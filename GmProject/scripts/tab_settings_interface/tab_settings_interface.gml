...
function tab_settings_interface()
{
	// Appearance
	tab_control_switch()
	draw_button_collapse("uiappearance", !collapse_map[?"uiappearance"], null, true, "settingsappearance")
	tab_next()
	
	if (!collapse_map[?"uiappearance"])
	{
		tab_collapse_start()
	...
	// Timeline
	tab_control_switch()
	draw_button_collapse("uitimeline", collapse_map[?"uitimeline"], null, true, "settingstimeline")
	tab_next()
		
	if (collapse_map[?"uitimeline"])
	{
		tab_collapse_start()
	...
	// Tools
	tab_control_switch()
	draw_button_collapse("uitools", collapse_map[?"uitools"], null, true, "settingstools")
	tab_next()
	
	if (collapse_map[?"uitools"])
	{
		tab_collapse_start()
	...
	// Viewport
	tab_control_switch()
	draw_button_collapse("uiviewport", collapse_map[?"uiviewport"], null, true, "settingsviewport")
	tab_next()
	
	if (collapse_map[?"uiviewport"])
	{
		tab_collapse_start()
		
		// Work camera FOV
		tab_control_meter()
		draw_meter("settingsworkcamerafov", dx, dy, dw, setting_work_camera_fov, 1, 170, 45, 1, tab.interface.tbx_work_camera_fov, action_setting_work_camera_fov)
		tab_next()
