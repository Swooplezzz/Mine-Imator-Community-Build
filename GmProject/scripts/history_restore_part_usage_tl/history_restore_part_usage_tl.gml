...
{	
	...
	with (hobj)
	{
		...
		for (var i = 0; i < usage_tl_flw_rot_target_amount; i++)
		{
			tl = save_id_find(usage_tl_flw_rot_target_save_id[i])
			tl.value[e_value.ROT_TARGET] = save_id_find(usage_tl_flw_rot_target_part_save_id[i])
		}
		for (var i = 0; i < usage_tl_flw_scale_target_amount; i++)
		{
			tl = save_id_find(usage_tl_flw_scale_target_save_id[i])
			tl.value[e_value.SCALE_TARGET] = save_id_find(usage_tl_flw_scale_target_part_save_id[i])
		}
		...
		for (var i = 0; i < usage_kf_flw_rot_target_amount; i++)
		{
			tl = save_id_find(usage_kf_flw_rot_target_tl_save_id[i])
			kfindex = usage_kf_flw_rot_target_index[i]
			tl.keyframe_list[|kfindex].value[e_value.ROT_TARGET] = save_id_find(usage_kf_flw_rot_target_tl_part_save_id[i])
		}
		
	    for (var i = 0; i < usage_kf_flw_pos_target_amount; i++)
		{
			tl = save_id_find(usage_kf_flw_pos_target_tl_save_id[i])
			kfindex = usage_kf_flw_pos_target_index[i]
			tl.keyframe_list[|kfindex].value[e_value.POS_TARGET] = save_id_find(usage_kf_flw_pos_target_tl_part_save_id[i])
		}
		
	    for (var i = 0; i < usage_kf_flw_scale_target_amount; i++)
		{
			tl = save_id_find(usage_kf_flw_scale_target_tl_save_id[i])
			kfindex = usage_kf_flw_scale_target_index[i]
