...
namespace CppProject
{
	...
	RealType draw_wheel(ScopeAny self, StringType name, VarType xx, VarType yy, VarType color, VarType value, RealType minval, RealType maxval, VarType def, RealType snapval, VarType tbx, IntType script, RealType rad, VarType sprite)
	{
		...
		StringType text;
		VarType labely;
		context_menu_area(self, { xx - rad, yy - rad, rad * IntType(2), rad * IntType(2), /*"contextmenuvalue"*/ STR(1070), value, e_context_type_NUMBER, script, def });
		draw_set_font(sInt(font_label));
		modval = mod_fix(value, IntType(360));
		capwid = string_width(text_get({ name }) + /*":"*/ STR(782)) + IntType(5);
	...
	RealType draw_wheel_sky(ScopeAny self, StringType name, VarType xx, VarType yy, VarType value, RealType def, IntType script, IntType tbx, RealType time)
	{
		...
		context_menu_area(self, { xx - (rad + IntType(10)), yy - (rad + IntType(10)), (rad + IntType(10)) * IntType(2), (rad + IntType(10)) * IntType(2), /*"contextmenuvalue"*/ STR(1070), value, (time > 0) ? e_context_type_TIME : e_context_type_NUMBER, script, def });
	...
	VarType ease(VarType func, VarType xx)
	{
		...
		switch (StringType(func).id)
		{
			...
			case 574: // easeinquint
			{
				return xx * xx * xx * xx * xx;
			}
			case 575: // easeoutquint
			{
				return ((xxm1) * xxm1 * xxm1 * xxm1 * xxm1 + IntType(1));
			}
			case 576: // easeinoutquint
			{
				if (xx2 < IntType(1))
			...
			case 578: // easeoutsine
			{
				return sin(xx * (pi_ / 2.0));
			}
			case 579: // easeinoutsine
			{
			...
			case 581: // easeoutexpo
			{
				return -power(2.0, -IntType(10) * xx) + IntType(1);
			}
			case 582: // easeinoutexpo
			{
				if (xx2 < IntType(1))
			...
			case 584: // easeoutcirc
			{
				return sqrt(IntType(1) - (xxm1) * xxm1);
			}
			case 585: // easeinoutcirc
			{
				if (xx2 < IntType(1))
	...
	VarType eval(VarType str, VarType def)
	{
		...
		for (i = IntType(1); i < string_length(str) + IntType(1); i++)
		{
			...
			if (char_ == /*" "*/ STR(17))
				...
				if (char_ == /*"("*/ STR(1101))
				{
					if (lastoperator == /*"--"*/ STR(1102))
						ds_stack_push({ ops, /*"-("*/ STR(1103) });
					else
						ds_stack_push({ ops, /*"("*/ STR(1101) });
					...
					if (eval_is_digit(char_))
					{
						...
						{
							if (string_char_at(str, i) == /*"."*/ STR(1104))
							{
								deccheck = true;
								i++;
								continue;
						...
						if (char_ == /*")"*/ STR(1105))
						{
							while (!ds_stack_empty(ops) && !(ds_stack_top(ops) == /*"("*/ STR(1101) || ds_stack_top(ops) == /*"-("*/ STR(1103)))
							{
						...
						{
							if (lastoperator != /*""*/ STR(0) && char_ == /*"-"*/ STR(1106))
							{
								lastoperator = /*"--"*/ STR(1102);
								continue;
							}
	...
	{
		if (a == VarType() && b != VarType() && op == /*"-"*/ STR(1106))
			a = IntType(0);
	...
	{
		ArrType digits = ArrType::From({ /*"0"*/ STR(1063), /*"1"*/ STR(1110), /*"2"*/ STR(1111), /*"3"*/ STR(1112), /*"4"*/ STR(1113), /*"5"*/ STR(1114), /*"6"*/ STR(1115), /*"7"*/ STR(1116), /*"8"*/ STR(1117), /*"9"*/ STR(1118), /*"."*/ STR(1104) });
