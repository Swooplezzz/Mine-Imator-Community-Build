...
namespace CppProject
{
	VarType model_file_load_shape(Scope<obj_model_part> self, VarType map, IntType res)
	{
		...
		withOne (obj_model_shape, (new obj_model_shape)->id, self->id)
		{
			...
			self->use_model_color = value_get_real(DsMap(map).Value(/*"use_model_color"*/ STR(1879)), false);
			...
			self->texture_mirror = value_get_real(DsMap(map).Value(/*"texture_mirror"*/ STR(1880)), false);
			self->invert = value_get_real(DsMap(map).Value(/*"invert"*/ STR(1873)), false);
			self->hide_front = value_get_real(DsMap(map).Value(/*"hide_front"*/ STR(1881)), false);
			self->hide_back = value_get_real(DsMap(map).Value(/*"hide_back"*/ STR(1882)), false);
			if (is_bool(DsMap(map).Value(/*"hide_backface"*/ STR(1883))))
				self->hide_back = DsMap(map).Value(/*"hide_backface"*/ STR(1883));
			self->face_camera = value_get_real(DsMap(map).Value(/*"face_camera"*/ STR(1884)), false);
			self->item_bounce = value_get_real(DsMap(map).Value(/*"item_bounce"*/ STR(1885)), false);
			self->move_required_array = value_get_point3D(DsMap(map).Value(/*"move_required"*/ STR(1886)), vec3(-IntType(1), -IntType(1), -IntType(1)));
			...
			if (self->type == /*"plane"*/ STR(1887))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->inflate = vec3(value_get_real(DsMap(map).Value(/*"inflate"*/ STR(1888)), IntType(0)));
			if (self->type == /*"plane"*/ STR(1887))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->is3d = false;
			if (self->type == /*"plane"*/ STR(1887))
			{
				self->is3d = value_get_real(DsMap(map).Value(/*"3d"*/ STR(1889)), false);
				if (self->is3d > 0)
				{
			...
			self->bend_shape = value_get_real(DsMap(map).Value(/*"bend"*/ STR(1857)), true);
			...
			VarType windmap = DsMap(map).Value(/*"wind"*/ STR(145));
			if (ds_map_valid(windmap))
			{
				...
				if (is_real(DsMap(windmap).Value(/*"ymax"*/ STR(1890))))
					self->wind_wave_zmax = DsMap(windmap).Value(/*"ymax"*/ STR(1890));
			}
			if (self->type == /*"block"*/ STR(4))
			{
				...
				else
				{
					...
					log({ /*"Invalid shape type"*/ STR(1891), self->type });
					return null_;
				}
	...
	RealType model_file_load_texture(VarType name, IntType res)
	{
		...
		if (ext != /*".png"*/ STR(10) && ext != /*".jpg"*/ STR(734) && ext != /*".jpeg"*/ STR(735))
			return IntType(0);
	...
	IntType model_load(VarType map, StringType dir)
	{
		withOne (obj_model, (new obj_model)->id, noone)
		{
			...
			{
				log({ /*"Could not find name for model"*/ STR(1892) });
				return null_;
			}
			
			if (dev_mode_debug_names && !text_exists(/*"model"*/ STR(8) + self->name))
				log({ /*"model/"*/ STR(1836) + self->name + dev_mode_name_translation_message });
			if (is_string(DsMap(map).Value(/*"file"*/ STR(122))))
				self->file = model_file_load(dir + DsMap(map).Value(/*"file"*/ STR(122)));
			else
				self->file = null_;
			...
			if (is_string(DsMap(map).Value(/*"texture"*/ STR(570))))
			{
				...
				DsMap(self->texture_name_map)[/*""*/ STR(0)] = DsMap(map).Value(/*"texture"*/ STR(570));
			}
			else
				if (ds_map_valid(DsMap(map).Value(/*"texture"*/ STR(570))))
				{
					self->texture_name_map = ds_string_map_create();
					ds_map_copy((IntType)(self->texture_name_map), (IntType)(DsMap(map).Value(/*"texture"*/ STR(570))));
				}
				else
					self->texture_name_map = null_;
			
			
			self->version = value_get_real(DsMap(map).Value(/*"version"*/ STR(92)), IntType(0));
			...
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(823))))
			{
				...
				{
					if (dev_mode_debug_names && !text_exists(/*"modelstate"*/ STR(771) + curstate))
						log({ /*"model/state/"*/ STR(1893) + curstate + dev_mode_name_translation_message });
					withOne (obj_model_state, (new obj_model_state)->id, self->id)
					{
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							if (dev_mode_debug_names && !text_exists(/*"modelstatevalue"*/ STR(9) + self->value_name.Value(v)))
								log({ /*"model/state/value/"*/ STR(1894) + self->value_name.Value(v) + dev_mode_name_translation_message });
							if (!is_undefined(DsMap(curvalue).Value(/*"file"*/ STR(122))))
								self->value_file[v] = model_file_load(dir + DsMap(curvalue).Value(/*"file"*/ STR(122)));
							if (is_string(DsMap(curvalue).Value(/*"texture"*/ STR(570))))
							{
								IntType texnamemap = ds_string_map_create();
								DsMap(texnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"texture"*/ STR(570));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"texture"*/ STR(570))))
								{
									self->value_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(570)), true);
									self->value_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(570)), true);
									self->value_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(570)), true);
								}
							
							if (is_string(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1895))))
							{
								IntType shapetexnamemap = ds_string_map_create();
								DsMap(shapetexnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"shape_texture"*/ STR(1895));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1895))))
								{
									self->value_shape_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1895)), true);
									self->value_shape_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1895)), true);
									self->value_shape_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1895)), true);
								}
							
							if (ds_list_valid(DsMap(curvalue).Value(/*"hide"*/ STR(746))))
							{
								self->value_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"hide"*/ STR(746))));
							}
							if (ds_list_valid(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1896))))
							{
								self->value_shape_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_shape_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1896))));
							}
							if (ds_map_valid(DsMap(curvalue).Value(/*"shape_color"*/ STR(1897))))
							{
								self->value_color_name_map[v] = ds_map_create();
								ds_map_merge(self->value_color_name_map.Value(v), DsMap(curvalue).Value(/*"shape_color"*/ STR(1897)), true);
							}
	...
	RealType model_part_fill_shape_alpha_map(VarType part, IntType alphamap, VarType res, IntType texnamemap, IntType shapetexnamemap)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			{
				if (self->type == /*"plane"*/ STR(1887) && self->is3d > 0)
				{
	...
	VarType model_shape_generate_block(ScopeAny self, VecType bend)
	{
		...
		VarType bendsize;
		BoolType invangle;
		sharpbend = global::_app->project_bend_style == /*"blocky"*/ STR(1898) && sVar(bend_size) == null_ && ((sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && sVec(bend_axis).Real(Z_)));
		bendsize = (sVar(bend_size) == null_ ? VarType((global::_app->project_bend_style == /*"realistic"*/ STR(1899) ? 4.0 : 1.0)) : sVar(bend_size));
