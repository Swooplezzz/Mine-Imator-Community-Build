...
namespace CppProject
{
		...
		for (IntType _it = 0, _it_max = listlen; _it < _it_max; _it++)
		{
			switch ((IntType)listtype)
			{
				...
				case e_nbt_TAG_LIST:
				{
					...
					{
						log({ /*"TAG_List error"*/ STR(1896), /*"Invalid tag"*/ STR(1895) });
	...
	IntType new_biome(StringType name, RealType tx, RealType ty, BoolType hardcoded, IntType grass, IntType foliage, IntType water, IntType base)
	{
		withOne (obj_biome, (new obj_biome)->id, noone)
		{
			...
			self->group = false;
			self->display_name = minecraft_asset_get_name(/*"biome"*/ STR(1686), name);
	...
	IntType new_res(ScopeAny self, VarType fn, VarType type)
	{
		...
		if (res != null_)
		{
			if (question(text_get({ /*"questionreplace"*/ STR(1900) })))
				replaced = true;
			else
			{
				res = null_;
		...
		log({ /*"Add resource"*/ STR(1901), DsList(global::res_type_name_list).Value(type) });
		log({ /*"filename"*/ STR(1247), ObjType(obj_resource, res)->filename });
		return res;
	}
	...
	IntType new_shader(StringType name)
	{
		withOne (obj_shader, (new obj_shader)->id, noone)
		{
			...
			self->script = asset_get_index(name + /*"_set"*/ STR(1902));
			self->uniform_map = ds_string_map_create();
			self->sampler_map = ds_string_map_create();
			new_shader_sampler(self, /*"uTexture"*/ STR(1903));
			new_shader_uniform(self, /*"uBlendColor"*/ STR(1904));
			new_shader_uniform(self, /*"uTime"*/ STR(1905));
			new_shader_uniform(self, /*"uWindEnable"*/ STR(1906));
			new_shader_uniform(self, /*"uWindTerrain"*/ STR(1907));
			new_shader_uniform(self, /*"uWindSpeed"*/ STR(1908));
			new_shader_uniform(self, /*"uWindStrength"*/ STR(1909));
			new_shader_uniform(self, /*"uWindDirection"*/ STR(1910));
			new_shader_uniform(self, /*"uWindDirectionalSpeed"*/ STR(1911));
			new_shader_uniform(self, /*"uWindDirectionalStrength"*/ STR(1912));
			new_shader_uniform(self, /*"uFogShow"*/ STR(1913));
			new_shader_uniform(self, /*"uFogColor"*/ STR(1914));
			new_shader_uniform(self, /*"uFogDistance"*/ STR(1915));
			new_shader_uniform(self, /*"uFogSize"*/ STR(1916));
			new_shader_uniform(self, /*"uFogHeight"*/ STR(1917));
			new_shader_uniform(self, /*"uCameraPosition"*/ STR(1918));
			new_shader_uniform(self, /*"uTAAMatrix"*/ STR(1919));
			new_shader_uniform(self, /*"uSampleIndex"*/ STR(1920));
			new_shader_uniform(self, /*"uAlphaHash"*/ STR(1921));
	...
	IntType new_tab(StringType location, BoolType show, IntType header, IntType window)
	{
		...
		location = string_replace(location, /*"_top"*/ STR(1922), /*""*/ STR(0));
		location = string_replace(location, /*"_bottom"*/ STR(1923), /*"_secondary"*/ STR(1924));
	...
	{
		return new_textbox(true, IntType(64), /*".0123456789"*/ STR(1925) + /*"+-/*^()"*/ STR(1926));
	}
	
	IntType new_textbox_hex()
	{
		return new_textbox(true, IntType(6), /*"0123456789ABCDEFabcdef"*/ STR(1927));
	}
	
	IntType new_textbox_integer()
	{
		return new_textbox(true, IntType(64), /*"0123456789"*/ STR(1033) + /*"+-/*^()"*/ STR(1926));
	}
	
	IntType new_textbox_ndecimals()
	{
		return new_textbox(true, IntType(64), /*"-.0123456789"*/ STR(1928) + /*"+-/*^()"*/ STR(1926));
	}
	
	IntType new_textbox_ninteger()
	{
		return new_textbox(true, IntType(64), /*" - 0123456789"*/ STR(1929) + /*"+-/*^()"*/ STR(1926));
	}
	...
	IntType new_transition_texture_map(RealType w, RealType h, RealType padding, BoolType center)
	{
			...
			surface_set_target(surf);
		{
			...
			for (RealType d = IntType(0); d <= quality; d++)
			{
				...
				if (DsList(global::transition_list).Value(t) == /*"bezier"*/ STR(560))
				{
	...
	RealType object_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miobject"*/ STR(707));
		log({ /*"Saving object"*/ STR(1930), fn });
		...
		project_save_done();
		log({ /*"Object saved"*/ STR(1931) });
		return 0.0;
	}
	
	void open_url(StringType url)
	{
		log({ /*"Open URL"*/ STR(1932), url });
		lib_open_url(url);
	}
	...
	void panel_area_draw(ScopeAny self)
	{
		...
		withAll (obj_panel, global::_app->id)
		{
			...
			self->size_real_ani = ceil((self->size_real - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(580), self->size_ani)));
			self->glow_ani += test_reduced_motion(IntType(1), 0.035 * global::delta);
			self->glow_ani = clamp(self->glow_ani, IntType(0), IntType(1));
			self->size_glow = (self->size - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(580), self->glow_ani));
		...
		if (sVar(window_busy) == /*"panelresize"*/ STR(1933))
		{
		...
		if (sVar(window_busy) == /*"tabmove"*/ STR(1934))
		{
	...
	RealType panel_draw(ScopeAny self, IntType panel)
	{
		...
		if (idBool(sVar(content_tab), movable))
		{
			...
			for (IntType t = IntType(0); t < ObjType(obj_panel, panel)->tab_list_amount; t++)
			{
				...
				if (idBool(tab, closeable) && (hover > 0 || sel > 0))
				{
					...
					if (draw_button_icon(self, { /*"tabclose"*/ STR(1935) + string(tab), floor(dx + dw - IntType(20)), dy + IntType(4), IntType(16), IntType(16), false, icons_CLOSE_SMALL }))
					{
						tab_close(self, tab);
						return IntType(0);
					}
				...
				if (sVar(window_busy) == /*"tabmove"*/ STR(1934))
				{
					...
					sVar(window_busy) = /*"tabmove"*/ STR(1934);
				}
		...
		{
			sVar(window_busy) = /*"panelresize"*/ STR(1933);
		...
		{
			sVar(window_busy) = /*"tabclick"*/ STR(1936);
			sVar(tab_move) = sVar(content_tab);
		}
		if (sVar(window_busy) == /*"tabclick"*/ STR(1936))
		{
