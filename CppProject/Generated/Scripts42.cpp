...
namespace CppProject
{
	RealType project_load(ScopeAny self, VarArgs argument)
	{
		...
		{
			unzip({ fn });
			fn = file_find_single(unzip_directory, /*".miproject;.mproj;.mani"*/ STR(2061));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*".miproject;.mproj;.mani"*/ STR(2061));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenprojectzip"*/ STR(2062));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		BoolType legacy;
		ext = filename_ext(fn);
		if (ext == /*".miproject"*/ STR(700) || string_contains(ext, /*".backup"*/ STR(2055)))
		{
			log({ /*"Opening project"*/ STR(2063), fn });
		...
		{
			log({ /*"Opening legacy project"*/ STR(2064), fn });
		...
		sVar(project_file) = filename_new_ext(fn, /*".miproject"*/ STR(700));
		...
		if (!legacy)
		{
			...
			project_load_markers(DsMap(rootmap).Value(/*"markers"*/ STR(2065)));
		}
		else
		{
		...
		project_reset_loaded(self);
		log({ /*"Project loaded"*/ STR(2066) });
		if (global::load_format < e_project_FORMAT_110_PRE_1)
		{
			if (!dev_mode)
				file_rename_lib(fn, fn + /*".old"*/ STR(2067));
			project_save(self);
		}
	...
	RealType project_load_background(ScopeAny self, VarType map)
	{
		...
		sVar(background_image_show) = value_get_real({ DsMap(map).Value(/*"image_show"*/ STR(2068)), sVar(background_image_show) });
		...
		sVar(background_image_type) = value_get_string({ DsMap(map).Value(/*"image_type"*/ STR(2069)), sVar(background_image_type) });
		sVar(background_image_stretch) = value_get_real({ DsMap(map).Value(/*"image_stretch"*/ STR(2070)), sVar(background_image_stretch) });
		sVar(background_image_box_mapped) = value_get_real({ DsMap(map).Value(/*"image_box_mapped"*/ STR(2071)), sVar(background_image_box_mapped) });
		sVar(background_image_rotation) = value_get_real({ DsMap(map).Value(/*"image_rotation"*/ STR(2072)), sVar(background_image_rotation) });
		idInt(sVar(background_sky_sun_tex), count)--;
		sVar(background_sky_sun_tex) = value_get_save_id({ DsMap(map).Value(/*"sky_sun_tex"*/ STR(2073)), sVar(background_sky_sun_tex) });
		idInt(sVar(background_sky_moon_tex), count)--;
		sVar(background_sky_moon_tex) = value_get_save_id({ DsMap(map).Value(/*"sky_moon_tex"*/ STR(2074)), sVar(background_sky_moon_tex) });
		sVar(background_sky_moon_phase) = value_get_real({ DsMap(map).Value(/*"sky_moon_phase"*/ STR(2075)), sVar(background_sky_moon_phase) });
		...
		if (global::load_format < e_project_FORMAT_200_PRE_5)
		{
			...
			if (faded > 0)
				sVar(background_sky_clouds_mode) = /*"faded"*/ STR(731);
			else
				if (flat > 0)
					sVar(background_sky_clouds_mode) = /*"flat"*/ STR(732);
		...
		{
			sVar(background_ground_name) = string_replace(sVar(background_ground_name), /*"blocks/"*/ STR(837), /*"block/"*/ STR(803));
		...
		sVar(background_biome) = value_get_string({ DsMap(map).Value(/*"biome"*/ STR(1671)), sVar(background_biome) });
		...
		sVar(background_fog_custom_object_color) = value_get_real({ DsMap(map).Value(/*"fog_custom_object_color"*/ STR(2116)), sVar(background_fog_custom_object_color) });
	...
	void project_load_find_save_ids(ScopeAny self)
	{
		...
		{
			if (DsMap(global::save_id_map).Value(key) != /*"root"*/ STR(2127) && DsMap(global::save_id_map).Value(key) != /*"default"*/ STR(98))
			{
		...
		withAll (obj_keyframe, self->id)
		{
			...
			if (self->value.Value(e_value_TEXTURE_OBJ) == /*"none"*/ STR(888))
			...
			if (self->value.Value(e_value_TEXTURE_MATERIAL_OBJ) == /*"none"*/ STR(888))
			...
			if (self->value.Value(e_value_TEXTURE_NORMAL_OBJ) == /*"none"*/ STR(888))
	...
	void project_load_legacy_background(ScopeAny self)
	{
		...
		ArrType imagetype = ArrType::From({ /*"image"*/ STR(90), /*"sphere"*/ STR(527), /*"box"*/ STR(1329) });
		...
		sVar(background_sky_clouds_mode) = ((flat > 0) ? /*"flat"*/ STR(732) : /*"faded"*/ STR(731));
	...
	void project_load_legacy_beta(ScopeAny self, BoolType loadbackground)
	{
		...
		withOne (obj_data, load, self->id)
		{
			...
			for (IntType a = IntType(0); a < self->lib_amount; a++)
			{
				...
				if (is_string(self->lib_char_model_legacy_name.Value(a)))
				{
					...
					{
						self->lib_char_model_name[a] = DsMap(modelmap).Value(/*"name"*/ STR(801));
						if (!is_undefined(DsMap(modelmap).Value(/*"state"*/ STR(997))))
							self->lib_char_model_state[a] = string_get_state_vars(DsMap(modelmap).Value(/*"state"*/ STR(997)));
						else
			...
			for (IntType a = IntType(0); a < self->tl_amount; a++)
			{
				...
				if (self->tl_type.Value(a) == /*"char"*/ STR(518) || self->tl_type.Value(a) == /*"spblock"*/ STR(519))
				{
		...
		{
			if (ObjType(obj_data, load)->lib_type.Value(a) == /*"light"*/ STR(628))
				continue;
		...
		for (IntType a = IntType(0); a < ObjType(obj_data, load)->tl_amount; a++)
		{
			...
			if (a == IntType(0))
			{
				...
				if (ObjType(obj_data, load)->tl_type.Value(a) == /*"light"*/ STR(628))
				{
					tl = (new obj_timeline)->id;
