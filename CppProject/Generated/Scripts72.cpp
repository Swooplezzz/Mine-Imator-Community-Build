...
namespace CppProject
{
	...
	{
		log({ /*"Create vertex format"*/ STR(3962) });
		vertex_format_begin();
	...
	{
		sReal(view_area_x) = sReal(panel_area_x) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(203)))->size_real_ani + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(199)))->size_real_ani;
		sReal(view_area_y) = sReal(panel_area_y) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(202)))->size_real_ani;
		sReal(view_area_width) = sReal(panel_area_width) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(203)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(199)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(204)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(200)))->size_real_ani;
		sReal(view_area_height) = sReal(panel_area_height) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(202)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(201)))->size_real_ani;
		view_draw(self, sInt(view_main));
		view_draw(self, sInt(view_second));
		if (sVar(window_busy) == /*"viewresizehor"*/ STR(3963) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3964))
		{
			sInt(mouse_cursor) = cr_size_we;
			if (string_contains(ObjType(obj_view, sInt(view_second))->location, /*"left"*/ STR(203)))
		...
		if (sVar(window_busy) == /*"viewresizever"*/ STR(3965) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3964))
		{
			sInt(mouse_cursor) = cr_size_ns;
			if (string_contains(ObjType(obj_view, sInt(view_second))->location, /*"top"*/ STR(202)))
		...
		if (sVar(window_busy) == /*"viewresizeboth"*/ STR(3964))
		{
			if (ObjType(obj_view, sInt(view_second))->location == /*"right"*/ STR(204) || ObjType(obj_view, sInt(view_second))->location == /*"left_secondary"*/ STR(199))
				sInt(mouse_cursor) = cr_size_nesw;
			else
				sInt(mouse_cursor) = cr_size_nwse;
			
		}
		if (sVar(window_busy) == /*"viewresizesplithor"*/ STR(3966))
		{
			sInt(mouse_cursor) = cr_size_we;
			sReal(view_split) = clamp((gmlGlobal::mouse_x - sReal(view_area_x)) / sReal(view_area_width), 0.1, 0.9);
			if (ObjType(obj_view, sInt(view_second))->location == /*"right"*/ STR(204))
				sReal(view_split) = IntType(1) - sReal(view_split);
			if (!sBool(mouse_left))
				sVar(window_busy) = /*""*/ STR(0);
		}
		if (sVar(window_busy) == /*"viewresizesplitver"*/ STR(3967))
		{
			sInt(mouse_cursor) = cr_size_ns;
			sReal(view_split) = clamp((gmlGlobal::mouse_y - sReal(view_area_y)) / sReal(view_area_height), 0.1, 0.9);
			if (ObjType(obj_view, sInt(view_second))->location == /*"bottom"*/ STR(201))
				sReal(view_split) = IntType(1) - sReal(view_split);
			if (!sBool(mouse_left))
				sVar(window_busy) = /*""*/ STR(0);
		}
	...
	void view_click(ScopeAny self, IntType view, VarType cam)
	{
		...
		global::render_ratio = ((RealType)sVar(content_width) / sVar(content_height)) * global::cam_aspect;
		surface_set_target(surf);
	{
		...
		if (tl > IntType(0))
		{
			if (!(global::tl_edit > 0) && !keyboard_check(vk_control) || sVar(setting_select_locked_parent) > 0 && idVar(tl, lock) > 0)
				while (idVar(tl, parent) != global::_app->id && (sVar(setting_select_locked_parent) > 0 && idVar(tl, lock) > 0 ? idVar(idVar(tl, parent), lock) : VarType(!(idVar(idVar(tl, parent), lock) > 0))) && tl_update_list_filter(idVar(tl, parent)))
					tl = idVar(tl, parent);
			
			if (idVar(tl, lock) > 0 && !(idVar(idVar(tl, parent), lock) > 0) && sVar(setting_select_locked_parent) > 0)
				tl = idVar(tl, parent);
