...
namespace CppProject
{
	...
	void view_shape_particles(IntType tl)
	{
		...
		if (temp == global::temp_edit && ObjType(obj_tab, global::_app->template_editor)->show > 0)
		{
			...
			{
				if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1350))
			...
			if (idVar(temp, pc_bounding_box_type) == /*"custom"*/ STR(612))
			{
				draw_set_color(c_red);
					...
					if (app_mouse_box(self, sVar(content_x) + (nextcoord2d.Real(X_) - IntType(13)), sVar(content_y) + (nextcoord2d.Real(Y_) - IntType(13)), IntType(26), IntType(26)) && sBool(content_mouseon) && ObjType(obj_view, view)->control_mouseon_last == null_)
					{
						...
						if (sBool(mouse_left_pressed))
						{
							...
							if (sVar(setting_timeline_select_jump) > 0)
								tl_jump(pointtl);
							sVar(window_busy) = /*"viewpathpointclick"*/ STR(3891);
						}
					}
				}
			}
	...
	RealType view_toolbar_draw(ScopeAny self, IntType view, RealType xx, RealType yy)
	{
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtoolselect"*/ STR(3892), xx, yy, IntType(24), IntType(24), sBool(setting_tool_select), icons_SELECT, null_, false, /*"viewtoolselecttip"*/ STR(3893) }))
			{
		...
		if (draw_button_icon(self, { /*"viewtoolmove"*/ STR(3894), xx, yy, IntType(24), IntType(24), sVar(setting_tool_move), icons_MOVE, null_, false, /*"viewtoolmovetip"*/ STR(3895) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (draw_button_icon(self, { /*"viewtoolrotate"*/ STR(3896), xx, yy, IntType(24), IntType(24), sBool(setting_tool_rotate), icons_ROTATE, null_, false, /*"viewtoolrotatetip"*/ STR(3897) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtoolscale"*/ STR(3898), xx, yy, IntType(24), IntType(24), sBool(setting_tool_scale), icons_SCALE, null_, false, /*"viewtoolscaletip"*/ STR(3899) }))
			{
				if (sVar(setting_separate_tool_modes) > 0)
				{
					action_tools_disable_all(self);
		...
		if (draw_button_icon(self, { /*"viewtoolbend"*/ STR(3900), xx, yy, IntType(24), IntType(24), sBool(setting_tool_bend), icons_BEND, null_, false, /*"viewtoolbendtip"*/ STR(3901) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtooltransform"*/ STR(3902), xx, yy, IntType(24), IntType(24), sBool(setting_tool_transform), icons_MULTITRANSFORM, null_, false, /*"viewtooltransformtip"*/ STR(3903) }))
			{
				action_tools_disable_all(self);
	...
	void view_update(ScopeAny self, IntType view, VarType cam)
	{
		...
		if (sBool(content_mouseon) && (sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(114)))
		{
			sInt(place_view_mouse) = view;
			sInt(mouse_cursor) = cr_handpoint;
			if (sBool(mouse_left_pressed))
			{
				sVar(window_busy) = /*"viewclick"*/ STR(3904);
			...
			if ((!(cam > 0) || editcamobj) && sBool(mouse_right_pressed))
			{
				...
				sVar(window_busy) = /*"viewmovecamera"*/ STR(3905);
				sVar(window_focus) = string(view);
				if (cam > 0)
		...
		if (((((sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(114)) && sBool(content_mouseon)) || (sVar(window_busy) == /*"viewrotatecamera"*/ STR(3906) && sVar(window_focus) == string(view)))) && sReal(mouse_wheel) != IntType(0))
		{
			if (!(cam > 0))
		...
		{
			if (sVar(window_busy) == /*"viewclick"*/ STR(3904))
			{
				sInt(mouse_cursor) = cr_handpoint;
				if ((!(cam > 0) || editcamobj) && sReal(mouse_move) > IntType(5))
				{
					...
					{
						sVar(window_busy) = /*"viewpancamera"*/ STR(3907);
						sVar(window_focus) = string(view);
						if (cam > 0)
							action_tl_select_single(self, { cam });
					}
					else
					{
						sInt(view_click_x) = display_mouse_get_x();
						sInt(view_click_y) = display_mouse_get_y();
						sVar(window_busy) = /*"viewrotatecamera"*/ STR(3906);
						if (cam > 0)
							action_tl_select_single(self, { cam });
					}
			...
			if (sVar(window_busy) == /*"viewrotatecamera"*/ STR(3906))
			{
				...
				if (!sBool(mouse_left))
					sVar(window_busy) = (sInt(place_tl) != null_ ? /*"place"*/ STR(114) : /*""*/ STR(0));
			}
			if (sVar(window_busy) == /*"viewmovecamera"*/ STR(3905))
			{
				global::render_samples = -IntType(1);
				if (cam == null_)
					sStr(shortcut_bar_state) = /*"cameramove"*/ STR(2892);
				else
					sStr(shortcut_bar_state) = /*"tlcameramove"*/ STR(2893);
			...
			if (sVar(window_busy) == /*"viewpancamera"*/ STR(3907))
			{
		...
		if (sVar(window_busy) == /*"viewpathpointclick"*/ STR(3891))
			sVar(window_busy) = /*""*/ STR(0);
	}
	...
	RealType window_draw(ScopeAny self)
	{
		switch ((IntType)window_get_current())
		{
			case e_window_MAIN:
			{
				if (sStr(window_state) == /*"load_assets"*/ STR(80))
				{
					window_draw_load_assets(self);
					return IntType(0);
				}
				if (sStr(window_state) == /*"new_assets"*/ STR(79))
				{
				...
				if (sStr(window_state) == /*"startup"*/ STR(25))
				{
					...
					if (sStr(window_state) == /*"world_import"*/ STR(2702))
					{
			...
			case e_window_VIEW_SECOND:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(ObjType(obj_view, sInt(view_second))->title + /*" - Mine-imator"*/ STR(702));
				break;
			}
			...
			case e_window_TIMELINE:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(text_get({ /*"tabtimeline"*/ STR(3177) }) + /*" - Mine-imator"*/ STR(702));
				break;
			}
	...
	RealType window_draw_export(ScopeAny self)
	{
		...
		if (timelefthours > IntType(0))
			timeleftstr += text_get({ ((timelefthours == IntType(1)) ? /*"exporttimelefthour"*/ STR(3908) : /*"exporttimelefthours"*/ STR(3909)), string(timelefthours) }) + /*", "*/ STR(1042);
		if (timeleftmins > IntType(0))
			timeleftstr += text_get({ ((timeleftmins == IntType(1)) ? /*"exporttimeleftminute"*/ STR(3910) : /*"exporttimeleftminutes"*/ STR(3911)), string(timeleftmins) }) + /*" "*/ STR(17) + text_get({ /*"exporttimeleftand"*/ STR(3912) }) + /*" "*/ STR(17);
		timeleftstr += text_get({ ((timeleftsecs == IntType(1)) ? /*"exporttimeleftsecond"*/ STR(3913) : /*"exporttimeleftseconds"*/ STR(3914)), string(timeleftsecs) });
		draw_label(self, { text_get({ /*"exporttimeleft"*/ STR(3915), timeleftstr }), framex + (RealType)framew / 2.0, framey + frameh + IntType(33), fa_center, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_heading_big) });
		VarType loadtext, loadw;
		RealType sw, sh;
		loadtext = text_get({ /*"exportloading"*/ STR(3916), string(floor(perc * IntType(100))) });
		loadw = framew;
		...
		if (usesamples > 0)
			samplecount = text_get({ /*"exportsamples"*/ STR(3917), string(max({ global::render_samples, IntType(1) })), string(sVar(project_render_samples)) });
		VarType text = /*""*/ STR(0);
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
			text = text_get({ /*"exportframe"*/ STR(3918), string(sReal(exportmovie_frame)), string(totalframes) }) + ((usesamples > 0) ? (/*" ("*/ STR(1129) + samplecount + /*")"*/ STR(1105)) : /*""*/ STR(0));
		else
			if (usesamples > 0)
				text = samplecount;
		...
		window_set_caption(loadtext + /*" - Mine-imator"*/ STR(702));
		draw_label(self, { sStr(window_state) == /*"export_movie"*/ STR(49) ? text_get({ /*"exportmovietitle"*/ STR(3919) }) : text_get({ /*"exportimagetitle"*/ STR(3920) }), framex + (RealType)framew / 2.0, framey - IntType(35), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading_big) });
		draw_label(self, { text_get({ /*"exportstop"*/ STR(3921) }), framex + (RealType)framew / 2.0, framey - IntType(16), fa_center, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		return 0.0;
	}
	...
	RealType window_draw_load_assets(ScopeAny self)
	{
		...
		{
			global::load_assets_stage = /*"exit"*/ STR(3922);
			return IntType(0);
		}
		else
			if (global::load_assets_stage == /*"exit"*/ STR(3922))
			{
		...
		draw_sprite_ext(ID_spr_startupasset_gradient, IntType(0), IntType(0), IntType(0), 1.0, 1.0, 0.0, merge_color((IntType)(global::c_accent), c_white, 0.2), .5);
		IntType pattern = (sInt(setting_theme) == global::theme_light ? IntType(0) : IntType(1));
		draw_sprite_ext(ID_spr_pattern_left, pattern, (IntType)(xoff - 138.0 / 4.0), (IntType)(yoff), 138.0 / sprite_get_width(ID_spr_pattern_left), 450.0 / sprite_get_height(ID_spr_pattern_left), 0.0, c_white, 1.0);
		draw_sprite(ID_spr_load_assets, IntType(0), (IntType)(xoff + IntType(95)), (IntType)(yoff + IntType(207)));
		draw_set_font(sInt(font_splash_bold_big));
		draw_set_font(sInt(font_splash_bold));
		draw_set_font(sInt(font_splash_bold_secondary));
		draw_label(self, { /*"Mine-imator "*/ STR(3923) + string(mineimator_version), xoff + IntType(95), yoff + IntType(289), fa_middle, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_splash_bold) });
		draw_label(self, { string(string_upper(mineimator_version_sub)), xoff + IntType(95), yoff + IntType(289) + IntType(12), fa_middle, fa_bottom, global::c_accent, global::a_accent, sInt(font_splash_bold_secondary) });
		draw_label(self, { string(string_upper(mineimator_version_extra)), xoff + IntType(95), yoff + IntType(289) + IntType(28), fa_middle, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		draw_label(self, { text_get({ /*"startuploadingassets"*/ STR(3924) }), xoff + IntType(12), yoff + IntType(418), fa_left, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_splash_bold_big) });
		draw_label(self, { string(string_upper(minecraft_version)), xoff + IntType(12), yoff + IntType(435), fa_left, fa_bottom, global::c_accent, global::a_accent, sInt(font_splash_bold_big) });
		draw_label(self, { string(floor(global::load_assets_progress * IntType(100))) + /*"%"*/ STR(209), IntType(180), yoff + IntType(435), fa_right, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_splash_bold_big) });
		...
		{
			draw_label(self, { text_get({ /*"startupsplashauthor"*/ STR(3925) }), xoff + IntType(95), yoff + IntType(289) + IntType(34), fa_middle, fa_top, global::c_text_main, global::a_text_main, sInt(font_splash_bold) });
			draw_label(self, { /*"@"*/ STR(3926) + global::load_assets_credits, xoff + IntType(95), yoff + IntType(289) + IntType(50), fa_middle, fa_top, global::c_accent, global::a_accent, sInt(font_splash_bold) });
		}
		if (dev_mode)
			draw_label(self, { /*"DEV MODE ENABLED"*/ STR(3927), xoff + IntType(95), yoff + IntType(4), fa_middle, fa_top, global::c_accent, IntType(1), sInt(font_heading_big) });
	...
	RealType window_draw_new_assets(ScopeAny self)
	{
		...
		draw_label(self, { text_get({ /*"newassetstitle"*/ STR(3928), sStr(new_assets_version) }), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy) + IntType(227), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading) });
		draw_label(self, { text_get({ /*"newassetssubtitle"*/ STR(3929) }), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy) + IntType(244), fa_center, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += IntType(264);
		sVar(dx) += IntType(12);
		sVar(dw) -= 24.0;
		if (sStr(new_assets_stage) == /*"download"*/ STR(105))
		{
			tab_control_loading(self);
			draw_loading_bar(self, sVar(dx), sVar(dy), sVar(dw), IntType(8), sReal(new_assets_download_progress), text_get({ /*"newassetsdownloading"*/ STR(3930) }));
			tab_next(self);
		}
		else
		{
			draw_set_font(sInt(font_button));
			IntType capwid = string_width(text_get({ /*"newassetsdownload"*/ STR(3931) })) + button_padding;
			if (draw_button_label(self, /*"newassetsdownload"*/ STR(3931), sVar(dx) + sVar(dw), (sVar(content_y) + sVar(content_height)) - IntType(44), null_, null_, e_button_PRIMARY, null_, fa_right))
			{
				if (sVar(new_assets_format) > minecraft_assets_format)
				{
					if (question(text_get({ /*"questionassetsnewer"*/ STR(3932) })))
					{
				...
				{
					sStr(new_assets_stage) = /*"download"*/ STR(105);
					sInt(http_download_assets_file) = http_get_file(link_assets + sStr(new_assets_version) + /*".midata"*/ STR(97), mc_file_directory + sStr(new_assets_version) + /*".midata"*/ STR(97));
				}
				
			}
			if (draw_button_label(self, /*"newassetslater"*/ STR(3933), (sVar(dx) + sVar(dw)) - (capwid + IntType(8)), (sVar(content_y) + sVar(content_height)) - IntType(44), null_, null_, e_button_SECONDARY, null_, fa_right))
			{
			...
			sVar(dy_start) = sVar(dy);
			sVar(dy) += IntType(16);
			draw_label(self, { text_get({ /*"newassetschangelog"*/ STR(3934) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(20);
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(sArr(new_assets_changes_lines))); i++)
			{
				...
				if (string_copy(text, IntType(1), IntType(2)) == /*"- "*/ STR(3935))
				{
				...
				{
					if (string_copy(text, IntType(1), IntType(2)) == /*"* "*/ STR(3936))
						text = /*"• "*/ STR(3937) + string_delete(text, IntType(1), IntType(2));
