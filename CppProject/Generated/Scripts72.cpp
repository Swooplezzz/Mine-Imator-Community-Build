...
namespace CppProject
{
	...
	{
		log({ /*"Create vertex format"*/ STR(3936) });
		vertex_format_begin();
	...
	{
		sReal(view_area_x) = sReal(panel_area_x) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(201)))->size_real_ani + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(197)))->size_real_ani;
		sReal(view_area_y) = sReal(panel_area_y) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(200)))->size_real_ani;
		sReal(view_area_width) = sReal(panel_area_width) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(201)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(197)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(202)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(198)))->size_real_ani;
		sReal(view_area_height) = sReal(panel_area_height) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(200)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(199)))->size_real_ani;
		view_draw(self, sInt(view_main));
		view_draw(self, sInt(view_second));
		if (sVar(window_busy) == /*"viewresizehor"*/ STR(3937) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3938))
		{
			sInt(mouse_cursor) = cr_size_we;
			if (string_contains(ObjType(obj_view, sInt(view_second))->location, /*"left"*/ STR(201)))
		...
		if (sVar(window_busy) == /*"viewresizever"*/ STR(3939) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3938))
		{
			sInt(mouse_cursor) = cr_size_ns;
			if (string_contains(ObjType(obj_view, sInt(view_second))->location, /*"top"*/ STR(200)))
		...
		if (sVar(window_busy) == /*"viewresizeboth"*/ STR(3938))
		{
			if (ObjType(obj_view, sInt(view_second))->location == /*"right"*/ STR(202) || ObjType(obj_view, sInt(view_second))->location == /*"left_secondary"*/ STR(197))
				sInt(mouse_cursor) = cr_size_nesw;
			else
				sInt(mouse_cursor) = cr_size_nwse;
			
		}
		if (sVar(window_busy) == /*"viewresizesplithor"*/ STR(3940))
		{
			sInt(mouse_cursor) = cr_size_we;
			sReal(view_split) = clamp((gmlGlobal::mouse_x - sReal(view_area_x)) / sReal(view_area_width), 0.1, 0.9);
			if (ObjType(obj_view, sInt(view_second))->location == /*"right"*/ STR(202))
				sReal(view_split) = IntType(1) - sReal(view_split);
			if (!sBool(mouse_left))
				sVar(window_busy) = /*""*/ STR(0);
		}
		if (sVar(window_busy) == /*"viewresizesplitver"*/ STR(3941))
		{
			sInt(mouse_cursor) = cr_size_ns;
			sReal(view_split) = clamp((gmlGlobal::mouse_y - sReal(view_area_y)) / sReal(view_area_height), 0.1, 0.9);
			if (ObjType(obj_view, sInt(view_second))->location == /*"bottom"*/ STR(199))
				sReal(view_split) = IntType(1) - sReal(view_split);
			if (!sBool(mouse_left))
				sVar(window_busy) = /*""*/ STR(0);
		}
