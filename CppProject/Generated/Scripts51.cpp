...
namespace CppProject
{
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2502) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2503) + filename_ext(fn));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2504);
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1288: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2511), /*"unzip"*/ STR(1288) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip(fname))
					{
						log({ /*"Error unzipping pack"*/ STR(2512) });
						error(/*"errorunzippack"*/ STR(2513));
						withOne (app, global::_app->id, self->id)
				...
				sStr(load_stage) = /*"modeltextures"*/ STR(1370);
				global::load_assets_dir = unzip_directory;
				res_load_pack_version(self);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2514) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2511), /*"modeltextures"*/ STR(1370) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2515);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2516) });
			...
			case 2515: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2511), /*"blocktextures"*/ STR(2515) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"blocks"*/ STR(1003), global::load_assets_dir + mc_textures_directory + /*"block"*/ STR(529));
				res_load_pack_block_textures(self);
				sStr(load_stage) = /*"itemtextures"*/ STR(2517);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2518) });
			...
			case 2517: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2511), /*"itemtextures"*/ STR(2517) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2519), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(528));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(628), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(633), /*"_s"*/ STR(2520));
				res_load_pack_item_textures(self, /*"normal"*/ STR(622), /*"_n"*/ STR(2521));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2522) });
