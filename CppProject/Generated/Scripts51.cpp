...
namespace CppProject
{
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2514) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2515) + filename_ext(fn));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2516);
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1294: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2523), /*"unzip"*/ STR(1294) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip(fname))
					{
						log({ /*"Error unzipping pack"*/ STR(2524) });
						error(/*"errorunzippack"*/ STR(2525));
						withOne (app, global::_app->id, self->id)
				...
				sStr(load_stage) = /*"modeltextures"*/ STR(1376);
				global::load_assets_dir = unzip_directory;
				res_load_pack_version(self);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2526) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2523), /*"modeltextures"*/ STR(1376) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2527);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2528) });
			...
			case 2527: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2523), /*"blocktextures"*/ STR(2527) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"blocks"*/ STR(1009), global::load_assets_dir + mc_textures_directory + /*"block"*/ STR(536));
				res_load_pack_block_textures(self);
				sStr(load_stage) = /*"itemtextures"*/ STR(2529);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2530) });
			...
			case 2529: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2523), /*"itemtextures"*/ STR(2529) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2531), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(535));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(635), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(640), /*"_s"*/ STR(2532));
				res_load_pack_item_textures(self, /*"normal"*/ STR(629), /*"_n"*/ STR(2533));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2534) });
