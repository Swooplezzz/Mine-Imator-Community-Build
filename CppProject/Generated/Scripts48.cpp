/*
	...
	[ Generated on 2023.09.14 05:35:35 ]
*/
...
namespace CppProject
{
			...
			withOne (obj_timeline, lightlist.Value(i), global::_app->id)
			{
				...
				if (self->type == e_tl_type_POINT_LIGHT)
				{
						...
						surface_set_target((IntType)(global::render_surface_point_buffer));
					{
						gpu_set_blendmode_ext(bm_one, bm_zero);
						draw_clear(c_white);
						render_world_start_light(self->world_pos, point3D_add(self->world_pos, look), sampleoffset, self->id, point3D(IntType(0), IntType(0), IntType(1)));
					...
					if (self->type == e_tl_type_SPOT_LIGHT)
					{
						...
						VecType up = point3D_mul_matrix(point3D(0.0001, 0.0001, IntType(1)), self->matrix);
						global::taa_matrix = MAT_IDENTITY;
						global::render_alpha_hash = global::_app->project_render_shadows_transparent;
						global::render_alpha_hash_force = true;
						...
						surface_set_target((IntType)(global::render_surface_spot_buffer));
					{
						gpu_set_blendmode_ext(bm_one, bm_zero);
						draw_clear(c_white);
						render_world_start_light(self->world_pos, lookat, sampleoffset, self->id, up);
						if (self->shadows > 0)
	...
	RealType render_set_texture(VarType tex, StringType type)
	{
		...
		sampler = DsMap(idInt(global::render_shader_obj, sampler_map)).Value(/*"uTexture"*/ STR(1907) + type);
		scalex = IntType(1);
		scaley = IntType(1);
