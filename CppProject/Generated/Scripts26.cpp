...
namespace CppProject
{
	...
	BoolType draw_button_menu(ScopeAny self, VarArgs argument)
	{
		...
		if (sVar(menu_model_current) != null_)
		{
			...
			{
				cap = minecraft_asset_get_name(/*"blockstate"*/ STR(790), name);
				name = /*"blockstate"*/ STR(790) + name;
			}
			else
				cap = text_get({ name });
	...
	RealType draw_button_transition(ScopeAny self, VarType xx, VarType yy, VarType transition)
	{
		...
		tipname = transition;
		if (tipname != /*"linear"*/ STR(562) && tipname != /*"instant"*/ STR(563) && tipname != /*"bezier"*/ STR(564))
		{
		...
		if (transition == /*"bezier"*/ STR(564) && !(sReal(setting_advanced_mode) > 0))
			return IntType(0);
	...
	RealType draw_keybind(ScopeAny self, IntType keybindID, VarType xx, VarType yy)
	{
		...
		draw_label(self, { text_get({ name }) + /*":"*/ STR(782), sVar(dx), sVar(dy) + (sReal(tab_control_h) / 2.0), fa_left, fa_middle, textcolor, textalpha, sInt(font_label) });
		...
		if (draw_button_icon(self, { name + /*"edit"*/ STR(116), sVar(dx) + w - IntType(24), sVar(dy) + IntType(2), IntType(24), IntType(24), sVar(window_busy) == name, icons_PENCIL, null_, false, /*"tooltipeditkeybind"*/ STR(1084) }))
		{
