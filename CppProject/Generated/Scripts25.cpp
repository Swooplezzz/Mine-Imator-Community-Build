...
namespace CppProject
{
	...
	RealType context_menu_draw_level(ScopeAny self, IntType argument0)
	{
		...
		if (sStr(context_menu_ani) == /*""*/ STR(0) || sStr(context_menu_ani) == /*"open"*/ STR(919))
		{
		...
		aniease = ease(/*"easeoutexpo"*/ STR(581), ObjType(obj_context_menu_level, level)->ani);
		alphaease = aniease;
		aniease = IntType(1);
	...
	VarType debug(VarArgs argument)
	{
		...
		cap = string_repeat(/*"\t"*/ STR(1040), global::debug_indent) + string(argument[IntType(0)]);
		valstr = /*""*/ STR(0);
	...
	void debug_ds_list(VarType argument0)
	{
		...
		debug({ /*"Elements"*/ STR(1043), ds_list_size((IntType)(list)) });
		for (IntType i = IntType(0); i < ds_list_size(list); i++)
			debug({ /*"    "*/ STR(1044) + string(DsList(list).Value(i)) });
	}
	...
	void debug_ds_map(IntType map)
	{
		...
		debug({ /*"Elements"*/ STR(1043), ds_list_size(keylist) });
		for (IntType i = IntType(0); i < ds_list_size(keylist); i++)
			debug({ /*"   "*/ STR(1045) + string(DsList(keylist).Value(i)) + /*" => "*/ STR(1046) + string(DsMap(map).Value(DsList(keylist).Value(i))) });
	...
	RealType debug_info_draw(ScopeAny self)
	{
		...
		str += /*"fps: "*/ STR(1048) + string(gmlGlobal::fps) + /*" \n"*/ STR(1049);
		str += /*"fps_real: "*/ STR(1050) + string(gmlGlobal::fps_real) + /*" \n"*/ STR(1049);
		str += /*"delta: "*/ STR(1051) + string(global::delta) + /*" \n"*/ STR(1049);
		str += /*"DPI: "*/ STR(1052) + string(display_get_dpi_x()) + /*","*/ STR(1053) + string(display_get_dpi_y()) + /*" \n"*/ STR(1049);
		str += /*"instance_count: "*/ STR(1054) + string(gmlGlobal::instance_count) + /*" \n"*/ STR(1049);
		str += /*"render_world() calls: "*/ STR(1055) + string(global::render_world_count) + /*" \n"*/ STR(1049);
		str += /*"Vertex buffer triangles: "*/ STR(1056) + string(get_vertex_buffer_triangles()) + /*" \n"*/ STR(1049);
		str += /*"Vertex buffer render calls: "*/ STR(1057) + string(get_vertex_buffer_render_calls()) + /*" \n"*/ STR(1049);
		str += /*"Primitive lines: "*/ STR(1058) + string(get_primitive_lines()) + /*" \n"*/ STR(1049);
		str += /*"Primitive triangles: "*/ STR(1059) + string(get_primitive_triangles()) + /*" \n"*/ STR(1049);
		str += /*"Primitive render calls: "*/ STR(1060) + string(get_primitive_render_calls()) + /*" \n"*/ STR(1049);
		str += /*"[F12 to disable]"*/ STR(1061);
	...
	{
		debug({ str, string(current_time() - global::debug_timer) + /*" msec"*/ STR(1062) });
	}
	...
	{
		return filename_name(filename_dir(dir + /*".ext"*/ STR(1064))) + /*"/"*/ STR(16);
	}
	...
	StringType dir_get_string(IntType dir)
	{
		switch ((IntType)dir)
		{
			...
			case e_dir_SOUTH:
			{
				return /*"south"*/ STR(896);
			}
			case e_dir_NORTH:
			{
				return /*"north"*/ STR(897);
			}
			case e_dir_UP:
			{
				return /*"up"*/ STR(913);
	...
	void draw_bezier_graph(ScopeAny self, VarType xx, VarType yy, RealType wid, RealType hei, ArrType points, BoolType sync)
	{
		...
		if (app_mouse_box(self, easeinxpos - IntType(5), easeinypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereasein"*/ STR(1065);
		...
		if (app_mouse_box(self, easeoutxpos - IntType(5), easeoutypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereaseout"*/ STR(1066);
				sVar(handle_drag_offset_x) = gmlGlobal::mouse_x - easeoutxpos;
				sVar(handle_drag_offset_y) = gmlGlobal::mouse_y - easeoutypos;
			}
		}
		...
		if (sVar(window_busy) == /*"beziereasein"*/ STR(1065))
		{
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (sVar(window_busy) == /*"beziereaseout"*/ STR(1066))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (!(sInt(mouse_still) > 0))
			{
	...
	RealType draw_button_accent(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, IntType index)
	{
		...
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		microani_set(/*"accentlistitem"*/ STR(1067) + string(index), null_, mouseon, mouseclick, sInt(setting_accent) == index, 0.5);
		...
		else
		{
			...
			tip_set(self, text_get({ /*"tooltipcustomaccentcolor"*/ STR(1068) }), xx, yy, width, height);
		}
	...
	RealType draw_button_collapse(ScopeAny self, StringType name, VarType open, IntType script, VarType active, StringType caption, StringType tip)
	{
		...
		if (xx + wid < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || sVar(dy) + IntType(24) < sVar(content_y) || sVar(dy) > sVar(content_y) + sVar(content_height))
			return IntType(0);
		draw_button_icon(self, { name + /*"collapse"*/ STR(1069), xx, yy, IntType(20), IntType(20), open > 0 && active > 0, null_, null_, !(active > 0), /*""*/ STR(0), ID_spr_chevron_ani });
	...
	void draw_button_color(ScopeAny self, VarType name, VarType xx, VarType yy, VarType w, VarType color, IntType def, BoolType hsvmode, IntType script)
	{
		...
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		context_menu_area(self, { xx, yy, w, h, /*"contextmenuvalue"*/ STR(1070), color, e_context_type_COLOR, script, def });
		...
		if (mouseon && sBool(mouse_left_released))
		{
			...
			if (hsvmode)
				ObjType(obj_colorpicker, sInt(colorpicker))->mode = /*"hsv"*/ STR(1026);
		}
	}
