...
namespace CppProject
{
	RealType block_tile_entity_skull(Scope<obj_builder_thread> self, VarType map)
	{
		...
		VarType texlist;
		mapname = /*"Owner"*/ STR(979);
		if (!ds_map_valid(DsMap(map).Value(mapname)))
		{
			mapname = /*"SkullOwner"*/ STR(980);
			if (!ds_map_valid(DsMap(map).Value(mapname)))
				return IntType(0);
		}
		map = DsMap(map).Value(mapname);
		ownerid = string(DsMap(map).Value(/*"Id"*/ STR(981)));
		map = DsMap(map).Value(/*"Properties"*/ STR(982));
		if (!ds_map_valid(map))
			return IntType(0);
		...
		texvalue = base64_decode(DsMap(map).Value(/*"Value"*/ STR(983)));
		...
		map = DsMap(map).Value(/*"SKIN"*/ STR(984));
		texurl = DsMap(map).Value(/*"url"*/ STR(985)).Str();
	...
	BoolType builder_read_schematic(Scope<obj_builder> self, VarType map)
	{
		if (!ds_map_valid(map))
		{
			log({ /*"Schematic error"*/ STR(986), /*"Not a schematic file"*/ STR(987) });
			return false;
		}
		self->builder_scenery_legacy = is_undefined(DsMap(map).Value(/*"Palette"*/ STR(988)));
		self->build_size_x = DsMap(map).Value(/*"Width"*/ STR(989));
		self->build_size_y = DsMap(map).Value(/*"Length"*/ STR(990));
		self->build_size_z = DsMap(map).Value(/*"Height"*/ STR(991));
		log({ /*"Size"*/ STR(59), string(self->build_size_x) + /*" x "*/ STR(992) + string(self->build_size_y) + /*" x "*/ STR(992) + string(self->build_size_z) });
		if (is_undefined(self->build_size_x) || is_undefined(self->build_size_y) || is_undefined(self->build_size_z))
		{
			log({ /*"Schematic error"*/ STR(986), /*"Size not fully defined"*/ STR(993) });
			return false;
		}
		if (self->build_size_x <= IntType(0) || self->build_size_y <= IntType(0) || self->build_size_z <= IntType(0))
		{
			log({ /*"Schematic error"*/ STR(986), /*"Size cannot be 0"*/ STR(994) });
			return false;
		}
		if (!self->builder_scenery_legacy)
		{
			VarType version = DsMap(map).Value(/*"Version"*/ STR(995));
			if (is_undefined(version))
			{
				log({ /*"Schematic error"*/ STR(986), /*"Version not available"*/ STR(996) });
				return false;
			}
			log({ /*"Version"*/ STR(995), DsMap(map).Value(/*"Version"*/ STR(995)) });
			if (version > IntType(1))
			{
				log({ /*"Schematic error"*/ STR(986), /*"Unsupported format, version too high"*/ STR(997) });
				return false;
			}
			VarType palettemap = DsMap(map).Value(/*"Palette"*/ STR(988));
			if (!ds_map_valid(palettemap))
			{
				log({ /*"Schematic error"*/ STR(986), /*"Palette not found"*/ STR(998) });
				return false;
			}
			...
			while (!is_undefined(key))
			{
				if (!string_contains(key, /*"_NBT_"*/ STR(999)))
				{
					VarType index;
					IntType bracketindex;
					index = DsMap(palettemap).Value(key);
					bracketindex = string_pos(/*"["*/ STR(1000), key);
					if (bracketindex > IntType(0))
					{
						...
						if (!is_undefined(block))
						{
							...
							if (state_vars_get_value(vars, /*"waterlogged"*/ STR(831)) != /*"false"*/ STR(773))
								if (ObjType(obj_block, block)->waterlogged > 0 || state_vars_get_value(vars, /*"waterlogged"*/ STR(831)) == /*"true"*/ STR(772))
			...
			self->sch_blockdata_array = DsMap(map).Value(/*"BlockData"*/ STR(1001));
			if (is_undefined(self->sch_blockdata_array))
			{
				log({ /*"Schematic error"*/ STR(986), /*"BlockData array not found"*/ STR(1002) });
				return false;
			}
			self->sch_blockdata_ints = (DsMap(map).Value(/*"BlockData_NBT_type"*/ STR(1003)) == e_nbt_TAG_INT_ARRAY);
			debug({ /*"blockdataints"*/ STR(1004), self->sch_blockdata_ints });
			VarType metadata = DsMap(map).Value(/*"Metadata"*/ STR(1005));
			if (ds_map_valid(metadata))
				self->file_map = DsMap(metadata).Value(/*"FromMap"*/ STR(1006));
		}
		else
		{
			self->sch_legacy_blocksarray = DsMap(map).Value(/*"Blocks"*/ STR(1007));
			if (is_undefined(self->sch_legacy_blocksarray))
			{
				log({ /*"Schematic error"*/ STR(986), /*"Blocks array not found"*/ STR(1008) });
				return false;
			}
			self->sch_legacy_dataarray = DsMap(map).Value(/*"Data"*/ STR(1009));
			if (is_undefined(self->sch_legacy_dataarray))
			{
				log({ /*"Schematic error"*/ STR(986), /*"Data array not found"*/ STR(1010) });
				return false;
			}
			self->file_map = DsMap(map).Value(/*"FromMap"*/ STR(1006));
		}
		
		if (is_undefined(self->file_map))
			self->file_map = /*""*/ STR(0);
		self->sch_tileentity_list = DsMap(map).Value(/*"TileEntities"*/ STR(1011));
		return true;
	}
	...
	BoolType builder_read_schematic_nbt(ScopeAny self, VarType structuremap)
	{
		...
		if (!is_undefined(DsMap(structuremap).Value(/*"DataVersion"*/ STR(1012))))
			structureversion = DsMap(structuremap).Value(/*"DataVersion"*/ STR(1012));
		if (structureversion < IntType(2000))
		{
			log({ /*"Structure error"*/ STR(1013), /*"Unsupported format, version too low"*/ STR(1014) });
			return false;
		}
		sizemap = DsMap(structuremap).Value(/*"size"*/ STR(1015));
		...
		{
			log({ /*"Structure error"*/ STR(1013), /*"Size cannot be 0"*/ STR(994) });
			return false;
		}
		VarType paletteslist, palettelist;
		paletteslist = DsMap(structuremap).Value(/*"palettes"*/ STR(1016));
		...
		{
			palettelist = DsMap(structuremap).Value(/*"palette"*/ STR(1017));
			if (!ds_list_valid(palettelist))
			{
				log({ /*"Structure error"*/ STR(1013), /*"Palette not found"*/ STR(998) });
				return false;
			}
		}
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(palettelist)); i++)
		{
			...
			mcid = DsMap(blockmap).Value(/*"Name"*/ STR(1018));
			...
			if (!is_undefined(block))
			{
				...
				if (ObjType(obj_block, block)->id_state_vars_map != null_ && is_array(DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid)))
					state_vars_add(VarType::CreateRef(vars), DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid));
				propertiesmap = DsMap(blockmap).Value(/*"Properties"*/ STR(982));
		...
		VarType blocklist = DsMap(structuremap).Value(/*"blocks"*/ STR(1019));
		if (!ds_list_valid(blocklist))
		{
			log({ /*"Structure error"*/ STR(1013), /*"Block list not found"*/ STR(1020) });
			return false;
		}
		withOne (obj_builder, global::mc_builder, self->id)
		{
			...
			for (IntType i = IntType(0); i < ds_list_size((IntType)(blocklist)); i++)
			{
				...
				pos = DsMap(blockmap).Value(/*"pos"*/ STR(1021));
				state = DsMap(blockmap).Value(/*"state"*/ STR(1022));
				...
				blocknbt = DsMap(blockmap).Value(/*"nbt"*/ STR(1023));
				if (!is_undefined(blocknbt))
				{
					VarType finalstate, script;
					finalstate = DsMap(blocknbt).Value(/*"final_state"*/ STR(1024));
				...
				if (entity != null_)
				{
					self->script = asset_get_index(/*"block_tile_entity_"*/ STR(1025) + string_replace(string_lower(entity), /*"minecraft:"*/ STR(834), /*""*/ STR(0)));
		...
		debug_timer_stop(/*"res_load_scenery, Parse blocks"*/ STR(1026));
		return true;
	}
		...
		if (ds_list_valid(self->sch_tileentity_list))
		{
			...
			withOne (obj_builder_thread, DsList(self->thread_list).Value(IntType(0)), self->id)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(ObjType(obj_builder, self.otherId)->sch_tileentity_list)); i++)
				{
					...
					if (!self->builder_scenery_legacy)
					{
						eid = DsMap(entity).Value(/*"Id"*/ STR(981));
						VarType poslist = DsMap(entity).Value(/*"Pos"*/ STR(1028));
					...
					else
					{
						...
						ex = DsMap(entity).Value(/*"x"*/ STR(846));
						ey = DsMap(entity).Value(/*"z"*/ STR(847));
						ez = DsMap(entity).Value(/*"y"*/ STR(829));
					}
					
					if (is_string(eid))
					{
						VarType script = asset_get_index(/*"block_tile_entity_"*/ STR(1025) + string_replace(string_lower(eid), /*"minecraft:"*/ STR(834), /*""*/ STR(0)));
						if (script > -IntType(1))
						{
							self->build_pos_x = ex;
							self->build_pos_y = ey;
				...
				debug_timer_stop(/*"Parse Tile Entities"*/ STR(1029));
			}
