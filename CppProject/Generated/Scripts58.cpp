...
namespace CppProject
{
	RealType sortlist_draw(ScopeAny self, IntType slist, VarType xx, VarType yy, VarType w, RealType h, VarType select, BoolType filter, VarType name)
	{
		...
		if (filter && name == /*""*/ STR(0))
		{
			...
			if ((sStr(settings_menu_name) == /*"listfilter"*/ STR(3031) + string(slist)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(751))
		...
		if (draw_textfield(self, /*"listsearch"*/ STR(3033) + string(slist), searchx, yy, searchw, IntType(24), idInt(slist, search_tbx), null_, text_get({ /*"listsearch"*/ STR(3033) }), /*"none"*/ STR(909)))
		{
		...
		if (sVar(window_focus) == string(idInt(slist, scroll)))
		{
			...
			if (!app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && sBool(mouse_left) && sVar(window_busy) != /*"scrollbar"*/ STR(2693))
	...
	RealType sortlist_filters_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(typelist); i++)
		{
			...
			{
				if (itemname == /*"packunzipped"*/ STR(566) || itemname == /*"legacyblocksheet"*/ STR(569) || itemname == /*"fromworld"*/ STR(571))
					continue;
			}
			capwid = max({ capwid, string_width(text_get({ /*"type"*/ STR(806) + itemname })) });
			...
			if (draw_checkbox(self, /*"type"*/ STR(806) + itemname, sVar(dx), sVar(dy) + scroll, active, null_))
			{
				if (active)
					...
					for (IntType i = IntType(0); i < array_length(VarType::CreateRef(ObjType(obj_block, block)->default_state)) && !match; i += IntType(2))
					{
						...
						for (IntType s = IntType(0); s < idReal(statecurrent, value_amount) && !match; s++)
							if (string_count(check, string_lower(minecraft_asset_get_name(/*"blockstatevalue"*/ STR(5), idVar(statecurrent, value_name).Value(s)))) > IntType(0))
								match = true;
						if (string_count(check, string_lower(text_get({ /*"blockstate"*/ STR(797) + state }))) > IntType(0))
							match = true;
					}
						...
						for (IntType i = IntType(0); i < array_length(VarType::CreateRef(ObjType(obj_model, model)->default_state)) && !match; i += IntType(2))
						{
							...
							for (IntType s = IntType(0); s < idReal(statecurrent, value_amount) && !match; s++)
								if (string_count(check, string_lower(minecraft_asset_get_name(/*"modelstatevalue"*/ STR(9), idVar(statecurrent, value_name).Value(s)))) > IntType(0))
									match = true;
							if (string_count(check, string_lower(text_get({ /*"modelstate"*/ STR(776) + state }))) > IntType(0))
								match = true;
						}
	...
	{
		return string_replace_all(string_upper(string_char_at(str, IntType(1))) + string_delete(str, IntType(1), IntType(1)), /*"_"*/ STR(1153), /*" "*/ STR(21));
	}
	...
	VarType string_get_real(VarType str, VarType inv)
	{
		...
		if (string_replace(string_replace(str, /*"-"*/ STR(1132), /*""*/ STR(0)), /*"."*/ STR(1130), /*""*/ STR(0)) == /*""*/ STR(0))
			return inv;
	...
	VarType string_get_state_vars(VarType argument0)
	{
		...
		IntType varslen;
		str = argument0;
		vars = string_split(str, /*","*/ STR(1070));
	...
	StringType string_limit_ext(VarType str, VarType w, VarType h)
	{
		...
		for (c = IntType(1); c <= string_length(str); c++)
		{
			...
			{
				if (char_ == /*" "*/ STR(21) || char_ == /*", "*/ STR(1057) || char_ == /*"."*/ STR(1130) || char_ == /*"/"*/ STR(20) || char_ == /*"-"*/ STR(1132))
	...
	ArrType string_split(VarType str, StringType sep)
	{
		...
		while (true)
		{
			...
			if (pos == IntType(0))
				break;
			IntType escapepos = string_pos(/*"\\"*/ STR(1076) + sep, str);
