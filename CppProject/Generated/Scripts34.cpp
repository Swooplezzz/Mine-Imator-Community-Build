...
namespace CppProject
{
	...
	RealType menu_settings_draw(ScopeAny self)
	{
		...
		if (sStr(settings_menu_name) == /*""*/ STR(0))
			return IntType(0);
		VarType settingsmenuease = ease(((sStr(settings_menu_ani_type) == /*"show"*/ STR(747)) ? /*"easeoutexpo"*/ STR(581) : /*"easeinexpo"*/ STR(580)), sVar(settings_menu_ani));
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			switch (StringType(global::load_assets_stage).id)
			{
				...
				case 1705: // biomes
				{
					...
					global::load_assets_stage = /*"textures"*/ STR(837);
				...
				case 837: // textures
				{
					...
					{
						if (string_char_at(DsMap(self->block_texture_color_map).Value(key), IntType(1)) == /*"#"*/ STR(854))
					...
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(537))))
					{
					...
					withOne (obj_resource, global::mc_res, self->id)
					{
						...
						minecraft_assets_load_particles(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(537)));
				...
				case 1009: // blocks
				{
					...
					{
						DsMap(self->block_name_map)[/*"flowing_water"*/ STR(1734)] = DsMap(self->block_name_map).Value(/*"water"*/ STR(812));
						DsMap(self->block_name_map)[/*"flowing_lava"*/ STR(1735)] = DsMap(self->block_name_map).Value(/*"lava"*/ STR(1736));
						DsMap(self->block_liquid_slot_map)[/*"water"*/ STR(812)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_still"*/ STR(1737));
						...
						while (!is_undefined(key))
						{
							...
							if (!is_undefined(newid))
								newidnomc = string_replace(DsMap(curmap).Value(/*"id"*/ STR(82)), /*"minecraft:"*/ STR(833), /*""*/ STR(0));
							else
								newidnomc = /*""*/ STR(0);
						...
						if (dev_mode_debug_unused)
						{
							...
							filesarr = file_find(global::load_assets_dir + mc_models_directory + /*"block/"*/ STR(816), /*".json"*/ STR(724));
	...
	void minecraft_assets_load_biomes(IntType list, VarType map)
	{
		for (IntType b = IntType(0); b < ds_list_size((IntType)(map)); b++)
		{
			...
			{
				biomeid = self->id;
				self->name = DsMap(biome).Value(/*"name"*/ STR(814));
				self->group = value_get_real(DsMap(biome).Value(/*"group"*/ STR(1749)), false);
				self->txy = vec2(IntType(0));
				if (is_string(DsMap(biome).Value(/*"foliage"*/ STR(859))))
				{
					self->color_foliage = hex_to_color(DsMap(biome).Value(/*"foliage"*/ STR(859)));
				...
				else
				{
					...
					self->txy = value_get_point2D(DsMap(biome).Value(/*"foliage"*/ STR(859)), vec2(IntType(0), IntType(0)));
					self->hardcoded = false;
				}
				
				if (is_string(DsMap(biome).Value(/*"grass"*/ STR(858))))
					self->color_grass = hex_to_color(DsMap(biome).Value(/*"grass"*/ STR(858)));
				if (is_string(DsMap(biome).Value(/*"water"*/ STR(812))))
					self->color_water = hex_to_color(DsMap(biome).Value(/*"water"*/ STR(812)));
				self->biome_base = null_;
				self->biome_variants = null_;
				self->variants_extend = false;
				if (ds_list_valid(DsMap(biome).Value(/*"variant"*/ STR(810))))
				{
					self->biome_variants = ds_list_create();
					VarType biomevariants = DsMap(biome).Value(/*"variant"*/ STR(810));
					for (IntType v = IntType(0); v < ds_list_size((IntType)(biomevariants)); v++)
					{
						...
						{
							self->name = DsMap(variant).Value(/*"name"*/ STR(814));
							self->group = false;
							...
							if (is_string(DsMap(variant).Value(/*"foliage"*/ STR(859))))
							{
								self->color_foliage = hex_to_color(DsMap(variant).Value(/*"foliage"*/ STR(859)));
								self->hardcoded = true;
							}
							else
								if (ds_list_valid(DsMap(variant).Value(/*"foliage"*/ STR(859))))
								{
									self->color_foliage = c_white;
									self->txy = value_get_point2D(DsMap(variant).Value(/*"foliage"*/ STR(859)), vec2(IntType(0), IntType(0)));
									self->hardcoded = false;
								}
							
							if (is_string(DsMap(variant).Value(/*"grass"*/ STR(858))))
								self->color_grass = hex_to_color(DsMap(variant).Value(/*"grass"*/ STR(858)));
							else
								self->color_grass = ObjType(obj_biome, self.otherId)->color_grass;
							
							if (is_string(DsMap(variant).Value(/*"water"*/ STR(812))))
								self->color_water = hex_to_color(DsMap(variant).Value(/*"water"*/ STR(812)));
							else
		...
		while (!is_undefined(key))
		{
			switch (StringType(key).id)
			{
				...
				default:
				{
					...
					if (newid != null_)
						newidnomc = string_replace(newid, /*"minecraft:"*/ STR(833), /*""*/ STR(0));
					block = null_;
					if (is_string(newid) && !is_undefined(DsMap(self->block_id_map).Value(newid)))
						block = DsMap(self->block_id_map).Value(newid);
