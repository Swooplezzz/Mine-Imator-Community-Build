...
namespace CppProject
{
	...
	{
		StringType fn = global::load_assets_dir + /*"pack.mcmeta"*/ STR(2665);
		if (file_exists_lib(fn))
		{
			IntType map = json_load({ fn });
			if (!ds_map_valid(map))
			{
				log({ /*"Error loading pack.mcmeta"*/ STR(2666) });
			...
			if (ds_map_valid(DsMap(map).Value(/*"pack"*/ STR(565))))
			{
				...
				sVar(pack_format) = value_get_real(DsMap(packmap).Value(/*"pack_format"*/ STR(2667)));
			}
		}
		else
			sVar(pack_format) = e_minecraft_pack_LATEST;
	...
	RealType res_load_scenery(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 925: // open
			{
				debug({ /*"res_load_scenery"*/ STR(2668), /*"open"*/ STR(925) });
				if (sVar(type) == e_res_type_FROM_WORLD)
				{
					if (!res_load_scenery_world(self))
					{
						error(/*"errorloadworld"*/ STR(2669));
				...
				else
				{
					...
					{
						log({ /*"Loading "*/ STR(2607) + ext, fname });
						debug_timer_start();
						...
						if (rootmap == null_)
							break;
						debug_timer_stop(/*"res_load_scenery, Parse NBT"*/ STR(2670));
						if (dev_mode_debug_schematics)
							nbt_debug_tag_compound(/*"root"*/ STR(2225), rootmap);
						if (ext == /*".schematic"*/ STR(727))
						{
							withOne (obj_builder, global::mc_builder, self->id)
							{
								openerr = !builder_read_schematic(self, DsMap(rootmap).Value(/*"Schematic"*/ STR(2671)));
								if (openerr)
									break;
					...
					{
						log({ /*"Loading .blocks"*/ STR(2672), fname });
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadsceneryblocks"*/ STR(2673) });
					if (ObjType(obj_builder, global::mc_builder)->file_map != /*""*/ STR(0))
						ObjType(obj_popup, global::_app->popup_loading)->caption = text_get({ /*"loadscenerycaptionpieceof"*/ STR(2674), ObjType(obj_builder, global::mc_builder)->file_map });
					else
						ObjType(obj_popup, global::_app->popup_loading)->caption = text_get({ /*"loadscenerycaption"*/ STR(2675), idVar(self.otherId, filename) });
				...
				if (sVar(scenery_tl_add) == null_)
				{
					if (ObjType(obj_builder, global::mc_builder)->sch_timeline_amount > IntType(500))
						...
						if (ObjType(obj_builder, global::mc_builder)->sch_timeline_amount > IntType(20))
							sVar(scenery_tl_add) = question(text_get({ /*"loadsceneryaddtimelines"*/ STR(2676), ObjType(obj_builder, global::mc_builder)->sch_timeline_amount }));
						else
			...
			case 1024: // blocks
			{
				...
				{
					debug_timer_stop(/*"res_load_scenery, Set models"*/ STR(2677));
					debug_timer_start();
					...
					ObjType(obj_builder, global::mc_builder)->build_pos = IntType(0);
					withOne (app, global::_app->id, self->id)
						ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadscenerymodel"*/ STR(2678) });
			...
			case 8: // model
			{
				...
				if (ObjType(obj_builder, global::mc_builder)->build_pos == ObjType(obj_builder, global::mc_builder)->build_size_total)
				{
					...
					debug_timer_stop(/*"res_load_scenery, Generate models"*/ STR(2679));
					...
					sBool(ready) = true;
					if (ObjType(obj_builder, global::mc_builder)->file_map != /*""*/ STR(0))
						sVar(display_name) = text_get({ /*"loadscenerypieceof"*/ STR(2680), ObjType(obj_builder, global::mc_builder)->file_map });
					res_save_block_cache(self, global::_app->project_folder + /*"/"*/ STR(20) + sVar(filename) + /*".meshcache"*/ STR(2609));
		...
		if (openerr)
		{
			error(/*"errorloadschematic"*/ STR(2681));
			buffer_delete(global::buffer_current);
			withOne (app, global::_app->id, self->id)
				load_next(ScopeAny(self));
	...
	RealType res_load_skulls(ScopeAny self)
	{
		...
		ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadscenerydownload"*/ STR(2682), ObjType(obj_builder, global::mc_builder)->block_skull_finish_count, ObjType(obj_builder, global::mc_builder)->block_skull_texture_count, ObjType(obj_builder, global::mc_builder)->block_skull_fail_count });
		withOne (obj_builder, global::mc_builder, self->id)
		{
			...
			if (self->block_skull_texture != null_ || self->block_skull_texture_fail == true || (current_time() - self->block_skull_download_time > IntType(3000)))
			{
				...
				else
				{
					...
					log({ /*"Failed to download texture"*/ STR(2683), self->block_skull_texture_name });
	...
	void res_load_start(ScopeAny self)
	{
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_FROM_WORLD:
			{
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadsceneryopen"*/ STR(2684) });
			...
			case e_res_type_SOUND:
			{
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadaudioread"*/ STR(2685) });
			...
			case e_res_type_PACK_UNZIPPED:
			{
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackunzip"*/ STR(2686) });
					ObjType(obj_popup, global::_app->popup_loading)->caption = idVar(self.otherId, filename);
	...
	{
		draw_clear_alpha(c_black, 0.0);
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/stone"*/ STR(2687)), IntType(0), IntType(0), size, size, block_sheet_width, block_sheet_height);
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/grass_side"*/ STR(2688)), IntType(16), IntType(0), size, size, block_sheet_width, block_sheet_height);
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/tnt_side"*/ STR(2689)), IntType(0), IntType(16), size, size, block_sheet_width, block_sheet_height);
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/sapling_oak"*/ STR(2690)), IntType(16), IntType(16), size, size, block_sheet_width, block_sheet_height);
	}
		surface_reset_target();
	...
	RealType res_update_colors(ScopeAny self, VarArgs argument)
	{
		...
		else
		{
			...
			sVar(color_leaves_spruce) = hex_to_color(/*"62A857"*/ STR(2691));
	...
	void res_update_display_name(Scope<obj_resource> self)
	{
		...
		if (self->type == e_res_type_DOWNLOADED_SKIN)
			self->display_name = text_get({ /*"downloadskinname"*/ STR(2692), self->display_name });
	...
	RealType scrollbar_draw(ScopeAny self, IntType sb, IntType dir, VarType xx, VarType yy, VarType size, VarType maxsize)
	{
		...
		if (dir == e_scroll_HORIZONTAL)
			ObjType(value_animation, idInt(sb, mousenear))->value = (app_mouse_box(self, xx, yy - nearsize, size, areasize + nearsize) && sBool(content_mouseon)) || (sVar(window_focus) == string(sb) && sVar(window_busy) == /*"scrollbar"*/ STR(2693));
		else
			ObjType(value_animation, idInt(sb, mousenear))->value = (app_mouse_box(self, xx - nearsize, yy, areasize + nearsize, size) && sBool(content_mouseon)) || (sVar(window_focus) == string(sb) && sVar(window_busy) == /*"scrollbar"*/ STR(2693));
		...
		if (mouseinarea)
		{
			...
			if (!sBool(mouse_left_pressed) && sBool(mouse_left) && !mouseinbar && ((dir > 0) ? (gmlGlobal::mouse_x < (xx + barpos) || gmlGlobal::mouse_x > (xx + barpos + barsize)) : (gmlGlobal::mouse_y < (yy + barpos) || gmlGlobal::mouse_y > (yy + barpos + barsize))))
			{
				...
				if (sBool(mouse_left) && mouseinbar)
				{
					...
					sVar(window_busy) = /*"scrollbar"*/ STR(2693);
				}
		...
		{
			if (sVar(window_busy) == /*"scrollbar"*/ STR(2693))
			{
				sInt(mouse_cursor) = cr_handpoint;
				if (!sBool(mouse_left))
				{
		...
		if (sVar(window_focus) == string(sb) && sVar(window_busy) == /*"scrollbar"*/ STR(2693))
			idVar(sb, value_goal) = idVar(sb, value);
		barpos = min({ size - barsize, floor(idVar(sb, value) * ((RealType)size / maxsize)) });
		pressed = (sVar(window_busy) == /*"scrollbar"*/ STR(2693) && sVar(window_focus) == string(sb));
		if (dir == e_scroll_HORIZONTAL)
		{
	...
	RealType settings_load(ScopeAny self)
	{
		...
		{
			log({ /*"Loading settings"*/ STR(2694), fn });
			IntType map = json_load({ fn });
			if (!ds_map_valid(map))
				return IntType(0);
			...
			if (ds_map_valid(assetsmap))
			{
				...
				VarType newmap = DsMap(assetsmap).Value(/*"new"*/ STR(2695));
			...
			VarType toastslist = DsMap(map).Value(/*"closed_toasts"*/ STR(2696));
			if (ds_list_valid(toastslist))
				ds_list_copy(sInt(closed_toast_list), (IntType)(toastslist));
			VarType programmap = DsMap(map).Value(/*"program"*/ STR(2697));
			if (ds_map_valid(programmap))
			{
				sReal(setting_advanced_mode) = value_get_real(DsMap(programmap).Value(/*"advanced_mode"*/ STR(2698)), sReal(setting_advanced_mode));
				gmlGlobal::room_speed = value_get_real(DsMap(programmap).Value(/*"fps"*/ STR(2699)), gmlGlobal::room_speed);
				if (!dev_mode)
					sStr(setting_project_folder) = value_get_string(DsMap(programmap).Value(/*"project_folder"*/ STR(2700)), sStr(setting_project_folder)).Str();
				...
				sReal(setting_backup_time) = value_get_real(DsMap(programmap).Value(/*"backup_time"*/ STR(2701)), sReal(setting_backup_time));
				sReal(setting_backup_amount) = value_get_real(DsMap(programmap).Value(/*"backup_amount"*/ STR(2702)), sReal(setting_backup_amount));
				sVar(setting_spawn_cameras) = value_get_real(DsMap(programmap).Value(/*"spawn_cameras"*/ STR(2703)), sVar(setting_spawn_cameras));
				sReal(setting_unlimited_values) = value_get_real(DsMap(programmap).Value(/*"unlimited_values"*/ STR(2704)), sReal(setting_unlimited_values));
				sVar(setting_watermark_custom) = value_get_real(DsMap(programmap).Value(/*"watermark_custom"*/ STR(2705)), sVar(setting_watermark_custom));
				sStr(setting_watermark_fn) = value_get_string(DsMap(programmap).Value(/*"watermark_fn"*/ STR(2706)), sStr(setting_watermark_fn)).Str();
				sVar(setting_watermark_halign) = value_get_string(DsMap(programmap).Value(/*"watermark_halign"*/ STR(2707)), sVar(setting_watermark_halign));
				sVar(setting_watermark_valign) = value_get_string(DsMap(programmap).Value(/*"watermark_valign"*/ STR(2708)), sVar(setting_watermark_valign));
				sReal(setting_watermark_padding) = value_get_real(DsMap(programmap).Value(/*"watermark_padding"*/ STR(2709)), sReal(setting_watermark_padding));
				sReal(setting_watermark_scale) = value_get_real(DsMap(programmap).Value(/*"watermark_scale"*/ STR(2710)), sReal(setting_watermark_scale));
				sReal(setting_watermark_opacity) = value_get_real(DsMap(programmap).Value(/*"watermark_opacity"*/ STR(2711)), sReal(setting_watermark_opacity));
				if (sStr(setting_watermark_fn) != /*""*/ STR(0))
					sInt(setting_watermark_image) = texture_create(sStr(setting_watermark_fn));
			}
			VarType interfacemap = DsMap(map).Value(/*"interface"*/ STR(2712));
			if (ds_map_valid(interfacemap))
			{
				sStr(setting_language_filename) = value_get_string(DsMap(interfacemap).Value(/*"language_filename"*/ STR(2713)), sStr(setting_language_filename)).Str();
				...
				themename = value_get_string(DsMap(interfacemap).Value(/*"theme"*/ STR(2714)), themename);
				...
				sInt(setting_accent) = value_get_real(DsMap(interfacemap).Value(/*"accent"*/ STR(2715)), sInt(setting_accent));
				sVar(setting_accent_custom) = value_get_color(DsMap(interfacemap).Value(/*"accent_custom"*/ STR(2716)), sVar(setting_accent_custom));
				sVar(setting_timeline_autoscroll) = value_get_real(DsMap(interfacemap).Value(/*"timeline_autoscroll"*/ STR(2717)), sVar(setting_timeline_autoscroll));
				sBool(setting_timeline_show_markers) = value_get_real(DsMap(interfacemap).Value(/*"timeline_show_markers"*/ STR(2718)), sBool(setting_timeline_show_markers));
				sVar(setting_interface_compact) = value_get_real(DsMap(interfacemap).Value(/*"interface_compact"*/ STR(2719)), sVar(setting_interface_compact));
				sReal(setting_timeline_compact) = value_get_real(DsMap(interfacemap).Value(/*"timeline_compact"*/ STR(2720)), sReal(setting_timeline_compact));
				sVar(setting_reduced_motion) = value_get_real(DsMap(interfacemap).Value(/*"reduced_motion"*/ STR(2721)), sVar(setting_reduced_motion));
				sVar(setting_timeline_select_jump) = value_get_real(DsMap(interfacemap).Value(/*"timeline_select_jump"*/ STR(2722)), sVar(setting_timeline_select_jump));
				sVar(setting_timeline_hide_ghosts) = value_get_real(DsMap(interfacemap).Value(/*"timeline_hide_ghosts"*/ STR(2723)), sVar(setting_timeline_hide_ghosts));
				sVar(setting_timeline_frame_snap) = value_get_real(DsMap(interfacemap).Value(/*"timeline_frame_snap"*/ STR(2724)), sVar(setting_timeline_frame_snap));
				sReal(setting_z_is_up) = value_get_real(DsMap(interfacemap).Value(/*"z_is_up"*/ STR(2725)), sReal(setting_z_is_up));
				sBool(setting_show_shortcuts_bar) = value_get_real(DsMap(interfacemap).Value(/*"show_shortcuts_bar"*/ STR(2726)), sBool(setting_show_shortcuts_bar));
				sVar(setting_gizmos_face_camera) = value_get_real(DsMap(interfacemap).Value(/*"gizmos_face_camera"*/ STR(2727)), sVar(setting_gizmos_face_camera));
				sVar(setting_fade_gizmos) = value_get_real(DsMap(interfacemap).Value(/*"fade_gizmos"*/ STR(2728)), sVar(setting_fade_gizmos));
				sVar(setting_camera_lock_mouse) = value_get_real(DsMap(interfacemap).Value(/*"camera_lock_mouse"*/ STR(2729)), sVar(setting_camera_lock_mouse));
				window_mouse_set_permission(sVar(setting_camera_lock_mouse));
				sVar(setting_place_new) = value_get_real(DsMap(interfacemap).Value(/*"place_new"*/ STR(2730)), sVar(setting_place_new));
				sVar(setting_interface_scale_auto) = value_get_real(DsMap(interfacemap).Value(/*"scale_auto"*/ STR(2731)), sVar(setting_interface_scale_auto));
				...
				sVar(setting_separate_tool_modes) = value_get_real(DsMap(interfacemap).Value(/*"separate_tool_modes"*/ STR(2732)), sVar(setting_separate_tool_modes));
				if (sVar(setting_separate_tool_modes) > 0)
				{
					action_tools_disable_all(self);
					sBool(setting_tool_select) = true;
				}
				sVar(setting_panel_left_bottom_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_left_bottom_size"*/ STR(2733)), sVar(setting_panel_left_bottom_size));
				sVar(setting_panel_right_bottom_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_right_bottom_size"*/ STR(2734)), sVar(setting_panel_right_bottom_size));
				sVar(setting_panel_bottom_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_bottom_size"*/ STR(2735)), sVar(setting_panel_bottom_size));
				sVar(setting_panel_top_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_top_size"*/ STR(2736)), sVar(setting_panel_top_size));
				sVar(setting_panel_left_top_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_left_top_size"*/ STR(2737)), sVar(setting_panel_left_top_size));
				sVar(setting_panel_right_top_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_right_top_size"*/ STR(2738)), sVar(setting_panel_right_top_size));
				sVar(setting_properties_location) = value_get_string(DsMap(interfacemap).Value(/*"properties_location"*/ STR(2739)), sVar(setting_properties_location));
				sVar(setting_ground_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"ground_editor_location"*/ STR(2740)), sVar(setting_ground_editor_location));
				sVar(setting_template_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"template_editor_location"*/ STR(2741)), sVar(setting_template_editor_location));
				sVar(setting_timeline_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"timeline_editor_location"*/ STR(2742)), sVar(setting_timeline_editor_location));
				sVar(setting_frame_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"frame_editor_location"*/ STR(2743)), sVar(setting_frame_editor_location));
				sVar(setting_settings_location) = value_get_string(DsMap(interfacemap).Value(/*"settings_location"*/ STR(2744)), sVar(setting_settings_location));
				if (ds_map_valid(DsMap(interfacemap).Value(/*"timeline_window"*/ STR(2745))))
					window_state_restore(e_window_TIMELINE, DsMap(interfacemap).Value(/*"timeline_window"*/ STR(2745)));
				sVar(setting_view_split) = value_get_real(DsMap(interfacemap).Value(/*"view_split"*/ STR(2746)), sVar(setting_view_split));
				sVar(setting_view_main_overlays) = value_get_real(DsMap(interfacemap).Value(/*"view_main_overlays"*/ STR(2747)), sVar(setting_view_main_overlays));
				sVar(setting_view_main_aspect_ratio) = value_get_real(DsMap(interfacemap).Value(/*"view_main_aspect_ratio"*/ STR(2748)), sVar(setting_view_main_aspect_ratio));
				sVar(setting_view_main_grid) = value_get_real(DsMap(interfacemap).Value(/*"view_main_grid"*/ STR(2749)), sVar(setting_view_main_grid));
				sVar(setting_view_main_gizmos) = value_get_real(DsMap(interfacemap).Value(/*"view_main_gizmos"*/ STR(2750)), sVar(setting_view_main_gizmos));
				sVar(setting_view_main_fog) = value_get_real(DsMap(interfacemap).Value(/*"view_main_fog"*/ STR(2751)), sVar(setting_view_main_fog));
				sVar(setting_view_main_effects) = value_get_real(DsMap(interfacemap).Value(/*"view_main_effects"*/ STR(2752)), sVar(setting_view_main_effects));
				sVar(setting_view_main_particles) = value_get_real(DsMap(interfacemap).Value(/*"view_main_particles"*/ STR(2753)), sVar(setting_view_main_particles));
				sVar(setting_view_main_location) = value_get_string(DsMap(interfacemap).Value(/*"view_main_location"*/ STR(2754)), sVar(setting_view_main_location));
				sVar(setting_view_second_show) = value_get_real(DsMap(interfacemap).Value(/*"view_second_show"*/ STR(2755)), sVar(setting_view_second_show));
				sVar(setting_view_second_overlays) = value_get_real(DsMap(interfacemap).Value(/*"view_second_overlays"*/ STR(2756)), sVar(setting_view_second_overlays));
				sVar(setting_view_second_aspect_ratio) = value_get_real(DsMap(interfacemap).Value(/*"view_second_aspect_ratio"*/ STR(2757)), sVar(setting_view_second_aspect_ratio));
				sVar(setting_view_second_grid) = value_get_real(DsMap(interfacemap).Value(/*"view_second_grid"*/ STR(2758)), sVar(setting_view_second_grid));
				sVar(setting_view_second_gizmos) = value_get_real(DsMap(interfacemap).Value(/*"view_second_gizmos"*/ STR(2759)), sVar(setting_view_second_gizmos));
				sVar(setting_view_second_fog) = value_get_real(DsMap(interfacemap).Value(/*"view_second_fog"*/ STR(2760)), sVar(setting_view_second_fog));
				sVar(setting_view_second_effects) = value_get_real(DsMap(interfacemap).Value(/*"view_second_effects"*/ STR(2761)), sVar(setting_view_second_effects));
				sVar(setting_view_second_particles) = value_get_real(DsMap(interfacemap).Value(/*"view_second_particles"*/ STR(2762)), sVar(setting_view_second_particles));
				sVar(setting_view_second_location) = value_get_string(DsMap(interfacemap).Value(/*"view_second_location"*/ STR(2763)), sVar(setting_view_second_location));
				sVar(setting_view_second_width) = value_get_real(DsMap(interfacemap).Value(/*"view_second_width"*/ STR(2764)), sVar(setting_view_second_width));
				sVar(setting_view_second_height) = value_get_real(DsMap(interfacemap).Value(/*"view_second_height"*/ STR(2765)), sVar(setting_view_second_height));
				if (ds_map_valid(DsMap(interfacemap).Value(/*"view_second_window"*/ STR(2766))))
					window_state_restore(e_window_VIEW_SECOND, DsMap(interfacemap).Value(/*"view_second_window"*/ STR(2766)));
				sReal(setting_snap) = value_get_real(DsMap(interfacemap).Value(/*"snap"*/ STR(304)), sReal(setting_snap));
				sVar(setting_snap_absolute) = value_get_real(DsMap(interfacemap).Value(/*"snap_absolute"*/ STR(2767)), sVar(setting_snap_absolute));
				sReal(setting_snap_size_position) = value_get_real(DsMap(interfacemap).Value(/*"snap_size_position"*/ STR(2768)), sReal(setting_snap_size_position));
				sReal(setting_snap_size_rotation) = value_get_real(DsMap(interfacemap).Value(/*"snap_size_rotation"*/ STR(2769)), sReal(setting_snap_size_rotation));
				sReal(setting_snap_size_scale) = value_get_real(DsMap(interfacemap).Value(/*"snap_size_scale"*/ STR(2770)), sReal(setting_snap_size_scale));
				sVar(setting_select_locked_parent) = value_get_real(DsMap(interfacemap).Value(/*"setting_select_locked_parent"*/ STR(2771)), sVar(setting_select_locked_parent));
