...
namespace CppProject
{
	...
	{
		if (block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"snowy"*/ STR(944)) == /*"true"*/ STR(773))
			return IntType(0);
	...
	RealType block_set_stairs(ScopeAny self)
	{
		...
		if (shape != /*"straight"*/ STR(946))
			sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"shape"*/ STR(550), shape);
		return IntType(0);
	}
	...
	RealType block_set_wall(ScopeAny self)
	{
		...
		states = ArrType::From({ /*"none"*/ STR(904), /*"low"*/ STR(610), /*"tall"*/ STR(956) });
		tall = false;
		i = IntType(0);
		if (!sBool(build_edge_zp))
		{
	...
	void block_tile_entity_banner(Scope<obj_builder_thread> self, IntType map)
	{
		...
		{
			VarType color = block_get_state_id_value(self->block_current, self->block_state_id_current, /*"color"*/ STR(916));
	...
	RealType block_tile_entity_bed(Scope<obj_builder_thread> self, IntType map)
	{
		...
		if (is_undefined(colorid) || colorid >= ds_list_size(global::minecraft_color_list))
			return IntType(0);
		RealType newstate = block_set_state_id_value(self->block_current, self->block_state_id_current, /*"color"*/ STR(916), DsList(global::minecraft_color_name_list).Value(colorid));
	...
	{
		VarType frontmap = DsMap(map).Value(/*"front_text"*/ STR(963));
		VarType text, colorname, glowing;
		IntType color;
		text = /*""*/ STR(0);
		if (ds_map_valid(frontmap))
		{
			colorname = value_get_string(DsMap(frontmap).Value(/*"color"*/ STR(916)), /*"black"*/ STR(628));
			color = DsList(global::minecraft_color_list).Value(ds_list_find_index(global::minecraft_color_name_list, colorname));
			glowing = value_get_real(DsMap(frontmap).Value(/*"has_glowing_text"*/ STR(964)), IntType(0));
			VarType messagemap = value_get_array(DsMap(frontmap).Value(/*"messages"*/ STR(965)), /*""*/ STR(0));
			for (IntType i = IntType(0); i < IntType(4); i++)
			{
				...
				{
					if (is_string(DsMap(textmap).Value(/*"text"*/ STR(100))))
						line = DsMap(textmap).Value(/*"text"*/ STR(100));
		...
		{
			colorname = value_get_string(DsMap(map).Value(/*"Color"*/ STR(962)), /*"black"*/ STR(628));
			color = DsList(global::minecraft_color_list).Value(ds_list_find_index(global::minecraft_color_name_list, colorname));
			...
			for (IntType i = IntType(0); i < IntType(4); i++)
			{
				...
				if (ds_map_valid(textmap))
				{
					if (ds_list_valid(DsMap(textmap).Value(/*"extra"*/ STR(968))) && ds_list_size((IntType)(DsMap(textmap).Value(/*"extra"*/ STR(968)))) > IntType(0))
					{
						...
						if (ds_map_valid(extramap) && is_string(DsMap(extramap).Value(/*"text"*/ STR(100))))
							textmap = extramap;
					}
					if (is_string(DsMap(textmap).Value(/*"text"*/ STR(100))))
						line = DsMap(textmap).Value(/*"text"*/ STR(100));
	...
	RealType block_tile_entity_skull(Scope<obj_builder_thread> self, VarType map)
	{
		...
		if (!ds_map_valid(map))
			return IntType(0);
		texlist = DsMap(map).Value(/*"textures"*/ STR(838));
		...
		if (!ds_map_valid(DsMap(map).Value(/*"textures"*/ STR(838))))
			return IntType(0);
		map = DsMap(map).Value(/*"textures"*/ STR(838));
	...
	RealType builder_generate(ScopeAny self)
	{
		...
		if (builder_get_waterlogged(Scope<obj_builder_thread>(self), sInt(build_pos_x), sInt(build_pos_y), sInt(build_pos_z)))
		{
			...
			sInt(block_pos_y) = sInt(build_pos_y) * block_size_;
			sInt(block_pos_z) = sInt(build_pos_z) * block_size_;
			sInt(block_current) = DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(/*"water"*/ STR(813));
