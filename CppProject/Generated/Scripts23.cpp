...
namespace CppProject
{
	VarType block_load_state_file(StringType fname, IntType block, ArrType state)
	{
		...
		{
			log({ /*"Could not find state file"*/ STR(868), filename_name(fname) });
			return null_;
		}
		...
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse state file"*/ STR(869), filename_name(fname) });
			ds_map_destroy(jsontypemap);
			return null_;
		}
		VarType variantsmap, multipartlist;
		variantsmap = DsMap(map).Value(/*"variants"*/ STR(870));
		multipartlist = DsMap(map).Value(/*"multipart"*/ STR(871));
		if (!ds_map_valid(variantsmap) && !ds_list_valid(multipartlist))
		{
			log({ /*"No models in the states file"*/ STR(872), fname });
			ds_map_destroy(map);
			return null_;
		}
		withOne (obj_block_load_state_file, (new obj_block_load_state_file)->id, noone)
		{
			...
			if (ds_map_valid(DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_preview_map).Value(sVar(name))))
			{
				...
				if (DsMap(blockmap).Value(/*"colorZ"*/ STR(873)) != VarType())
				{
					if (is_string(DsMap(blockmap).Value(/*"colorZ"*/ STR(873))))
						self->model_preview_color_yp = hex_to_color(DsMap(blockmap).Value(/*"colorZ"*/ STR(873)));
					else
					{
				...
				if (self->model_preview_alpha_yp != null_ && DsMap(blockmap).Value(/*"alphaZ"*/ STR(874)) != VarType())
					self->model_preview_alpha_yp = DsMap(blockmap).Value(/*"alphaZ"*/ STR(874));
				if (DsMap(blockmap).Value(/*"colorY"*/ STR(875)) != VarType())
				{
					if (is_string(DsMap(blockmap).Value(/*"colorY"*/ STR(875))))
						self->model_preview_color_zp = hex_to_color(DsMap(blockmap).Value(/*"colorY"*/ STR(875)));
					else
					{
				...
				if (self->model_preview_alpha_zp != null_ && DsMap(blockmap).Value(/*"alphaY"*/ STR(876)) != VarType())
					self->model_preview_alpha_zp = DsMap(blockmap).Value(/*"alphaY"*/ STR(876));
			}
			...
			if (ds_map_valid(variantsmap))
			{
				...
				if (ds_list_valid(multipartlist))
				{
					for (IntType c = IntType(0); c < ds_list_size((IntType)(multipartlist)); c++)
					{
						...
						{
							VarType whenmap = DsMap(mcase).Value(/*"when"*/ STR(877));
							if (ds_map_valid(whenmap) && ds_map_size((IntType)(whenmap)) > IntType(0))
							{
								VarType orlist = DsMap(whenmap).Value(/*"OR"*/ STR(878));
								VarType andlist = DsMap(whenmap).Value(/*"AND"*/ STR(879));
									...
									for (IntType oc = IntType(0); oc < ds_list_size((IntType)(orlist)); oc++)
									{
										...
										while (!is_undefined(cond))
										{
											...
											if (ds_map_find_value((IntType)(DsMap(jsontypemap).Value(curcondmap)), cond) == e_json_type_BOOL)
												val = ((val > 0) ? /*"true"*/ STR(772) : /*"false"*/ STR(773));
											if (string_contains(val, /*"|"*/ STR(880)))
												state_vars_set_value(VarType::CreateRef(condvars), cond, string_split(val, /*"|"*/ STR(880)));
											else
							...
							self->model_amount = IntType(0);
							self->total_weight = IntType(0);
							if (ds_map_find_value((IntType)(DsMap(jsontypemap).Value(mcase)), /*"apply"*/ STR(881)) == e_json_type_ARRAY)
							{
								VarType modellist = DsMap(mcase).Value(/*"apply"*/ STR(881));
								for (IntType i = IntType(0); i < ds_list_size((IntType)(modellist)); i++)
									if (!block_load_variant_model(ScopeAny(self), DsList(modellist).Value(i), ObjType(obj_block, block)->type))
										return false;
							}
							else
								if (!block_load_variant_model(ScopeAny(self), DsMap(mcase).Value(/*"apply"*/ STR(881)), ObjType(obj_block, block)->type))
									return null_;
	...
	void block_load_timeline(Scope<obj_block> self, VarType map, VarType typemap)
	{
		...
		if (ds_map_valid(DsMap(map).Value(/*"text"*/ STR(105))))
		{
			...
			if (ds_map_valid(DsMap(textmap).Value(/*"front"*/ STR(884))))
			{
				VarType front = DsMap(textmap).Value(/*"front"*/ STR(884));
				self->tl_text_front_position = value_get_point3D(DsMap(front).Value(/*"position"*/ STR(44)), point3D(IntType(0), IntType(0), IntType(0)));
				self->tl_text_front_rotation = value_get_point3D(DsMap(front).Value(/*"rotation"*/ STR(842)), point3D(IntType(0), IntType(0), IntType(0)));
				self->tl_text_front_scale = value_get_real(DsMap(front).Value(/*"scale"*/ STR(885)), IntType(1));
			}
			if (ds_map_valid(DsMap(textmap).Value(/*"back"*/ STR(886))))
			{
				VarType back = DsMap(textmap).Value(/*"back"*/ STR(886));
				self->tl_text_back_position = value_get_point3D(DsMap(back).Value(/*"position"*/ STR(44)), point3D(IntType(0), IntType(0), IntType(0)));
				self->tl_text_back_rotation = value_get_point3D(DsMap(back).Value(/*"rotation"*/ STR(842)), point3D(IntType(0), IntType(0), IntType(0)));
				self->tl_text_back_scale = value_get_real(DsMap(back).Value(/*"scale"*/ STR(885)), IntType(1));
			}
	...
	BoolType block_load_variant_model(ScopeAny self, VarType map, VarType type)
	{
		...
		VarType blockmodel;
		blockname = string_replace(DsMap(map).Value(/*"model"*/ STR(8)), /*"minecraft:"*/ STR(834), /*""*/ STR(0));
		blockmodel = block_load_model_file(global::load_assets_dir + mc_models_directory + blockname + /*".json"*/ STR(723));
		if (!(blockmodel > 0))
			return false;
		...
		if (is_bool(DsMap(map).Value(/*"uvlock"*/ STR(890))))
			...
			if (is_string(DsMap(map).Value(/*"uvlock"*/ STR(890))))
				uvlock = (DsMap(map).Value(/*"uvlock"*/ STR(890)) == /*"true"*/ STR(772));
	...
	VarType block_set_door(ScopeAny self)
	{
		...
		{
			location = (hinge == /*"right"*/ STR(203) ? /*"south_west"*/ STR(921) : /*"north_west"*/ STR(922));
			if (open == /*"false"*/ STR(773))
				dir = (hinge == /*"right"*/ STR(203) ? /*"north"*/ STR(898) : /*"south"*/ STR(897));
		}
		else
			if (facing == /*"west"*/ STR(896))
			{
				location = (hinge == /*"right"*/ STR(203) ? /*"north_east"*/ STR(923) : /*"south_east"*/ STR(924));
				if (open == /*"false"*/ STR(773))
					dir = (hinge == /*"right"*/ STR(203) ? /*"south"*/ STR(897) : /*"north"*/ STR(898));
			}
			else
				if (facing == /*"south"*/ STR(897))
				{
					location = (hinge == /*"right"*/ STR(203) ? /*"north_west"*/ STR(922) : /*"north_east"*/ STR(923));
					if (open == /*"false"*/ STR(773))
						dir = (hinge == /*"right"*/ STR(203) ? /*"east"*/ STR(895) : /*"west"*/ STR(896));
				}
				else
					if (facing == /*"north"*/ STR(898))
					{
						location = (hinge == /*"right"*/ STR(203) ? /*"south_east"*/ STR(924) : /*"south_west"*/ STR(921));
						if (open == /*"false"*/ STR(773))
							dir = (hinge == /*"right"*/ STR(203) ? /*"west"*/ STR(896) : /*"east"*/ STR(895));
					}
