...
namespace CppProject
{
	...
	{
		if (block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"snowy"*/ STR(937)) == /*"true"*/ STR(766))
			return IntType(0);
		StringType snowy = /*"false"*/ STR(767);
		if (!sBool(build_edge_zp))
		{
			...
			if (otherblock != null_ && idVar(otherblock, type) == /*"snow"*/ STR(938))
				snowy = /*"true"*/ STR(766);
		}
		sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"snowy"*/ STR(937), snowy);
		return IntType(0);
	}
	...
	RealType block_set_stairs(ScopeAny self)
	{
		...
		StringType shape;
		VarType half, facing;
		shape = /*"straight"*/ STR(939);
		half = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"half"*/ STR(910));
		facing = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"facing"*/ STR(895));
		if (!sBool(build_edge_xp) && (facing == /*"east"*/ STR(888) || facing == /*"west"*/ STR(889)))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(940))
			{
				IntType stateid = builder_get_state_id(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z));
				if (block_get_state_id_value(block, stateid, /*"half"*/ STR(910)) == half)
				{
					VarType otherfacing = block_get_state_id_value(block, stateid, /*"facing"*/ STR(895));
					if (facing == /*"east"*/ STR(888))
					{
						if (otherfacing == /*"south"*/ STR(890))
							shape = /*"outer_right"*/ STR(941);
						else
							if (otherfacing == /*"north"*/ STR(891))
								shape = /*"outer_left"*/ STR(942);
						
					}
					else
					{
						if (otherfacing == /*"south"*/ STR(890))
							shape = /*"inner_left"*/ STR(943);
						else
							if (otherfacing == /*"north"*/ STR(891))
								shape = /*"inner_right"*/ STR(944);
		...
		if (!sBool(build_edge_xn) && (facing == /*"east"*/ STR(888) || facing == /*"west"*/ STR(889)))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) - IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(940))
			{
		...
		if (!sBool(build_edge_yp) && (facing == /*"south"*/ STR(890) || facing == /*"north"*/ STR(891)))
		{
			...
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(940))
			{
				...
				if (block_get_state_id_value(block, stateid, /*"half"*/ STR(910)) == half)
				{
					...
					{
						if (otherfacing == /*"east"*/ STR(888))
							shape = /*"outer_left"*/ STR(942);
						else
							if (otherfacing == /*"west"*/ STR(889))
								shape = /*"outer_right"*/ STR(941);
		...
		if (!sBool(build_edge_yn) && (facing == /*"south"*/ STR(890) || facing == /*"north"*/ STR(891)))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x), sInt(build_pos_y) - IntType(1), sInt(build_pos_z));
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(940))
			{
		...
		if (shape != /*"straight"*/ STR(939))
			sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"shape"*/ STR(543), shape);
		return IntType(0);
	}
	...
	RealType block_set_tripwire(ScopeAny self)
	{
		...
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if (block != null_ && (idVar(block, name) == /*"tripwire"*/ STR(945) || idVar(block, name) == /*"tripwire_hook"*/ STR(946)))
				east = /*"true"*/ STR(766);
		}
	...
	RealType block_set_twisting_vines(ScopeAny self)
	{
		if (!sBool(build_edge_zn))
		{
			...
			if (block == null_ || idVar(block, type) != /*"twisting_vines"*/ STR(947))
				global::vertex_wave_zmin = sInt(block_pos_z);
		}
		else
			global::vertex_wave_zmin = sInt(block_pos_z);
		
		if ((sBool(builder_scenery) && !sBool(builder_scenery_legacy)) || block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"variant"*/ STR(804)) == /*"twisting_vines_plant"*/ STR(948))
			return IntType(0);
		if (sInt(build_pos_z) != (sInt(build_size_z) - IntType(1)))
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(804), /*"twisting_vines_plant"*/ STR(948) }));
		return IntType(0);
	}
	...
	RealType block_set_vine(ScopeAny self)
	{
		...
		sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"up"*/ STR(907), up);
		return IntType(0);
	}
	...
	RealType block_set_wall(ScopeAny self)
	{
		...
		states = ArrType::From({ /*"none"*/ STR(897), /*"low"*/ STR(603), /*"tall"*/ STR(949) });
		tall = false;
		i = IntType(0);
		if (!sBool(build_edge_zp))
		{
			...
			if (!sBool(build_edge_xp) && (i == IntType(0) || (i == IntType(1) && east > 0)))
			{
				...
				if (block != null_)
				{
					if (idVar(block, type) == idVar(sInt(block_current), type) || (sInt(block_face_min_depth_xp) == e_block_depth_DEPTH0 && sBool(block_face_full_xp)))
						...
						{
							VarType facing = block_get_state_id_value(block, builder_get_state_id(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z) + i), /*"facing"*/ STR(895));
			...
			if (!sBool(build_edge_yp) && (i == IntType(0) || (i == IntType(1) && south > 0)))
			{
				...
				if (block != null_)
				{
					if (idVar(block, type) == idVar(sInt(block_current), type) || (sInt(block_face_min_depth_yp) == e_block_depth_DEPTH0 && sBool(block_face_full_yp)))
						...
						if (idVar(block, type) == /*"fence_gate"*/ STR(925))
						{
							VarType facing = block_get_state_id_value(block, builder_get_state_id(self, sInt(build_pos_x), sInt(build_pos_y) + IntType(1), sInt(build_pos_z) + i), /*"facing"*/ STR(895));
		...
		sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(804), variant, /*"east"*/ STR(888), states.Value(east), /*"west"*/ STR(889), states.Value(west), /*"south"*/ STR(890), states.Value(south), /*"north"*/ STR(891), states.Value(north), /*"up"*/ STR(907), up }));
		return IntType(0);
	}
	...
	RealType block_set_weeping_vines(ScopeAny self)
	{
		...
		{
			IntType block = builder_get_block(self, sInt(build_pos_x), sInt(build_pos_y), sInt(build_pos_z) + IntType(1));
			if (block == null_ || idVar(block, type) != /*"weeping_vines"*/ STR(950))
				global::vertex_wave_zmax = sInt(block_pos_z) + block_size_;
		}
		else
			global::vertex_wave_zmax = sInt(block_pos_z) + block_size_;
		
		if ((sBool(builder_scenery) && !sBool(builder_scenery_legacy)) || block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"variant"*/ STR(804)) == /*"weeping_vines_plant"*/ STR(951))
			return IntType(0);
		if (sInt(build_pos_z) == IntType(0))
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(804), /*"weeping_vines"*/ STR(950) }));
		else
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(804), /*"weeping_vines_plant"*/ STR(951) }));
	...
	void block_tile_entity_banner(Scope<obj_builder_thread> self, IntType map)
	{
		...
		patterns = DsMap(map).Value(/*"Patterns"*/ STR(952));
		if (ds_map_exists(map, /*"Base"*/ STR(953)))
		{
			base = DsMap(map).Value(/*"Base"*/ STR(953));
			legacy = true;
		}
		else
		{
		...
		if (ds_list_valid(patterns))
		{
			for (IntType i = IntType(0); i < ds_list_size((IntType)(patterns)); i++)
			{
				...
				pattern = value_get_string(DsMap(patternmap).Value(/*"Pattern"*/ STR(954)), DsList(global::minecraft_pattern_short_list).Value(IntType(1)));
				color = value_get_real(DsMap(patternmap).Value(/*"Color"*/ STR(955)), IntType(0));
		...
		{
			VarType color = block_get_state_id_value(self->block_current, self->block_state_id_current, /*"color"*/ STR(909));
	...
	{
		VarType colorid = DsMap(map).Value(/*"color"*/ STR(909));
		if (!is_real(colorid))
			return IntType(0);
		if (is_undefined(colorid) || colorid >= ds_list_size(global::minecraft_color_list))
			return IntType(0);
		RealType newstate = block_set_state_id_value(self->block_current, self->block_state_id_current, /*"color"*/ STR(909), DsList(global::minecraft_color_name_list).Value(colorid));
	...
	{
		VarType frontmap = DsMap(map).Value(/*"front_text"*/ STR(956));
		VarType text, colorname, glowing;
		IntType color;
		text = /*""*/ STR(0);
		if (ds_map_valid(frontmap))
		{
			colorname = value_get_string(DsMap(frontmap).Value(/*"color"*/ STR(909)), /*"black"*/ STR(621));
			color = DsList(global::minecraft_color_list).Value(ds_list_find_index(global::minecraft_color_name_list, colorname));
			glowing = value_get_real(DsMap(frontmap).Value(/*"has_glowing_text"*/ STR(957)), IntType(0));
			VarType messagemap = value_get_array(DsMap(frontmap).Value(/*"messages"*/ STR(958)), /*""*/ STR(0));
		...
		{
			colorname = value_get_string(DsMap(map).Value(/*"Color"*/ STR(955)), /*"black"*/ STR(621));
			color = DsList(global::minecraft_color_list).Value(ds_list_find_index(global::minecraft_color_name_list, colorname));
			glowing = value_get_real(DsMap(map).Value(/*"GlowingText"*/ STR(960)), IntType(0));
			for (IntType i = IntType(0); i < IntType(4); i++)
			{
				VarType line = DsMap(map).Value(/*"Text"*/ STR(961) + string(i + IntType(1)));
				if (!is_string(line))
					return IntType(0);
				IntType textmap = json_decode(line);
				if (ds_map_valid(textmap))
				{
					if (ds_list_valid(DsMap(textmap).Value(/*"extra"*/ STR(962))) && ds_list_size((IntType)(DsMap(textmap).Value(/*"extra"*/ STR(962)))) > IntType(0))
					{
						IntType extramap = ds_list_find_value(DsMap(textmap).Value(/*"extra"*/ STR(962)), IntType(0));
	...
	RealType block_tile_entity_skull(Scope<obj_builder_thread> self, VarType map)
	{
		...
		VarType texlist;
		mapname = /*"Owner"*/ STR(963);
		if (!ds_map_valid(DsMap(map).Value(mapname)))
		{
			mapname = /*"SkullOwner"*/ STR(964);
			if (!ds_map_valid(DsMap(map).Value(mapname)))
				return IntType(0);
		}
		map = DsMap(map).Value(mapname);
		ownerid = string(DsMap(map).Value(/*"Id"*/ STR(965)));
		map = DsMap(map).Value(/*"Properties"*/ STR(966));
		if (!ds_map_valid(map))
			return IntType(0);
		texlist = DsMap(map).Value(/*"textures"*/ STR(831));
		if (!ds_list_valid(texlist))
			return IntType(0);
		map = DsList(texlist).Value(IntType(0));
		texvalue = base64_decode(DsMap(map).Value(/*"Value"*/ STR(967)));
		map = json_decode(texvalue);
		if (!ds_map_valid(map))
			return IntType(0);
		if (!ds_map_valid(DsMap(map).Value(/*"textures"*/ STR(831))))
			return IntType(0);
		map = DsMap(map).Value(/*"textures"*/ STR(831));
		map = DsMap(map).Value(/*"SKIN"*/ STR(968));
		texurl = DsMap(map).Value(/*"url"*/ STR(969)).Str();
	...
	RealType builder_generate(ScopeAny self)
	{
		...
		if (builder_get_waterlogged(Scope<obj_builder_thread>(self), sInt(build_pos_x), sInt(build_pos_y), sInt(build_pos_z)))
		{
			...
			sInt(block_pos_y) = sInt(build_pos_y) * block_size_;
			sInt(block_pos_z) = sInt(build_pos_z) * block_size_;
			sInt(block_current) = DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(/*"water"*/ STR(806));
