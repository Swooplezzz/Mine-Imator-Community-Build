...
namespace CppProject
{
	...
	RealType draw_wheel(ScopeAny self, StringType name, VarType xx, VarType yy, VarType color, VarType value, RealType minval, RealType maxval, VarType def, RealType snapval, VarType tbx, IntType script, RealType rad, VarType sprite)
	{
		...
		StringType text;
		VarType labely;
		context_menu_area(self, { xx - rad, yy - rad, rad * IntType(2), rad * IntType(2), /*"contextmenuvalue"*/ STR(1082), value, e_context_type_NUMBER, script, def });
		draw_set_font(sInt(font_label));
		modval = mod_fix(value, IntType(360));
		capwid = string_width(text_get({ name }) + /*":"*/ STR(784)) + IntType(5);
	...
	RealType draw_wheel_sky(ScopeAny self, StringType name, VarType xx, VarType yy, VarType value, RealType def, IntType script, IntType tbx, RealType time)
	{
		...
		context_menu_area(self, { xx - (rad + IntType(10)), yy - (rad + IntType(10)), (rad + IntType(10)) * IntType(2), (rad + IntType(10)) * IntType(2), /*"contextmenuvalue"*/ STR(1082), value, (time > 0) ? e_context_type_TIME : e_context_type_NUMBER, script, def });
	...
	VarType ease(VarType func, VarType xx)
	{
		...
		switch (StringType(func).id)
		{
			...
			case 585: // easeinquint
			{
				return xx * xx * xx * xx * xx;
			}
			case 586: // easeoutquint
			{
				return ((xxm1) * xxm1 * xxm1 * xxm1 * xxm1 + IntType(1));
			}
			case 587: // easeinoutquint
			{
				if (xx2 < IntType(1))
			...
			case 589: // easeoutsine
			{
				return sin(xx * (pi_ / 2.0));
			}
			case 590: // easeinoutsine
			{
			...
			case 592: // easeoutexpo
			{
				return -power(2.0, -IntType(10) * xx) + IntType(1);
			}
			case 593: // easeinoutexpo
			{
				if (xx2 < IntType(1))
			...
			case 595: // easeoutcirc
			{
				return sqrt(IntType(1) - (xxm1) * xxm1);
			}
			case 596: // easeinoutcirc
			{
				if (xx2 < IntType(1))
	...
	VarType eval(VarType str, VarType def)
	{
		...
		for (i = IntType(1); i < string_length(str) + IntType(1); i++)
		{
			...
			if (char_ == /*" "*/ STR(21))
				...
				if (char_ == /*"("*/ STR(1109))
				{
					if (lastoperator == /*"--"*/ STR(1110))
						ds_stack_push({ ops, /*"-("*/ STR(1111) });
					else
						ds_stack_push({ ops, /*"("*/ STR(1109) });
					...
					if (eval_is_digit(char_))
					{
						...
						{
							if (string_char_at(str, i) == /*"."*/ STR(1112))
							{
								deccheck = true;
								i++;
								continue;
						...
						if (char_ == /*")"*/ STR(1113))
						{
							while (!ds_stack_empty(ops) && !(ds_stack_top(ops) == /*"("*/ STR(1109) || ds_stack_top(ops) == /*"-("*/ STR(1111)))
							{
						...
						{
							if (lastoperator != /*""*/ STR(0) && char_ == /*"-"*/ STR(1114))
							{
								lastoperator = /*"--"*/ STR(1110);
								continue;
							}
	...
	{
		if (a == VarType() && b != VarType() && op == /*"-"*/ STR(1114))
			a = IntType(0);
	...
	{
		ArrType digits = ArrType::From({ /*"0"*/ STR(1075), /*"1"*/ STR(1118), /*"2"*/ STR(1119), /*"3"*/ STR(1120), /*"4"*/ STR(1121), /*"5"*/ STR(1122), /*"6"*/ STR(1123), /*"7"*/ STR(1124), /*"8"*/ STR(1125), /*"9"*/ STR(1126), /*"."*/ STR(1112) });
	...
	{
		log({ /*"execute"*/ STR(1127), file, parameters, wait });
	...
	void export_done_image(ScopeAny self)
	{
		...
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportimage"*/ STR(1128) }));
		toast_add_action(self, /*"alertexportimageview"*/ STR(1129), ID_popup_open_url, sStr(export_filename));
	}
	...
	void export_done_movie(ScopeAny self, BoolType cancel)
	{
		...
		if (sStr(exportmovie_format) == /*"png"*/ STR(64))
			fn = filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_1.png"*/ STR(1130);
		else
			fn = sStr(export_filename);
		
		if (!cancel)
		{
			toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportmovie"*/ STR(1131) }));
			toast_add_action(self, /*"alertexportmovieview"*/ STR(1132), ID_popup_open_url, fn);
		}
	}
	...
	RealType export_update(ScopeAny self)
	{
		...
		if (sInt(export_escape_time) > IntType(0) && current_time() - sInt(export_escape_time) > IntType(1000))
		{
			...
			if (question(text_get({ /*"questionstoprender"*/ STR(1133) })))
			{
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(53))
		{
			if (sStr(exportmovie_format) == /*"png"*/ STR(64))
			{
				...
				numstr = string_repeat(/*"0"*/ STR(1075), (IntType)((totallen - string_length(numstr)))) + numstr;
				surface_save_lib(sVar(export_surface), filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_"*/ STR(1135) + numstr + /*".png"*/ STR(10));
			}
			else
			{
				if (!is_cpp())
				...
				{
					export_done_movie(self);
					log({ /*"Error when adding frame, error code"*/ STR(1136), err });
	...
	StringType filename_get_unique(StringType fn)
	{
		...
		while (file_exists_lib(newfn))
			newfn = path + noext + /*" ("*/ STR(1137) + string(num++) + /*")"*/ STR(1113) + ext;
	...
	{
		fn = string_replace_all(fn, /*"/"*/ STR(20), /*"_"*/ STR(1135));
		fn = string_replace_all(fn, /*"\\"*/ STR(1138), /*"_"*/ STR(1135));
		fn = string_replace_all(fn, /*":"*/ STR(784), /*"_"*/ STR(1135));
		fn = string_replace_all(fn, /*"*"*/ STR(1116), /*"_"*/ STR(1135));
		fn = string_replace_all(fn, /*"?"*/ STR(1139), /*"_"*/ STR(1135));
		fn = string_replace_all(fn, /*"\""*/ STR(1140), /*"_"*/ STR(1135));
		fn = string_replace_all(fn, /*"<"*/ STR(1141), /*"_"*/ STR(1135));
		fn = string_replace_all(fn, /*">"*/ STR(1142), /*"_"*/ STR(1135));
		fn = string_replace_all(fn, /*"|"*/ STR(880), /*"_"*/ STR(1135));
	...
	VarType filename_new_ext(VarType fn, StringType newext)
	{
		...
		for (p = string_length(fn); p >= IntType(0); p--)
		{
			...
			if (p == IntType(0) || c == /*"\\"*/ STR(1138) || c == /*"/"*/ STR(20))
				return fn + newext;
			if (c == /*"."*/ STR(1112))
				break;
		}
	...
	{
		return file_dialog_open_multiple(text_get({ /*"filedialogopenasset"*/ STR(1143) }) + /*"|"*/ STR(880) + asset_exts, /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenassetcaption"*/ STR(1144) }));
	}
	
	StringType file_dialog_open_font()
	{
		return file_dialog_open(text_get({ /*"filedialogopenfont"*/ STR(1145) }) + /*" (*.ttf)|*.ttf"*/ STR(1146), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenfontcaption"*/ STR(1147) }));
	}
	
	StringType file_dialog_open_image()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimage"*/ STR(1148) }) + /*" (*.png; *.jpg; *.dat)|*.png;*.jpg;*.jpeg;*.dat;"*/ STR(1149), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimagecaption"*/ STR(1150) }));
	}
	
	StringType file_dialog_open_image_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimageorpack"*/ STR(1151) }) + /*" (*.png; *.jpg; *.zip)|*.png;*.jpg;*.jpeg;*.zip"*/ STR(1152), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimageorpackcaption"*/ STR(1153) }));
	}
	
	StringType file_dialog_open_language()
	{
		return file_dialog_open(text_get({ /*"filedialogopenlanguage"*/ STR(1154) }) + /*" (*.milanguage;*.txt)|*.milanguage;*.txt"*/ STR(1155), /*""*/ STR(0), languages_directory, text_get({ /*"filedialogopenlanguagecaption"*/ STR(1156) }));
	}
	
	StringType file_dialog_open_model()
	{
		return file_dialog_open(text_get({ /*"filedialogopenmodel"*/ STR(1157) }) + /*" (*.mimodel;*.json;*.zip)|*.mimodel;*.json;*.zip"*/ STR(1158), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenmodelcaption"*/ STR(1159) }));
	}
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenpack"*/ STR(1160) }) + /*" (*.zip)|*.zip"*/ STR(1161), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenpackcaption"*/ STR(1162) }));
	}
	
	StringType file_dialog_open_particles()
	{
		return file_dialog_open(text_get({ /*"filedialogopenparticles"*/ STR(1163) }) + /*" (*.miparticles; *.zip; *.particles)|*miparticles;*.zip;*.particles;"*/ STR(1164), /*""*/ STR(0), particles_directory, text_get({ /*"filedialogopenparticlecaption"*/ STR(1165) }));
	}
