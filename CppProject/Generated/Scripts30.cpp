...
namespace CppProject
{
	...
	BoolType history_save_part_usage_tl(IntType tl, VarType hobj)
	{
		...
		withAll (obj_timeline, noone)
		{
			...
			{
				idArr(hobj, usage_tl_flw_rot_target_save_id)[idInt(hobj, usage_tl_flw_rot_target_amount)] = self->save_id;
				idArr(hobj, usage_tl_flw_rot_target_part_save_id)[idInt(hobj, usage_tl_flw_rot_target_amount)] = idVar(tl, save_id);
				idInt(hobj, usage_tl_flw_rot_target_amount)++;
				used = true;
			}
		}
		...
		withAll (obj_keyframe, noone)
		{
			...
			if (self->value.Value(e_value_ROT_TARGET) == tl)
			{
				idArr(hobj, usage_kf_flw_rot_target_tl_save_id)[idInt(hobj, usage_kf_flw_rot_target_amount)] = save_id_get(self->timeline);
				idArr(hobj, usage_kf_flw_rot_target_tl_part_save_id)[idInt(hobj, usage_kf_flw_rot_target_amount)] = idVar(tl, save_id);
				idArr(hobj, usage_kf_flw_rot_target_index)[idInt(hobj, usage_kf_flw_rot_target_amount)] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
				idInt(hobj, usage_kf_flw_rot_target_amount)++;
				used = true;
			}
			if (self->value.Value(e_value_POS_TARGET) == tl)
			{
				idArr(hobj, usage_kf_flw_pos_target_tl_save_id)[idInt(hobj, usage_kf_flw_pos_target_amount)] = save_id_get(self->timeline);
				idArr(hobj, usage_kf_flw_pos_target_tl_part_save_id)[idInt(hobj, usage_kf_flw_pos_target_amount)] = idVar(tl, save_id);
				idArr(hobj, usage_kf_flw_pos_target_index)[idInt(hobj, usage_kf_flw_pos_target_amount)] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
				idInt(hobj, usage_kf_flw_pos_target_amount)++;
				used = true;
			}
			if (self->value.Value(e_value_SCALE_TARGET) == tl)
			{
				idArr(hobj, usage_kf_flw_scale_target_tl_save_id)[idInt(hobj, usage_kf_flw_scale_target_amount)] = save_id_get(self->timeline);
				idArr(hobj, usage_kf_flw_scale_target_tl_part_save_id)[idInt(hobj, usage_kf_flw_scale_target_amount)] = idVar(tl, save_id);
				idArr(hobj, usage_kf_flw_scale_target_index)[idInt(hobj, usage_kf_flw_scale_target_amount)] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
				idInt(hobj, usage_kf_flw_scale_target_amount)++;
				used = true;
			}
			if (self->value.Value(e_value_IK_TARGET_ANGLE) == tl)
			{
