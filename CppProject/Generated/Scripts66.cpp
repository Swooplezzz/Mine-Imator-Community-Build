...
namespace CppProject
{
	...
	RealType tl_update_matrix(ScopeAny self, BoolType usepaths, BoolType updateik, BoolType updateflw)
	{
		...
		for (IntType i = start; i < tlamount; i++)
		{
			...
			withOne (Object, curtl, self->id)
			{
				...
				if (sVar(value).Value(e_value_ROT_TARGET) != null_)
				{
					...
					matrix_remove_rotation(VarType::CreateRef(sVar(matrix)));
					sVar(matrix) = matrix_multiply(matrix_create(point3D(IntType(0), IntType(0), IntType(0)), vec3((sVar(value).Value(e_value_FLW_ROT_X) == IntType(1) ? idVar(sVar(value).Value(e_value_ROT_TARGET), value).Value(e_value_ROT_X) : sVar(value).Value(e_value_ROT_X)), (sVar(value).Value(e_value_FLW_ROT_Y) == IntType(1) ? idVar(sVar(value).Value(e_value_ROT_TARGET), value).Value(e_value_ROT_Y) : sVar(value).Value(e_value_ROT_Y)), (sVar(value).Value(e_value_FLW_ROT_Z) == IntType(1) ? idVar(sVar(value).Value(e_value_ROT_TARGET), value).Value(e_value_ROT_Z) : sVar(value).Value(e_value_ROT_Z))), vec3(IntType(1))), sVar(matrix));
				}
				if (sVar(value).Value(e_value_POS_TARGET) != null_)
				{
					sBool(update_matrix) = true;
					matrix_remove_rotation(VarType::CreateRef(sMat(matrix_parent)));
					sVar(matrix)[MAT_X] = sVar(value).Value(e_value_POS_X) + idVar(sVar(value).Value(e_value_POS_TARGET), value).Value(e_value_POS_X);
					sVar(matrix)[MAT_Y] = sVar(value).Value(e_value_POS_Y) + idVar(sVar(value).Value(e_value_POS_TARGET), value).Value(e_value_POS_Y);
					sVar(matrix)[MAT_Z] = sVar(value).Value(e_value_POS_Z) + idVar(sVar(value).Value(e_value_POS_TARGET), value).Value(e_value_POS_Z);
				}
		...
		if (updateflw)
		{
			if (global::_app->project_flw_obj_array == null_)
			{
				global::_app->project_flw_obj_array = ArrType::From({});
				withAll (obj_timeline, self->id)
			...
			tl_update_flw(global::_app->project_flw_obj_array);
		}
		return 0.0;
	}
	...
	{
		sVar(type_name) = text_get({ /*"type"*/ STR(787) + DsList(global::tl_type_name_list).Value(sVar(type)) });
		if (sVar(part_of) != null_)
		{
			if (sVar(type) == e_tl_type_BODYPART)
			{
				if (sVar(model_part) != null_)
					sVar(type_name) = minecraft_asset_get_name(/*"modelpart"*/ STR(762), idVar(sVar(model_part), name));
				else
					sVar(type_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3769) });
				...
				if (sVar(type) == e_tl_type_SPECIAL_BLOCK)
				{
					if (sVar(model_file) != null_)
						sVar(type_name) = minecraft_asset_get_name(/*"model"*/ STR(529), idVar(sVar(model_file), name));
				}
				else
					if (sVar(type) == e_tl_type_BLOCK)
					{
						sVar(type_name) = minecraft_asset_get_name(/*"block"*/ STR(522), ObjType(obj_block, DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(sVar(block_name)))->name);
					}
			
			
			sVar(type_name) = text_get({ /*"timelinepartof"*/ STR(3770), sVar(type_name), string_remove_newline(idVar(sVar(part_of), display_name)) });
		}
		else
			if (sVar(temp) != null_)
				sVar(type_name) = text_get({ /*"timelineinstanceof"*/ STR(3771), idVar(sVar(temp), display_name) });
	...
	void tl_update_values(ScopeAny self)
	{
		...
		tl_update_values_ease(Scope<obj_timeline>(self), e_value_FLW_ROT_X);
		tl_update_values_ease(Scope<obj_timeline>(self), e_value_FLW_ROT_Y);
		tl_update_values_ease(Scope<obj_timeline>(self), e_value_FLW_ROT_Z);
		tl_update_values_ease(Scope<obj_timeline>(self), e_value_FLW_POS_X);
		tl_update_values_ease(Scope<obj_timeline>(self), e_value_FLW_POS_Y);
		tl_update_values_ease(Scope<obj_timeline>(self), e_value_FLW_POS_Z);
		tl_update_values_ease(Scope<obj_timeline>(self), e_value_FLW_SCALE_X);
		tl_update_values_ease(Scope<obj_timeline>(self), e_value_FLW_SCALE_Y);
		tl_update_values_ease(Scope<obj_timeline>(self), e_value_FLW_SCALE_Z);
		...
		if (sVar(type) == e_tl_type_AUDIO && !(sVar(hide) > 0) && global::_app->timeline_marker > global::_app->timeline_marker_previous && global::_app->timeline_playing > 0)
		{
			if (sVar(keyframe_current) > 0)
			{
				...
				{
					ObjType(obj_keyframe, sVar(keyframe_current))->sound_play_index = audio_play_sound(idInt(sVar(value).Value(e_value_SOUND_OBJ), sound_index), IntType(0), false);
					audio_sound_set_track_position(ObjType(obj_keyframe, sVar(keyframe_current))->sound_play_index, mod(sVar(value).Value(e_value_SOUND_START), (idReal(sVar(value).Value(e_value_SOUND_OBJ), sound_samples) / sample_rate_)));
