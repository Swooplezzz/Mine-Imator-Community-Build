...
namespace CppProject
{
		...
		for (IntType i = start; i < tlamount; i++)
		{
			...
			withOne (Object, curtl, self->id)
			{
				...
				if (sVar(value).Value(e_value_POS_TARGET) != null_)
				{
					sBool(update_matrix) = true;
					matrix_remove_rotation(VarType::CreateRef(sMat(matrix_parent)));
					sVar(matrix)[MAT_X] = sVar(value).Value(e_value_POS_X) + idVar(sVar(value).Value(e_value_POS_TARGET), world_pos).Value(X_);
					sVar(matrix)[MAT_Y] = sVar(value).Value(e_value_POS_Y) + idVar(sVar(value).Value(e_value_POS_TARGET), world_pos).Value(Y_);
					sVar(matrix)[MAT_Z] = sVar(value).Value(e_value_POS_Z) + idVar(sVar(value).Value(e_value_POS_TARGET), world_pos).Value(Z_);
				}
	...
	{
		sVar(type_name) = text_get({ /*"type"*/ STR(800) + DsList(global::tl_type_name_list).Value(sVar(type)) });
		if (sVar(part_of) != null_)
		{
			if (sVar(type) == e_tl_type_BODYPART)
			{
				if (sVar(model_part) != null_)
					sVar(type_name) = minecraft_asset_get_name(/*"modelpart"*/ STR(775), idVar(sVar(model_part), name));
				else
					sVar(type_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3815) });
				...
				if (sVar(type) == e_tl_type_SPECIAL_BLOCK)
				{
					if (sVar(model_file) != null_)
						sVar(type_name) = minecraft_asset_get_name(/*"model"*/ STR(542), idVar(sVar(model_file), name));
				}
				else
					if (sVar(type) == e_tl_type_BLOCK)
					{
						sVar(type_name) = minecraft_asset_get_name(/*"block"*/ STR(535), ObjType(obj_block, DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(sVar(block_name)))->name);
					}
			
			
			sVar(type_name) = text_get({ /*"timelinepartof"*/ STR(3816), sVar(type_name), string_remove_newline(idVar(sVar(part_of), display_name)) });
		}
		else
			if (sVar(temp) != null_)
				sVar(type_name) = text_get({ /*"timelineinstanceof"*/ STR(3817), idVar(sVar(temp), display_name) });
	...
	void tl_update_values(ScopeAny self)
	{
		...
		if (sVar(keyframe_transition) == /*"bezier"*/ STR(564))
