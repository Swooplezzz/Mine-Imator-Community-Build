...
namespace CppProject
{
	RealType render_world_tl(ScopeAny self)
	{
		...
		{
			if (sReal(selected) > 0 || (sVar(lock) > 0 && !(global::_app->setting_select_locked_parent > 0)) || !tl_update_list_filter(self->id))
				return IntType(0);
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2593), self->id, IntType(1));
		}
		...
		if (global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2593), (sVar(ssao) > 0) ? merge_color(c_black, c_white, global::shader_blend_alpha) : c_black, IntType(1));
		if (sBool(colors_ext) != global::shader_uniform_color_ext || sArr(value_inherit).Value(e_value_RGB_ADD) != global::shader_uniform_rgb_add || sArr(value_inherit).Value(e_value_HSB_ADD) != global::shader_uniform_hsb_add || sArr(value_inherit).Value(e_value_RGB_SUB) != global::shader_uniform_rgb_sub || sArr(value_inherit).Value(e_value_HSB_SUB) != global::shader_uniform_hsb_sub || sArr(value_inherit).Value(e_value_HSB_MUL) != global::shader_uniform_hsb_mul || sArr(value_inherit).Value(e_value_MIX_COLOR) != global::shader_uniform_mix_color || sArr(value_inherit).Value(e_value_MIX_PERCENT) != global::shader_uniform_mix_percent)
		{
			...
			render_set_uniform_int(/*"uColorsExt"*/ STR(2134), global::shader_uniform_color_ext);
			render_set_uniform_color(/*"uRGBAdd"*/ STR(2135), global::shader_uniform_rgb_add, IntType(1));
			render_set_uniform_color(/*"uHSBAdd"*/ STR(2137), global::shader_uniform_hsb_add, IntType(1));
			render_set_uniform_color(/*"uRGBSub"*/ STR(2136), global::shader_uniform_rgb_sub, IntType(1));
			render_set_uniform_color(/*"uHSBSub"*/ STR(2138), global::shader_uniform_hsb_sub, IntType(1));
			render_set_uniform_color(/*"uHSBMul"*/ STR(2139), global::shader_uniform_hsb_mul, IntType(1));
			render_set_uniform_color(/*"uMixColor"*/ STR(2140), global::shader_uniform_mix_color, global::shader_uniform_mix_percent);
		}
		...
		if (sVar(wind) != global::shader_uniform_wind)
		{
			...
			render_set_uniform(/*"uWindEnable"*/ STR(1964), global::shader_uniform_wind);
		}
		if (sVar(wind_terrain) != global::shader_uniform_wind_terrain)
		{
			global::shader_uniform_wind_terrain = sVar(wind_terrain);
			render_set_uniform(/*"uWindTerrain"*/ STR(1965), global::shader_uniform_wind_terrain);
		}
		if ((global::_app->background_fog_show > 0 && sVar(fog) > 0) != global::shader_uniform_fog)
		{
			...
			render_set_uniform_int(/*"uFogShow"*/ STR(1971), global::shader_uniform_fog);
		}
		if (sArr(value_inherit).Value(e_value_SUBSURFACE) != global::shader_uniform_sss || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_RED) != global::shader_uniform_sss_red || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_GREEN) != global::shader_uniform_sss_green || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_BLUE) != global::shader_uniform_sss_blue || sArr(value_inherit).Value(e_value_SUBSURFACE_COLOR) != global::shader_uniform_sss_color)
		{
			...
			render_set_uniform(/*"uSSS"*/ STR(2599), global::shader_uniform_sss);
			render_set_uniform_vec3(/*"uSSSRadius"*/ STR(2600), global::shader_uniform_sss_red, global::shader_uniform_sss_green, global::shader_uniform_sss_blue);
			render_set_uniform_color(/*"uSSSColor"*/ STR(2601), global::shader_uniform_sss_color, 1.0);
		}
		if (sArr(value_inherit).Value(e_value_WIND_INFLUENCE) != global::shader_uniform_wind_strength)
		{
			...
			render_set_uniform(/*"uWindStrength"*/ STR(1967), global::shader_uniform_wind_strength);
			render_set_uniform(/*"uWindDirectionalStrength"*/ STR(1970), global::shader_uniform_wind_strength * global::_app->background_wind_directional_strength);
		}
		IntType prevblend = null_;
		if (sVar(blend_mode) != /*"normal"*/ STR(631) && (global::render_mode == e_render_mode_COLOR || global::render_mode == e_render_mode_COLOR_FOG || global::render_mode == e_render_mode_COLOR_FOG_LIGHTS || global::render_mode == e_render_mode_ALPHA_FIX))
		{
		...
		if (sVar(glow) != global::shader_uniform_glow || sVar(glow_texture) != global::shader_uniform_glow_texture || sArr(value_inherit).Value(e_value_GLOW_COLOR) != global::shader_uniform_glow_color)
		{
			...
			{
				render_set_uniform_int(/*"uGlow"*/ STR(2602), IntType(1));
				render_set_uniform_int(/*"uGlowTexture"*/ STR(2596), sVar(glow_texture));
				render_set_uniform_color(/*"uGlowColor"*/ STR(2595), global::shader_uniform_glow_color, IntType(1));
			...
			{
				render_set_uniform_int(/*"uGlow"*/ STR(2602), IntType(0));
				render_set_uniform_int(/*"uGlowTexture"*/ STR(2596), IntType(0));
				render_set_uniform_color(/*"uGlowColor"*/ STR(2595), c_black, IntType(0));
			}
		...
		if (DsMap(idInt(global::render_shader_obj, uniform_map)).Value(/*"uGlintEnabled"*/ STR(2603)) > -IntType(1))
			texture_set_stage(DsMap(idInt(global::render_shader_obj, sampler_map)).Value(/*"uGlintTexture"*/ STR(2604)), sprite_get_texture((IntType)(tex), IntType(0)));
		spd = global::_app->background_time * sVar(glint_speed) * global::_app->project_render_glint_speed;
		render_set_uniform_int(/*"uGlintEnabled"*/ STR(2603), sVar(glint_mode) == e_glint_NONE ? IntType(0) : IntType(1));
		render_set_uniform_vec2(/*"uGlintOffset"*/ STR(2605), spd * (0.000625), spd * (0.00125));
		render_set_uniform(/*"uGlintStrength"*/ STR(2606), global::_app->project_render_glint_strength * sVar(glint_strength));
		render_set_uniform_vec2(/*"uGlintSize"*/ STR(2607), sprite_get_width((IntType)(tex)) * IntType(2) * sVar(glint_scale), sprite_get_height((IntType)(tex)) * IntType(2) * sVar(glint_scale));
		if (sVar(type) != e_tl_type_PARTICLE_SPAWNER)
		{
			...
			switch ((IntType)sVar(type))
			{
				...
				case e_tl_type_PATH:
				{
					if (sVar(path_vbuffer) != null_)
					{
						...
						else
						{
							...
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2586), idVar(sArr(value_inherit).Value(e_value_TEXTURE_MATERIAL_OBJ), material_format));
						}
						...
						render_set_texture(texmat, /*"Material"*/ STR(2587));
						render_set_texture(texnorm, /*"Normal"*/ STR(2588));
				...
				default:
				{
					...
					withOne (Object, sVar(temp), self->id)
					{
						...
						if (matres != null_)
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2586), idVar(matres, material_format));
						else
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2586), e_material_FORMAT_NONE);
	...
	void render_world_tl_reset()
	{
		...
		render_set_uniform_int(/*"uGlintEnabled"*/ STR(2603), IntType(0));
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2608) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2609) + filename_ext(fn));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2610);
			...
			case e_res_type_TEXTURE:
			{
				if (sVar(texture) > 0)
					texture_free(sVar(texture));
				sVar(texture) = null_;
				if (filename_ext(fn) == /*".dat"*/ STR(716))
				{
					IntType map = new_minecraft_map(fn);
					if (!(map > 0))
					{
						error(/*"errorloadmap"*/ STR(2611));
						break;
					}
					else
