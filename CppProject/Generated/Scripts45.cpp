...
namespace CppProject
{
		...
		switch ((IntType)vid)
		{
			...
			case e_value_TRANSITION:
			{
				switch ((IntType)val)
				{
					...
					case IntType(1):
					{
						return /*"easeinquad"*/ STR(578);
					}
					case IntType(2):
					{
						return /*"easeoutquad"*/ STR(579);
					}
					case IntType(3):
					{
						return /*"easeinoutquad"*/ STR(580);
					}
					case IntType(4):
					{
						return /*"instant"*/ STR(576);
					}
				}
	...
	void project_load_legacy_particles(ScopeAny self)
	{
		...
		ArrType boxtype = ArrType::From({ /*"none"*/ STR(908), /*"ground"*/ STR(143), /*"spawn"*/ STR(1373), /*"custom"*/ STR(628) });
	...
	void project_load_legacy_resource()
	{
		withOne (obj_resource, (new obj_resource)->id, noone)
		{
			...
			if (typename_ == /*"item"*/ STR(549))
				typename_ = /*"itemsheet"*/ STR(567);
			if (typename_ == /*"block"*/ STR(4) || typename_ == /*"blocksheet"*/ STR(569))
				typename_ = /*"legacyblocksheet"*/ STR(568);
			if (typename_ == /*"particles"*/ STR(551))
				typename_ = /*"particlesheet"*/ STR(571);
			if (typename_ == /*"schematic"*/ STR(2232))
				typename_ = /*"scenery"*/ STR(548);
	...
	BoolType project_load_legacy_start(VarType argument0)
	{
		...
		if (global::load_format > e_project_FORMAT_CB_103)
		{
			...
			{
				log({ /*"Too old legacy project, format"*/ STR(2235), global::load_format });
				error(/*"errorfilecorrupted"*/ STR(2234));
				buffer_delete(global::buffer_current);
				return false;
			}
	...
	void project_load_legacy_template()
	{
		withOne (obj_template, (new obj_template)->id, noone)
		{
			...
			if (self->type == e_temp_type_CHARACTER || self->type == e_temp_type_SPECIAL_BLOCK || self->type == e_temp_type_BODYPART)
			{
				...
				{
					self->model_name = DsMap(modelmap).Value(/*"name"*/ STR(819));
					if (!is_undefined(DsMap(modelmap).Value(/*"state"*/ STR(1026))))
						self->model_state = string_get_state_vars(DsMap(modelmap).Value(/*"state"*/ STR(1026)));
					else
					...
					log({ /*"Could not convert model "*/ STR(2236), self->legacy_model_name });
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (ds_list_valid(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
					...
					log({ /*"Could not convert model part of "*/ STR(2237), self->model_name, self->legacy_bodypart_id });
			...
			if (self->text_font == /*"root"*/ STR(2221))
		...
		withOne (obj_timeline, (new obj_timeline)->id, noone)
		{
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (!is_undefined(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
				{
					...
					log({ /*"Could not find model part for"*/ STR(2238), ObjType(obj_template, findtemp)->model_name, ObjType(obj_template, findtemp)->legacy_model_name, self->legacy_bodypart_id });
	...
	void project_load_legacy_values(ScopeAny self, IntType tl)
	{
		...
		if (idArr(tl, value_type).Value(e_value_type_PARTICLES))
		{
			...
			if (sVar(value).Value(e_value_ATTRACTOR) == /*"root"*/ STR(2221))
		...
		if (idArr(tl, value_type).Value(e_value_type_MATERIAL_TEXTURE))
		{
			...
			if (sVar(value).Value(e_value_TEXTURE_OBJ) == /*"root"*/ STR(2221))
	...
	RealType project_load_markers(VarType list)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			...
			withOne (obj_marker, (new obj_marker)->id, noone)
			{
				...
				self->name = value_get_string(DsMap(markermap).Value(/*"name"*/ STR(819)), /*""*/ STR(0));
				self->color = value_get_real(DsMap(markermap).Value(/*"color"*/ STR(920)), IntType(0));
			}
	...
	{
		VarType templist = DsMap(map).Value(/*"templates"*/ STR(2230));
		...
		VarType tllist = DsMap(map).Value(/*"timelines"*/ STR(2231));
