/*
	...
	[ Generated on 2023.10.16 22:12:02 ]
*/
...
namespace CppProject
{
	...
	void action_toolbar_new(ScopeAny self)
	{
		...
		{
			IntType res = show_message_ext(/*"Mine-imator"*/ STR(19), text_get({ /*"questionconfirmnew"*/ STR(73), sVar(project_name) }), text_get({ /*"questionsave"*/ STR(21) }), text_get({ /*"questiondontsave"*/ STR(22) }), text_get({ /*"questioncancel"*/ STR(23) }));
			if (res == IntType(0))
				project_save(self);
			else
				if (res != IntType(1))
	...
	{
		StringType fn = sStr(project_folder) + /*"/"*/ STR(16) + filename_name(sStr(project_folder)) + /*".backup1"*/ STR(75);
		if (!file_exists(fn))
			return IntType(0);
	...
	RealType action_toolbar_redo(ScopeAny self)
	{
		...
		log({ /*"Redo"*/ STR(76), script_get_name((IntType)(ObjType(obj_history, global::history_data)->script)) });
	...
	{
		project_save(self);
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertprojectsaved"*/ STR(77) }));
	}
	...
	RealType action_toolbar_undo(ScopeAny self)
	{
		...
		global::axis_edit = ObjType(obj_history, global::history_data)->save_axis_edit;
		global::save_id_seed = ObjType(obj_history, global::history_data)->save_save_id_seed;
		log({ /*"Undo"*/ STR(78), script_get_name((IntType)(ObjType(obj_history, global::history_data)->script)) });
	...
	BoolType app_event_game_end(ScopeAny self)
	{
		if (sVar(startup_error) > 0)
			return true;
		audio_stop_all();
		if (sStr(window_state) != /*"new_assets"*/ STR(79) && sStr(window_state) != /*"load_assets"*/ STR(80))
		{
			if (sInt(project_changed) > 0)
			{
				IntType res = show_message_ext(/*"Mine-imator"*/ STR(19), text_get({ /*"questionconfirmexit"*/ STR(81), sVar(project_name) }), text_get({ /*"questionsave"*/ STR(21) }), text_get({ /*"questiondontsave"*/ STR(22) }), text_get({ /*"questioncancel"*/ STR(23) }));
				if (res == IntType(0))
					project_save(self);
				else
					if (res != IntType(1))
	...
	{
		if (DsMap(gmlGlobal::async_load)[/*"id"*/ STR(83)] == sInt(http_assets) && DsMap(gmlGlobal::async_load)[/*"status"*/ STR(84)] < IntType(1) && (!dev_mode || dev_mode_check_assets))
		{
			sInt(http_assets) = null_;
			if (DsMap(gmlGlobal::async_load)[/*"status"*/ STR(84)] == IntType(0) && DsMap(gmlGlobal::async_load)[/*"http_status"*/ STR(85)] == http_ok)
			{
				IntType decodedmap = json_decode(DsMap(gmlGlobal::async_load)[/*"result"*/ STR(86)]);
				if (ds_map_valid(decodedmap))
				{
					VarType versionslist = DsMap(decodedmap).Value(/*"versions"*/ STR(87));
					if (ds_list_valid(versionslist))
					{
						VarType newversionmap = DsList(versionslist).Value(ds_list_size((IntType)(versionslist)) - IntType(1));
						if (ds_map_valid(newversionmap) && !file_exists_lib(minecraft_directory + DsMap(newversionmap).Value(/*"version"*/ STR(88)) + /*".zip"*/ STR(5)))
						{
							if (DsMap(newversionmap).Value(/*"format"*/ STR(89)) >= minecraft_assets_format)
							{
								sVar(setting_minecraft_assets_new_version) = DsMap(newversionmap).Value(/*"version"*/ STR(88));
								sVar(setting_minecraft_assets_new_format) = DsMap(newversionmap).Value(/*"format"*/ STR(89));
								sVar(setting_minecraft_assets_new_changes) = DsMap(newversionmap).Value(/*"changes"*/ STR(90));
								if (is_string(DsMap(newversionmap).Value(/*"image"*/ STR(91))))
								{
									sStr(setting_minecraft_assets_new_image) = mc_file_directory + DsMap(newversionmap).Value(/*"image"*/ STR(91));
									sInt(assets_http_image) = http_get_file(link_assets + DsMap(newversionmap).Value(/*"image"*/ STR(91)), sStr(setting_minecraft_assets_new_image));
								}
								else
									sStr(setting_minecraft_assets_new_image) = /*""*/ STR(0);
								
								toast_new(self, e_toast_INFO, text_get({ /*"alertnewassets"*/ STR(92), sVar(setting_minecraft_assets_new_version) }));
								ObjType(obj_toast, sInt(toast_last))->dismiss_time = no_limit;
								log({ /*"New assets found"*/ STR(93), sVar(setting_minecraft_assets_new_version) });
							}
						}
						else
							log({ /*"Using the latest assets"*/ STR(94) });
			...
			if (DsMap(gmlGlobal::async_load)[/*"id"*/ STR(83)] == sInt(http_download_assets_file))
			{
				if (DsMap(gmlGlobal::async_load)[/*"status"*/ STR(84)] == IntType(1))
					sReal(new_assets_download_progress) = ((RealType)DsMap(gmlGlobal::async_load)[/*"sizeDownloaded"*/ STR(95)] / DsMap(gmlGlobal::async_load)[/*"contentLength"*/ STR(96)]) * 0.25;
				else
				{
				...
				if (DsMap(gmlGlobal::async_load)[/*"id"*/ STR(83)] == sInt(http_download_assets_zip))
				{
					if (DsMap(gmlGlobal::async_load)[/*"status"*/ STR(84)] == IntType(1))
						sReal(new_assets_download_progress) = 0.25 + ((RealType)DsMap(gmlGlobal::async_load)[/*"sizeDownloaded"*/ STR(95)] / DsMap(gmlGlobal::async_load)[/*"contentLength"*/ STR(96)]) * 0.75;
					else
					{
						...
						file_copy_lib(mc_file_directory + sStr(new_assets_version) + /*".midata"*/ STR(97), minecraft_directory + sStr(new_assets_version) + /*".midata"*/ STR(97));
						file_copy_lib(mc_file_directory + sStr(new_assets_version) + /*".zip"*/ STR(5), minecraft_directory + sStr(new_assets_version) + /*".zip"*/ STR(5));
						file_delete_lib(mc_file_directory + sStr(new_assets_version) + /*".midata"*/ STR(97));
					...
					if (DsMap(gmlGlobal::async_load)[/*"id"*/ STR(83)] == sInt(http_alert_news) && DsMap(gmlGlobal::async_load)[/*"status"*/ STR(84)] < IntType(1))
					{
						...
						if (DsMap(gmlGlobal::async_load)[/*"status"*/ STR(84)] == IntType(0) && DsMap(gmlGlobal::async_load)[/*"http_status"*/ STR(85)] == http_ok)
						{
							...
							if (ds_map_valid(decodedmap))
							{
								VarType newslist = DsMap(decodedmap).Value(/*"default"*/ STR(99));
								for (IntType n = IntType(0); n < ds_list_size((IntType)(newslist)); n++)
								{
									...
									{
										text = DsMap(newsmap).Value(/*"text"*/ STR(100));
										icon = DsMap(newsmap).Value(/*"icon"*/ STR(101));
										...
										button = DsMap(newsmap).Value(/*"button"*/ STR(108));
										buttonurl = DsMap(newsmap).Value(/*"buttonurl"*/ STR(109));
										if (!is_undefined(DsMap(newsmap).Value(/*"saveclose"*/ STR(110))))
											iid = DsMap(newsmap).Value(/*"id"*/ STR(83));
										else
											iid = null_;
								...
								log({ /*"Failed to decode"*/ STR(111) });
							
						}
					}
					else
						if (DsMap(gmlGlobal::async_load)[/*"id"*/ STR(83)] == sInt(http_downloadskin) && DsMap(gmlGlobal::async_load)[/*"status"*/ STR(84)] < IntType(1))
						{
							sInt(http_downloadskin) = null_;
							if (sInt(popup) == sInt(popup_downloadskin))
							{
								ObjType(obj_popup, sInt(popup_downloadskin))->fail_message = text_get({ /*"errordownloadskininternet"*/ STR(112) });
							...
							if (DsMap(gmlGlobal::async_load)[/*"status"*/ STR(84)] == IntType(0))
							{
								if (sInt(popup) == sInt(popup_downloadskin))
									ObjType(obj_popup, sInt(popup_downloadskin))->fail_message = text_get({ /*"errordownloadskinuser"*/ STR(113), string_remove_newline(ObjType(obj_popup, sInt(popup_downloadskin))->username) });
								else
									ObjType(obj_builder, global::mc_builder)->block_skull_texture_fail = true;
								
								if (DsMap(gmlGlobal::async_load)[/*"http_status"*/ STR(85)] == http_ok && file_exists_lib(download_image_file))
								{
									if (sInt(popup) == sInt(popup_downloadskin))
									{
	...
	RealType app_event_step(ScopeAny self)
	{
		...
		if (window_get_current() == e_window_MAIN || (window_get_current() != e_window_MAIN && dev_mode && !is_cpp()))
		{
			...
			if (sStr(window_state) == /*"load_assets"*/ STR(80))
				return IntType(0);
			if (sStr(window_state) == /*"new_assets"*/ STR(79) || sStr(window_state) == /*"export_movie"*/ STR(49) || sStr(window_state) == /*"export_image"*/ STR(56))
				return IntType(0);
