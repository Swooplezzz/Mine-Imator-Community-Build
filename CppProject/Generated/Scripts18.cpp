...
namespace CppProject
{
	...
	BoolType asset_load(ScopeAny self, VarArgs argument)
	{
		...
		if (ext == /*".zip"*/ STR(5))
		{
			...
			{
				if (directory_exists_lib(unzip_directory + /*"assets"*/ STR(710)))
					action_res_pack_load(self, fn);
				else
					error(/*"erroropenassetzip"*/ STR(711));
		...
		switch (StringType(ext).id)
		{
			...
			case 722: // .blocks
			{
				log({ /*"Opening scenery"*/ STR(723), fn });
			...
			case 732: // .m4a
			{
				log({ /*"Opening audio"*/ STR(733), fn });
				action_res_sound_load(self, fn);
				return true;
			}
			case 734: // .ttf
			{
				log({ /*"Adding font"*/ STR(735), fn });
		...
		switch (StringType(ext).id)
		{
			...
			case 713: // .miparticles
			{
				...
				withOne (obj_template, temp, self->id)
				{
					...
					project_load_particles(ScopeAny(self), DsMap(rootmap).Value(/*"particles"*/ STR(538)));
