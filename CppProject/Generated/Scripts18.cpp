...
namespace CppProject
{
	...
	void app_update_tl_edit_select(ScopeAny self)
	{
		...
		if (sInt(timeline_settings_import_loop_tl) != null_)
		{
			...
			sStr(timeline_settings_walk_fn) = global::load_assets_startup_dir + mc_loops_directory + name + /*"_walk.miframes"*/ STR(694);
			sStr(timeline_settings_run_fn) = global::load_assets_startup_dir + mc_loops_directory + name + /*"_run.miframes"*/ STR(695);
	...
	BoolType asset_load(ScopeAny self, VarArgs argument)
	{
		...
		if (ext == /*".zip"*/ STR(5))
		{
			...
			{
				if (directory_exists_lib(unzip_directory + /*"assets"*/ STR(697)))
					action_res_pack_load(self, fn);
				else
					error(/*"erroropenassetzip"*/ STR(698));
		...
		switch (StringType(ext).id)
		{
			...
			case 709: // .blocks
			{
				log({ /*"Opening scenery"*/ STR(710), fn });
			...
			case 719: // .m4a
			{
				log({ /*"Opening audio"*/ STR(720), fn });
				action_res_sound_load(self, fn);
				return true;
			}
			case 721: // .ttf
			{
				log({ /*"Adding font"*/ STR(722), fn });
		...
		{
			log({ /*"Opening legacy asset"*/ STR(727), fn });
	...
	void background_ground_startup()
	{
		...
		log({ /*"Ground vbuffer init"*/ STR(731) });
