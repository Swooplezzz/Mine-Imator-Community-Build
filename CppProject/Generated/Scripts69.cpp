/*
	...
	[ Generated on 2024.01.16 23:32:14 ]
*/
...
namespace CppProject
{
	...
	RealType tl_update_model_shape_bend(ScopeAny self)
	{
		...
		if (sVar(value).Value(e_value_BEND_IK_TARGET) != null_)
		{
			...
			bend = rot_target;
			bend.Real(X_) *= (idVec(sVar(model_part), bend_invert).Real(X_) ? -1.0 : 1.0);
			bend.Real(Y_) *= (idVec(sVar(model_part), bend_invert).Real(Y_) ? -1.0 : 1.0);
			bend.Real(Z_) *= (idVec(sVar(model_part), bend_invert).Real(Z_) ? -1.0 : 1.0);
			sArr(value_inherit)[e_value_BEND_ANGLE_X] = bend.Real(X_);
			sArr(value_inherit)[e_value_BEND_ANGLE_Y] = bend.Real(Y_);
			sArr(value_inherit)[e_value_BEND_ANGLE_Z] = bend.Real(Z_);
		}
	...
	{
		sVar(type_name) = text_get({ /*"type"*/ STR(806) + DsList(global::tl_type_name_list).Value(sVar(type)) });
		if (sVar(part_of) != null_)
		{
			if (sVar(type) == e_tl_type_BODYPART)
			{
				if (sVar(model_part) != null_)
					sVar(type_name) = minecraft_asset_get_name(/*"modelpart"*/ STR(779), idVar(sVar(model_part), name));
				else
					sVar(type_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3935) });
			...
			sVar(type_name) = text_get({ /*"timelinepartof"*/ STR(3936), sVar(type_name), string_remove_newline(idVar(sVar(part_of), display_name)) });
		}
		else
			if (sVar(temp) != null_)
				sVar(type_name) = text_get({ /*"timelineinstanceof"*/ STR(3937), idVar(sVar(temp), display_name) });
	...
	void tl_update_values(ScopeAny self)
	{
		...
		if (sVar(keyframe_transition) == /*"bezier"*/ STR(578))
