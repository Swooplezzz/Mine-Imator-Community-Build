...
namespace CppProject
{
	...
	void tl_update_type_name(ScopeAny self)
	{
		...
		if (sVar(part_of) != null_)
		{
			...
			{
				if (sVar(model_part) != null_)
					sVar(type_name) = minecraft_asset_get_name(/*"modelpart"*/ STR(774), idVar(sVar(model_part), name));
				else
					sVar(type_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3908) });
			...
			sVar(type_name) = text_get({ /*"timelinepartof"*/ STR(3909), sVar(type_name), string_remove_newline(idVar(sVar(part_of), display_name)) });
		}
		else
			if (sVar(temp) != null_)
				sVar(type_name) = text_get({ /*"timelineinstanceof"*/ STR(3910), idVar(sVar(temp), display_name) });
