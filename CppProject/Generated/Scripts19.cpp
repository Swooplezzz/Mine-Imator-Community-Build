/*
	...
	[ Generated on 2023.12.17 21:19:21 ]
*/
...
namespace CppProject
{
	...
	void app_update_tl_edit_select(ScopeAny self)
	{
		...
		if (sInt(timeline_settings_import_loop_tl) != null_)
		{
			...
			sStr(timeline_settings_walk_fn) = global::load_assets_startup_dir + mc_loops_directory + name + /*"_walk.miframes"*/ STR(705);
			sStr(timeline_settings_run_fn) = global::load_assets_startup_dir + mc_loops_directory + name + /*"_run.miframes"*/ STR(706);
	...
	BoolType asset_load(ScopeAny self, VarArgs argument)
	{
		...
		show_debug_message(/*"FILE COUNT = "*/ STR(709) + string(path_array_count));
		...
		for (IntType i = IntType(0); i < path_array_count; i += IntType(1))
		{
			...
			if (ext == /*".zip"*/ STR(7))
			{
				...
				{
					if (directory_exists_lib(unzip_directory + /*"assets"*/ STR(714)))
						action_res_pack_load(self, fn);
					else
						error(/*"erroropenassetzip"*/ STR(715));
			...
			switch (StringType(ext).id)
			{
				...
				case 726: // .blocks
				{
					log({ /*"Opening scenery"*/ STR(727), fn });
				...
				case 736: // .m4a
				{
					log({ /*"Opening audio"*/ STR(737), fn });
					action_res_sound_load(self, fn);
					return true;
				}
				case 738: // .ttf
				{
					log({ /*"Adding font"*/ STR(739), fn });
			...
			{
				log({ /*"Opening legacy asset"*/ STR(741), fn });
			...
			switch (StringType(ext).id)
			{
				...
				case 717: // .miparticles
				{
					...
					withOne (obj_template, temp, self->id)
					{
						...
						project_load_particles(ScopeAny(self), DsMap(rootmap).Value(/*"particles"*/ STR(549)));
	...
	void background_ground_startup()
	{
		...
		log({ /*"Ground vbuffer init"*/ STR(745) });
