...
namespace CppProject
{
	...
	BoolType asset_load(ScopeAny self, VarArgs argument)
	{
		...
		if (ext == /*".zip"*/ STR(7))
		{
			...
			{
				if (directory_exists_lib(unzip_directory + /*"assets"*/ STR(708)))
					action_res_pack_load(self, fn);
				else
					error(/*"erroropenassetzip"*/ STR(709));
		...
		switch (StringType(ext).id)
		{
			...
			case 720: // .blocks
			{
				log({ /*"Opening scenery"*/ STR(721), fn });
			...
			case 730: // .m4a
			{
				log({ /*"Opening audio"*/ STR(731), fn });
				action_res_sound_load(self, fn);
				return true;
			}
			case 732: // .ttf
			{
				log({ /*"Adding font"*/ STR(733), fn });
