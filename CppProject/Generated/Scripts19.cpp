...
namespace CppProject
{
	...
	void app_update_tl_edit_select(ScopeAny self)
	{
		...
		withAll (obj_timeline, self->id)
		{
			...
			if (self->value_type.Value(e_value_type_TRANSFORM_POS))
		...
		if (sInt(timeline_settings_import_loop_tl) != null_)
		{
			...
			sStr(timeline_settings_walk_fn) = global::load_assets_startup_dir + mc_loops_directory + name + /*"_walk.miframes"*/ STR(708);
			sStr(timeline_settings_run_fn) = global::load_assets_startup_dir + mc_loops_directory + name + /*"_run.miframes"*/ STR(709);
	...
	BoolType asset_load(ScopeAny self, VarArgs argument)
	{
		...
		show_debug_message(/*"FILE COUNT = "*/ STR(712) + string(path_array_count));
		...
		for (IntType i = IntType(0); i < path_array_count; i += IntType(1))
		{
			...
			if (ext == /*".zip"*/ STR(7))
			{
				...
				{
					if (directory_exists_lib(unzip_directory + /*"assets"*/ STR(717)))
						action_res_pack_load(self, fn);
					else
						error(/*"erroropenassetzip"*/ STR(718));
			...
			switch (StringType(ext).id)
			{
				...
				case 729: // .blocks
				{
					log({ /*"Opening scenery"*/ STR(730), fn });
				...
				case 739: // .m4a
				{
					log({ /*"Opening audio"*/ STR(740), fn });
					action_res_sound_load(self, fn);
					return true;
				}
				case 741: // .ttf
				{
					log({ /*"Adding font"*/ STR(742), fn });
			...
			{
				log({ /*"Opening legacy asset"*/ STR(744), fn });
			...
			switch (StringType(ext).id)
			{
				...
				case 720: // .miparticles
				{
					...
					withOne (obj_template, temp, self->id)
					{
						...
						project_load_particles(ScopeAny(self), DsMap(rootmap).Value(/*"particles"*/ STR(552)));
	...
	void background_ground_startup()
	{
		...
		log({ /*"Ground vbuffer init"*/ STR(748) });
