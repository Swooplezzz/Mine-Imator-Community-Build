...
namespace CppProject
{
	...
	RealType view_draw(ScopeAny self, IntType view)
	{
		...
		if (sVar(window_busy) == /*"viewmove"*/ STR(3945) && view == sInt(view_second))
		{
		...
		if (view == sInt(view_main) && ObjType(obj_view, sInt(view_second))->show > 0 && !window_exists(e_window_VIEW_SECOND) && sVar(window_busy) != /*"viewmove"*/ STR(3945))
		{
			if (ObjType(obj_view, sInt(view_second))->location == /*"left_top"*/ STR(3944))
				captionx += ObjType(obj_view, sInt(view_second))->width;
			if (ObjType(obj_view, sInt(view_second))->location == /*"right_top"*/ STR(3942))
				dx -= ObjType(obj_view, sInt(view_second))->width;
			if (ObjType(obj_view, sInt(view_second))->location == /*"left_top"*/ STR(3944) || ObjType(obj_view, sInt(view_second))->location == /*"right_top"*/ STR(3942))
		...
		if (view == sInt(view_second))
		{
			if (window_get_current() != e_window_VIEW_SECOND)
			{
				if (draw_button_icon(self, { /*"viewclose"*/ STR(3946), dx, dy, sVar(dw), sVar(dh), false, icons_CLOSE, null_, false, /*"viewclose"*/ STR(3946) }))
				{
					ObjType(obj_view, view)->show = false;
					sBool(view_render) = false;
				}
				dx -= sVar(dw) + padding;
				if (draw_button_icon(self, { /*"viewpopout"*/ STR(3947), dx, dy, sVar(dw), sVar(dh), false, icons_EXTERNAL, null_, false, /*"viewpopout"*/ STR(3947) }))
					window_create(e_window_VIEW_SECOND, boxx, boxy, boxw, boxh);
			}
			else
			{
				if (draw_button_icon(self, { /*"viewpopout"*/ STR(3947), dx, dy, sVar(dw), sVar(dh), false, icons_INTERNAL, null_, false, /*"viewpopin"*/ STR(3948) }))
				{
		...
		else
		{
			...
			if (draw_button_icon(self, { /*"viewsecond"*/ STR(3949), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, sInt(view_second))->show, icons_VIEWPORT_SECONDARY, null_, false, (ObjType(obj_view, sInt(view_second))->show > 0) ? /*"viewseconddisable"*/ STR(3950) : /*"viewsecondenable"*/ STR(3951) }))
		...
		dx -= 16.0 + padding;
		if (draw_button_icon(self, { /*"viewqualitysettings"*/ STR(3952), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3952)), icons_CHEVRON_DOWN_TINY }))
		{
			menu_settings_set(self, dx, dy, (string(view) + /*"viewqualitysettings"*/ STR(3952)), IntType(24));
			sInt(settings_menu_view) = view;
			sInt(settings_menu_script) = ID_menu_quality_settings;
		}
		if (sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3952)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(748))
		...
		if (draw_button_icon(self, { /*"viewmoderender"*/ STR(3953), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_RENDER, ObjType(obj_theme, sInt(setting_theme))->dark ? icons_SPHERE_MATERIAL__DARK : icons_SPHERE_MATERIAL, null_, false, /*"viewmoderender"*/ STR(3953) }))
		{
		...
		dx -= sVar(dw) + padding;
		tip_set_keybind(self, e_keybind_RENDER_MODE);
		if (draw_button_icon(self, { /*"viewmodeshaded"*/ STR(3954), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_SHADED, ObjType(obj_theme, sInt(setting_theme))->dark ? icons_SPHERE_SHADING__DARK : icons_SPHERE_SHADING, null_, false, /*"viewmodeshaded"*/ STR(3954) }))
			ObjType(obj_view, view)->quality = e_view_mode_SHADED;
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewmodeflat"*/ STR(3955), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_FLAT, icons_SPHERE_FLAT, null_, false, /*"viewmodeflat"*/ STR(3955) }))
			ObjType(obj_view, view)->quality = e_view_mode_FLAT;
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewparticles"*/ STR(3956), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->particles, icons_FIREWORKS, null_, false, (ObjType(obj_view, view)->particles > 0) ? /*"viewparticlesdisable"*/ STR(3957) : /*"viewparticlesenable"*/ STR(3958) }))
			ObjType(obj_view, view)->particles = !(ObjType(obj_view, view)->particles > 0);
		if (cam != null_)
		{
			dx -= sVar(dw) + padding;
			if (draw_button_icon(self, { /*"vieweffects"*/ STR(3959), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->effects, icons_WAND, null_, false, (ObjType(obj_view, view)->effects > 0) ? /*"vieweffectsdisable"*/ STR(3960) : /*"vieweffectsenable"*/ STR(3961) }))
			{
		...
		draw_divide_vertical(dx, dy, sVar(dh));
		dx -= 16.0 + padding;
		if (draw_button_icon(self, { /*"viewgridsettings"*/ STR(3962), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3962)), icons_CHEVRON_DOWN_TINY }))
		{
			menu_settings_set(self, dx, dy, (string(view) + /*"viewgridsettings"*/ STR(3962)), IntType(24));
			sInt(settings_menu_view) = view;
			sInt(settings_menu_script) = ID_menu_grid_settings;
		}
		if (sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3962)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(748))
			ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
		dx -= sVar(dw);
		if (draw_button_icon(self, { /*"viewgrid"*/ STR(3963), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->grid, icons_GRID, null_, false, (ObjType(obj_view, view)->grid > 0) ? /*"viewgriddisable"*/ STR(3964) : /*"viewgridenable"*/ STR(3965) }))
			ObjType(obj_view, view)->grid = !(ObjType(obj_view, view)->grid > 0);
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewaspectratio"*/ STR(3966), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->aspect_ratio, icons_LETTERBOX, null_, false, (ObjType(obj_view, view)->aspect_ratio > 0) ? /*"viewaspectratiodisable"*/ STR(3967) : /*"viewaspectratioenable"*/ STR(3968) }))
			ObjType(obj_view, view)->aspect_ratio = !(ObjType(obj_view, view)->aspect_ratio > 0);
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewoverlays"*/ STR(3969), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->gizmos, icons_OVERLAYS, null_, false, (ObjType(obj_view, view)->gizmos > 0) ? /*"viewoverlaysdisable"*/ STR(3970) : /*"viewoverlaysenable"*/ STR(3971) }))
		...
		if (view == sInt(view_main))
		{
			...
			draw_divide_vertical(dx, dy, sVar(dh));
			dx -= 16.0 + padding;
			if (draw_button_icon(self, { /*"viewsnapsettings"*/ STR(3972), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3972)), icons_CHEVRON_DOWN_TINY }))
			{
				menu_settings_set(self, dx, dy, (string(view) + /*"viewsnapsettings"*/ STR(3972)), IntType(24));
				sInt(settings_menu_view) = view;
				sInt(settings_menu_script) = ID_menu_snap_settings;
			}
			if (sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3972)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(748))
				ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
			dx -= sVar(dw);
			tip_set_keybind(self, e_keybind_SNAP);
			if (draw_button_icon(self, { /*"viewsnap"*/ STR(3973), dx, dy, sVar(dw), sVar(dh), sReal(setting_snap), icons_MAGNET, null_, false, (sReal(setting_snap) > 0) ? /*"viewsnapdisable"*/ STR(3974) : /*"viewsnapenable"*/ STR(3975) }))
				sReal(setting_snap) = !(sReal(setting_snap) > 0);
		}
		StringType listname;
		BoolType menuactive;
		listname = (view == sInt(view_main) ? /*"viewcameramain"*/ STR(1496) : /*"viewcamerasecond"*/ STR(1497));
		menuactive = false;
		...
		if (ObjType(obj_view, view)->camera == -IntType(4) && cam == -IntType(4))
			camname = text_get({ /*"viewworkcamera"*/ STR(3976) });
		else
			if (ObjType(obj_view, view)->camera == -IntType(5))
				camname = text_get({ /*"viewactivecamera"*/ STR(3977), (cam == -IntType(4) ? text_get({ /*"viewworkcamera"*/ STR(3976) }) : string_remove_newline(idVar(cam, display_name))) });
			else
		...
		if (location != /*"full"*/ STR(2788) && location != /*"top"*/ STR(200) && location != /*"bottom"*/ STR(199))
		{
			if (!string_contains(location, /*"left"*/ STR(201)))
			{
				sVar(content_x) += IntType(3);
				sVar(content_width) -= 3.0;
			}
			if (!string_contains(location, /*"right"*/ STR(202)))
				sVar(content_width) -= 3.0;
			if (location == /*"right_top"*/ STR(3942) || location == /*"left_top"*/ STR(3944))
		...
		if (sVar(content_width) > IntType(0) && sVar(content_height) > IntType(0))
		{
			...
			if (sBool(content_mouseon))
				sStr(shortcut_bar_state) = /*"viewport"*/ STR(124) + (cam == null_ ? /*""*/ STR(0) : /*"cam"*/ STR(3978));
		...
		if (view == sInt(view_main))
		{
			...
			if (sBool(bench_open))
			{
				sStr(bench_show_ani_type) = /*"show"*/ STR(749);
			...
			if (ObjType(obj_view, sInt(view_main))->mouseon && app_mouse_box(self, benchx, benchy, IntType(86), IntType(86)) && !sBool(popup_mouseon) && !sBool(toast_mouseon) && !sBool(context_menu_mouseon) && !(ObjType(obj_view, sInt(view_second))->show > 0 && ObjType(obj_view, sInt(view_second))->mouseon))
			{
				...
				tip_set(self, text_get({ /*"viewworkbenchtip"*/ STR(3979) }), benchx, benchy, IntType(86), IntType(86), false);
			...
			if (sVar(window_busy) == /*"bench"*/ STR(750) || sVar(bench_show_ani) == IntType(1))
			{
				sReal(bench_hover_ani_goal) = .5;
				sReal(bench_click_ani_goal) = .9;
			}
		...
		if (view == sInt(view_second) && window_get_current() != e_window_VIEW_SECOND)
		{
			if (ObjType(obj_view, view)->mouseon && (sInt(mouse_cursor) == cr_default || sBool(content_mouseon)))
			{
				...
				{
					if (ObjType(obj_view, view)->location == /*"right_top"*/ STR(3942) || ObjType(obj_view, view)->location == /*"left_bottom"*/ STR(3943))
					...
					if (sBool(mouse_left_pressed))
					{
						sVar(window_busy) = /*"viewresizeboth"*/ STR(3938);
					...
					if (mouseonresizehor)
					{
						...
						if (sBool(mouse_left_pressed))
						{
							sVar(window_busy) = /*"viewresizehor"*/ STR(3937);
							sVar(view_resize_width) = ObjType(obj_view, view)->width;
						}
					}
					else
						if (mouseonresizever)
						{
							...
							if (sBool(mouse_left_pressed))
							{
								sVar(window_busy) = /*"viewresizever"*/ STR(3939);
								sVar(view_resize_height) = ObjType(obj_view, view)->height;
							}
						}
						else
							if (app_mouse_box(self, boxx, boxy, boxw, captionh) && !sBool(popup_mouseon) && sBool(mouse_left_pressed))
								sVar(window_busy) = /*"viewclickcaption"*/ STR(3980);
					
				
				
			}
			if (sVar(window_busy) == /*"viewclickcaption"*/ STR(3980))
			{
				if (sReal(mouse_move) > IntType(10))
				{
					ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2788);
					sVar(window_busy) = /*"viewmove"*/ STR(3945);
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3945))
			{
				...
				if (!sBool(mouse_left))
				{
					...
					switch (StringType(ObjType(obj_view, view)->location).id)
					{
						case 200: // top
						{
							ObjType(obj_view, sInt(view_main))->location = /*"bottom"*/ STR(199);
							break;
						}
						case 199: // bottom
						{
							ObjType(obj_view, sInt(view_main))->location = /*"top"*/ STR(200);
							break;
						}
						case 202: // right
						{
							ObjType(obj_view, sInt(view_main))->location = /*"left"*/ STR(201);
							break;
						}
						case 201: // left
						{
							ObjType(obj_view, sInt(view_main))->location = /*"right"*/ STR(202);
							break;
						}
						default:
							ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2788);
		...
		{
			StringType mouselocation = /*""*/ STR(0);
			if (ObjType(obj_view, view)->location == /*"right"*/ STR(202) || ObjType(obj_view, view)->location == /*"left"*/ STR(201))
			{
				sInt(mouse_cursor) = cr_size_we;
				mouselocation = /*"hor"*/ STR(3981);
			}
			else
			{
				sInt(mouse_cursor) = cr_size_ns;
				mouselocation = /*"ver"*/ STR(3982);
			}
			
			if (sBool(mouse_left_pressed))
				sVar(window_busy) = /*"viewresizesplit"*/ STR(3983) + mouselocation;
		}
		if (view == sInt(view_second))
		{
			...
			lineleft = false;
			lineright = false;
			if (location == /*"top"*/ STR(200) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3941)))
				linetop = true;
			if (location == /*"bottom"*/ STR(199) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3941)))
				linebottom = true;
			if (location == /*"left"*/ STR(201) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3940)))
				lineright = true;
			if (location == /*"right"*/ STR(202) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3940)))
				lineleft = true;
			if ((mouseonresizever || mouseonresizehor) || (sVar(window_busy) == /*"viewresizeboth"*/ STR(3938) || sVar(window_busy) == /*"viewresizever"*/ STR(3939) || sVar(window_busy) == /*"viewresizehor"*/ STR(3937)))
			{
				if (location == /*"left_top"*/ STR(3944))
				{
					lineright = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3937) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3938));
					linetop = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3939) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3938));
				}
				if (location == /*"right_top"*/ STR(3942))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3937) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3938));
					linetop = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3939) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3938));
				}
				if (location == /*"left_bottom"*/ STR(3943))
				{
					lineright = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3937) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3938));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3939) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3938));
				}
				if (location == /*"right_bottom"*/ STR(2789))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3937) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3938));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3939) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3938));
				}
		...
		if (ObjType(obj_view, view)->quality == e_view_mode_RENDER)
		{
			...
			if (sBool(view_render_real_time))
				infotext = text_get({ /*"viewrenderfps"*/ STR(3984), string(gmlGlobal::fps), max({ IntType(1), global::render_samples }), sVar(project_render_samples) });
			else
				infotext = /*""*/ STR(0);
			...
			{
				infotext = text_get({ /*"viewrenderpass"*/ STR(3985), text_get({ /*"viewmodepass"*/ STR(1495) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) }) });
	...
	{
		self->show = true;
		self->location = /*"full"*/ STR(2788);
		self->location_last = /*"full"*/ STR(2788);
		self->surface = null_;
