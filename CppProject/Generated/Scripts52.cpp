...
namespace CppProject
{
	...
	RealType render_world_ground(ScopeAny self)
	{
		...
		if (global::render_mode == e_render_mode_SCENE_TEST || global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2593), c_white, IntType(1));
		VarType blend = block_texture_get_blend(sVar(background_ground_name), sVar(background_ground_tex));
		BoolType iswater = (sVar(background_ground_name) == /*"block/water_flow"*/ STR(1780) || sVar(background_ground_name) == /*"block/water_still"*/ STR(1778));
		render_set_uniform_int(/*"uIsGround"*/ STR(2594), IntType(1));
		render_set_uniform_color(/*"uBlendColor"*/ STR(1962), blend, IntType(1));
		render_set_uniform_color(/*"uGlowColor"*/ STR(2595), c_black, IntType(1));
		render_set_uniform_int(/*"uGlowTexture"*/ STR(2596), IntType(0));
		render_set_uniform_int(/*"uFogShow"*/ STR(1971), global::_app->background_fog_show);
		render_set_uniform_int(/*"uIsWater"*/ STR(2592), iswater);
		render_set_uniform_int(/*"uMaterialFormat"*/ STR(2586), idVar(sVar(background_ground_tex_material), material_format));
		if (sVar(background_ground_tex_material) == global::mc_res)
		{
			render_set_uniform(/*"uMetallic"*/ STR(2589), IntType(0));
			render_set_uniform(/*"uRoughness"*/ STR(2590), (iswater && global::_app->project_render_water_reflections > 0 ? .07 : 1.0));
			render_set_uniform(/*"uEmissive"*/ STR(2591), IntType(0));
		}
		else
		{
			render_set_uniform(/*"uMetallic"*/ STR(2589), IntType(0));
			render_set_uniform(/*"uRoughness"*/ STR(2590), IntType(0));
			render_set_uniform(/*"uEmissive"*/ STR(2591), IntType(0));
		}
		...
		if (sBool(background_ground_material_ani))
			render_set_texture(sArr(background_ground_ani_texture_material).Value(block_texture_get_frame()), /*"Material"*/ STR(2587));
		else
			render_set_texture(sInt(background_ground_texture_material), /*"Material"*/ STR(2587));
		
		if (sBool(background_ground_normal_ani))
			render_set_texture(sArr(background_ground_ani_texture_normal).Value(block_texture_get_frame()), /*"Normal"*/ STR(2588));
		else
			render_set_texture(sInt(background_ground_texture_normal), /*"Normal"*/ STR(2588));
	...
	void render_world_item(VarType vbuffer, VarType is3d, VarType facecamera, VarType bounce, VarType rotate, ArrType res)
	{
		...
		if (bounce > 0)
		{
			...
			if (t < d)
				offz = ease(/*"easeinoutquad"*/ STR(582), t / d) * IntType(2) - IntType(1);
			else
				offz = IntType(1) - ease(/*"easeinoutquad"*/ STR(582), (t - d) / d) * IntType(2);
		...
		if (res.Value(IntType(1)) != null_ && res.Value(IntType(1)) != global::mc_res)
		{
			if (global::shader_uniform_metallic != IntType(0))
			{
				...
				render_set_uniform(/*"uMetallic"*/ STR(2589), global::shader_uniform_metallic);
			}
			if (global::shader_uniform_roughness != IntType(0))
			{
				global::shader_uniform_roughness = IntType(0);
				render_set_uniform(/*"uRoughness"*/ STR(2590), global::shader_uniform_roughness);
			}
			if (global::shader_uniform_emissive != IntType(0))
			{
				global::shader_uniform_emissive = IntType(0);
				render_set_uniform(/*"uEmissive"*/ STR(2591), global::shader_uniform_emissive);
			}
			if (idInt(res.Value(IntType(1)), item_sheet_texture_material) != null_)
				render_set_texture(idInt(res.Value(IntType(1)), item_sheet_texture_material), /*"Material"*/ STR(2587));
			else
				render_set_texture(idVar(res.Value(IntType(1)), texture), /*"Material"*/ STR(2587));
			
			render_set_uniform_int(/*"uMaterialFormat"*/ STR(2586), idVar(res.Value(IntType(1)), material_format));
		}
		else
		{
			render_set_texture(ID_spr_default_material, /*"Material"*/ STR(2587));
			render_set_uniform_int(/*"uMaterialFormat"*/ STR(2586), e_material_FORMAT_NONE);
		}
		
		if (res.Value(IntType(2)) != null_ && res.Value(IntType(2)) != global::mc_res)
		{
			if (idInt(res.Value(IntType(2)), item_sheet_tex_normal) != null_)
				render_set_texture(idInt(res.Value(IntType(2)), item_sheet_tex_normal), /*"Normal"*/ STR(2588));
			else
				render_set_texture(idVar(res.Value(IntType(2)), texture), /*"Normal"*/ STR(2588));
			
		}
		else
			render_set_texture(ID_spr_default_normal, /*"Normal"*/ STR(2588));
	...
	void render_world_model_file_parts(ScopeAny self, VarType modelfile, VarType res, IntType texnamemap, IntType hidelist, IntType shapevbuffermap, IntType colornamemap, IntType shapehidelist, IntType shapetexnamemap, IntType matrixmap)
	{
		...
		for (IntType p = IntType(0); p < ds_list_size(partlist); p++)
		{
			...
			if (idBool(part, part_mixing_shapes))
				render_set_uniform_int(/*"uColorsExt"*/ STR(2134), idBool(part, part_mixing_shapes));
	...
	RealType render_world_model_part(ScopeAny self, VarType part, VarType res, IntType texnamemap, IntType shapevbuffermap, IntType colornamemap, IntType shapehidelist, IntType shapetexnamemap, VarType tlobject)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			if (tlobject > 0)
			{
				...
				render_set_uniform_int(/*"uMaterialFormat"*/ STR(2586), sArr(model_part_shape_material_res).Value(s));
				...
				{
					render_set_texture(sArr(model_part_shape_tex_material).Value(s), /*"Material"*/ STR(2587));
				...
				if (sArr(model_part_shape_tex_normal).Value(s) == null_)
					...
					render_set_texture(sArr(model_part_shape_tex_normal).Value(s), /*"Normal"*/ STR(2588));
			...
			if (self->subAssetId == ID_obj_preview && Obj(sVar(select))->subAssetId != ID_obj_resource && idVar(sVar(select), model_name) == /*"armor"*/ STR(783))
			{
				if (ObjType(obj_model_shape, shape)->description == /*"helmet"*/ STR(2045))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"chestplate"*/ STR(2046))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"leggings"*/ STR(2047))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"boots"*/ STR(2048))
				{
			...
			if (tlobject != null_ && idVar(idVar(tlobject, temp), model_name) == /*"armor"*/ STR(783))
			{
				if (ObjType(obj_model_shape, shape)->description == /*"helmet"*/ STR(2045))
				{
			...
			{
				render_set_uniform_color(/*"uBlendColor"*/ STR(1962), blendcolor, alpha);
			...
			{
				if (tlobject != null_)
					render_set_uniform_color(/*"uMixColor"*/ STR(2140), merge_color((IntType)(ObjType(obj_model_shape, shape)->color_mix), (IntType)(sArr(value_inherit).Value(e_value_MIX_COLOR)), sArr(value_inherit).Value(e_value_MIX_PERCENT)), lerp(ObjType(obj_model_shape, shape)->color_mix_percent, sArr(value_inherit).Value(e_value_MIX_PERCENT), sArr(value_inherit).Value(e_value_MIX_PERCENT)));
				else
					render_set_uniform_color(/*"uMixColor"*/ STR(2140), ObjType(obj_model_shape, shape)->color_mix, ObjType(obj_model_shape, shape)->color_mix_percent);
	...
	RealType render_world_particle(ScopeAny self)
	{
		...
		render_set_uniform_color(/*"uBlendColor"*/ STR(1962), global::shader_blend_color, global::shader_blend_alpha);
	...
	void render_world_shape(VarType type, VarType vbuf, VarType facecamera, ArrType tex)
	{
		...
		render_set_texture(tex.Value(IntType(1)), /*"Material"*/ STR(2587));
		render_set_texture(tex.Value(IntType(2)), /*"Normal"*/ STR(2588));
		vbuffer_render({ vbuf });
	}
	...
	RealType render_world_sky()
	{
		...
		{
			VarType vbuf;
			if (global::_app->background_image_type == /*"sphere"*/ STR(556))
			{
				...
				if (global::_app->background_image_type == /*"box"*/ STR(1370))
				{
		...
		if (global::_app->background_fog_show > 0 && global::_app->background_fog_sky > 0)
		{
			...
			render_set_uniform_color(/*"uBlendColor"*/ STR(1962), global::_app->background_fog_color_final, IntType(1));
		...
		if (!(global::_app->background_image_show > 0))
		{
			...
			if (global::_app->background_night_alpha > IntType(0))
			{
				...
				render_set_uniform_color(/*"uBlendColor"*/ STR(1962), c_white, 0.4 * global::_app->background_night_alpha);
			...
			render_set_uniform_color(/*"uBlendColor"*/ STR(1962), c_white, vis);
	...
	RealType render_world_sky_clouds(ScopeAny self)
	{
		if (!(sVar(background_sky_clouds_show) > 0) || !(global::render_background > 0) || global::render_mode == e_render_mode_DEPTH_NO_SKY)
			return IntType(0);
		if (global::render_mode == e_render_mode_SCENE_TEST || global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2593), c_black, IntType(1));
		VarType res = sVar(background_sky_clouds_tex);
		if (!res_is_ready(res))
			res = global::mc_res;
		render_set_uniform_int(/*"uIsSky"*/ STR(2597), IntType(1));
		render_set_uniform_color(/*"uBlendColor"*/ STR(1962), sInt(background_sky_clouds_final), sReal(background_clouds_alpha));
		render_set_uniform_color(/*"uGlowColor"*/ STR(2595), c_black, IntType(1));
		render_set_uniform_int(/*"uGlowTexture"*/ STR(2596), IntType(0));
		render_set_uniform(/*"uMetallic"*/ STR(2589), IntType(0));
		render_set_uniform(/*"uRoughness"*/ STR(2590), IntType(1));
		render_set_uniform(/*"uEmissive"*/ STR(2591), IntType(0));
		render_set_uniform(/*"uLightSpecular"*/ STR(2598), IntType(0));
		...
		if (!(sVar(background_fog_show) > 0) || !(sVar(background_fog_sky) > 0))
			render_set_uniform(/*"uFogShow"*/ STR(1971), IntType(0));
		...
		if (!(sVar(background_fog_show) > 0) || !(sVar(background_fog_sky) > 0))
			render_set_uniform(/*"uFogShow"*/ STR(1971), global::_app->background_fog_show);
		return 0.0;
	}
	...
	void render_world_text(VarType vbuffer, VarType tex, VarType facecamera, VarType res, VarType outline)
	{
		...
		if (outline != null_)
		{
			render_set_uniform_color(/*"uBlendColor"*/ STR(1962), outline, global::shader_blend_alpha);
