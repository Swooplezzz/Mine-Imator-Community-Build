...
namespace CppProject
{
	...
	void tab_frame_editor_camera(ScopeAny self)
	{
		...
		draw_dragger(self, /*"frameeditorcameraaspect"*/ STR(3049), sVar(dx), sVar(dy), dragger_width, idVar(global::tl_edit, value).Value(e_value_CAM_ASPECT), 0.01, 0.0001, no_limit, IntType(1), 0.0001, idInt(idVar(sVar(tab), camera), tbx_aspect), ID_action_tl_frame_cam_aspect);
		tab_next(self);
		if (sReal(setting_advanced_mode) > 0)
		{
			tab_control_switch(self);
		...
		if (idVar(global::tl_edit, value).Value(e_value_CAM_ROTATE) > 0 && DsMap(global::collapse_map).Value(/*"rotatepoint"*/ STR(147)) > 0)
		{
			...
			draw_wheel(self, /*"frameeditorcamerarotateanglezwheel"*/ STR(3062), sVar(dx) + floor(sVar(dw) * 0.75), sVar(dy) + IntType(24), c_aqua, idVar(global::tl_edit, value).Value(e_value_CAM_ROTATE_ANGLE_Z), -89.9, 89.9, IntType(0), snapval, idInt(idVar(sVar(tab), camera), tbx_rotate_angle_z), ID_action_tl_frame_cam_rotate_angle_z);
			tab_next(self);
			...
			textfield_group_add(self, { /*"frameeditorcamerarotateanglez"*/ STR(3064), idVar(global::tl_edit, value).Value(e_value_CAM_ROTATE_ANGLE_Z), IntType(0), ID_action_tl_frame_cam_rotate_angle_z, global::axis_edit, idInt(idVar(sVar(tab), camera), tbx_rotate_angle_z), null_, .1, -89.9, 89.9 });
			tab_control_textfield_group(self, false);
		...
		if (idVar(global::tl_edit, value).Value(e_value_CAM_BLOOM) > 0 && DsMap(global::collapse_map).Value(/*"bloom"*/ STR(159)) > 0)
		{
			tab_collapse_start(self);
			tab_control_dragger(self);
			draw_dragger(self, /*"frameeditorcamerabloomthreshold"*/ STR(3102), sVar(dx), sVar(dy), dragger_width, round(idVar(global::tl_edit, value).Value(e_value_CAM_BLOOM_THRESHOLD) * IntType(100)), .1, IntType(0), no_limit, IntType(85), IntType(1), idInt(idVar(sVar(tab), camera), tbx_bloom_threshold), ID_action_tl_frame_cam_bloom_threshold);
			tab_next(self);
			tab_control_dragger(self);
