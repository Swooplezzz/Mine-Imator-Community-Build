...
namespace CppProject
{
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1319: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2617), /*"unzip"*/ STR(1319) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip(fname))
					{
						log({ /*"Error unzipping pack"*/ STR(2618) });
						error(/*"errorunzippack"*/ STR(2619));
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2620) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2617), /*"modeltextures"*/ STR(1401) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2621);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2622) });
			...
			case 2621: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2617), /*"blocktextures"*/ STR(2621) });
				...
				sStr(load_stage) = /*"itemtextures"*/ STR(2623);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2624) });
			...
			case 2623: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2617), /*"itemtextures"*/ STR(2623) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2625), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(550));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(636), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(641), /*"_s"*/ STR(2626));
				res_load_pack_item_textures(self, /*"normal"*/ STR(630), /*"_n"*/ STR(2627));
	...
	void res_load_pack_block_sheet(ScopeAny self, StringType type, StringType suffix)
	{
		...
		log({ /*"Block textures"*/ STR(2629), type, /*"load static"*/ STR(2630) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Block texture not found"*/ STR(2631), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
			
		}
		log({ /*"Block textures"*/ STR(2629), type, /*"load animated"*/ STR(2632) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Animated block texture not found"*/ STR(2633), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix });
		...
		log({ /*"Block textures, blocksize"*/ STR(2635), type, blocksize });
		draw_texture_start();
		surf = surface_create((IntType)(block_sheet_width * blocksize), (IntType)(block_sheet_height * blocksize));
		log({ /*"Block textures"*/ STR(2629), /*"static block surface"*/ STR(2636) });
		surface_set_target(surf);
	{
		...
		if (self->id == global::mc_res)
		{
			...
			log({ /*"Block textures"*/ STR(2629), /*"static block preview"*/ STR(2637) });
		...
		log({ /*"Block textures"*/ STR(2629), type, /*"animated block surfaces"*/ STR(2638) });
		...
		draw_texture_start();
		gpu_set_blendmode_ext(bm_one, bm_inv_src_alpha);
		log({ /*"Block textures"*/ STR(2629), type, /*"animated frames"*/ STR(2639) });
		for (IntType t = IntType(0); t < ds_list_size(texanilist); t++)
		{
			...
			fname = global::load_assets_dir + mc_textures_directory + DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix + /*".png.mcmeta"*/ STR(2640);
			if (file_exists_lib(fname))
			{
				IntType map = json_load({ fname });
				if (ds_map_valid(map))
				{
					VarType animation = DsMap(map).Value(/*"animation"*/ STR(2641));
					if (!is_undefined(animation))
					{
						if (!is_undefined(DsMap(animation).Value(/*"interpolate"*/ STR(2642))))
							framefade = DsMap(animation).Value(/*"interpolate"*/ STR(2642));
						if (!is_undefined(DsMap(animation).Value(/*"frametime"*/ STR(2643))))
							frametime = DsMap(animation).Value(/*"frametime"*/ STR(2643));
		...
		if (self->id == global::mc_res)
		{
			log({ /*"Block textures"*/ STR(2629), /*"animated block preview"*/ STR(2644) });
		...
		if (type == /*"diffuse"*/ STR(636))
		{
			...
			log({ /*"Block textures"*/ STR(2629), /*"find static block depths"*/ STR(2645) });
			...
			log({ /*"Block textures"*/ STR(2629), /*"find animated block depths"*/ STR(2646) });
		...
		log({ /*"Block textures"*/ STR(2629), type, /*"done"*/ STR(1772) });
		debug_timer_stop(/*"Block textures: "*/ STR(2647) + type);
	}
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		if (self->id != global::mc_res)
		{
			res_load_pack_block_sheet(self, /*"material"*/ STR(641), /*"_s"*/ STR(2626));
			res_load_pack_block_sheet(self, /*"normal"*/ STR(630), /*"_n"*/ STR(2627));
		}
		else
		{
		...
		log({ /*"Block textures all"*/ STR(2648), /*"done"*/ STR(1772) });
	}
	...
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		...
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"item/"*/ STR(2497), /*".png"*/ STR(10));
			fileslist = ds_list_create();
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(filesarr)); i++)
				ds_list_add({ fileslist, filesarr.Value(i) });
		}
		log({ /*"Item textures"*/ STR(2649), type, /*"load"*/ STR(2650) });
		texlist = ds_list_create();
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Item texture not found"*/ STR(2651), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
		...
		if (itemsize == null_)
			itemsize = item_size;
		log({ /*"Item textures"*/ STR(2649), /*"surface"*/ STR(556) });
		draw_texture_start();
		...
		surface_set_target(surf);
	{
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2649), type, /*"done"*/ STR(1772) });
		debug_timer_stop(/*"Item textures: "*/ STR(2653) + type);
	}
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Model textures"*/ STR(2654), /*"load"*/ STR(2650) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list)); t++)
		{
			...
			matfname = global::load_assets_dir + mc_textures_directory + name + /*"_s.png"*/ STR(2655);
			norfname = global::load_assets_dir + mc_textures_directory + name + /*"_n.png"*/ STR(2656);
			if (file_exists_lib(fname))
			{
				if (name == /*"entity/steve"*/ STR(2657))
				...
				{
					if (dev_mode)
						log({ /*"Model texture not found"*/ STR(2658), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list).Value(t) });
			...
			{
				if (matfname == /*"entity/steve_s"*/ STR(2659))
			...
			{
				if (norfname == /*"entity/steve_n"*/ STR(2660))
		...
		log({ /*"Model textures"*/ STR(2654), /*"done"*/ STR(1772) });
		debug_timer_stop(/*"Model textures"*/ STR(2654));
	}
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Particle textures"*/ STR(2661), /*"load"*/ STR(2650) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list)); t++)
		{
			...
			if (file_exists_lib(fname))
			{
				...
				{
					if (dev_mode)
						log({ /*"Particle texture not found"*/ STR(2662), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list).Value(t) });
					tex = texture_create_missing();
				}
			...
			{
				if (string_contains(name, /*"explosion"*/ STR(2663)))
				{
		...
		log({ /*"Particle textures"*/ STR(2661), /*"legacy sheets"*/ STR(2664) });
		...
		log({ /*"Particle textures"*/ STR(2661), /*"done"*/ STR(1772) });
		debug_timer_stop(/*"Particle textures"*/ STR(2661));
	}
