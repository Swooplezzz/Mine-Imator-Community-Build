...
namespace CppProject
{
	...
	VarType text_control_name(VarType keybind)
	{
		...
		switch ((IntType)keybind.Value(e_keybind_key_CHAR))
		{
			case vk_left:
			{
				char_ = text_get({ /*"keyleft"*/ STR(3859) });
				break;
			}
			case vk_right:
			{
				char_ = text_get({ /*"keyright"*/ STR(3860) });
				break;
			}
			case vk_up:
			{
				char_ = text_get({ /*"keyup"*/ STR(3861) });
				break;
			}
			case vk_down:
			{
				char_ = text_get({ /*"keydown"*/ STR(3862) });
				break;
			}
			case vk_enter:
			{
				char_ = text_get({ /*"keyenter"*/ STR(3863) });
				break;
			}
			case vk_escape:
			{
				char_ = text_get({ /*"keyescape"*/ STR(3864) });
				break;
			}
			case vk_space:
			{
				char_ = text_get({ /*"keyspace"*/ STR(3865) });
				break;
			}
			case vk_lshift:
			{
				char_ = text_get({ /*"keyleftshift"*/ STR(3866) });
				break;
			}
			case vk_rshift:
			{
				char_ = text_get({ /*"keyrightshift"*/ STR(3867) });
				break;
			}
			case vk_lalt:
			{
				char_ = text_get({ /*"keyleftalt"*/ STR(3868) });
				break;
			}
			case vk_ralt:
			{
				char_ = text_get({ /*"keyrightalt"*/ STR(3869) });
				break;
			}
			case vk_lcontrol:
			{
				char_ = text_get({ /*"keyleftcontrol"*/ STR(3870) });
				break;
			}
			case vk_rcontrol:
			{
				char_ = text_get({ /*"keyrightcontrol"*/ STR(3871) });
				break;
			}
			case vk_backspace:
			{
				char_ = text_get({ /*"keybackspace"*/ STR(3872) });
				break;
			}
			case vk_tab:
			{
				char_ = text_get({ /*"keytab"*/ STR(3873) });
				break;
			}
			case vk_home:
			{
				char_ = text_get({ /*"keyhome"*/ STR(3874) });
				break;
			}
			case vk_end:
			{
				char_ = text_get({ /*"keyend"*/ STR(3875) });
				break;
			}
			case vk_delete:
			{
				char_ = text_get({ /*"keydelete"*/ STR(3876) });
				break;
			}
			case vk_insert:
			{
				char_ = text_get({ /*"keyinsert"*/ STR(3877) });
				break;
			}
			case vk_pageup:
			{
				char_ = text_get({ /*"keypageup"*/ STR(3878) });
				break;
			}
			case vk_pagedown:
			{
				char_ = text_get({ /*"keypagedown"*/ STR(3879) });
				break;
			}
			case vk_pause:
			{
				char_ = text_get({ /*"keypause"*/ STR(3880) });
				break;
			}
			case vk_printscreen:
			{
				char_ = text_get({ /*"keyprintscreen"*/ STR(3881) });
				break;
			}
			case vk_f1:
			{
		...
		if (keybind.Value(e_keybind_key_ALT) > 0)
			text = text_get({ /*"keyalt"*/ STR(3894) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(3003) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_CTRL) > 0)
			text = text_get({ /*"keycontrol"*/ STR(3895) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(3003) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_SHIFT) > 0)
			text = text_get({ /*"keyshift"*/ STR(3896) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(3003) + text) : /*""*/ STR(0));
		return text;
	}
	...
	IntType text_max_width(VarArgs argument)
	{
		...
		IntType wid = IntType(0);
		for (IntType a = IntType(0); a < argument_count; a++)
			wid = max({ wid, string_width(text_get({ argument[a] }) + /*":"*/ STR(784)) });
		return wid;
	}
	...
	RealType time_get_rotation(VarType str)
	{
		...
		colons = string_count(/*":"*/ STR(784), str);
		days = IntType(0);
		hours = IntType(0);
		minutes = IntType(0);
		if (colons > IntType(2))
		...
		if (colons == IntType(1))
		{
			colonpos = string_pos(/*":"*/ STR(784), str);
	...
	void tl_event_create(Scope<obj_timeline> self)
	{
		...
		self->temp = null_;
		self->text = text_get({ /*"timelineeditortextsample"*/ STR(3899) });
		...
		self->text_res = null_;
		self->text_3d = false;
		self->text_halign_prev = /*"center"*/ STR(1448);
		self->text_valign_prev = /*"center"*/ STR(1448);
