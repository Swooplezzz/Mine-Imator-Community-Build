...
namespace CppProject
{
	...
	VarType text_control_name(VarType keybind)
	{
		...
		switch ((IntType)keybind.Value(e_keybind_key_CHAR))
		{
			case vk_left:
			{
				char_ = text_get({ /*"keyleft"*/ STR(3886) });
				break;
			}
			case vk_right:
			{
				char_ = text_get({ /*"keyright"*/ STR(3887) });
				break;
			}
			case vk_up:
			{
				char_ = text_get({ /*"keyup"*/ STR(3888) });
				break;
			}
			case vk_down:
			{
				char_ = text_get({ /*"keydown"*/ STR(3889) });
				break;
			}
			case vk_enter:
			{
				char_ = text_get({ /*"keyenter"*/ STR(3890) });
				break;
			}
			case vk_escape:
			{
				char_ = text_get({ /*"keyescape"*/ STR(3891) });
				break;
			}
			case vk_space:
			{
				char_ = text_get({ /*"keyspace"*/ STR(3892) });
				break;
			}
			case vk_lshift:
			{
				char_ = text_get({ /*"keyleftshift"*/ STR(3893) });
				break;
			}
			case vk_rshift:
			{
				char_ = text_get({ /*"keyrightshift"*/ STR(3894) });
				break;
			}
			case vk_lalt:
			{
				char_ = text_get({ /*"keyleftalt"*/ STR(3895) });
				break;
			}
			case vk_ralt:
			{
				char_ = text_get({ /*"keyrightalt"*/ STR(3896) });
				break;
			}
			case vk_lcontrol:
			{
				char_ = text_get({ /*"keyleftcontrol"*/ STR(3897) });
				break;
			}
			case vk_rcontrol:
			{
				char_ = text_get({ /*"keyrightcontrol"*/ STR(3898) });
				break;
			}
			case vk_backspace:
			{
				char_ = text_get({ /*"keybackspace"*/ STR(3899) });
				break;
			}
			case vk_tab:
			{
				char_ = text_get({ /*"keytab"*/ STR(3900) });
				break;
			}
			case vk_home:
			{
				char_ = text_get({ /*"keyhome"*/ STR(3901) });
				break;
			}
			case vk_end:
			{
				char_ = text_get({ /*"keyend"*/ STR(3902) });
				break;
			}
			case vk_delete:
			{
				char_ = text_get({ /*"keydelete"*/ STR(3903) });
				break;
			}
			case vk_insert:
			{
				char_ = text_get({ /*"keyinsert"*/ STR(3904) });
				break;
			}
			case vk_pageup:
			{
				char_ = text_get({ /*"keypageup"*/ STR(3905) });
				break;
			}
			case vk_pagedown:
			{
				char_ = text_get({ /*"keypagedown"*/ STR(3906) });
				break;
			}
			case vk_pause:
			{
				char_ = text_get({ /*"keypause"*/ STR(3907) });
				break;
			}
			case vk_printscreen:
			{
				char_ = text_get({ /*"keyprintscreen"*/ STR(3908) });
				break;
			}
			case vk_f1:
			{
		...
		if (keybind.Value(e_keybind_key_ALT) > 0)
			text = text_get({ /*"keyalt"*/ STR(3921) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(3025) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_CTRL) > 0)
			text = text_get({ /*"keycontrol"*/ STR(3922) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(3025) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_SHIFT) > 0)
			text = text_get({ /*"keyshift"*/ STR(3923) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(3025) + text) : /*""*/ STR(0));
		return text;
	}
	...
	VarType text_get(VarArgs argument)
	{
		...
		{
			if (!text_exists(key))
				return /*"<No text found for \""*/ STR(3924) + key + /*"\">"*/ STR(3925);
	...
	void tl_event_create(Scope<obj_timeline> self)
	{
		...
		self->temp = null_;
		self->text = text_get({ /*"timelineeditortextsample"*/ STR(3926) });
