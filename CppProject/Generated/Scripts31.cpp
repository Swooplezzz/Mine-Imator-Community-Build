...
namespace CppProject
{
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenproject"*/ STR(1166) }) + /*" (*.miproject; *.zip; *.mproj; *.mani)|*miproject;*.zip;*.mproj;*.mani|"*/ STR(1167) + text_get({ /*"filedialogopenbackup"*/ STR(1168) }) + /*" (*.backup*; *.mbackup*)|*.backup*;*.mbackup*"*/ STR(1169), /*""*/ STR(0), sStr(setting_project_folder), text_get({ /*"filedialogopenprojectcaption"*/ STR(1170) }));
	}
	
	StringType file_dialog_open_render()
	{
		return file_dialog_open(text_get({ /*"filedialogopenrender"*/ STR(1171) }) + /*" (*.mirender)|*mirender"*/ STR(1172), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenrendercaption"*/ STR(1173) }));
	}
	
	StringType file_dialog_open_scenery()
	{
		return file_dialog_open(text_get({ /*"filedialogopenscenery"*/ STR(1174) }) + /*" (*.schematic; *.nbt; *.blocks)|*.schematic;*.nbt;*.blocks"*/ STR(1175), /*""*/ STR(0), schematics_directory, text_get({ /*"filedialogopenscenerycaption"*/ STR(1176) }));
	}
	
	StringType file_dialog_open_sound()
	{
		return file_dialog_open(text_get({ /*"filedialogopensound"*/ STR(1177) }) + /*" (*.mp3; *.wav; *.ogg; *.flac; *.wma; *.m4a)|*.mp3;*.wav;*.ogg;*.flac;*.wma;*.m4a;"*/ STR(1178), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopensoundcaption"*/ STR(1179) }));
	}
	...
	{
		return file_dialog_save(text_get({ /*"filedialogsaveimage"*/ STR(1180) }) + /*" (*.png)|*.png"*/ STR(1181), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveimagecaption"*/ STR(1182) }));
	}
	
	StringType file_dialog_save_keyframes(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavekeyframes"*/ STR(1183) }) + /*" (*.miframes)|*.miframes"*/ STR(1184), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsavekeyframescaption"*/ STR(1185) }));
	}
	
	StringType file_dialog_save_movie_mov(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviemov"*/ STR(1186) }) + /*" (*.mov)|*.mov"*/ STR(1187), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1188) }));
	}
	
	StringType file_dialog_save_movie_mp4(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviemp4"*/ STR(1189) }) + /*" (*.mp4)|*.mp4"*/ STR(1190), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1188) }));
	}
	
	StringType file_dialog_save_movie_png(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviepng"*/ STR(1191) }) + /*" (*.png)|*.png"*/ STR(1181), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1188) }));
	}
	
	StringType file_dialog_save_movie_wmv(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviewmv"*/ STR(1192) }) + /*" (*.wmv)|*.wmv"*/ STR(1193), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1188) }));
	}
	
	StringType file_dialog_save_object(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveobject"*/ STR(1194) }) + /*" (*.miobject)|*.miobject"*/ STR(1195), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveobjectcaption"*/ STR(1196) }));
	}
	
	StringType file_dialog_save_particles(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveparticles"*/ STR(1197) }) + /*" (*.miparticles)|*.miparticles"*/ STR(1198), filename_get_valid(fn), particles_directory, text_get({ /*"filedialogsaveparticlescaption"*/ STR(1199) }));
	}
	
	StringType file_dialog_save_project(ScopeAny self, VarType fn)
	{
		return file_dialog_save(/*""*/ STR(0), fn, sStr(setting_project_folder), text_get({ /*"filedialogsaveprojectcaption"*/ STR(1200) }));
	}
	
	StringType file_dialog_save_render(StringType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaverender"*/ STR(1201) }) + /*" (*.mirender)|*.mirender"*/ STR(1202), filename_get_valid(fn), render_directory, text_get({ /*"filedialogsaverendercaption"*/ STR(1203) }));
	}
	
	StringType file_dialog_save_resource(VarType fn, StringType exts)
	{
		return file_dialog_save(/*" * "*/ STR(699) + exts + /*"|*"*/ STR(1204) + exts, filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveresourcecaption"*/ STR(1205) }));
	}
	...
	ArrType file_find(StringType dir, StringType exts)
	{
		...
		StringType f;
		ret = ArrType();
		f = file_find_first(dir + /*"*"*/ STR(1116), IntType(0));
		while (f != /*""*/ STR(0))
		{
	...
	{
		log({ /*"working_directory"*/ STR(1206), gmlGlobal::working_directory });
		if (!directory_exists_lib(gmlGlobal::working_directory))
		{
			log({ /*"directory broken"*/ STR(1207) });
			access_error();
			return false;
		}
		IntType surf;
		...
		tmpfile1 = file_directory + /*"tmp.png"*/ STR(1208);
		tmpfile2 = data_directory + /*"tmp.png"*/ STR(1208);
		log({ /*"Trying to save files"*/ STR(1209) });
		...
		{
			log({ /*"Couldn't save to file_directory"*/ STR(1210) });
			access_error();
			return false;
		}
		if (!file_exists_lib(tmpfile2))
		{
			log({ /*"Couldn't save to data_directory"*/ STR(1211) });
			access_error();
			return false;
		}
		log({ /*"surface_save OK"*/ STR(1212) });
		IntType tex1, tex2;
		...
		{
			log({ /*"Couldn't load texture from file_directory"*/ STR(1213) });
			access_error();
			return false;
		}
		if (tex2 < IntType(0) || texture_width(tex2) != IntType(32) || texture_height(tex2) != IntType(32))
		{
			log({ /*"Couldn't load texture from data_directory"*/ STR(1214) });
			access_error();
			return false;
		}
		log({ /*"texture_create OK"*/ STR(1215) });
		file_delete_lib(tmpfile1);
		file_delete_lib(tmpfile2);
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from file_directory"*/ STR(1216) });
			access_error();
			return false;
		}
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from data_directory"*/ STR(1217) });
			access_error();
			return false;
		}
		log({ /*"file_delete_lib OK"*/ STR(1218) });
		return true;
	}
	...
	{
		StringType tmpfile = file_directory + /*"tmp.ttf"*/ STR(1219);
	...
	VarType hex_to_color(VarType oldstr)
	{
		...
		str = string_upper(str + string_repeat(/*"0"*/ STR(1075), (IntType)(IntType(6) - string_length(oldstr))));
