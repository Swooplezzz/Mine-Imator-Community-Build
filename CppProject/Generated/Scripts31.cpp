/*
	...
	[ Generated on 2024.01.18 18:30:57 ]
*/
...
namespace CppProject
{
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenproject"*/ STR(1183) }) + /*" (*.miproject; *.zip; *.mproj; *.mani)|*miproject;*.zip;*.mproj;*.mani|"*/ STR(1184) + text_get({ /*"filedialogopenbackup"*/ STR(1185) }) + /*" (*.backup*; *.mbackup*)|*.backup*;*.mbackup*"*/ STR(1186), /*""*/ STR(0), sStr(setting_project_folder), text_get({ /*"filedialogopenprojectcaption"*/ STR(1187) }));
	}
	
	StringType file_dialog_open_render()
	{
		return file_dialog_open(text_get({ /*"filedialogopenrender"*/ STR(1188) }) + /*" (*.mirender)|*mirender"*/ STR(1189), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenrendercaption"*/ STR(1190) }));
	}
	
	StringType file_dialog_open_scenery()
	{
		return file_dialog_open(text_get({ /*"filedialogopenscenery"*/ STR(1191) }) + /*" (*.schematic; *.nbt; *.blocks)|*.schematic;*.nbt;*.blocks"*/ STR(1192), /*""*/ STR(0), schematics_directory, text_get({ /*"filedialogopenscenerycaption"*/ STR(1193) }));
	}
	
	StringType file_dialog_open_sound()
	{
		return file_dialog_open(text_get({ /*"filedialogopensound"*/ STR(1194) }) + /*" (*.mp3; *.wav; *.ogg; *.flac; *.wma; *.m4a)|*.mp3;*.wav;*.ogg;*.flac;*.wma;*.m4a;"*/ STR(1195), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopensoundcaption"*/ STR(1196) }));
	}
	...
	{
		return file_dialog_save(text_get({ /*"filedialogsaveimage"*/ STR(1197) }) + /*" (*.png)|*.png"*/ STR(1198), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveimagecaption"*/ STR(1199) }));
	}
	
	StringType file_dialog_save_keyframes(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavekeyframes"*/ STR(1200) }) + /*" (*.miframes)|*.miframes"*/ STR(1201), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsavekeyframescaption"*/ STR(1202) }));
	}
	
	StringType file_dialog_save_movie_mov(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviemov"*/ STR(1203) }) + /*" (*.mov)|*.mov"*/ STR(1204), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1205) }));
	}
	
	StringType file_dialog_save_movie_mp4(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviemp4"*/ STR(1206) }) + /*" (*.mp4)|*.mp4"*/ STR(1207), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1205) }));
	}
	
	StringType file_dialog_save_movie_png(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviepng"*/ STR(1208) }) + /*" (*.png)|*.png"*/ STR(1198), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1205) }));
	}
	
	StringType file_dialog_save_movie_wmv(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviewmv"*/ STR(1209) }) + /*" (*.wmv)|*.wmv"*/ STR(1210), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1205) }));
	}
	
	StringType file_dialog_save_object(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveobject"*/ STR(1211) }) + /*" (*.miobject)|*.miobject"*/ STR(1212), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveobjectcaption"*/ STR(1213) }));
	}
	
	StringType file_dialog_save_particles(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveparticles"*/ STR(1214) }) + /*" (*.miparticles)|*.miparticles"*/ STR(1215), filename_get_valid(fn), particles_directory, text_get({ /*"filedialogsaveparticlescaption"*/ STR(1216) }));
	}
	
	StringType file_dialog_save_project(ScopeAny self, VarType fn)
	{
		return file_dialog_save(/*""*/ STR(0), fn, sStr(setting_project_folder), text_get({ /*"filedialogsaveprojectcaption"*/ STR(1217) }));
	}
	
	StringType file_dialog_save_render(StringType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaverender"*/ STR(1218) }) + /*" (*.mirender)|*.mirender"*/ STR(1219), filename_get_valid(fn), render_directory, text_get({ /*"filedialogsaverendercaption"*/ STR(1220) }));
	}
	
	StringType file_dialog_save_resource(VarType fn, StringType exts)
	{
		return file_dialog_save(/*" * "*/ STR(702) + exts + /*"|*"*/ STR(1221) + exts, filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveresourcecaption"*/ STR(1222) }));
	}
	...
	ArrType file_find(StringType dir, StringType exts)
	{
		...
		StringType f;
		ret = ArrType();
		f = file_find_first(dir + /*"*"*/ STR(1134), IntType(0));
		while (f != /*""*/ STR(0))
		{
	...
	{
		log({ /*"working_directory"*/ STR(1223), gmlGlobal::working_directory });
		if (!directory_exists_lib(gmlGlobal::working_directory))
		{
			log({ /*"directory broken"*/ STR(1224) });
			access_error();
			return false;
		}
		IntType surf;
		...
		tmpfile1 = file_directory + /*"tmp.png"*/ STR(1225);
		tmpfile2 = data_directory + /*"tmp.png"*/ STR(1225);
		log({ /*"Trying to save files"*/ STR(1226) });
		...
		{
			log({ /*"Couldn't save to file_directory"*/ STR(1227) });
			access_error();
			return false;
		}
		if (!file_exists_lib(tmpfile2))
		{
			log({ /*"Couldn't save to data_directory"*/ STR(1228) });
			access_error();
			return false;
		}
		log({ /*"surface_save OK"*/ STR(1229) });
		IntType tex1, tex2;
		...
		{
			log({ /*"Couldn't load texture from file_directory"*/ STR(1230) });
			access_error();
			return false;
		}
		if (tex2 < IntType(0) || texture_width(tex2) != IntType(32) || texture_height(tex2) != IntType(32))
		{
			log({ /*"Couldn't load texture from data_directory"*/ STR(1231) });
			access_error();
			return false;
		}
		log({ /*"texture_create OK"*/ STR(1232) });
		file_delete_lib(tmpfile1);
		file_delete_lib(tmpfile2);
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from file_directory"*/ STR(1233) });
			access_error();
			return false;
		}
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from data_directory"*/ STR(1234) });
			access_error();
			return false;
		}
		log({ /*"file_delete_lib OK"*/ STR(1235) });
		return true;
	}
	...
	{
		StringType tmpfile = file_directory + /*"tmp.ttf"*/ STR(1236);
	...
	{
		StringType str, hex;
		str = string_replace(oldstr, /*"#"*/ STR(860), /*""*/ STR(0));
		str = string_upper(str + string_repeat(/*"0"*/ STR(1093), (IntType)(IntType(6) - string_length(oldstr))));
		hex = /*"0123456789ABCDEF"*/ STR(1053);
	...
	void history_restore_part_usage_tl(VarType hobj)
	{
		...
		withOne (obj_history, hobj, noone)
		{
			...
			for (IntType i = IntType(0); i < self->usage_tl_copy_rot_target_amount; i++)
			{
				tl = save_id_find(self->usage_tl_copy_rot_target_save_id.Value(i));
				idVar(tl, value)[e_value_ROT_TARGET] = save_id_find(self->usage_tl_copy_rot_target_part_save_id.Value(i));
			}
			for (IntType i = IntType(0); i < self->usage_tl_copy_scale_target_amount; i++)
			{
				tl = save_id_find(self->usage_tl_copy_scale_target_save_id.Value(i));
				idVar(tl, value)[e_value_SCALE_TARGET] = save_id_find(self->usage_tl_copy_scale_target_part_save_id.Value(i));
			}
			for (IntType i = IntType(0); i < self->usage_tl_bnd_ik_target_amount; i++)
			{
				tl = save_id_find(self->usage_tl_bnd_ik_target_save_id.Value(i));
				idVar(tl, value)[e_value_BEND_IK_TARGET] = save_id_find(self->usage_tl_bnd_ik_target_part_save_id.Value(i));
			}
			...
			for (IntType i = IntType(0); i < self->usage_kf_copy_rot_target_amount; i++)
			{
				tl = save_id_find(self->usage_kf_copy_rot_target_tl_save_id.Value(i));
				kfindex = self->usage_kf_copy_rot_target_index.Value(i);
				ObjType(obj_keyframe, DsList(idInt(tl, keyframe_list))[kfindex])->value[e_value_ROT_TARGET] = save_id_find(self->usage_kf_copy_rot_target_tl_part_save_id.Value(i));
			}
			for (IntType i = IntType(0); i < self->usage_kf_copy_pos_target_amount; i++)
			{
				tl = save_id_find(self->usage_kf_copy_pos_target_tl_save_id.Value(i));
				kfindex = self->usage_kf_copy_pos_target_index.Value(i);
				ObjType(obj_keyframe, DsList(idInt(tl, keyframe_list))[kfindex])->value[e_value_POS_TARGET] = save_id_find(self->usage_kf_copy_pos_target_tl_part_save_id.Value(i));
			}
			for (IntType i = IntType(0); i < self->usage_kf_copy_scale_target_amount; i++)
			{
				tl = save_id_find(self->usage_kf_copy_scale_target_tl_save_id.Value(i));
				kfindex = self->usage_kf_copy_scale_target_index.Value(i);
				ObjType(obj_keyframe, DsList(idInt(tl, keyframe_list))[kfindex])->value[e_value_SCALE_TARGET] = save_id_find(self->usage_kf_copy_scale_target_tl_part_save_id.Value(i));
			}
			for (IntType i = IntType(0); i < self->usage_kf_bnd_ik_target_amount; i++)
			{
				tl = save_id_find(self->usage_kf_bnd_ik_target_tl_save_id.Value(i));
				kfindex = self->usage_kf_bnd_ik_target_index.Value(i);
				ObjType(obj_keyframe, DsList(idInt(tl, keyframe_list))[kfindex])->value[e_value_BEND_IK_TARGET] = save_id_find(self->usage_kf_bnd_ik_target_tl_part_save_id.Value(i));
			}
	...
	IntType history_restore_tl(VarType save, IntType tl)
	{
		...
		withOne (obj_timeline, tl, noone)
		{
			...
			for (IntType s = IntType(0); s < ObjType(obj_history_save, save)->usage_tl_copy_rot_target_amount; s++)
			{
				withOne (Object, save_id_find(ObjType(obj_history_save, save)->usage_tl_copy_rot_target_save_id.Value(s)), self->id)
				{
			...
			for (IntType s = IntType(0); s < ObjType(obj_history_save, save)->usage_tl_copy_pos_target_amount; s++)
			{
				withOne (Object, save_id_find(ObjType(obj_history_save, save)->usage_tl_copy_pos_target_save_id.Value(s)), self->id)
				{
			...
			for (IntType s = IntType(0); s < ObjType(obj_history_save, save)->usage_tl_copy_scale_target_amount; s++)
			{
				withOne (Object, save_id_find(ObjType(obj_history_save, save)->usage_tl_copy_scale_target_save_id.Value(s)), self->id)
				{
			...
			for (IntType s = IntType(0); s < ObjType(obj_history_save, save)->usage_tl_bnd_ik_target_amount; s++)
			{
				withOne (Object, save_id_find(ObjType(obj_history_save, save)->usage_tl_bnd_ik_target_save_id.Value(s)), self->id)
				{
					sVar(value)[e_value_BEND_IK_TARGET] = tl;
					sBool(update_matrix) = true;
				}
			...
			for (IntType s = IntType(0); s < ObjType(obj_history_save, save)->usage_kf_copy_rot_target_amount; s++)
				withOne (Object, save_id_find(ObjType(obj_history_save, save)->usage_kf_copy_rot_target_tl_save_id.Value(s)), self->id)
					ObjType(obj_keyframe, DsList(sInt(keyframe_list))[ObjType(obj_history_save, save)->usage_kf_copy_rot_target_index.Value(s)])->value[e_value_ROT_TARGET] = tl;
			
			for (IntType s = IntType(0); s < ObjType(obj_history_save, save)->usage_kf_copy_pos_target_amount; s++)
				withOne (Object, save_id_find(ObjType(obj_history_save, save)->usage_kf_copy_pos_target_tl_save_id.Value(s)), self->id)
					ObjType(obj_keyframe, DsList(sInt(keyframe_list))[ObjType(obj_history_save, save)->usage_kf_copy_pos_target_index.Value(s)])->value[e_value_POS_TARGET] = tl;
			
			for (IntType s = IntType(0); s < ObjType(obj_history_save, save)->usage_kf_copy_scale_target_amount; s++)
				withOne (Object, save_id_find(ObjType(obj_history_save, save)->usage_kf_copy_scale_target_tl_save_id.Value(s)), self->id)
					ObjType(obj_keyframe, DsList(sInt(keyframe_list))[ObjType(obj_history_save, save)->usage_kf_copy_scale_target_index.Value(s)])->value[e_value_SCALE_TARGET] = tl;
			
			for (IntType s = IntType(0); s < ObjType(obj_history_save, save)->usage_kf_bnd_ik_target_amount; s++)
				withOne (Object, save_id_find(ObjType(obj_history_save, save)->usage_kf_bnd_ik_target_tl_save_id.Value(s)), self->id)
					ObjType(obj_keyframe, DsList(sInt(keyframe_list))[ObjType(obj_history_save, save)->usage_kf_bnd_ik_target_index.Value(s)])->value[e_value_BEND_IK_TARGET] = tl;
