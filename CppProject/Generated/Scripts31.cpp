...
namespace CppProject
{
	VarType history_set_var(ScopeAny self, IntType script, VarType oldval, VarType newval, RealType combine)
	{
		...
		{
			history_push(self);
			log({ /*"Action Set variable"*/ STR(1211), script_get_name(script), oldval, newval, combine });
	...
	BoolType json_load_array()
	{
		...
		while (json_load_char())
		{
			...
			{
				global::json_error = /*"Expected \",\""*/ STR(1216);
				break;
			}
		}
	...
	BoolType json_load_char()
	{
		if (buffer_is_eof())
		{
			global::json_error = /*"Unexpected end of file"*/ STR(1217);
			return false;
		}
		do
		{
	...
	BoolType json_load_number()
	{
		...
		do
		{
			...
			if (global::json_char == e_json_char_E || global::json_char == e_json_char_CAPITAL_E)
			{
				if (expart)
				{
					global::json_error = /*"Unexpected symbol"*/ STR(1218);
					return false;
				}
				global::json_char = buffer_read_byte();
				global::json_column++;
		...
		{
			global::json_error = /*"Invalid number \""*/ STR(1219) + str + /*"\""*/ STR(1128);
			return false;
		}
		return true;
	}
	...
	BoolType json_load_string()
	{
		...
		while (true)
		{
			...
			{
				global::json_error = /*"Unexpected linebreak in string"*/ STR(1221);
				return false;
			}
	...
	BoolType json_load_word()
	{
		if (global::json_char == e_json_char_T)
		{
			...
			if (global::json_char == e_json_char_F)
			{
				...
				{
					global::json_error = /*"Unrecognized word"*/ STR(1222);
					return false;
				}
	...
	void json_save_var_bool(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		buffer_write_string((val > 0) ? /*"true"*/ STR(768) : /*"false"*/ STR(769));
		global::json_add_comma = true;
	}
	...
	void json_save_var_color(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		json_save_value(/*"#"*/ STR(850) + color_to_hex(val));
		global::json_add_comma = true;
	}
	...
	BoolType keybind_check(VarType keybindobj, StringType checkscript)
	{
		...
		if (checkscript == /*"keyboard_check"*/ STR(698))
		{
	...
	RealType keyframes_save(ScopeAny self)
	{
		...
		log({ /*"Saving keyframes"*/ STR(1238), fn });
	...
	BoolType key_valid(VarType key)
	{
		...
		keystr = /*"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"*/ STR(1240);
	...
	void langauge_new(StringType fn)
	{
		...
		withOne (obj_language, obj, noone)
		{
			...
			self->name = text_get({ /*"filelanguage"*/ STR(1241) });
			self->locale = text_exists(/*"filelocale"*/ STR(1242)) ? string(text_get({ /*"filelocale"*/ STR(1242) })) : /*""*/ STR(0);
		}
	...
	RealType languages_load()
	{
		...
		if (!file_exists_lib(fn))
			return IntType(0);
		log({ /*"Loading languages"*/ STR(1243), fn });
		...
		VarType list, obj;
		list = DsMap(map).Value(/*"languages"*/ STR(1245));
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			map = DsList(list).Value(i);
			VarType name, locale, fn;
			name = value_get_string(DsMap(map).Value(/*"name"*/ STR(810)), /*""*/ STR(0));
			locale = value_get_string(DsMap(map).Value(/*"locale"*/ STR(1246)), /*""*/ STR(0));
			fn = value_get_string(DsMap(map).Value(/*"filename"*/ STR(1247)), /*""*/ STR(0));
			language_remove(fn);
			IntType obj = (new obj_language)->id;
	...
	void languages_save()
	{
		log({ /*"Saving languages"*/ STR(1248), languages_file });
		json_save_start(languages_file);
		json_save_object_start();
		json_save_var(/*"format"*/ STR(88), settings_format);
		json_save_array_start({ /*"languages"*/ STR(1245) });
		withAll (obj_language, noone)
		{
			json_save_object_start();
			json_save_var(/*"name"*/ STR(810), json_string_encode(self->name));
			json_save_var(/*"locale"*/ STR(1246), self->locale);
			json_save_var(/*"filename"*/ STR(1247), self->filename);
			json_save_object_done();
		}
