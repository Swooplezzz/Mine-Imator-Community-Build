...
namespace CppProject
{
	...
	RealType sortlist_filters_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(typelist); i++)
		{
			...
			{
				if (itemname == /*"packunzipped"*/ STR(546) || itemname == /*"legacyblocksheet"*/ STR(549) || itemname == /*"fromworld"*/ STR(551))
					continue;
			}
			capwid = max({ capwid, string_width(text_get({ /*"type"*/ STR(794) + itemname })) });
			...
			if (draw_checkbox(self, /*"type"*/ STR(794) + itemname, sVar(dx), sVar(dy) + scroll, active, null_))
			{
				if (active)
				...
				for (IntType p = IntType(0); p < ds_list_size(idInt(slist, display_list)); p++)
				{
					...
					if (sVar(setting_search_variants) > 0)
					{
						...
						for (IntType i = IntType(0); i < statelen; i += IntType(2))
						{
							...
							VarType statename = text_get({ /*"blockstate"*/ STR(784) + state });
							...
							{
								if (string_count(check, string_lower(minecraft_asset_get_name(/*"blockstatevalue"*/ STR(785), idVar(statecurrent, value_name).Value(s)))) > IntType(0))
								{
									variantmatch = true;
									break;
								}
					...
					if (!match && string_count(check, string_lower(minecraft_asset_get_name(/*"block"*/ STR(529), baseblock))) > IntType(0))
						match = true;
					if (!match)
					{
						ds_list_delete(idInt(slist, display_list), p);
	...
	{
		return string_replace_all(string_upper(string_char_at(str, IntType(1))) + string_delete(str, IntType(1), IntType(1)), /*"_"*/ STR(1121), /*" "*/ STR(17));
	}
	...
	VarType string_get_real(VarType str, VarType inv)
	{
		...
		if (string_replace(string_replace(str, /*"-"*/ STR(1100), /*""*/ STR(0)), /*"."*/ STR(1098), /*""*/ STR(0)) == /*""*/ STR(0))
			return inv;
		if (string_contains(str, /*"-."*/ STR(2922)))
			str = string_replace(str, /*"-."*/ STR(2922), /*"-0."*/ STR(2923));
		if (state >= IntType(2))
			return real(str);
		return inv;
	}
	...
	StringType string_limit_ext(VarType str, VarType w, VarType h)
	{
		...
		dx = IntType(0);
		dy = IntType(0);
		if (lh > h || string_width(/*"..."*/ STR(2925)) > w)
			return /*""*/ STR(0);
		linestart = IntType(1);
		wordstart = IntType(1);
		for (c = IntType(1); c <= string_length(str); c++)
		{
			...
			if (char_ == /*"\n"*/ STR(959))
			{
				...
				if (dy + lh > h)
				{
					while (c > linestart && string_width(string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2925)) > w)
						c--;
					
					nstr += string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2925);
					c = linestart;
					break;
				}
			...
			{
				if (char_ == /*" "*/ STR(17) || char_ == /*", "*/ STR(1036) || char_ == /*"."*/ STR(1098) || char_ == /*"/"*/ STR(16) || char_ == /*"-"*/ STR(1100))
	...
	ArrType string_split(VarType str, StringType sep)
	{
		...
		while (true)
		{
			...
			if (pos == IntType(0))
				break;
			IntType escapepos = string_pos(/*"\\"*/ STR(1124) + sep, str);
	...
	{
		StringType sep = /*":"*/ STR(776);
		return string_repeat(/*"0"*/ STR(1057), (IntType)(hours < IntType(10))) + string(hours) + sep + string_repeat(/*"0"*/ STR(1057), (IntType)(mnts < IntType(10))) + string(mnts) + sep + string_repeat(/*"0"*/ STR(1057), (IntType)(secs < IntType(10))) + string_replace_all(string_format(secs, IntType(2), IntType(3)), /*" "*/ STR(17), /*""*/ STR(0));
	}
