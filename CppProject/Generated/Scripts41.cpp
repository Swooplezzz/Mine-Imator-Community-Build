...
namespace CppProject
{
	void panel_draw_content(ScopeAny self)
	{
		...
		else
		{
			...
			if (sVar(dh) < minh)
			{
				if (idInt(sVar(tab), panel) != DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(200)))
	...
	RealType particles_load(ScopeAny self, VarType fn, VarType temp)
	{
		...
		if (filename_ext(fn) == /*".zip"*/ STR(7))
		{
			...
			unzip(fn);
			fn = file_find_single(unzip_directory, /*".miparticles;*.particles"*/ STR(1977));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(20), /*".miparticles;*.particles"*/ STR(1977));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenparticleszip"*/ STR(1978));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		IntType rootmap;
		BoolType legacy;
		if (string_contains(filename_ext(fn), /*".miparticles"*/ STR(711)))
		{
			log({ /*"Opening particles"*/ STR(1979), fn });
		...
		{
			log({ /*"Opening legacy particles"*/ STR(1980), fn });
	...
	RealType particles_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miparticles"*/ STR(711));
		log({ /*"Saving particles"*/ STR(1982), fn });
								...
								if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1358))
								{
									...
									if (idVar(temp, pc_spawn_region_type) == /*"sphere"*/ STR(552))
									{
										...
										if (idVar(temp, pc_spawn_region_type) == /*"cube"*/ STR(3))
										{
											...
											if (idVar(temp, pc_spawn_region_type) == /*"box"*/ STR(1352))
											{
												for (IntType a = X_; a <= Z_; a++)
												{
