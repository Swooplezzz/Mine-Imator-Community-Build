...
namespace CppProject
{
	void debug_ds_list(VarType argument0)
	{
		...
		debug({ /*"Elements"*/ STR(1059), ds_list_size((IntType)(list)) });
		for (IntType i = IntType(0); i < ds_list_size(list); i++)
			debug({ /*"    "*/ STR(1060) + string(DsList(list).Value(i)) });
	}
	...
	void debug_ds_map(IntType map)
	{
		...
		debug({ /*"Elements"*/ STR(1059), ds_list_size(keylist) });
		for (IntType i = IntType(0); i < ds_list_size(keylist); i++)
			debug({ /*"   "*/ STR(1061) + string(DsList(keylist).Value(i)) + /*" => "*/ STR(1062) + string(DsMap(map).Value(DsList(keylist).Value(i))) });
	...
	RealType debug_info_draw(ScopeAny self)
	{
		...
		str += /*"FPS real: "*/ STR(1067) + string(gmlGlobal::fps_real) + /*" \n"*/ STR(1066);
		str += /*"delta: "*/ STR(1068) + string(global::delta) + /*" \n"*/ STR(1066);
		str += /*"\n"*/ STR(712);
		str += /*"Window: \n"*/ STR(1069);
		str += /*"======================================= \n"*/ STR(1064);
		str += /*"DPI: "*/ STR(1070) + string(display_get_dpi_x()) + /*","*/ STR(1071) + string(display_get_dpi_y()) + /*" \n"*/ STR(1066);
		str += /*"Size: "*/ STR(1072) + string(sReal(window_width)) + /*","*/ STR(1071) + string(sReal(window_height)) + /*" \n"*/ STR(1066);
		str += /*"window_busy: "*/ STR(1073) + string(sVar(window_busy)) + /*" \n"*/ STR(1066);
		str += /*"window_focus: "*/ STR(1074) + string(sVar(window_focus)) + /*" \n"*/ STR(1066);
		str += /*"current_step: "*/ STR(1075) + string(global::current_step) + /*" \n"*/ STR(1066);
		str += /*"\n"*/ STR(712);
		StringType userprofile = string_replace_all(string(environment_get_variable(/*"USERPROFILE"*/ STR(1076))), /*"/"*/ STR(20), /*"\\"*/ STR(1077));
		StringType nodox_userprofile = string_replace_all(userprofile, string(environment_get_variable(/*"USERNAME"*/ STR(1078))), /*"?????"*/ STR(1079));
		StringType nodox_project_file = string_replace_all(sVar(project_file), /*"/"*/ STR(20), /*"\\"*/ STR(1077));
		StringType nodox_project_folder = string_replace_all(sStr(project_folder), /*"/"*/ STR(20), /*"\\"*/ STR(1077));
		StringType nodox_working_directory = string_replace_all(gmlGlobal::working_directory, /*"/"*/ STR(20), /*"\\"*/ STR(1077));
		StringType nodox_file_directory = string_replace_all(file_directory, /*"/"*/ STR(20), /*"\\"*/ STR(1077));
		str += /*"Project: \n"*/ STR(1080);
		str += /*"======================================= \n"*/ STR(1064);
		str += /*"project_file: "*/ STR(1081) + string_replace_all(nodox_project_file, userprofile, nodox_userprofile) + /*" \n"*/ STR(1066);
		str += /*"project_folder: "*/ STR(1082) + string_replace_all(nodox_project_folder, userprofile, nodox_userprofile) + /*" \n"*/ STR(1066);
		str += /*"working_directory: "*/ STR(1083) + string_replace_all(nodox_working_directory, userprofile, nodox_userprofile) + /*" \n"*/ STR(1066);
		str += /*"file_directory: "*/ STR(1084) + string_replace_all(nodox_file_directory, userprofile, nodox_userprofile) + /*" \n"*/ STR(1066);
		str += /*"\n"*/ STR(712);
		str += /*"instance_count: "*/ STR(1085) + string(gmlGlobal::instance_count) + /*" \n"*/ STR(1066);
		str += /*"render_world() calls: "*/ STR(1086) + string(global::render_world_count) + /*" \n"*/ STR(1066);
		str += /*"Vertex buffer triangles: "*/ STR(1087) + string(get_vertex_buffer_triangles()) + /*" \n"*/ STR(1066);
		str += /*"Vertex buffer render calls: "*/ STR(1088) + string(get_vertex_buffer_render_calls()) + /*" \n"*/ STR(1066);
		str += /*"Primitive lines: "*/ STR(1089) + string(get_primitive_lines()) + /*" \n"*/ STR(1066);
		str += /*"Primitive triangles: "*/ STR(1090) + string(get_primitive_triangles()) + /*" \n"*/ STR(1066);
		str += /*"Primitive render calls: "*/ STR(1091) + string(get_primitive_render_calls()) + /*" \n"*/ STR(1066);
		str += /*"\n"*/ STR(712);
		str += /*"[F12 to disable]"*/ STR(1092);
	...
	{
		debug({ str, string(current_time() - global::debug_timer) + /*" msec"*/ STR(1093) });
	}
	...
	{
		return filename_name(filename_dir(dir + /*".ext"*/ STR(1095))) + /*"/"*/ STR(20);
	}
	...
	StringType dir_get_string(IntType dir)
	{
		switch ((IntType)dir)
		{
			...
			case e_dir_SOUTH:
			{
				return /*"south"*/ STR(903);
			}
			case e_dir_NORTH:
			{
				return /*"north"*/ STR(904);
			}
			case e_dir_UP:
			{
				return /*"up"*/ STR(920);
	...
	void draw_bezier_graph(ScopeAny self, VarType xx, VarType yy, RealType wid, RealType hei, ArrType points, BoolType sync)
	{
		...
		if (app_mouse_box(self, easeinxpos - IntType(5), easeinypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereasein"*/ STR(1096);
		...
		if (app_mouse_box(self, easeoutxpos - IntType(5), easeoutypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereaseout"*/ STR(1097);
				sVar(handle_drag_offset_x) = gmlGlobal::mouse_x - easeoutxpos;
				sVar(handle_drag_offset_y) = gmlGlobal::mouse_y - easeoutypos;
			}
		}
		...
		if (sVar(window_busy) == /*"beziereasein"*/ STR(1096))
		{
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (sVar(window_busy) == /*"beziereaseout"*/ STR(1097))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (!(sInt(mouse_still) > 0))
			{
	...
	RealType draw_button_accent(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, IntType index)
	{
		...
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		microani_set(/*"accentlistitem"*/ STR(1098) + string(index), null_, mouseon, mouseclick, sInt(setting_accent) == index, 0.5);
		...
		else
		{
			...
			tip_set(self, text_get({ /*"tooltipcustomaccentcolor"*/ STR(1099) }), xx, yy, width, height);
		}
	...
	RealType draw_button_collapse(ScopeAny self, StringType name, VarType open, IntType script, VarType active, StringType caption, StringType tip)
	{
		...
		if (xx + wid < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || sVar(dy) + IntType(24) < sVar(content_y) || sVar(dy) > sVar(content_y) + sVar(content_height))
			return IntType(0);
		draw_button_icon(self, { name + /*"collapse"*/ STR(1100), xx, yy, IntType(20), IntType(20), open > 0 && active > 0, null_, null_, !(active > 0), /*""*/ STR(0), ID_spr_chevron_ani });
	...
	BoolType draw_button_color(ScopeAny self, VarType name, VarType xx, VarType yy, VarType w, VarType color, VarType def, BoolType hsvmode, IntType script)
	{
		...
		active = (sStr(settings_menu_name) == /*"colorpicker"*/ STR(708) && ObjType(obj_colorpicker, sInt(colorpicker))->value_name == name);
		mouseon = app_mouse_box(self, buttonx, yy, buttonw, h) && sBool(content_mouseon);
		mouseclick = mouseon && sBool(mouse_left);
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		click = context_menu_area(self, { xx, yy, w, h, /*"contextmenuvalue"*/ STR(1101), color, e_context_type_COLOR, script, def });
		...
		if (mouseon && sBool(mouse_left_released))
		{
			...
			if (hsvmode)
				ObjType(obj_colorpicker, sInt(colorpicker))->mode = /*"hsv"*/ STR(1042);
		}
		return click;
	}
	...
	BoolType draw_button_menu(ScopeAny self, VarArgs argument)
	{
		...
		{
			cap = minecraft_asset_get_name(/*"modelstate"*/ STR(777), name);
			name = /*"modelstate"*/ STR(777) + name;
		}
		else
			if (sVar(menu_block_current) != null_)
			{
				cap = minecraft_asset_get_name(/*"blockstate"*/ STR(798), name);
				name = /*"blockstate"*/ STR(798) + name;
			}
			else
				cap = text_get({ name });
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			{
				menuactive = true;
				menuhide = ObjType(obj_menu, DsList(sInt(menu_list)).Value(i))->menu_ani_type == /*"hide"*/ STR(752);
		...
		draw_box_hover({ xx, yy, wid, hei, global::microani_arr.Value(e_microani_PRESS) });
		mouseon = (app_mouse_box(self, xx, yy, wid, hei) && !(disabled > 0) && sBool(content_mouseon)) || (menuactive && app_mouse_box(self, xx, yy, wid, hei, /*"menu"*/ STR(1102)));
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (showsearch)
		{
			...
			if (sVar(window_busy) == /*"menu"*/ STR(1102))
			{
		...
		if (!menuactive && mouseon && keyboard_check(vk_control) && sReal(mouse_wheel) != IntType(0))
		{
			...
			ObjType(obj_menu, m)->menu_include_tl_edit = (ObjType(obj_menu, m)->menu_name != /*"timelineeditorparent"*/ STR(1103));
			sVar(menu_current) = m;
		...
		if (mouseon && sBool(mouse_left_released) && menuhide)
		{
			...
			sVar(window_focus) = string(sInt(menu_search_tbx));
			ObjType(obj_menu, menuid)->menu_ani_type = /*"show"*/ STR(753);
			app_mouse_clear(self);
		}
		if (!menuactive && mouseon && sBool(mouse_left_released) && !menuhide)
		{
			...
			ObjType(obj_menu, m)->menu_value = value;
			ObjType(obj_menu, m)->menu_ani = IntType(0);
			ObjType(obj_menu, m)->menu_ani_type = /*"show"*/ STR(753);
