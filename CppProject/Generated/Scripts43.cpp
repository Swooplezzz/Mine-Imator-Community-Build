...
namespace CppProject
{
		...
		switch ((IntType)vid)
		{
			...
			case e_value_TRANSITION:
			{
				switch ((IntType)val)
				{
					case IntType(0):
					{
						return /*"linear"*/ STR(563);
					}
					case IntType(1):
					{
						return /*"easeinquad"*/ STR(566);
					}
					case IntType(2):
					{
						return /*"easeoutquad"*/ STR(567);
					}
					case IntType(3):
					{
						return /*"easeinoutquad"*/ STR(568);
					}
					case IntType(4):
					{
						return /*"instant"*/ STR(564);
					}
				}
	...
	void project_load_legacy_particles(ScopeAny self)
	{
		...
		ArrType boxtype = ArrType::From({ /*"none"*/ STR(904), /*"ground"*/ STR(136), /*"spawn"*/ STR(1350), /*"custom"*/ STR(612) });
	...
	void project_load_legacy_resource()
	{
		withOne (obj_resource, (new obj_resource)->id, noone)
		{
			...
			if (typename_ == /*"item"*/ STR(535))
				typename_ = /*"itemsheet"*/ STR(555);
			if (typename_ == /*"block"*/ STR(536) || typename_ == /*"blocksheet"*/ STR(557))
				typename_ = /*"legacyblocksheet"*/ STR(556);
			if (typename_ == /*"particles"*/ STR(538))
				typename_ = /*"particlesheet"*/ STR(559);
			if (typename_ == /*"schematic"*/ STR(2172))
				typename_ = /*"scenery"*/ STR(534);
			...
			{
				DsMap(global::save_id_map)[self->load_id] = /*"default"*/ STR(99);
	...
	VarType project_load_legacy_save_id()
	{
		...
		if (saveid < IntType(0))
			return null_;
		if (saveid == IntType(0))
			return /*"root"*/ STR(2161);
		if (saveid == IntType(1))
			return /*"default"*/ STR(99);
		return saveid;
	}
	...
	BoolType project_load_legacy_start(VarType argument0)
	{
		...
		if (global::load_format > e_project_FORMAT_CB_103)
		{
			...
			{
				log({ /*"Too old legacy project, format"*/ STR(2175), global::load_format });
				error(/*"errorfilecorrupted"*/ STR(2174));
				buffer_delete(global::buffer_current);
				return false;
			}
		...
		withOne (obj_template, (new obj_template)->id, noone)
		{
			...
			if (self->type == e_temp_type_CHARACTER || self->type == e_temp_type_SPECIAL_BLOCK || self->type == e_temp_type_BODYPART)
			{
				...
				if (ds_map_valid(modelmap))
				{
					...
					if (!is_undefined(DsMap(modelmap).Value(/*"state"*/ STR(1012))))
						self->model_state = string_get_state_vars(DsMap(modelmap).Value(/*"state"*/ STR(1012)));
					else
						self->model_state = ArrType();
					
				}
				else
					log({ /*"Could not convert model "*/ STR(2176), self->legacy_model_name });
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (ds_list_valid(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
					...
					log({ /*"Could not convert model part of "*/ STR(2177), self->model_name, self->legacy_bodypart_id });
			...
			if (self->text_font == /*"root"*/ STR(2161))
		...
		withOne (obj_timeline, (new obj_timeline)->id, noone)
		{
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (!is_undefined(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
				{
					...
					log({ /*"Could not find model part for"*/ STR(2178), ObjType(obj_template, findtemp)->model_name, ObjType(obj_template, findtemp)->legacy_model_name, self->legacy_bodypart_id });
	...
	void project_load_legacy_values(ScopeAny self, IntType tl)
	{
		...
		if (idArr(tl, value_type).Value(e_value_type_PARTICLES))
		{
			...
			if (sVar(value).Value(e_value_ATTRACTOR) == /*"root"*/ STR(2161))
		...
		if (idArr(tl, value_type).Value(e_value_type_MATERIAL_TEXTURE))
		{
			...
			if (sVar(value).Value(e_value_TEXTURE_OBJ) == /*"root"*/ STR(2161))
	...
	RealType project_load_markers(VarType list)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			...
			{
				self->loaded = true;
				self->load_id = value_get_string(DsMap(markermap).Value(/*"id"*/ STR(83)), self->save_id);
				DsMap(global::save_id_map)[self->load_id] = self->load_id;
	...
	{
		VarType templist = DsMap(map).Value(/*"templates"*/ STR(2170));
		...
		VarType tllist = DsMap(map).Value(/*"timelines"*/ STR(2171));
		...
		VarType reslist = DsMap(map).Value(/*"resources"*/ STR(217));
