				...
				if (!string_contains(key, /*"_NBT_"*/ STR(989)))
				{
					...
					if (bracketindex > IntType(0))
					{
						...
						if (!is_undefined(block))
						{
							...
							if (state_vars_get_value(vars, /*"waterlogged"*/ STR(831)) != /*"false"*/ STR(774))
								if (ObjType(obj_block, block)->waterlogged > 0 || state_vars_get_value(vars, /*"waterlogged"*/ STR(831)) == /*"true"*/ STR(773))
			...
			for (IntType i = IntType(0); i < ds_list_size((IntType)(blocklist)); i++)
			{
				...
				if (!is_undefined(blocknbt))
				{
					...
					if (!is_undefined(finalstate))
					{
						...
						entity = DsMap(blocknbt).Value(/*"id"*/ STR(83));
				...
				if (entity != null_)
				{
					self->script = asset_get_index(/*"block_tile_entity_"*/ STR(1015) + string_replace(string_lower(entity), /*"minecraft:"*/ STR(834), /*""*/ STR(0)));
		...
		if (ds_list_valid(self->sch_tileentity_list))
		{
			...
			withOne (obj_builder_thread, DsList(self->thread_list).Value(IntType(0)), self->id)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(ObjType(obj_builder, self.otherId)->sch_tileentity_list)); i++)
				{
					...
					{
						eid = DsMap(entity).Value(/*"id"*/ STR(83));
					...
					if (is_string(eid))
					{
						VarType script = asset_get_index(/*"block_tile_entity_"*/ STR(1015) + string_replace(string_lower(eid), /*"minecraft:"*/ STR(834), /*""*/ STR(0)));
						if (script > -IntType(1))
						{
							self->build_pos_x = ex;
							self->build_pos_y = ey;
