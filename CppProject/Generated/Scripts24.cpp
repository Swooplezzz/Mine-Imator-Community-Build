...
namespace CppProject
{
	BoolType builder_read_schematic(Scope<obj_builder> self, VarType map)
	{
		if (!ds_map_valid(map))
		{
			log({ /*"Schematic error"*/ STR(961), /*"Not a schematic file"*/ STR(962) });
			return false;
		}
		self->builder_scenery_legacy = is_undefined(DsMap(map).Value(/*"Palette"*/ STR(963)));
		self->build_size_x = DsMap(map).Value(/*"Width"*/ STR(964));
		self->build_size_y = DsMap(map).Value(/*"Length"*/ STR(965));
		self->build_size_z = DsMap(map).Value(/*"Height"*/ STR(966));
		log({ /*"Size"*/ STR(55), string(self->build_size_x) + /*" x "*/ STR(967) + string(self->build_size_y) + /*" x "*/ STR(967) + string(self->build_size_z) });
		if (is_undefined(self->build_size_x) || is_undefined(self->build_size_y) || is_undefined(self->build_size_z))
		{
			log({ /*"Schematic error"*/ STR(961), /*"Size not fully defined"*/ STR(968) });
			return false;
		}
		if (self->build_size_x <= IntType(0) || self->build_size_y <= IntType(0) || self->build_size_z <= IntType(0))
		{
			log({ /*"Schematic error"*/ STR(961), /*"Size cannot be 0"*/ STR(969) });
			return false;
		}
		if (!self->builder_scenery_legacy)
		{
			VarType version = DsMap(map).Value(/*"Version"*/ STR(970));
			if (is_undefined(version))
			{
				log({ /*"Schematic error"*/ STR(961), /*"Version not available"*/ STR(971) });
				return false;
			}
			log({ /*"Version"*/ STR(970), DsMap(map).Value(/*"Version"*/ STR(970)) });
			if (version > IntType(1))
			{
				log({ /*"Schematic error"*/ STR(961), /*"Unsupported format, version too high"*/ STR(972) });
				return false;
			}
			VarType palettemap = DsMap(map).Value(/*"Palette"*/ STR(963));
			if (!ds_map_valid(palettemap))
			{
				log({ /*"Schematic error"*/ STR(961), /*"Palette not found"*/ STR(973) });
				return false;
			}
			...
			while (!is_undefined(key))
			{
				if (!string_contains(key, /*"_NBT_"*/ STR(974)))
				{
					...
					bracketindex = string_pos(/*"["*/ STR(975), key);
					if (bracketindex > IntType(0))
					{
						...
						if (!is_undefined(block))
						{
							...
							if (state_vars_get_value(vars, /*"waterlogged"*/ STR(817)) != /*"false"*/ STR(760))
								if (ObjType(obj_block, block)->waterlogged > 0 || state_vars_get_value(vars, /*"waterlogged"*/ STR(817)) == /*"true"*/ STR(759))
			...
			self->sch_blockdata_array = DsMap(map).Value(/*"BlockData"*/ STR(976));
			if (is_undefined(self->sch_blockdata_array))
			{
				log({ /*"Schematic error"*/ STR(961), /*"BlockData array not found"*/ STR(977) });
				return false;
			}
			self->sch_blockdata_ints = (DsMap(map).Value(/*"BlockData_NBT_type"*/ STR(978)) == e_nbt_TAG_INT_ARRAY);
			debug({ /*"blockdataints"*/ STR(979), self->sch_blockdata_ints });
			VarType metadata = DsMap(map).Value(/*"Metadata"*/ STR(980));
			if (ds_map_valid(metadata))
				self->file_map = DsMap(metadata).Value(/*"FromMap"*/ STR(981));
		}
		else
		{
			self->sch_legacy_blocksarray = DsMap(map).Value(/*"Blocks"*/ STR(982));
			if (is_undefined(self->sch_legacy_blocksarray))
			{
				log({ /*"Schematic error"*/ STR(961), /*"Blocks array not found"*/ STR(983) });
				return false;
			}
			self->sch_legacy_dataarray = DsMap(map).Value(/*"Data"*/ STR(984));
			if (is_undefined(self->sch_legacy_dataarray))
			{
				log({ /*"Schematic error"*/ STR(961), /*"Data array not found"*/ STR(985) });
				return false;
			}
			self->file_map = DsMap(map).Value(/*"FromMap"*/ STR(981));
		}
		
		if (is_undefined(self->file_map))
			self->file_map = /*""*/ STR(0);
		self->sch_tileentity_list = DsMap(map).Value(/*"TileEntities"*/ STR(986));
		return true;
	}
	...
	BoolType builder_read_schematic_nbt(ScopeAny self, VarType structuremap)
	{
		...
		if (!is_undefined(DsMap(structuremap).Value(/*"DataVersion"*/ STR(987))))
			structureversion = DsMap(structuremap).Value(/*"DataVersion"*/ STR(987));
		if (structureversion < IntType(2000))
		{
			log({ /*"Structure error"*/ STR(988), /*"Unsupported format, version too low"*/ STR(989) });
			return false;
		}
		sizemap = DsMap(structuremap).Value(/*"size"*/ STR(990));
		...
		{
			log({ /*"Structure error"*/ STR(988), /*"Size cannot be 0"*/ STR(969) });
			return false;
		}
		VarType paletteslist, palettelist;
		paletteslist = DsMap(structuremap).Value(/*"palettes"*/ STR(991));
		...
		{
			palettelist = DsMap(structuremap).Value(/*"palette"*/ STR(992));
			if (!ds_list_valid(palettelist))
			{
				log({ /*"Structure error"*/ STR(988), /*"Palette not found"*/ STR(973) });
				return false;
			}
		}
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(palettelist)); i++)
		{
			...
			mcid = DsMap(blockmap).Value(/*"Name"*/ STR(993));
			...
			if (!is_undefined(block))
			{
				...
				if (ObjType(obj_block, block)->id_state_vars_map != null_ && is_array(DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid)))
					state_vars_add(VarType::CreateRef(vars), DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid));
				propertiesmap = DsMap(blockmap).Value(/*"Properties"*/ STR(957));
		...
		VarType blocklist = DsMap(structuremap).Value(/*"blocks"*/ STR(994));
		if (!ds_list_valid(blocklist))
		{
			log({ /*"Structure error"*/ STR(988), /*"Block list not found"*/ STR(995) });
			return false;
		}
		withOne (obj_builder, global::mc_builder, self->id)
		{
			...
			for (IntType i = IntType(0); i < ds_list_size((IntType)(blocklist)); i++)
			{
				...
				pos = DsMap(blockmap).Value(/*"pos"*/ STR(996));
				state = DsMap(blockmap).Value(/*"state"*/ STR(997));
				...
				blocknbt = DsMap(blockmap).Value(/*"nbt"*/ STR(998));
				if (!is_undefined(blocknbt))
				{
					VarType finalstate, script;
					finalstate = DsMap(blocknbt).Value(/*"final_state"*/ STR(999));
				...
				if (entity != null_)
				{
					self->script = asset_get_index(/*"block_tile_entity_"*/ STR(1000) + string_replace(string_lower(entity), /*"minecraft:"*/ STR(820), /*""*/ STR(0)));
		...
		debug_timer_stop(/*"res_load_scenery, Parse blocks"*/ STR(1001));
		return true;
	}
		...
		if (ds_list_valid(self->sch_tileentity_list))
		{
			...
			withOne (obj_builder_thread, DsList(self->thread_list).Value(IntType(0)), self->id)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(ObjType(obj_builder, self.otherId)->sch_tileentity_list)); i++)
				{
					...
					if (!self->builder_scenery_legacy)
					{
						eid = DsMap(entity).Value(/*"Id"*/ STR(956));
						VarType poslist = DsMap(entity).Value(/*"Pos"*/ STR(1003));
					...
					else
					{
						...
						ex = DsMap(entity).Value(/*"x"*/ STR(832));
						ey = DsMap(entity).Value(/*"z"*/ STR(833));
						ez = DsMap(entity).Value(/*"y"*/ STR(815));
					}
					
					if (is_string(eid))
					{
						VarType script = asset_get_index(/*"block_tile_entity_"*/ STR(1000) + string_replace(string_lower(eid), /*"minecraft:"*/ STR(820), /*""*/ STR(0)));
						if (script > -IntType(1))
						{
							self->build_pos_x = ex;
							self->build_pos_y = ey;
				...
				debug_timer_stop(/*"Parse Tile Entities"*/ STR(1004));
			}
	...
	void colorpicker_draw(ScopeAny self)
	{
		...
		{
			textfield_group_add(self, { /*"colorpickerr"*/ STR(1015), color_get_red((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color)), color_get_red(ObjType(obj_colorpicker, sInt(colorpicker))->def), ID_colorpicker_red, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_red });
			textfield_group_add(self, { /*"colorpickerg"*/ STR(1016), color_get_green((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color)), color_get_green(ObjType(obj_colorpicker, sInt(colorpicker))->def), ID_colorpicker_green, Y_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_green });
			textfield_group_add(self, { /*"colorpickerb"*/ STR(1017), color_get_blue((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color)), color_get_blue(ObjType(obj_colorpicker, sInt(colorpicker))->def), ID_colorpicker_blue, Z_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_blue });
			...
			{
				textfield_group_add(self, { /*"colorpickerh"*/ STR(1018), floor(color_get_hue((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color))), floor(color_get_hue(ObjType(obj_colorpicker, sInt(colorpicker))->def)), ID_colorpicker_hue, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_hue });
				textfield_group_add(self, { /*"colorpickers"*/ STR(1019), floor(color_get_saturation((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color))), floor(color_get_saturation(ObjType(obj_colorpicker, sInt(colorpicker))->def)), ID_colorpicker_saturation, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_saturation });
				textfield_group_add(self, { /*"colorpickerv"*/ STR(1020), floor(color_get_value((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color))), floor(color_get_value(ObjType(obj_colorpicker, sInt(colorpicker))->def)), ID_colorpicker_brightness, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_brightness });
				...
				if (ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"hex"*/ STR(1013))
				{
					...
					if (ObjType(obj_textbox, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_hexadecimal)->text == /*""*/ STR(0) && sVar(window_focus) == /*""*/ STR(0))
						ObjType(obj_textbox, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_hexadecimal)->text = /*"000000"*/ STR(672);
				}
	...
	{
		if (sStr(settings_menu_name) == /*"colorpicker"*/ STR(692) && ObjType(obj_colorpicker, sInt(colorpicker))->value_script == script)
			popup_close(self);
		else
		{
			...
			sStr(settings_menu_name) = /*"colorpicker"*/ STR(692);
