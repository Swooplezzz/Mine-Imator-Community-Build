...
namespace CppProject
{
	...
	BoolType history_save_part_usage_tl(IntType tl, VarType hobj)
	{
		...
		withAll (obj_timeline, noone)
		{
			...
			{
				ObjType(obj_history, hobj)->usage_tl_copy_rot_target_save_id[ObjType(obj_history, hobj)->usage_tl_copy_rot_target_amount] = self->save_id;
				ObjType(obj_history, hobj)->usage_tl_copy_rot_target_part_save_id[ObjType(obj_history, hobj)->usage_tl_copy_rot_target_amount] = idVar(tl, save_id);
				ObjType(obj_history, hobj)->usage_tl_copy_rot_target_amount++;
				used = true;
			}
			if (self->value.Value(e_value_SCALE_TARGET) == tl)
			{
				ObjType(obj_history, hobj)->usage_tl_copy_scale_target_save_id[ObjType(obj_history, hobj)->usage_tl_copy_scale_target_amount] = self->save_id;
				ObjType(obj_history, hobj)->usage_tl_copy_scale_target_part_save_id[ObjType(obj_history, hobj)->usage_tl_copy_scale_target_amount] = idVar(tl, save_id);
				ObjType(obj_history, hobj)->usage_tl_copy_scale_target_amount++;
				used = true;
			}
			if (self->value.Value(e_value_BEND_IK_TARGET) == tl)
			{
				ObjType(obj_history, hobj)->usage_tl_bnd_ik_target_save_id[ObjType(obj_history, hobj)->usage_tl_bnd_ik_target_amount] = self->save_id;
				ObjType(obj_history, hobj)->usage_tl_bnd_ik_target_part_save_id[ObjType(obj_history, hobj)->usage_tl_bnd_ik_target_amount] = idVar(tl, save_id);
				ObjType(obj_history, hobj)->usage_tl_bnd_ik_target_amount++;
				used = true;
			}
		}
		...
		withAll (obj_keyframe, noone)
		{
			...
			if (self->value.Value(e_value_ROT_TARGET) == tl)
			{
				ObjType(obj_history, hobj)->usage_kf_copy_rot_target_tl_save_id[ObjType(obj_history, hobj)->usage_kf_copy_rot_target_amount] = save_id_get(self->timeline);
				ObjType(obj_history, hobj)->usage_kf_copy_rot_target_tl_part_save_id[ObjType(obj_history, hobj)->usage_kf_copy_rot_target_amount] = idVar(tl, save_id);
				ObjType(obj_history, hobj)->usage_kf_copy_rot_target_index[ObjType(obj_history, hobj)->usage_kf_copy_rot_target_amount] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
				ObjType(obj_history, hobj)->usage_kf_copy_rot_target_amount++;
				used = true;
			}
			if (self->value.Value(e_value_POS_TARGET) == tl)
			{
				ObjType(obj_history, hobj)->usage_kf_copy_pos_target_tl_save_id[ObjType(obj_history, hobj)->usage_kf_copy_pos_target_amount] = save_id_get(self->timeline);
				ObjType(obj_history, hobj)->usage_kf_copy_pos_target_tl_part_save_id[ObjType(obj_history, hobj)->usage_kf_copy_pos_target_amount] = idVar(tl, save_id);
				ObjType(obj_history, hobj)->usage_kf_copy_pos_target_index[ObjType(obj_history, hobj)->usage_kf_copy_pos_target_amount] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
				ObjType(obj_history, hobj)->usage_kf_copy_pos_target_amount++;
				used = true;
			}
			if (self->value.Value(e_value_SCALE_TARGET) == tl)
			{
				ObjType(obj_history, hobj)->usage_kf_copy_scale_target_tl_save_id[ObjType(obj_history, hobj)->usage_kf_copy_scale_target_amount] = save_id_get(self->timeline);
				ObjType(obj_history, hobj)->usage_kf_copy_scale_target_tl_part_save_id[ObjType(obj_history, hobj)->usage_kf_copy_scale_target_amount] = idVar(tl, save_id);
				ObjType(obj_history, hobj)->usage_kf_copy_scale_target_index[ObjType(obj_history, hobj)->usage_kf_copy_scale_target_amount] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
				ObjType(obj_history, hobj)->usage_kf_copy_scale_target_amount++;
				used = true;
			}
			if (self->value.Value(e_value_BEND_IK_TARGET) == tl)
			{
				ObjType(obj_history, hobj)->usage_kf_bnd_ik_target_tl_save_id[ObjType(obj_history, hobj)->usage_kf_bnd_ik_target_amount] = save_id_get(self->timeline);
				ObjType(obj_history, hobj)->usage_kf_bnd_ik_target_tl_part_save_id[ObjType(obj_history, hobj)->usage_kf_bnd_ik_target_amount] = idVar(tl, save_id);
				ObjType(obj_history, hobj)->usage_kf_bnd_ik_target_index[ObjType(obj_history, hobj)->usage_kf_bnd_ik_target_amount] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
				ObjType(obj_history, hobj)->usage_kf_bnd_ik_target_amount++;
				used = true;
			}
			if (self->value.Value(e_value_IK_TARGET_ANGLE) == tl)
			{
	...
	IntType history_save_tl(ScopeAny self, VarType tl)
	{
		...
		withOne (obj_history_save, save, self->id)
		{
			...
			self->usage_tl_copy_rot_target_amount = IntType(0);
			self->usage_tl_copy_pos_target_amount = IntType(0);
			self->usage_tl_copy_scale_target_amount = IntType(0);
			...
			withAll (obj_timeline, self->id)
			{
				...
				if (self->value.Value(e_value_ROT_TARGET) == tl)
				{
					ObjType(obj_history_save, save)->usage_tl_copy_rot_target_save_id[ObjType(obj_history_save, save)->usage_tl_copy_rot_target_amount] = self->save_id;
					ObjType(obj_history_save, save)->usage_tl_copy_rot_target_amount++;
				}
				if (self->value.Value(e_value_POS_TARGET) == tl)
				{
					ObjType(obj_history_save, save)->usage_tl_copy_pos_target_save_id[ObjType(obj_history_save, save)->usage_tl_copy_pos_target_amount] = self->save_id;
					ObjType(obj_history_save, save)->usage_tl_copy_pos_target_amount++;
				}
				if (self->value.Value(e_value_SCALE_TARGET) == tl)
				{
					ObjType(obj_history_save, save)->usage_tl_copy_scale_target_save_id[ObjType(obj_history_save, save)->usage_tl_copy_scale_target_amount] = self->save_id;
					ObjType(obj_history_save, save)->usage_tl_copy_scale_target_amount++;
				}
				if (self->value.Value(e_value_BEND_IK_TARGET) == tl)
				{
					ObjType(obj_history_save, save)->usage_tl_bnd_ik_target_save_id[ObjType(obj_history_save, save)->usage_tl_bnd_ik_target_amount] = self->save_id;
					ObjType(obj_history_save, save)->usage_tl_bnd_ik_target_amount++;
				}
				if (self->value.Value(e_value_IK_TARGET_ANGLE) == tl)
				{
			...
			self->usage_kf_copy_rot_target_amount = IntType(0);
			self->usage_kf_copy_pos_target_amount = IntType(0);
			self->usage_kf_copy_scale_target_amount = IntType(0);
			...
			withAll (obj_keyframe, self->id)
			{
				...
				if (self->value.Value(e_value_ROT_TARGET) == tl)
				{
					ObjType(obj_history_save, save)->usage_kf_copy_rot_target_tl_save_id[ObjType(obj_history_save, save)->usage_kf_copy_rot_target_amount] = save_id_get(self->timeline);
					ObjType(obj_history_save, save)->usage_kf_copy_rot_target_index[ObjType(obj_history_save, save)->usage_kf_copy_rot_target_amount] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
					ObjType(obj_history_save, save)->usage_kf_copy_rot_target_amount++;
				}
				if (self->value.Value(e_value_POS_TARGET) == tl)
				{
					ObjType(obj_history_save, save)->usage_kf_copy_pos_target_tl_save_id[ObjType(obj_history_save, save)->usage_kf_copy_pos_target_amount] = save_id_get(self->timeline);
					ObjType(obj_history_save, save)->usage_kf_copy_pos_target_index[ObjType(obj_history_save, save)->usage_kf_copy_pos_target_amount] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
					ObjType(obj_history_save, save)->usage_kf_copy_pos_target_amount++;
				}
				if (self->value.Value(e_value_SCALE_TARGET) == tl)
				{
					ObjType(obj_history_save, save)->usage_kf_copy_scale_target_tl_save_id[ObjType(obj_history_save, save)->usage_kf_copy_scale_target_amount] = save_id_get(self->timeline);
					ObjType(obj_history_save, save)->usage_kf_copy_scale_target_index[ObjType(obj_history_save, save)->usage_kf_copy_scale_target_amount] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
					ObjType(obj_history_save, save)->usage_kf_copy_scale_target_amount++;
				}
				if (self->value.Value(e_value_BEND_IK_TARGET) == tl)
				{
					ObjType(obj_history_save, save)->usage_kf_bnd_ik_target_tl_save_id[ObjType(obj_history_save, save)->usage_kf_bnd_ik_target_amount] = save_id_get(self->timeline);
					ObjType(obj_history_save, save)->usage_kf_bnd_ik_target_index[ObjType(obj_history_save, save)->usage_kf_bnd_ik_target_amount] = ds_list_find_index(idInt(self->timeline, keyframe_list), self->id);
					ObjType(obj_history_save, save)->usage_kf_bnd_ik_target_amount++;
				}
				if (self->value.Value(e_value_IK_TARGET_ANGLE) == tl)
				{
	...
	VarType history_save_var_start(ScopeAny self, IntType script, RealType combine)
	{
		...
		{
			history_push(self);
			debug({ /*"history_save_var_start("*/ STR(1237) + script_get_name(script) + /*", "*/ STR(1057) + string(combine) + /*")"*/ STR(1131) });
	...
	IntType history_set(ScopeAny self, IntType script)
	{
		...
		history_pop(self);
		history_push(self);
		log({ /*"Action"*/ STR(1238), script_get_name(script) });
		hobj = new_history(script);
		sArr(history)[IntType(0)] = hobj;
		return hobj;
	}
	...
	IntType history_set_res(ScopeAny self, IntType script, VarType fn, VarType oldres, IntType newres)
	{
		history_pop(self);
		history_push(self);
		log({ /*"Action Load resource"*/ STR(1239), script_get_name(script), fn });
	...
	VarType history_set_var(ScopeAny self, IntType script, VarType oldval, VarType newval, RealType combine)
	{
		...
		{
			history_push(self);
			log({ /*"Action Set variable"*/ STR(1240), script_get_name(script), oldval, newval, combine });
			hobj = new_history(script);
	...
	BoolType json_load_number()
	{
		...
		do
		{
			...
			if (global::json_char == e_json_char_E || global::json_char == e_json_char_CAPITAL_E)
			{
				if (expart)
				{
					global::json_error = /*"Unexpected symbol"*/ STR(1247);
					return false;
				}
				global::json_char = buffer_read_byte();
				global::json_column++;
		...
		{
			global::json_error = /*"Invalid number \""*/ STR(1248) + str + /*"\""*/ STR(1157);
			return false;
		}
		return true;
	}
