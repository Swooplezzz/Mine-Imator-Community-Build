...
namespace CppProject
{
	...
	VarType history_save_var_start(ScopeAny self, IntType script, RealType combine)
	{
		...
		{
			history_push(self);
			debug({ /*"history_save_var_start("*/ STR(1238) + script_get_name(script) + /*", "*/ STR(1058) + string(combine) + /*")"*/ STR(1132) });
	...
	IntType history_set_res(ScopeAny self, IntType script, VarType fn, VarType oldres, IntType newres)
	{
		history_pop(self);
		history_push(self);
		log({ /*"Action Load resource"*/ STR(1240), script_get_name(script), fn });
	...
	VarType history_set_var(ScopeAny self, IntType script, VarType oldval, VarType newval, RealType combine)
	{
		...
		{
			history_push(self);
			log({ /*"Action Set variable"*/ STR(1241), script_get_name(script), oldval, newval, combine });
			hobj = new_history(script);
	...
	BoolType json_load_number()
	{
		...
		do
		{
			...
			if (global::json_char == e_json_char_E || global::json_char == e_json_char_CAPITAL_E)
			{
				if (expart)
				{
					global::json_error = /*"Unexpected symbol"*/ STR(1248);
					return false;
				}
				global::json_char = buffer_read_byte();
				global::json_column++;
		...
		{
			global::json_error = /*"Invalid number \""*/ STR(1249) + str + /*"\""*/ STR(1158);
			return false;
		}
		return true;
	}
