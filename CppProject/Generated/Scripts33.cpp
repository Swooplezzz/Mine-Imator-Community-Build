...
namespace CppProject
{
	BoolType json_load_object(BoolType root)
	{
		...
		while (json_load_char())
		{
			...
			{
				global::json_error = /*"Expected \":\""*/ STR(1244);
				break;
			}
			if (!json_load_char())
				break;
			...
			{
				global::json_error = /*"Expected \",\""*/ STR(1240);
				break;
			}
		}
	...
	BoolType json_load_string()
	{
		...
		while (true)
		{
			if (buffer_is_eof())
			{
				global::json_error = /*"Unexpected end of file"*/ STR(1241);
				return false;
			}
			...
			{
				global::json_error = /*"Unexpected linebreak in string"*/ STR(1245);
				return false;
			}
	...
	void json_save_var_color(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		json_save_value(/*"#"*/ STR(856) + color_to_hex(val));
		global::json_add_comma = true;
	}
	...
	RealType languages_load()
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			...
			name = value_get_string(DsMap(map).Value(/*"name"*/ STR(816)), /*""*/ STR(0));
		...
		if (!reload && map != global::language_english_map)
		{
			...
			if (ds_list_size(missingkeyslist) > IntType(0))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(missingkeyslist); i++)
					msg += DsList(missingkeyslist).Value(i) + /*": "*/ STR(1052) + string_replace_all(DsMap(global::language_english_map).Value(DsList(missingkeyslist).Value(i)), /*"\n"*/ STR(76), /*"\\n"*/ STR(1257)) + /*"\n"*/ STR(76);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1279));
	...
	RealType language_load_legacy(VarType fn, IntType map)
	{
		...
		while (!file_text_eof(f))
		{
			...
			commapos = string_pos(/*":"*/ STR(785), line);
	...
	BoolType lib_startup()
	{
		...
		global::lib_execute = external_define({ pathfile, /*"execute"*/ STR(1140), dll_cdecl, ty_real, IntType(3), ty_string, ty_string, ty_real });
