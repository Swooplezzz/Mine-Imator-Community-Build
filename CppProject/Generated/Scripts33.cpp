...
namespace CppProject
{
	IntType list_init_context_menu(ScopeAny self, VarType name)
	{
		...
		switch (StringType(name).id)
		{
			...
			{
				list_item_add(text_get({ /*"contextmenutextboxcut"*/ STR(1519) }), null_, text_control_name(keybind_new(/*"X"*/ STR(271), true)), null_, icons_CUT, null_, ID_action_textbox_cut, true);
				ObjType(obj_list_item, global::list_item_last)->disabled = (sVar(textbox_select_startpos) == sVar(textbox_select_endpos));
				list_item_add(text_get({ /*"contextmenutextboxcopy"*/ STR(1520) }), null_, text_control_name(keybind_new(/*"C"*/ STR(269), true)), null_, icons_COPY, null_, ID_action_textbox_copy, false);
				ObjType(obj_list_item, global::list_item_last)->disabled = (sVar(textbox_select_startpos) == sVar(textbox_select_endpos));
				list_item_add(text_get({ /*"contextmenutextboxpaste"*/ STR(1521) }), null_, text_control_name(keybind_new(/*"V"*/ STR(273), true)), null_, icons_PASTE, null_, ID_action_textbox_paste, false);
				ObjType(obj_list_item, global::list_item_last)->disabled = (clipboard_get_text() == /*""*/ STR(0) || !clipboard_has_text());
				list_item_add(text_get({ /*"contextmenutextboxselectall"*/ STR(1522) }), null_, text_control_name(keybind_new(/*"A"*/ STR(260), true)), null_, icons_SELECT_ALL, null_, ID_action_textbox_select_all, false);
				break;
			}
			...
			case 1523: // timelinelist
			{
				...
				if (sReal(setting_advanced_mode) > 0)
				{
					...
					ObjType(obj_list_item, global::list_item_last)->context_menu_name = /*"color"*/ STR(915);
					ObjType(obj_list_item, global::list_item_last)->disabled = (sVar(context_menu_value) == null_ && global::tl_edit == null_);
				}
	...
	RealType menu_add_timeline(VarType tl, RealType level, VarType menu)
	{
		...
		else
		{
			...
			if (ds_list_size(idInt(tl, tree_list)))
				list_item_add_action(global::list_item_last, string(tl) + /*"extend"*/ STR(1687), ID_menu_item_set_extend, idVar(tl, tree_extend), tl, null_, /*"left"*/ STR(192), (idVar(tl, tree_extend) > 0) ? /*"tooltiptlcollapse"*/ STR(1688) : /*"tooltiptlexpand"*/ STR(1689), ID_spr_chevron_ani);
		}
	...
	IntType menu_biome_init(ScopeAny self, VarType menu)
	{
		...
		for (IntType b = IntType(0); b < ds_list_size(global::biome_list); b++)
		{
			...
			{
				list_item_add_action(global::list_item_last, string(biome) + /*"extend"*/ STR(1687), ID_menu_item_set_extend, ObjType(obj_biome, biome)->variants_extend, biome, null_, /*"left"*/ STR(192), (ObjType(obj_biome, biome)->variants_extend > 0) ? /*"tooltiptlcollapse"*/ STR(1688) : /*"tooltiptlexpand"*/ STR(1689), ID_spr_chevron_ani);
				if (ObjType(obj_biome, biome)->variants_extend > 0)
				{
					...
					{
						if (ObjType(obj_biome, DsList(ObjType(obj_biome, biome)->biome_variants).Value(v))->name == /*"normal"*/ STR(628))
							continue;
	...
	void menu_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			if (ObjType(obj_menu, m)->menu_window != window_get_current())
				continue;
			if (ObjType(obj_menu, m)->menu_ani_type == /*"hide"*/ STR(746))
			{
				...
				if (ObjType(obj_menu, m)->menu_ani_type == /*"show"*/ STR(747))
				{
					ObjType(obj_menu, m)->menu_ani += test_reduced_motion(IntType(1), (0.06 * global::delta));
			...
			if (ObjType(obj_menu, m)->menu_steps == IntType(0))
				draw_set_alpha(0.0);
			ObjType(obj_menu, m)->menu_ani_ease = ease(((ObjType(obj_menu, m)->menu_ani_type == /*"show"*/ STR(747)) ? /*"easeoutexpo"*/ STR(581) : /*"easeinexpo"*/ STR(580)), ObjType(obj_menu, m)->menu_ani);
			...
			if (sVar(window_busy) == /*"menu"*/ STR(1071) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(746) && menu_active)
			...
			if (!(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->mouseon) && !(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->mouseon) && sBool(mouse_left_released) && menu_active && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(746))
			{
				...
				{
					ObjType(obj_menu, m)->menu_ani = IntType(1);
					ObjType(obj_menu, m)->menu_ani_type = /*"hide"*/ STR(746);
			...
			if (sVar(window_busy) == /*""*/ STR(0) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(746) && menu_active)
