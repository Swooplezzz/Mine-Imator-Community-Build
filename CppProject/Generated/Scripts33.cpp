...
namespace CppProject
{
		...
		switch (StringType(name).id)
		{
			...
			case 1506: // timelinelist
			{
				...
				if (sReal(setting_advanced_mode) > 0)
				{
					...
					ObjType(obj_list_item, global::list_item_last)->context_menu_name = /*"color"*/ STR(900);
					ObjType(obj_list_item, global::list_item_last)->disabled = (sVar(context_menu_value) == null_ && global::tl_edit == null_);
				}
	...
	{
		log({ /*"External library"*/ STR(1275), path });
	...
	RealType menu_add_timeline(VarType tl, RealType level, VarType menu)
	{
		...
		else
		{
			...
			if (ds_list_size(idInt(tl, tree_list)))
				list_item_add_action({ global::list_item_last, string(tl) + /*"extend"*/ STR(1668), ID_menu_item_set_extend, idVar(tl, tree_extend), tl, null_, /*"left"*/ STR(191), (idVar(tl, tree_extend) > 0) ? /*"tooltiptlcollapse"*/ STR(1669) : /*"tooltiptlexpand"*/ STR(1670), ID_spr_chevron_ani });
		}
	...
	IntType menu_biome_init(ScopeAny self, VarType menu)
	{
		...
		for (IntType b = IntType(0); b < ds_list_size(global::biome_list); b++)
		{
			...
			menu_add_item({ ObjType(obj_biome, biome)->name, minecraft_asset_get_name(/*"biome"*/ STR(1671), ObjType(obj_biome, biome)->name) });
			if (ObjType(obj_biome, biome)->biome_variants != null_ && ds_list_size(ObjType(obj_biome, biome)->biome_variants) > IntType(0))
			{
				list_item_add_action({ global::list_item_last, string(biome) + /*"extend"*/ STR(1668), ID_menu_item_set_extend, ObjType(obj_biome, biome)->variants_extend, biome, null_, /*"left"*/ STR(191), (ObjType(obj_biome, biome)->variants_extend > 0) ? /*"tooltiptlcollapse"*/ STR(1669) : /*"tooltiptlexpand"*/ STR(1670), ID_spr_chevron_ani });
				if (ObjType(obj_biome, biome)->variants_extend > 0)
				{
					...
					{
						if (ObjType(obj_biome, DsList(ObjType(obj_biome, biome)->biome_variants).Value(v))->name == /*"normal"*/ STR(615))
							continue;
						menu_add_item({ ObjType(obj_biome, DsList(ObjType(obj_biome, biome)->biome_variants).Value(v))->name, minecraft_asset_get_name(/*"biome"*/ STR(1671), ObjType(obj_biome, DsList(ObjType(obj_biome, biome)->biome_variants).Value(v))->name) });
	...
	void menu_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			if (ObjType(obj_menu, m)->menu_window != window_get_current())
				continue;
			if (ObjType(obj_menu, m)->menu_ani_type == /*"hide"*/ STR(733))
			{
				...
				if (ObjType(obj_menu, m)->menu_ani_type == /*"show"*/ STR(734))
				{
					ObjType(obj_menu, m)->menu_ani += test_reduced_motion(IntType(1), (0.06 * global::delta));
			...
			if (ObjType(obj_menu, m)->menu_steps == IntType(0))
				draw_set_alpha(0.0);
			ObjType(obj_menu, m)->menu_ani_ease = ease(((ObjType(obj_menu, m)->menu_ani_type == /*"show"*/ STR(734)) ? /*"easeoutexpo"*/ STR(568) : /*"easeinexpo"*/ STR(567)), ObjType(obj_menu, m)->menu_ani);
			...
			if (sVar(window_busy) == /*"menu"*/ STR(1056) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(733) && menu_active)
			...
			if (!(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->mouseon) && !(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->mouseon) && sBool(mouse_left_released) && menu_active && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(733))
			{
				...
				{
					ObjType(obj_menu, m)->menu_ani = IntType(1);
					ObjType(obj_menu, m)->menu_ani_type = /*"hide"*/ STR(733);
					sVar(window_busy) = (ds_list_size(sInt(menu_list)) > IntType(1) ? /*"menu"*/ STR(1056) : /*""*/ STR(0));
				}
			}
			if (ObjType(obj_menu, m)->menu_steps == IntType(0))
				draw_set_alpha(1.0);
			ObjType(obj_menu, m)->menu_steps++;
			if (sVar(window_busy) == /*""*/ STR(0) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(733) && menu_active)
