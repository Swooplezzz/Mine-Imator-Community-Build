...
namespace CppProject
{
	BoolType json_load_object(BoolType root)
	{
		...
		while (json_load_char())
		{
			...
			{
				global::json_error = /*"Expected \":\""*/ STR(1234);
				break;
			}
			if (!json_load_char())
				break;
			...
			{
				global::json_error = /*"Expected \",\""*/ STR(1230);
				break;
			}
		}
	...
	BoolType json_load_string()
	{
		...
		while (true)
		{
			if (buffer_is_eof())
			{
				global::json_error = /*"Unexpected end of file"*/ STR(1231);
				return false;
			}
			...
			{
				global::json_error = /*"Unexpected linebreak in string"*/ STR(1235);
				return false;
			}
	...
	BoolType json_load_word()
	{
		if (global::json_char == e_json_char_T)
		{
			...
			if (global::json_char == e_json_char_F)
			{
				...
				{
					global::json_error = /*"Unrecognized word"*/ STR(1236);
					return false;
				}
	...
	void json_save_var_bool(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		buffer_write_string((val > 0) ? /*"true"*/ STR(774) : /*"false"*/ STR(775));
		global::json_add_comma = true;
	}
	...
	void json_save_var_color(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		json_save_value(/*"#"*/ STR(857) + color_to_hex(val));
		global::json_add_comma = true;
	}
	...
	RealType keyframes_save(ScopeAny self)
	{
		...
		log({ /*"Saving keyframes"*/ STR(1252), fn });
	...
	BoolType key_valid(VarType key)
	{
		...
		keystr = /*"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"*/ STR(1254);
	...
	void langauge_new(StringType fn)
	{
		...
		withOne (obj_language, obj, noone)
		{
			...
			self->name = text_get({ /*"filelanguage"*/ STR(1255) });
			self->locale = text_exists(/*"filelocale"*/ STR(1256)) ? string(text_get({ /*"filelocale"*/ STR(1256) })) : /*""*/ STR(0);
		}
	...
	RealType languages_load()
	{
		...
		if (!file_exists_lib(fn))
			return IntType(0);
		log({ /*"Loading languages"*/ STR(1257), fn });
		...
		VarType list, obj;
		list = DsMap(map).Value(/*"languages"*/ STR(1259));
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			map = DsList(list).Value(i);
			VarType name, locale, fn;
			name = value_get_string(DsMap(map).Value(/*"name"*/ STR(817)), /*""*/ STR(0));
			locale = value_get_string(DsMap(map).Value(/*"locale"*/ STR(1260)), /*""*/ STR(0));
			fn = value_get_string(DsMap(map).Value(/*"filename"*/ STR(1261)), /*""*/ STR(0));
			language_remove(fn);
			IntType obj = (new obj_language)->id;
	...
	void languages_save()
	{
		log({ /*"Saving languages"*/ STR(1262), languages_file });
		json_save_start(languages_file);
		json_save_object_start();
		json_save_var(/*"format"*/ STR(92), settings_format);
		json_save_array_start({ /*"languages"*/ STR(1259) });
		withAll (obj_language, noone)
		{
			json_save_object_start();
			json_save_var(/*"name"*/ STR(817), json_string_encode(self->name));
			json_save_var(/*"locale"*/ STR(1260), self->locale);
			json_save_var(/*"filename"*/ STR(1261), self->filename);
			json_save_object_done();
		}
	...
	RealType language_add(ScopeAny self)
	{
		...
		if (file_exists_lib(languages_directory + filename_name(fn)))
			if (!question(/*"This file already exists the language file directory. Do you want to replace the file?"*/ STR(1264)))
				return IntType(0);
	...
	{
		log({ /*"Loading language file"*/ STR(1265), fn });
		ds_map_clear(map);
		if (filename_ext(fn) == /*".milanguage"*/ STR(1266))
		{
			StringType convfn = file_directory + /*"conv.tmp"*/ STR(1267);
			json_file_convert_unicode(fn, convfn);
			if (!file_exists_lib(convfn))
			{
				StringType msg = /*"An error occurred while reading the language file:\nCould not convert."*/ STR(1268);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1269));
				show_message(msg);
			...
			{
				StringType msg = /*"An error occured while reading the language file:\n"*/ STR(1270) + global::json_error + /*" on line "*/ STR(1228) + string(global::json_line) + /*", column "*/ STR(1229) + string(global::json_column);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1269));
				show_message(msg);
				window_set_caption(/*""*/ STR(0));
		...
		if (!reload && map != global::language_english_map)
		{
			...
			if (ds_list_size(missingkeyslist) > IntType(0))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(missingkeyslist); i++)
					msg += DsList(missingkeyslist).Value(i) + /*": "*/ STR(1053) + string_replace_all(DsMap(global::language_english_map).Value(DsList(missingkeyslist).Value(i)), /*"\n"*/ STR(708), /*"\\n"*/ STR(1247)) + /*"\n"*/ STR(708);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1269));
	...
	RealType language_load_legacy(VarType fn, IntType map)
	{
		...
		while (!file_text_eof(f))
		{
			...
			commapos = string_pos(/*":"*/ STR(786), line);
	...
	{
		log({ /*"Loading legacy file"*/ STR(1273) });
		IntType map = json_load({ legacy_file });
		if (!ds_map_valid(map))
		{
			log({ /*"Error loading legacy.midata"*/ STR(1274) });
			return false;
		}
		global::legacy_model_id_05_map = DsMap(map).Value(/*"legacy_model_id_05"*/ STR(1275));
		global::legacy_model_id_06_map = DsMap(map).Value(/*"legacy_model_id_06"*/ STR(1276));
		global::legacy_model_id_100_demo_map = DsMap(map).Value(/*"legacy_model_id_100_demo"*/ STR(1277));
		ds_map_merge(global::legacy_model_id_05_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1278)));
		ds_map_merge(global::legacy_model_id_06_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1278)));
		ds_map_merge(global::legacy_model_id_100_demo_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1278)));
		global::legacy_model_part_map = DsMap(map).Value(/*"legacy_model_part"*/ STR(1279));
		global::legacy_model_name_map = DsMap(map).Value(/*"legacy_model_name"*/ STR(1280));
		global::legacy_block_set[IntType(255)] = false;
		global::legacy_block_id = DsMap(map).Value(/*"legacy_block_id"*/ STR(1281));
		global::legacy_block_texture_name_map = DsMap(map).Value(/*"legacy_block_texture_name"*/ STR(1282));
		global::legacy_block_05_texture_list = DsMap(map).Value(/*"legacy_block_05_textures"*/ STR(1283));
		global::legacy_block_07_demo_texture_list = DsMap(map).Value(/*"legacy_block_07_demo_textures"*/ STR(1284));
		global::legacy_block_100_texture_list = DsMap(map).Value(/*"legacy_block_100_textures"*/ STR(1285));
		...
		global::legacy_item_texture_name_map = DsMap(map).Value(/*"legacy_item_texture_name"*/ STR(1286));
		global::legacy_biomes_map = DsMap(map).Value(/*"legacy_biomes"*/ STR(1287));
		global::legacy_biomes_ids_map = ds_int_map_create();
		VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1288))));
		while (!is_undefined(key))
		{
			DsMap(global::legacy_biomes_ids_map)[string_get_real(key)] = ds_map_find_value((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1288))), key);
			key = ds_map_find_next((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1288))), key);
		}
		
		global::biomes_ids_map = ds_int_map_create();
		key = ds_map_find_first((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1289))));
		while (!is_undefined(key))
		{
			DsMap(global::biomes_ids_map)[string_get_real(key)] = ds_map_find_value((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1289))), key);
			key = ds_map_find_next((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1289))), key);
		}
		
		global::legacy_model_names_map = DsMap(map).Value(/*"legacy_model_names"*/ STR(1290));
		global::legacy_model_states_map = DsMap(map).Value(/*"legacy_model_states"*/ STR(1291));
		global::legacy_model_state_values_map = DsMap(map).Value(/*"legacy_model_state_values"*/ STR(1292));
		global::legacy_block_names_map = DsMap(map).Value(/*"legacy_block_names"*/ STR(1293));
		global::legacy_particles_map = DsMap(map).Value(/*"legacy_particles"*/ STR(1294));
		return true;
	}
	...
	BoolType lib_startup()
	{
		...
		if (is_cpp())
			return true;
		log({ /*"External library init"*/ STR(1295) });
		StringType libpath, pathfile, pathmovie, pathwindow;
		libpath = /*"Data/Libraries/"*/ STR(1296);
		pathfile = libpath + /*"file.dll"*/ STR(1297);
		pathmovie = libpath + /*"movie.dll"*/ STR(1298);
		pathwindow = libpath + /*"window.dll"*/ STR(1299);
		...
		log({ /*"External library"*/ STR(1300), pathwindow });
		global::lib_window_maximize = external_define({ pathwindow, /*"window_maximize"*/ STR(1301), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_window_set_focus = external_define({ pathwindow, /*"window_set_focus"*/ STR(1302), dll_cdecl, ty_real, IntType(1), ty_string });
		log({ /*"External library"*/ STR(1300), pathfile });
		global::file_copy_temp = true;
		global::lib_open_url = external_define({ pathfile, /*"open_url"*/ STR(1303), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_execute = external_define({ pathfile, /*"execute"*/ STR(1129), dll_cdecl, ty_real, IntType(3), ty_string, ty_string, ty_real });
		global::lib_unzip = external_define({ pathfile, /*"unzip"*/ STR(1304), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_gzunzip = external_define({ pathfile, /*"gzunzip"*/ STR(1305), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_rename = external_define({ pathfile, /*"file_rename"*/ STR(1306), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_copy = external_define({ pathfile, /*"file_copy"*/ STR(1307), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_delete = external_define({ pathfile, /*"file_delete"*/ STR(1308), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_file_exists = external_define({ pathfile, /*"file_exists"*/ STR(1309), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_create = external_define({ pathfile, /*"directory_create"*/ STR(1310), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_delete = external_define({ pathfile, /*"directory_delete"*/ STR(1311), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_exists = external_define({ pathfile, /*"directory_exists"*/ STR(1312), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_json_file_convert_unicode = external_define({ pathfile, /*"json_file_convert_unicode"*/ STR(1313), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		log({ /*"External library"*/ STR(1300), pathmovie });
		global::lib_movie_init = external_define({ pathmovie, /*"movie_init"*/ STR(1314), dll_cdecl, ty_real, IntType(0) });
		global::lib_movie_set = external_define({ pathmovie, /*"movie_set"*/ STR(1315), dll_cdecl, ty_real, IntType(5), ty_real, ty_real, ty_real, ty_real, ty_real });
		global::lib_movie_start = external_define({ pathmovie, /*"movie_start"*/ STR(1316), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_movie_audio_file_decode = external_define({ pathmovie, /*"movie_audio_file_decode"*/ STR(1317), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_movie_audio_file_add = external_define({ pathmovie, /*"movie_audio_file_add"*/ STR(1318), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_movie_audio_sound_add = external_define({ pathmovie, /*"movie_audio_sound_add"*/ STR(1319), dll_cdecl, ty_real, IntType(6), ty_real, ty_real, ty_real, ty_real, ty_real, ty_real });
		global::lib_movie_frame = external_define({ pathmovie, /*"movie_frame"*/ STR(1320), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_movie_done = external_define({ pathmovie, /*"movie_done"*/ STR(1321), dll_cdecl, ty_real, IntType(0) });
		log({ /*"External library"*/ STR(1300), /*"movie init"*/ STR(1322) });
		lib_movie_init();
