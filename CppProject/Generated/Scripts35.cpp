...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1742), /*"b"*/ STR(1743), /*"c"*/ STR(1744), /*"d"*/ STR(1745), /*"e"*/ STR(1218), /*"f"*/ STR(1219), /*"g"*/ STR(1746), /*"h"*/ STR(1747), /*"i"*/ STR(1748), /*"j"*/ STR(1749), /*"k"*/ STR(1750), /*"l"*/ STR(1751), /*"m"*/ STR(1752), /*"n"*/ STR(1220), /*"o"*/ STR(1753), /*"p"*/ STR(1754), /*"q"*/ STR(1755), /*"r"*/ STR(1756), /*"s"*/ STR(1757), /*"t"*/ STR(1221), /*"u"*/ STR(1222), /*"v"*/ STR(1758), /*"w"*/ STR(1759), /*"x"*/ STR(835), /*"y"*/ STR(818), /*"z"*/ STR(836) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->name = DsMap(tempmap).Value(/*"name"*/ STR(804));
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(549));
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1760))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1760));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real(DsMap(tempmap).Value(/*"size"*/ STR(991)), IntType(8));
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real(DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1761)), false);
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1113) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1113) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		global::load_assets_stage = /*"unzip"*/ STR(1280);
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1762);
		...
		{
			log({ /*"Could not load "*/ STR(1763) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1764), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1765)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1765) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1766));
			...
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1767));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1768), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1769), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1770), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1771), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1772), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1773));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1113) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(16);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1774) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1771), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1709));
			...
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1713));
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1775) + name + /*"]"*/ STR(1216) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1776) + name + /*"]"*/ STR(1216) : /*""*/ STR(0));
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1579) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(801) ? /*"entity/banner_base"*/ STR(1777) : /*"entity/shield_base"*/ STR(1778));
		patterndir = (type == /*"banner"*/ STR(801) ? /*"entity/banner/"*/ STR(1779) : /*"entity/shield/"*/ STR(1780));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1781), fn });
		show_message(/*"The file "*/ STR(1782) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1783));
		return false;
	}
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		{
			log({ /*"Could not find model file"*/ STR(824), fname });
			return null_;
		}
		IntType map = json_load({ fname });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse model file"*/ STR(825), fname });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(804))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(805) });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(549))))
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1784) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1785))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1786) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1787))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1788) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(804));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(532) + self->name))
				log({ /*"model/"*/ STR(1789) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string(DsMap(map).Value(/*"description"*/ STR(1790)), /*""*/ STR(0));
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string(DsMap(map).Value(/*"texture_material"*/ STR(1791)), /*""*/ STR(0));
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string(DsMap(map).Value(/*"texture_normal"*/ STR(1792)), /*""*/ STR(0));
			...
			self->texture_size = value_get_point2D(DsMap(map).Value(/*"texture_size"*/ STR(1785)));
			...
			self->player_skin = value_get_real(DsMap(map).Value(/*"player_skin"*/ STR(1793)), false);
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(1794)), vec3(IntType(1), IntType(1), IntType(1)));
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1787));
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1795))) && !(DsMap(map).Value(/*"visible"*/ STR(1795)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(804))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(805) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1796) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			if (ObjType(obj_model_part, DsList(ObjType(obj_model_file, root)->file_part_list).Value(i))->name == DsMap(map).Value(/*"name"*/ STR(804)))
			{
				log({ /*"Duplicate part name found"*/ STR(1797), DsMap(map).Value(/*"name"*/ STR(804)) });
				return null_;
			}
		}
		...
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(804));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(765) + self->name))
				log({ /*"model/part/"*/ STR(1798) + self->name + dev_mode_name_translation_message });
			self->depth = value_get_real(DsMap(map).Value(/*"depth"*/ STR(628)), IntType(0));
			IntType pos;
			...
			self->color_inherit = value_get_real(DsMap(map).Value(/*"color_inherit"*/ STR(1799)), true);
			self->color_blend = value_get_color(DsMap(map).Value(/*"color_blend"*/ STR(1800)), c_white);
			self->color_alpha = value_get_real(DsMap(map).Value(/*"color_alpha"*/ STR(1801)), IntType(1));
			if (is_real(/*"color_emissive"*/ STR(1802)))
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_emissive"*/ STR(1802)), IntType(0));
			else
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_brightness"*/ STR(1803)), IntType(0));
			
			self->color_mix = value_get_color(DsMap(map).Value(/*"color_mix"*/ STR(1804)), c_black);
			self->color_mix_percent = value_get_real(DsMap(map).Value(/*"color_mix_percent"*/ STR(1805)), IntType(0));
			self->part_mixing_shapes = false;
			...
			self->rotation = value_get_point3D(DsMap(map).Value(/*"rotation"*/ STR(831)), vec3(IntType(0), IntType(0), IntType(0)));
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(1794)), vec3(IntType(1), IntType(1), IntType(1)));
			self->scale = vec3_mul(self->scale, idVar(self.otherId, scale));
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1806)), false);
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idVar(self.otherId, locked) > 0)
				self->locked = true;
			else
				self->locked = value_get_real(DsMap(map).Value(/*"locked"*/ STR(926)), false);
			
			self->lock_bend = true;
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idInt(self.otherId, bend_part) != null_)
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1807))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1807))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1807));
				if (self->lock_bend > 0)
				{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1808)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1809))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1809));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1810)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1811))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1812) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1811));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1813)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(991)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(888))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1814) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(888))).id)
				{
					case 192: // right
					{
				...
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(817))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(817))).id)
					{
						case 835: // x
						{
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"axis"*/ STR(817))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(817)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(817))), i)).id)
							{
								case 835: // x
								{
									self->bend_axis.Real(X_) = true;
				...
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(915))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(915))).id)
					{
						case 1823: // forward
						{
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(915))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(915)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(915))), i)).id)
							{
								case 1823: // forward
								{
				...
				if (is_real(DsMap(bendmap).Value(/*"angle"*/ STR(833))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_default_angle = vec3(DsMap(bendmap).Value(/*"angle"*/ STR(833)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"angle"*/ STR(833))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(833)))); i++)
							self->bend_default_angle.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(833))), i);
					}
