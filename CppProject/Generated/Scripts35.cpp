...
namespace CppProject
{
	IntType list_init_context_menu(ScopeAny self, VarType name)
	{
		...
		switch (StringType(name).id)
		{
			...
			{
				list_item_add(text_get({ /*"contextmenutextboxcut"*/ STR(1560) }), null_, text_control_name(keybind_new(/*"X"*/ STR(283), true)), null_, icons_CUT, null_, ID_action_textbox_cut, true);
				ObjType(obj_list_item, global::list_item_last)->disabled = (sVar(textbox_select_startpos) == sVar(textbox_select_endpos));
				list_item_add(text_get({ /*"contextmenutextboxcopy"*/ STR(1561) }), null_, text_control_name(keybind_new(/*"C"*/ STR(281), true)), null_, icons_COPY, null_, ID_action_textbox_copy, false);
				ObjType(obj_list_item, global::list_item_last)->disabled = (sVar(textbox_select_startpos) == sVar(textbox_select_endpos));
				list_item_add(text_get({ /*"contextmenutextboxpaste"*/ STR(1562) }), null_, text_control_name(keybind_new(/*"V"*/ STR(285), true)), null_, icons_PASTE, null_, ID_action_textbox_paste, false);
				ObjType(obj_list_item, global::list_item_last)->disabled = (clipboard_get_text() == /*""*/ STR(0) || !clipboard_has_text());
				list_item_add(text_get({ /*"contextmenutextboxselectall"*/ STR(1563) }), null_, text_control_name(keybind_new(/*"A"*/ STR(272), true)), null_, icons_SELECT_ALL, null_, ID_action_textbox_select_all, false);
				break;
			}
