...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1738), /*"b"*/ STR(1739), /*"c"*/ STR(1740), /*"d"*/ STR(1741), /*"e"*/ STR(1217), /*"f"*/ STR(1218), /*"g"*/ STR(1742), /*"h"*/ STR(1743), /*"i"*/ STR(1744), /*"j"*/ STR(1745), /*"k"*/ STR(1746), /*"l"*/ STR(1747), /*"m"*/ STR(1748), /*"n"*/ STR(1219), /*"o"*/ STR(1749), /*"p"*/ STR(1750), /*"q"*/ STR(1751), /*"r"*/ STR(1752), /*"s"*/ STR(1753), /*"t"*/ STR(1220), /*"u"*/ STR(1221), /*"v"*/ STR(1754), /*"w"*/ STR(1755), /*"x"*/ STR(832), /*"y"*/ STR(815), /*"z"*/ STR(833) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->name = DsMap(tempmap).Value(/*"name"*/ STR(801));
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(546));
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1756))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1756));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real({ DsMap(tempmap).Value(/*"size"*/ STR(990)), IntType(8) });
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real({ DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1757)), false });
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1112) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1112) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		global::load_assets_stage = /*"unzip"*/ STR(1279);
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1758);
		...
		{
			log({ /*"Could not load "*/ STR(1759) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1760), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1761)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1761) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1762));
			...
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1763));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1764), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1765), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1766), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1767), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1768), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1769));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1112) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(16);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1770) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1767), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1705));
			if (is_undefined(characterslist))
			{
				log({ /*"No character list found"*/ STR(1706) });
				break;
			}
			for (IntType i = IntType(0); i < ds_list_size((IntType)(characterslist)); i++)
			{
				...
				if (!(model > 0))
				{
					log({ /*"Could not load model"*/ STR(1707) });
					continue;
				}
				DsMap(self->model_name_map)[ObjType(obj_model, model)->name] = model;
				ds_list_add({ self->char_list, model });
			}
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1709));
			if (is_undefined(specialblockslist))
			{
				log({ /*"No special block list found"*/ STR(1710) });
				break;
			}
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1771) + name + /*"]"*/ STR(1215) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1772) + name + /*"]"*/ STR(1215) : /*""*/ STR(0));
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(798) ? /*"entity/banner_base"*/ STR(1773) : /*"entity/shield_base"*/ STR(1774));
		patterndir = (type == /*"banner"*/ STR(798) ? /*"entity/banner/"*/ STR(1775) : /*"entity/shield/"*/ STR(1776));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1777), fn });
		show_message(/*"The file "*/ STR(1778) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1779));
		return false;
	}
	...
	IntType model_file_load(VarArgs argument)
	{
		...
		{
			log({ /*"Could not find model file"*/ STR(821), fname });
			return null_;
		}
		IntType map = json_load({ fname });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse model file"*/ STR(822), fname });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(801))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(802) });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(546))))
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1780) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1781))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1782) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1783))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1784) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(801));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(529) + self->name))
				log({ /*"model/"*/ STR(1785) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string({ DsMap(map).Value(/*"description"*/ STR(1786)), /*""*/ STR(0) });
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string({ DsMap(map).Value(/*"texture_material"*/ STR(1787)), /*""*/ STR(0) });
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string({ DsMap(map).Value(/*"texture_normal"*/ STR(1788)), /*""*/ STR(0) });
			...
			self->texture_size = value_get_point2D({ DsMap(map).Value(/*"texture_size"*/ STR(1781)) });
			...
			self->player_skin = value_get_real({ DsMap(map).Value(/*"player_skin"*/ STR(1789)), false });
			self->scale = value_get_point3D({ DsMap(map).Value(/*"scale"*/ STR(1790)), vec3(IntType(1), IntType(1), IntType(1)) });
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1783));
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1791))) && !(DsMap(map).Value(/*"visible"*/ STR(1791)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(801))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(802) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1792) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			if (ObjType(obj_model_part, DsList(ObjType(obj_model_file, root)->file_part_list).Value(i))->name == DsMap(map).Value(/*"name"*/ STR(801)))
			{
				log({ /*"Duplicate part name found"*/ STR(1793), DsMap(map).Value(/*"name"*/ STR(801)) });
				return null_;
			}
		}
		...
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(801));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(762) + self->name))
				log({ /*"model/part/"*/ STR(1794) + self->name + dev_mode_name_translation_message });
			self->depth = value_get_real({ DsMap(map).Value(/*"depth"*/ STR(625)), IntType(0) });
			IntType pos;
			...
			self->color_inherit = value_get_real({ DsMap(map).Value(/*"color_inherit"*/ STR(1795)), true });
			self->color_blend = value_get_color({ DsMap(map).Value(/*"color_blend"*/ STR(1796)), c_white });
			self->color_alpha = value_get_real({ DsMap(map).Value(/*"color_alpha"*/ STR(1797)), IntType(1) });
			if (is_real(/*"color_emissive"*/ STR(1798)))
				self->color_emissive = value_get_real({ DsMap(map).Value(/*"color_emissive"*/ STR(1798)), IntType(0) });
			else
				self->color_emissive = value_get_real({ DsMap(map).Value(/*"color_brightness"*/ STR(1799)), IntType(0) });
			
			self->color_mix = value_get_color({ DsMap(map).Value(/*"color_mix"*/ STR(1800)), c_black });
			self->color_mix_percent = value_get_real({ DsMap(map).Value(/*"color_mix_percent"*/ STR(1801)), IntType(0) });
			self->part_mixing_shapes = false;
			...
			self->rotation = value_get_point3D({ DsMap(map).Value(/*"rotation"*/ STR(828)), vec3(IntType(0), IntType(0), IntType(0)) });
			self->scale = value_get_point3D({ DsMap(map).Value(/*"scale"*/ STR(1790)), vec3(IntType(1), IntType(1), IntType(1)) });
			self->scale = vec3_mul(self->scale, idVar(self.otherId, scale));
			self->show_position = value_get_real({ DsMap(map).Value(/*"show_position"*/ STR(1802)), false });
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idVar(self.otherId, locked) > 0)
				self->locked = true;
			else
				self->locked = value_get_real({ DsMap(map).Value(/*"locked"*/ STR(921)), false });
			
			self->lock_bend = true;
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idInt(self.otherId, bend_part) != null_)
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1803))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1803))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1803));
				if (self->lock_bend > 0)
				{
			...
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1804))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1804));
				self->bend_inherit = value_get_real({ DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1805)), false });
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1806))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1807) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1806));
				self->bend_end_offset = value_get_real({ DsMap(bendmap).Value(/*"end_offset"*/ STR(1808)), IntType(0) });
				self->bend_size = value_get_real({ DsMap(bendmap).Value(/*"size"*/ STR(990)), null_ });
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(883))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1809) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(883))).id)
				{
					case 192: // right
					{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1813) });
						return null_;
				}
				
				self->bend_axis = ArrType::From({ false, false, false });
				ArrType axis = ArrType();
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(814))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(814))).id)
					{
						case 832: // x
						{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1814) });
							return null_;
					}
					
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"axis"*/ STR(814))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(814)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(814))), i)).id)
							{
								case 832: // x
								{
									self->bend_axis.Real(X_) = true;
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1815) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1816))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1816));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1816))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1816)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1816))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1817))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1817));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1817))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1817)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1817))), i);
					}
				
				self->bend_direction = ArrType::From({ IntType(0), IntType(0), IntType(0) });
				self->bend_direction_legacy = false;
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(910))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(910))).id)
					{
						case 1818: // forward
						{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1821) });
							return null_;
					}
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(910))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(910)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(910))), i)).id)
							{
								case 1818: // forward
								{
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1822))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1822)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1822)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1822))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1822)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1822))), i);
					}
					else
				...
				if (is_real(DsMap(bendmap).Value(/*"angle"*/ STR(830))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_default_angle = vec3(DsMap(bendmap).Value(/*"angle"*/ STR(830)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"angle"*/ STR(830))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(830)))); i++)
							self->bend_default_angle.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(830))), i);
					}
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1823));
