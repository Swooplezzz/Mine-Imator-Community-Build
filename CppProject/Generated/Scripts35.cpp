...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1757), /*"b"*/ STR(1758), /*"c"*/ STR(1759), /*"d"*/ STR(1760), /*"e"*/ STR(1232), /*"f"*/ STR(1233), /*"g"*/ STR(1761), /*"h"*/ STR(1762), /*"i"*/ STR(1763), /*"j"*/ STR(1764), /*"k"*/ STR(1765), /*"l"*/ STR(1766), /*"m"*/ STR(1767), /*"n"*/ STR(1234), /*"o"*/ STR(1768), /*"p"*/ STR(1769), /*"q"*/ STR(1770), /*"r"*/ STR(1771), /*"s"*/ STR(1772), /*"t"*/ STR(1235), /*"u"*/ STR(1236), /*"v"*/ STR(1773), /*"w"*/ STR(1774), /*"x"*/ STR(845), /*"y"*/ STR(828), /*"z"*/ STR(846) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->name = DsMap(tempmap).Value(/*"name"*/ STR(814));
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(559));
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(811) ? /*"entity/banner_base"*/ STR(1792) : /*"entity/shield_base"*/ STR(1793));
		patterndir = (type == /*"banner"*/ STR(811) ? /*"entity/banner/"*/ STR(1794) : /*"entity/shield/"*/ STR(1795));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		{
			log({ /*"Could not find model file"*/ STR(834), fname });
			return null_;
		}
		IntType map = json_load({ fname });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse model file"*/ STR(835), fname });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(814))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(815) });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(559))))
		{
		...
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(814));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(542) + self->name))
			...
			self->texture_name = DsMap(map).Value(/*"texture"*/ STR(559));
	...
	VarType model_file_load_part(ScopeAny self, VarType map, IntType root, IntType res, IntType model)
	{
		...
		{
			if (ObjType(obj_model_part, DsList(ObjType(obj_model_file, root)->file_part_list).Value(i))->name == DsMap(map).Value(/*"name"*/ STR(814)))
			{
				log({ /*"Duplicate part name found"*/ STR(1812), DsMap(map).Value(/*"name"*/ STR(814)) });
				return null_;
			}
		}
		...
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(814));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(775) + self->name))
				log({ /*"model/part/"*/ STR(1813) + self->name + dev_mode_name_translation_message });
			self->depth = value_get_real(DsMap(map).Value(/*"depth"*/ STR(638)), IntType(0));
			IntType pos;
			...
			self->rotation = value_get_point3D(DsMap(map).Value(/*"rotation"*/ STR(841)), vec3(IntType(0), IntType(0), IntType(0)));
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(1809)), vec3(IntType(1), IntType(1), IntType(1)));
			...
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idVar(self.otherId, locked) > 0)
				...
				self->locked = value_get_real(DsMap(map).Value(/*"locked"*/ STR(936)), false);
			...
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1824))))
			{
				...
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(898))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1829) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(898))).id)
				{
					case 193: // right
					{
				...
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(827))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(827))).id)
					{
						case 845: // x
						{
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"axis"*/ STR(827))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(827)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(827))), i)).id)
							{
								case 845: // x
								{
									self->bend_axis.Real(X_) = true;
				...
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(925))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(925))).id)
					{
						case 1838: // forward
						{
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(925))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(925)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(925))), i)).id)
							{
								case 1838: // forward
								{
				...
				if (is_real(DsMap(bendmap).Value(/*"angle"*/ STR(843))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_default_angle = vec3(DsMap(bendmap).Value(/*"angle"*/ STR(843)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"angle"*/ STR(843))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(843)))); i++)
							self->bend_default_angle.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(843))), i);
					}
