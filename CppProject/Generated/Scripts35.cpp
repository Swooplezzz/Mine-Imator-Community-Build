...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1757), /*"b"*/ STR(1758), /*"c"*/ STR(1759), /*"d"*/ STR(1760), /*"e"*/ STR(1232), /*"f"*/ STR(1233), /*"g"*/ STR(1761), /*"h"*/ STR(1762), /*"i"*/ STR(1763), /*"j"*/ STR(1764), /*"k"*/ STR(1765), /*"l"*/ STR(1766), /*"m"*/ STR(1767), /*"n"*/ STR(1234), /*"o"*/ STR(1768), /*"p"*/ STR(1769), /*"q"*/ STR(1770), /*"r"*/ STR(1771), /*"s"*/ STR(1772), /*"t"*/ STR(1235), /*"u"*/ STR(1236), /*"v"*/ STR(1773), /*"w"*/ STR(1774), /*"x"*/ STR(846), /*"y"*/ STR(829), /*"z"*/ STR(847) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(560));
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1775))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1775));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real(DsMap(tempmap).Value(/*"size"*/ STR(1005)), IntType(8));
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real(DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1776)), false);
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1127) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1127) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		sStr(window_state) = /*"load_assets"*/ STR(80);
		global::load_assets_stage = /*"unzip"*/ STR(1294);
		...
		{
			self->save_id = /*"default"*/ STR(99);
			self->type = e_res_type_PACK;
			self->display_name = /*"Minecraft"*/ STR(1777);
		...
		{
			log({ /*"Could not load "*/ STR(1778) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1779), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1780)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1780) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1781));
			splash = DsList(splashlist).Value(irandom((IntType)(ds_list_size((IntType)(splashlist)) - IntType(1))));
			splashfile = splash_directory + DsMap(splash).Value(/*"file"*/ STR(116));
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1782));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1783), version });
		global::load_assets_file = minecraft_directory + version + /*".midata"*/ STR(97);
		global::load_assets_zip_file = minecraft_directory + version + /*".zip"*/ STR(5);
		if (!file_exists_lib(global::load_assets_file))
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1784), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1785), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1786), global::load_assets_file });
			ds_map_destroy(global::load_assets_type_map);
			return false;
		}
		VarType format = DsMap(global::load_assets_map).Value(/*"format"*/ STR(89));
		if (!is_real(format))
			format = e_minecraft_assets_FORMAT_110_PRE_1;
		if (format != minecraft_assets_format)
		{
			log({ /*"Unsupported archive, format"*/ STR(1787), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1788));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1127) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(16);
	...
	{
		StringType fname = minecraft_directory + minecraft_version + /*".midata"*/ STR(97);
		log({ /*"Reloading models"*/ STR(1789) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1786), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1724));
			if (is_undefined(characterslist))
			{
				log({ /*"No character list found"*/ STR(1725) });
				break;
			}
			for (IntType i = IntType(0); i < ds_list_size((IntType)(characterslist)); i++)
			{
				...
				if (!(model > 0))
				{
					log({ /*"Could not load model"*/ STR(1726) });
					continue;
				}
				DsMap(self->model_name_map)[ObjType(obj_model, model)->name] = model;
				ds_list_add({ self->char_list, model });
			}
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1728));
			if (is_undefined(specialblockslist))
			{
				log({ /*"No special block list found"*/ STR(1729) });
				break;
			}
	...
	BoolType minecraft_assets_startup(ScopeAny self)
	{
		...
		if (sVar(setting_minecraft_assets_new_version) != /*""*/ STR(0))
		{
			...
			sStr(window_state) = /*"new_assets"*/ STR(79);
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1790) + name + /*"]"*/ STR(1230) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1791) + name + /*"]"*/ STR(1230) : /*""*/ STR(0));
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1594) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(812) ? /*"entity/banner_base"*/ STR(1792) : /*"entity/shield_base"*/ STR(1793));
		patterndir = (type == /*"banner"*/ STR(812) ? /*"entity/banner/"*/ STR(1794) : /*"entity/shield/"*/ STR(1795));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1796), fn });
		show_message(/*"The file "*/ STR(1797) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1798));
		return false;
	}
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(560))))
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1799) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1800))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1801) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1802))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1803) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(815));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(543) + self->name))
				log({ /*"model/"*/ STR(1804) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string(DsMap(map).Value(/*"description"*/ STR(1805)), /*""*/ STR(0));
			self->texture_name = DsMap(map).Value(/*"texture"*/ STR(560));
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string(DsMap(map).Value(/*"texture_material"*/ STR(1806)), /*""*/ STR(0));
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string(DsMap(map).Value(/*"texture_normal"*/ STR(1807)), /*""*/ STR(0));
			...
			self->texture_size = value_get_point2D(DsMap(map).Value(/*"texture_size"*/ STR(1800)));
			...
			self->player_skin = value_get_real(DsMap(map).Value(/*"player_skin"*/ STR(1808)), false);
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(1809)), vec3(IntType(1), IntType(1), IntType(1)));
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1802));
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1810))) && !(DsMap(map).Value(/*"visible"*/ STR(1810)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(815))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(816) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1811) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			...
			{
				log({ /*"Duplicate part name found"*/ STR(1812), DsMap(map).Value(/*"name"*/ STR(815)) });
				return null_;
			}
		}
		withOne (obj_model_part, (new obj_model_part)->id, self->id)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(776) + self->name))
				log({ /*"model/part/"*/ STR(1813) + self->name + dev_mode_name_translation_message });
			...
			self->color_inherit = value_get_real(DsMap(map).Value(/*"color_inherit"*/ STR(1814)), true);
			self->color_blend = value_get_color(DsMap(map).Value(/*"color_blend"*/ STR(1815)), c_white);
			self->color_alpha = value_get_real(DsMap(map).Value(/*"color_alpha"*/ STR(1816)), IntType(1));
			if (is_real(/*"color_emissive"*/ STR(1817)))
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_emissive"*/ STR(1817)), IntType(0));
			else
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_brightness"*/ STR(1818)), IntType(0));
			
			self->color_mix = value_get_color(DsMap(map).Value(/*"color_mix"*/ STR(1819)), c_black);
			self->color_mix_percent = value_get_real(DsMap(map).Value(/*"color_mix_percent"*/ STR(1820)), IntType(0));
			self->part_mixing_shapes = false;
			...
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1821)), false);
			...
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1822))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1822))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1822));
				if (self->lock_bend > 0)
				{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1823)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1824))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1824));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1825)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1826))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1827) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1826));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1828)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(1005)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(899))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1829) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(899))).id)
				{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1833) });
						return null_;
				}
				...
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(828))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(828))).id)
					{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1834) });
							return null_;
					}
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1835) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1836))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1836));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1836))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1836)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1836))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1837))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1837));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1837))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1837)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1837))), i);
					}
				...
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(926))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(926))).id)
					{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1841) });
							return null_;
					}
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1842))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1842)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1842)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1842))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1842)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1842))), i);
					}
					else
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1843));
