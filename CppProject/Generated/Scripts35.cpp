...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1750), /*"b"*/ STR(1751), /*"c"*/ STR(1752), /*"d"*/ STR(1753), /*"e"*/ STR(1226), /*"f"*/ STR(1227), /*"g"*/ STR(1754), /*"h"*/ STR(1755), /*"i"*/ STR(1756), /*"j"*/ STR(1757), /*"k"*/ STR(1758), /*"l"*/ STR(1759), /*"m"*/ STR(1760), /*"n"*/ STR(1228), /*"o"*/ STR(1761), /*"p"*/ STR(1762), /*"q"*/ STR(1763), /*"r"*/ STR(1764), /*"s"*/ STR(1765), /*"t"*/ STR(1229), /*"u"*/ STR(1230), /*"v"*/ STR(1766), /*"w"*/ STR(1767), /*"x"*/ STR(839), /*"y"*/ STR(822), /*"z"*/ STR(840) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->name = DsMap(tempmap).Value(/*"name"*/ STR(808));
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(553));
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1768))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1768));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real(DsMap(tempmap).Value(/*"size"*/ STR(999)), IntType(8));
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real(DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1769)), false);
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1121) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1121) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		global::load_assets_stage = /*"unzip"*/ STR(1288);
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1770);
		...
		{
			log({ /*"Could not load "*/ STR(1771) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1772), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1773)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1773) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1774));
			...
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1775));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1776), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1777), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1778), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1779), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1780), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1781));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1121) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(16);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1782) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1779), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1717));
			...
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1721));
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1783) + name + /*"]"*/ STR(1224) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1784) + name + /*"]"*/ STR(1224) : /*""*/ STR(0));
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1587) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(805) ? /*"entity/banner_base"*/ STR(1785) : /*"entity/shield_base"*/ STR(1786));
		patterndir = (type == /*"banner"*/ STR(805) ? /*"entity/banner/"*/ STR(1787) : /*"entity/shield/"*/ STR(1788));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1789), fn });
		show_message(/*"The file "*/ STR(1790) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1791));
		return false;
	}
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		{
			log({ /*"Could not find model file"*/ STR(828), fname });
			return null_;
		}
		IntType map = json_load({ fname });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse model file"*/ STR(829), fname });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(808))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(809) });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(553))))
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1792) });
			return null_;
		}
		...
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1795))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1796) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(808));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(536) + self->name))
			...
			self->player_skin = value_get_real(DsMap(map).Value(/*"player_skin"*/ STR(1801)), false);
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1795));
	...
	VarType model_file_load_part(ScopeAny self, VarType map, IntType root, IntType res, IntType model)
	{
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1804) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			if (ObjType(obj_model_part, DsList(ObjType(obj_model_file, root)->file_part_list).Value(i))->name == DsMap(map).Value(/*"name"*/ STR(808)))
			{
				log({ /*"Duplicate part name found"*/ STR(1805), DsMap(map).Value(/*"name"*/ STR(808)) });
				return null_;
			}
		}
		...
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(808));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(769) + self->name))
				log({ /*"model/part/"*/ STR(1806) + self->name + dev_mode_name_translation_message });
			self->depth = value_get_real(DsMap(map).Value(/*"depth"*/ STR(632)), IntType(0));
			IntType pos;
			...
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1814)), false);
			...
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1815))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1815))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1815));
				if (self->lock_bend > 0)
				{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1816)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1817))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1817));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1818)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1819))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1820) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1819));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1821)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(999)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(892))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1822) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(892))).id)
				{
					case 192: // right
					{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1826) });
						return null_;
				}
				
				self->bend_axis = ArrType::From({ false, false, false });
				ArrType axis = ArrType();
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(821))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(821))).id)
					{
						case 839: // x
						{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1827) });
							return null_;
					}
					
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"axis"*/ STR(821))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(821)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(821))), i)).id)
							{
								case 839: // x
								{
									self->bend_axis.Real(X_) = true;
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1828) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1829))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1829));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1829))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1829)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1829))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1830))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1830));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1830))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1830)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1830))), i);
					}
				
				self->bend_direction = ArrType::From({ IntType(0), IntType(0), IntType(0) });
				self->bend_direction_legacy = false;
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(919))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(919))).id)
					{
						case 1831: // forward
						{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1834) });
							return null_;
					}
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(919))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(919)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(919))), i)).id)
							{
								case 1831: // forward
								{
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1835))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1835)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1835)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1835))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1835)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1835))), i);
					}
					else
				...
				if (is_real(DsMap(bendmap).Value(/*"angle"*/ STR(837))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_default_angle = vec3(DsMap(bendmap).Value(/*"angle"*/ STR(837)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"angle"*/ STR(837))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(837)))); i++)
							self->bend_default_angle.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(837))), i);
					}
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1836));
