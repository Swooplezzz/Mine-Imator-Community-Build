...
namespace CppProject
{
	...
	RealType particles_load(ScopeAny self, VarType fn, VarType temp)
	{
		...
		if (!legacy)
		{
			...
			{
				temp_particles_type_clear(self);
				project_load_particles(self, DsMap(rootmap).Value(/*"particles"*/ STR(537)));
				temp_particles_restart(self);
			}
	...
	RealType particles_save(ScopeAny self)
	{
		...
		if (idVar(global::temp_edit, pc_spawn_region_type) == /*"path"*/ STR(547) && idVar(global::temp_edit, pc_spawn_region_path) != null_)
		{
			...
			for (RealType s = sReal(spawn_laststep); s < sReal(spawn_currentstep); s++)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(sInt(particle_list)); i++)
				{
					...
					if (!sBool(is_timeline) || !(idVar(ObjType(obj_particle, pt)->creator, value).Value(e_value_FREEZE) > 0))
					{
						...
						if (idVar(temp, pc_bounding_box_type) != /*"none"*/ STR(903) && idVar(ObjType(obj_particle, pt)->type, bounding_box) > 0)
						{
							...
							else
							{
								...
								if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1350))
								{
									if (!(idVar(temp, pc_spawn_region_use) > 0))
										continue;
									if (idVar(temp, pc_spawn_region_type) == /*"sphere"*/ STR(540))
									{
										...
										if (idVar(temp, pc_spawn_region_type) == /*"cube"*/ STR(3))
										{
											...
											if (idVar(temp, pc_spawn_region_type) == /*"box"*/ STR(1344))
											{
												...
												if (idVar(temp, pc_spawn_region_type) == /*"path"*/ STR(547) && idVar(temp, pc_spawn_region_path) != null_)
												{
