...
namespace CppProject
{
	...
	RealType particles_load(ScopeAny self, VarType fn, VarType temp)
	{
		...
		if (filename_ext(fn) == /*".zip"*/ STR(5))
		{
			...
			unzip(fn);
			fn = file_find_single(unzip_directory, /*".miparticles;*.particles"*/ STR(1924));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*".miparticles;*.particles"*/ STR(1924));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenparticleszip"*/ STR(1925));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		IntType rootmap;
		BoolType legacy;
		if (string_contains(filename_ext(fn), /*".miparticles"*/ STR(700)))
		{
			log({ /*"Opening particles"*/ STR(1926), fn });
		...
		{
			log({ /*"Opening legacy particles"*/ STR(1927), fn });
	...
	RealType particles_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miparticles"*/ STR(700));
		log({ /*"Saving particles"*/ STR(1929), fn });
		...
		if (idVar(global::temp_edit, pc_spawn_region_type) == /*"path"*/ STR(535) && idVar(global::temp_edit, pc_spawn_region_path) != null_)
		{
			...
			for (RealType s = sReal(spawn_laststep); s < sReal(spawn_currentstep); s++)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(sInt(particle_list)); i++)
				{
					...
					if (!sBool(is_timeline) || !(idVar(ObjType(obj_particle, pt)->creator, value).Value(e_value_FREEZE) > 0))
					{
						...
						if (idVar(temp, pc_bounding_box_type) != /*"none"*/ STR(891) && idVar(ObjType(obj_particle, pt)->type, bounding_box) > 0)
						{
								...
								if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1334))
								{
									...
									if (idVar(temp, pc_spawn_region_type) == /*"sphere"*/ STR(528))
									{
										...
										if (idVar(temp, pc_spawn_region_type) == /*"cube"*/ STR(3))
										{
											...
											if (idVar(temp, pc_spawn_region_type) == /*"box"*/ STR(1328))
											{
												for (IntType a = X_; a <= Z_; a++)
												{
												...
												if (idVar(temp, pc_spawn_region_type) == /*"path"*/ STR(535) && idVar(temp, pc_spawn_region_path) != null_)
												{
