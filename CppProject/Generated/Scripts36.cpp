...
namespace CppProject
{
	VarType model_file_load_shape(Scope<obj_model_part> self, VarType map, IntType res)
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1810))) && !(DsMap(map).Value(/*"visible"*/ STR(1810)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"type"*/ STR(800))))
		{
			log({ /*"Missing parameter \"type\""*/ STR(1844) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"from"*/ STR(839))))
		{
			log({ /*"Missing array \"from\""*/ STR(1845) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"to"*/ STR(840))))
		{
			log({ /*"Missing array \"to\""*/ STR(1846) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"uv"*/ STR(848))))
		{
		...
		{
			self->type = DsMap(map).Value(/*"type"*/ STR(800));
			...
			self->from_noscale = value_get_point3D(DsMap(map).Value(/*"from"*/ STR(839)));
			self->to_noscale = value_get_point3D(DsMap(map).Value(/*"to"*/ STR(840)));
			...
			self->uv = value_get_point2D(DsMap(map).Value(/*"uv"*/ STR(848)));
			...
			{
				if (is_string(DsMap(windmap).Value(/*"axis"*/ STR(827))))
				{
					if (DsMap(windmap).Value(/*"axis"*/ STR(827)) == /*"y"*/ STR(828))
						self->wind_wave = e_vertex_wave_Z_ONLY;
					else
						self->wind_wave = e_vertex_wave_ALL;
					
				}
				if (is_real(DsMap(windmap).Value(/*"ymin"*/ STR(829))))
					self->wind_wave_zmin = DsMap(windmap).Value(/*"ymin"*/ STR(829));
				if (is_real(DsMap(windmap).Value(/*"ymax"*/ STR(1858))))
					self->wind_wave_zmax = DsMap(windmap).Value(/*"ymax"*/ STR(1858));
			}
			if (self->type == /*"block"*/ STR(535))
			{
				self->vbuffer_default = model_shape_generate_block(ScopeAny(self), vec3(IntType(0)));
				if (ObjType(obj_model_part, self.otherId)->name == /*"head"*/ STR(899) && global::player_head_vbuffer == null_)
	...
	RealType model_file_load_texture(VarType name, IntType res)
	{
		...
		if (ext != /*".png"*/ STR(6) && ext != /*".jpg"*/ STR(735) && ext != /*".jpeg"*/ STR(736))
			return IntType(0);
	...
	IntType model_load(VarType map, StringType dir)
	{
		withOne (obj_model, (new obj_model)->id, noone)
		{
			...
			if (dev_mode_debug_names && !text_exists(/*"model"*/ STR(542) + self->name))
			...
			if (is_string(DsMap(map).Value(/*"texture"*/ STR(559))))
			{
				...
				DsMap(self->texture_name_map)[/*""*/ STR(0)] = DsMap(map).Value(/*"texture"*/ STR(559));
			}
			else
				if (ds_map_valid(DsMap(map).Value(/*"texture"*/ STR(559))))
				{
					self->texture_name_map = ds_string_map_create();
					ds_map_copy((IntType)(self->texture_name_map), (IntType)(DsMap(map).Value(/*"texture"*/ STR(559))));
				}
				else
			...
			self->pattern_type = value_get_string(DsMap(map).Value(/*"pattern_type"*/ STR(886)), /*""*/ STR(0));
			self->states_map = null_;
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(822))))
			{
				self->states_map = ds_map_create();
				VarType curstate = ds_map_find_first((IntType)(DsMap(map).Value(/*"states"*/ STR(822))));
				while (!is_undefined(curstate))
				{
					...
					{
						self->name = curstate;
						IntType valuelist = ds_map_find_value((IntType)(DsMap(map).Value(/*"states"*/ STR(822))), curstate);
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							self->value_name[v] = DsMap(curvalue).Value(/*"value"*/ STR(824));
							...
							if (dev_mode_debug_names && !text_exists(/*"modelstatevalue"*/ STR(774) + self->value_name.Value(v)))
							...
							if (is_string(DsMap(curvalue).Value(/*"texture"*/ STR(559))))
							{
								IntType texnamemap = ds_string_map_create();
								DsMap(texnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"texture"*/ STR(559));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"texture"*/ STR(559))))
								{
									self->value_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(559)), true);
									self->value_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(559)), true);
									self->value_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(559)), true);
								}
							...
							if (!is_undefined(DsMap(curvalue).Value(/*"pattern_type"*/ STR(886))))
								self->value_pattern_type[v] = DsMap(curvalue).Value(/*"pattern_type"*/ STR(886));
						}
						DsMap(ObjType(obj_model, self.otherId)->states_map)[curstate] = self->id;
						curstate = ds_map_find_next((IntType)(DsMap(map).Value(/*"states"*/ STR(822))), curstate);
					}
					
				}
				
			}
			if (is_string(DsMap(map).Value(/*"default_state"*/ STR(826))))
				self->default_state = string_get_state_vars(DsMap(map).Value(/*"default_state"*/ STR(826)));
			else
	...
	RealType model_part_fill_shape_vbuffer_map(VarType part, IntType vbufmap, IntType alphamap, VecType bend)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			withOne (obj_model_shape, DsList(idInt(part, shape_list)).Value(s), noone)
			{
				...
				if (self->type == /*"block"*/ STR(535) && self->bend_shape > 0 && isbent)
