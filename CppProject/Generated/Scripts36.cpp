...
namespace CppProject
{
	void menu_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			if (ObjType(obj_menu, m)->menu_window != window_get_current())
				continue;
			if (ObjType(obj_menu, m)->menu_ani_type == /*"hide"*/ STR(751))
			{
				...
				if (ObjType(obj_menu, m)->menu_ani_type == /*"show"*/ STR(752))
				{
					ObjType(obj_menu, m)->menu_ani += test_reduced_motion(IntType(1), (0.1 * global::delta));
			...
			if (ObjType(obj_menu, m)->menu_steps == IntType(0))
				draw_set_alpha(0.0);
			ObjType(obj_menu, m)->menu_ani_ease = ease(((ObjType(obj_menu, m)->menu_ani_type == /*"show"*/ STR(752)) ? /*"easeoutexpo"*/ STR(595) : /*"easeinexpo"*/ STR(594)), ObjType(obj_menu, m)->menu_ani);
			...
			if (sVar(window_busy) == /*"menu"*/ STR(1101) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(751) && menu_active)
			...
			if (keyboard_check_pressed(vk_enter) || keyboard_check_pressed(vk_escape))
			{
				if (ObjType(obj_menu, m)->menu_nav_use)
				{
					...
					ObjType(obj_menu, m)->menu_ani_type = /*"hide"*/ STR(751);
			...
			if ((!(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->mouseon) && !(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->mouseon) && sBool(mouse_left_released) && menu_active && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(751) && sVar(menu_search_busy) == /*""*/ STR(0)) || nav_close)
			{
				...
				if (close)
				{
					...
					if (ds_list_size(sInt(menu_list)) > IntType(1))
						sVar(window_busy) = /*"menu"*/ STR(1101);
					else
			...
			if (sVar(window_busy) == /*""*/ STR(0) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(751) && menu_active)
				sVar(window_busy) = /*"menu"*/ STR(1101);
		}
	...
	void menu_grid_settings(ScopeAny self)
	{
		...
		IntType draggerwid;
		draggerwid = text_max_width({ /*"viewgridrows"*/ STR(1720), /*"viewgridcolumns"*/ STR(1721) }) + IntType(16) + dragger_width;
		tab_control_dragger(self);
		draw_dragger(self, /*"viewgridrows"*/ STR(1720), sVar(dx), sVar(dy), dragger_width, sVar(project_grid_rows), 0.05, IntType(1), IntType(10), IntType(3), IntType(1), sInt(tbx_grid_rows), ID_action_project_grid_rows, null_, true);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewgridcolumns"*/ STR(1721), sVar(dx), sVar(dy), dragger_width, sVar(project_grid_columns), 0.05, IntType(1), IntType(10), IntType(3), IntType(1), sInt(tbx_grid_columns), ID_action_project_grid_columns, null_, true);
		tab_next(self);
	...
	void menu_quality_settings(ScopeAny self)
	{
		...
		VarType text;
		draw_set_font(sInt(font_label));
		capwid = max({ IntType(176), text_max_width({ /*"viewmodepass"*/ STR(1510) }) + IntType(16) });
		text = text_get({ /*"viewmodepass"*/ STR(1510) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) });
		tab_control_menu(self);
		draw_button_menu(self, { /*"viewmodepass"*/ STR(1510), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sInt(project_render_pass), text, ID_action_project_render_pass });
		tab_next(self);
	...
	RealType menu_settings_draw(ScopeAny self)
	{
		if (sInt(settings_menu_window) != window_get_current())
			return IntType(0);
		if (sStr(settings_menu_ani_type) == /*"hide"*/ STR(751))
		{
			...
			if (sStr(settings_menu_ani_type) == /*"show"*/ STR(752))
			{
				if (sReal(settings_menu_quick) > 0)
		...
		if (sStr(settings_menu_name) == /*""*/ STR(0))
			return IntType(0);
		VarType settingsmenuease = ease(((sStr(settings_menu_ani_type) == /*"show"*/ STR(752)) ? /*"easeoutexpo"*/ STR(595) : /*"easeinexpo"*/ STR(594)), sVar(settings_menu_ani));
		...
		if (sVar(window_busy) == /*"settingsmenu"*/ STR(1051) && sStr(settings_menu_ani_type) == /*""*/ STR(0) && sVar(settings_menu_ani) == IntType(1))
			sVar(window_busy) = /*""*/ STR(0);
		sBool(content_mouseon) = app_mouse_box(self, sVar(content_x), sVar(content_y), sVar(content_width), sVar(content_height));
		...
		if (sStr(settings_menu_ani_type) == /*""*/ STR(0) && sBool(mouse_left_released) && !sBool(context_menu_mouseon) && (ds_list_size(sInt(menu_list)) == IntType(0)) && (sVar(window_focus) == /*""*/ STR(0) || sVar(window_focus) == string(sInt(settings_menu_scroll))))
		{
			if (sInt(settings_menu_script) > 0)
			{
				...
				{
					sVar(settings_menu_ani) = IntType(1);
					sStr(settings_menu_ani_type) = /*"hide"*/ STR(751);
					sVar(window_busy) = /*"settingsmenu"*/ STR(1051);
		...
		if (sVar(window_busy) == /*""*/ STR(0) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(751))
			sVar(window_busy) = /*"settingsmenu"*/ STR(1051);
		return 0.0;
	}
	...
	void menu_settings_set(ScopeAny self, VarType xx, VarType yy, StringType name, RealType buttonheight)
	{
		...
		sVar(settings_menu_ani) = IntType(0);
		sStr(settings_menu_ani_type) = /*"show"*/ STR(752);
	...
	void menu_snap_settings(ScopeAny self)
	{
		...
		draggerwid = text_max_width({ /*"viewsnapmove"*/ STR(1722), /*"viewsnaprotate"*/ STR(1723), /*"viewsnapscale"*/ STR(1724) }) + IntType(16) + dragger_width;
		switchwid = text_max_width({ /*"viewsnapabsolute"*/ STR(1725) }) + IntType(28) + IntType(16) + IntType(24);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewsnapmove"*/ STR(1722), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_position), 0.01, snap_min, no_limit, IntType(1), snap_min, sInt(tbx_snap_position), ID_action_setting_snap_size_position);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewsnaprotate"*/ STR(1723), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_rotation), 0.01, snap_min, no_limit, IntType(15), snap_min, sInt(tbx_snap_rotation), ID_action_setting_snap_size_rotation);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewsnapscale"*/ STR(1724), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_scale), 0.01, snap_min, no_limit, IntType(1), snap_min, sInt(tbx_snap_scale), ID_action_setting_snap_size_scale);
		tab_next(self);
		draw_divide(sVar(content_x), sVar(dy), sVar(content_width));
		sVar(dy) += IntType(8);
		tab_control_switch(self);
		draw_switch(self, /*"viewsnapabsolute"*/ STR(1725), sVar(dx), sVar(dy), sVar(setting_snap_absolute), ID_action_setting_snap_absolute, /*"viewsnapabsolutetip"*/ STR(1726));
		tab_next(self);
	...
	BoolType menu_swatches_draw(ScopeAny self)
	{
		...
		for (IntType s = IntType(0); s < array_length(VarType::CreateRef(global::minecraft_swatch_array)); s++)
		{
			...
			sVar(dy) += IntType(14);
			draw_label(self, { text_get({ /*"swatch"*/ STR(1727) + ObjType(obj_swatch, swatch)->name }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(9);
			VarType xx;
			xx = sVar(dx);
			for (IntType c = IntType(0); c < array_length(VarType::CreateRef(ObjType(obj_swatch, swatch)->colors)); c++)
			{
				if (draw_button_swatch(self, xx, sVar(dy), IntType(20), IntType(20), /*"swatch"*/ STR(1727) + ObjType(obj_swatch, swatch)->name + ObjType(obj_swatch, swatch)->color_names.Value(c), ObjType(obj_swatch, swatch)->colors.Value(c)))
				{
					if (ObjType(obj_context_menu_level, sVar(context_menu_current))->name == /*"contextmenuswatchset"*/ STR(1557))
	...
	VarType menu_transitions(ScopeAny self, VarType xx, VarType yy, VarType wid, VarType hei)
	{
		...
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenuother"*/ STR(1728) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		...
		sVar(dx) = sVar(dx_start);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueasein"*/ STR(1729) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
		...
		sVar(dy) += IntType(46);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueaseout"*/ STR(1730) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
		...
		sVar(dy) += IntType(46);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueaseinout"*/ STR(1731) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
		...
		if (sVar(context_menu_name) != /*""*/ STR(0) && transition != null_)
		{
			...
			if (transition != null_)
			{
				...
				ObjType(obj_menu, sVar(menu_current))->menu_ani_type = /*"hide"*/ STR(751);
				sVar(window_busy) = /*""*/ STR(0);
				return true;
			}
			...
			switch (StringType(global::load_assets_stage).id)
			{
				case 1319: // unzip
				{
					...
					if (!(exists > 0))
					{
						...
						{
							log({ /*"Could not unzip archive"*/ STR(1732) });
							directory_delete_lib(global::load_assets_dir);
							access_error();
							return false;
						}
						debug_timer_stop(/*"Unzip archive"*/ STR(1733));
					}
					else
						log({ /*"Archive already unzipped, re-using"*/ STR(1734), global::load_assets_dir + mc_assets_directory });
					
					global::load_assets_stage = /*"biomes"*/ STR(1735);
				...
				case 1735: // biomes
				{
					withOne (obj_resource, global::mc_res, self->id)
						minecraft_assets_load_biomes(global::biome_list, DsMap(global::load_assets_map).Value(/*"biomes"*/ STR(1735)));
					...
					global::load_assets_stage = /*"textures"*/ STR(843);
				...
				case 843: // textures
				{
					VarType modeltextureslist = DsMap(global::load_assets_map).Value(/*"model_textures"*/ STR(1736));
					if (is_undefined(modeltextureslist))
					{
						log({ /*"No model textures found"*/ STR(1737) });
						return false;
					}
					ds_list_copy((IntType)(self->model_texture_list), (IntType)(modeltextureslist));
					VarType blocktextureslist = DsMap(global::load_assets_map).Value(/*"block_textures"*/ STR(1738));
					if (is_undefined(blocktextureslist))
					{
						log({ /*"No block textures found"*/ STR(1739) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_list), (IntType)(blocktextureslist));
					VarType blocktexturesanimatedlist = DsMap(global::load_assets_map).Value(/*"block_textures_animated"*/ STR(1740));
					if (is_undefined(blocktexturesanimatedlist))
					{
						log({ /*"No animated block textures found"*/ STR(1741) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_ani_list), (IntType)(blocktexturesanimatedlist));
					VarType blocktexturescolorlist = DsMap(global::load_assets_map).Value(/*"block_textures_color"*/ STR(1742));
					if (is_undefined(blocktexturescolorlist))
					{
						log({ /*"No block texture colors found"*/ STR(1743) });
						return false;
					}
					...
					{
						if (string_char_at(DsMap(self->block_texture_color_map).Value(key), IntType(1)) == /*"#"*/ STR(860))
					...
					VarType blocktexturepreview = DsMap(global::load_assets_map).Value(/*"block_textures_preview"*/ STR(1744));
					if (!is_undefined(blocktexturepreview))
						ds_map_copy((IntType)(self->block_texture_preview_map), (IntType)(blocktexturepreview));
					VarType itemtextureslist = DsMap(global::load_assets_map).Value(/*"item_textures"*/ STR(1745));
					if (is_undefined(itemtextureslist))
					{
						log({ /*"No item textures found"*/ STR(1746) });
						return false;
					}
					ds_list_copy((IntType)(self->item_texture_list), (IntType)(itemtextureslist));
					VarType particletextureslist = DsMap(global::load_assets_map).Value(/*"particle_textures"*/ STR(1747));
					if (is_undefined(particletextureslist))
					{
						log({ /*"No particle textures found"*/ STR(1748) });
						return false;
					}
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(552))))
					{
						log({ /*"No particle list found"*/ STR(1749) });
						return false;
					}
					...
					withOne (obj_resource, global::mc_res, self->id)
					{
						...
						res_load_pack_item_textures(ScopeAny(self), /*"diffuse"*/ STR(636), /*""*/ STR(0));
						...
						minecraft_assets_load_particles(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(552)));
					...
					global::load_assets_stage = /*"misc"*/ STR(1750);
				...
				case 1750: // misc
				{
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1751))))
					{
						log({ /*"No pattern designs list found"*/ STR(1752) });
						return false;
					}
					VarType patternlist = DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1751));
					...
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"sherds"*/ STR(968))))
					{
						log({ /*"No sherd keys found"*/ STR(1753) });
						return false;
					}
					else
						ds_map_copy((IntType)(global::minecraft_sherd_map), (IntType)(DsMap(global::load_assets_map).Value(/*"sherds"*/ STR(968))));
					
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"armor"*/ STR(782))))
					{
						log({ /*"No armor data found"*/ STR(1754) });
						return false;
					}
					else
					{
						VarType armormap = DsMap(global::load_assets_map).Value(/*"armor"*/ STR(782));
						ds_list_copy((IntType)(global::minecraft_armor_trim_pattern_list), (IntType)(DsMap(armormap).Value(/*"pattern"*/ STR(1755))));
						ds_list_copy((IntType)(global::minecraft_armor_trim_material_list), (IntType)(DsMap(armormap).Value(/*"material"*/ STR(641))));
					}
					
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"map_colors"*/ STR(1756))))
					{
						log({ /*"No map colors found"*/ STR(1757) });
						return false;
					}
					VarType colors = DsMap(global::load_assets_map).Value(/*"map_colors"*/ STR(1756));
					...
					global::load_assets_stage = /*"models"*/ STR(1758);
					global::load_assets_progress = 0.5;
					VarType swatches = DsMap(global::load_assets_map).Value(/*"swatches"*/ STR(1759));
					if (is_undefined(swatches))
					{
						log({ /*"No swatches found"*/ STR(1760) });
						return false;
					}
					for (IntType i = IntType(0); i < ds_list_size((IntType)(swatches)); i++)
					{
						...
						ObjType(obj_swatch, s)->name = DsMap(swatch).Value(/*"name"*/ STR(820));
						...
						VarType colors = DsMap(swatch).Value(/*"colors"*/ STR(1761));
						for (IntType j = IntType(0); j < ds_list_size((IntType)(colors)); j++)
						{
							VarType c = DsList(colors).Value(j);
							array_add(VarType::CreateRef(ObjType(obj_swatch, s)->color_names), DsMap(c).Value(/*"name"*/ STR(820)));
							array_add(VarType::CreateRef(ObjType(obj_swatch, s)->colors), hex_to_color(DsMap(c).Value(/*"value"*/ STR(830))));
							ds_map_add(ObjType(obj_swatch, s)->map, DsMap(c).Value(/*"name"*/ STR(820)), hex_to_color(DsMap(c).Value(/*"value"*/ STR(830))));
							ds_map_add(global::minecraft_swatch_color_map, DsMap(swatch).Value(/*"name"*/ STR(820)) + /*":"*/ STR(789) + DsMap(c).Value(/*"name"*/ STR(820)), hex_to_color(DsMap(c).Value(/*"value"*/ STR(830))));
						}
						ObjType(obj_swatch, s)->size = array_length(VarType::CreateRef(ObjType(obj_swatch, s)->colors));
					}
					global::minecraft_swatch_dyes = minecraft_get_swatch(/*"dye"*/ STR(1762));
					if (global::minecraft_swatch_dyes == null_)
					{
						log({ /*"No dyes found"*/ STR(1763) });
						return false;
					}
					break;
				}
				...
				case 1758: // models
				{
					VarType characterslist = DsMap(global::load_assets_map).Value(/*"characters"*/ STR(1764));
					...
					debug_timer_stop(/*"Load characters"*/ STR(1767));
					VarType specialblockslist = DsMap(global::load_assets_map).Value(/*"special_blocks"*/ STR(1768));
					...
					debug_timer_stop(/*"Load special blocks"*/ STR(1770));
					debug_timer_start();
					global::load_assets_stage = /*"blocks"*/ STR(1024);
				...
				case 1024: // blocks
				{
					VarType blockslist = DsMap(global::load_assets_map).Value(/*"blocks"*/ STR(1024));
					if (is_undefined(blockslist))
					{
						log({ /*"No block list found"*/ STR(1771) });
						return false;
					}
					for (IntType _it = 0, _it_max = IntType(20); _it < _it_max; _it++)
					{
						...
						{
							global::load_assets_stage = /*"done"*/ STR(1772);
							break;
						}
						VarType blockmap;
						IntType block;
						...
						if (!(block > 0))
						{
							log({ /*"Could not load block"*/ STR(1773) });
							continue;
						}
					...
					if (dev_mode_skip_blocks)
						global::load_assets_stage = /*"done"*/ STR(1772);
					if (global::load_assets_stage == /*"done"*/ STR(1772))
					{
						DsMap(self->block_name_map)[/*"flowing_water"*/ STR(1774)] = DsMap(self->block_name_map).Value(/*"water"*/ STR(818));
						DsMap(self->block_name_map)[/*"flowing_lava"*/ STR(1775)] = DsMap(self->block_name_map).Value(/*"lava"*/ STR(1776));
						DsMap(self->block_liquid_slot_map)[/*"water"*/ STR(818)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_still"*/ STR(1777));
						DsMap(self->block_liquid_slot_map)[/*"lava"*/ STR(1776)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_still"*/ STR(1778));
						DsMap(self->block_liquid_slot_map)[/*"flowing_water"*/ STR(1774)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_flow"*/ STR(1779));
						DsMap(self->block_liquid_slot_map)[/*"flowing_lava"*/ STR(1775)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_flow"*/ STR(1780));
						if (!dev_mode_skip_blocks)
							DsMap(self->block_id_map)[/*"minecraft:grass_path"*/ STR(1781)] = DsMap(self->block_id_map).Value(/*"minecraft:dirt_path"*/ STR(1782));
						...
						while (!is_undefined(key))
						{
							...
							if (!is_undefined(newid))
								newidnomc = string_replace(DsMap(curmap).Value(/*"id"*/ STR(86)), /*"minecraft:"*/ STR(839), /*""*/ STR(0));
							else
								newidnomc = /*""*/ STR(0);
							...
							if (!is_undefined(DsMap(curmap).Value(/*"data"*/ STR(1783))))
								minecraft_assets_load_legacy_block_data(self, curid, DsMap(curmap).Value(/*"data"*/ STR(1783)), IntType(0), IntType(1));
							for (IntType d = IntType(0); d < IntType(16); d++)
						...
						debug_timer_stop(/*"Load blocks"*/ STR(1784));
						...
						{
							ArrType filesarr = file_find(global::load_assets_dir + mc_blockstates_directory, /*".json"*/ STR(732));
							...
							if (ds_list_size(unusedlist) > IntType(0))
							{
								...
								for (IntType i = IntType(0); i < ds_list_size(unusedlist); i++)
									str += /*"  "*/ STR(1786) + filename_name(DsList(unusedlist).Value(i)) + /*"\n"*/ STR(711);
								log({ str });
							}
							filesarr = file_find(global::load_assets_dir + mc_models_directory + /*"block/"*/ STR(822), /*".json"*/ STR(732));
						...
						log({ /*"Loaded assets successfully"*/ STR(1788) });
