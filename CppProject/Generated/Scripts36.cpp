...
namespace CppProject
{
	VarType model_file_load_shape(Scope<obj_model_part> self, VarType map, IntType res)
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1795))) && !(DsMap(map).Value(/*"visible"*/ STR(1795)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"type"*/ STR(790))))
		{
			log({ /*"Missing parameter \"type\""*/ STR(1829) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"from"*/ STR(829))))
		{
			log({ /*"Missing array \"from\""*/ STR(1830) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"to"*/ STR(830))))
		{
			log({ /*"Missing array \"to\""*/ STR(1831) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"uv"*/ STR(838))))
		{
		...
		{
			self->type = DsMap(map).Value(/*"type"*/ STR(790));
			...
			self->texture_mirror = value_get_real(DsMap(map).Value(/*"texture_mirror"*/ STR(1833)), false);
			self->invert = value_get_real(DsMap(map).Value(/*"invert"*/ STR(1827)), false);
			self->hide_front = value_get_real(DsMap(map).Value(/*"hide_front"*/ STR(1834)), false);
			self->hide_back = value_get_real(DsMap(map).Value(/*"hide_back"*/ STR(1835)), false);
			if (is_bool(DsMap(map).Value(/*"hide_backface"*/ STR(1836))))
				self->hide_back = DsMap(map).Value(/*"hide_backface"*/ STR(1836));
			self->face_camera = value_get_real(DsMap(map).Value(/*"face_camera"*/ STR(1837)), false);
			self->item_bounce = value_get_real(DsMap(map).Value(/*"item_bounce"*/ STR(1838)), false);
			self->move_required_array = value_get_point3D(DsMap(map).Value(/*"move_required"*/ STR(1839)), vec3(-IntType(1), -IntType(1), -IntType(1)));
			...
			self->from_noscale = value_get_point3D(DsMap(map).Value(/*"from"*/ STR(829)));
			self->to_noscale = value_get_point3D(DsMap(map).Value(/*"to"*/ STR(830)));
			if (self->type == /*"plane"*/ STR(1840))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->inflate = vec3(value_get_real(DsMap(map).Value(/*"inflate"*/ STR(1841)), IntType(0)));
			if (self->type == /*"plane"*/ STR(1840))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->is3d = false;
			if (self->type == /*"plane"*/ STR(1840))
			{
				self->is3d = value_get_real(DsMap(map).Value(/*"3d"*/ STR(1842)), false);
				if (self->is3d > 0)
				{
			...
			self->bend_shape = value_get_real(DsMap(map).Value(/*"bend"*/ STR(1809)), true);
			...
			self->uv = value_get_point2D(DsMap(map).Value(/*"uv"*/ STR(838)));
			...
			{
				if (is_string(DsMap(windmap).Value(/*"axis"*/ STR(817))))
				{
					if (DsMap(windmap).Value(/*"axis"*/ STR(817)) == /*"y"*/ STR(818))
						self->wind_wave = e_vertex_wave_Z_ONLY;
					else
						self->wind_wave = e_vertex_wave_ALL;
					
				}
				if (is_real(DsMap(windmap).Value(/*"ymin"*/ STR(819))))
					self->wind_wave_zmin = DsMap(windmap).Value(/*"ymin"*/ STR(819));
				if (is_real(DsMap(windmap).Value(/*"ymax"*/ STR(1843))))
					self->wind_wave_zmax = DsMap(windmap).Value(/*"ymax"*/ STR(1843));
			}
			if (self->type == /*"block"*/ STR(525))
			{
				self->vbuffer_default = model_shape_generate_block(ScopeAny(self), vec3(IntType(0)));
				if (ObjType(obj_model_part, self.otherId)->name == /*"head"*/ STR(889) && global::player_head_vbuffer == null_)
				...
				else
				{
					...
					log({ /*"Invalid shape type"*/ STR(1844), self->type });
					return null_;
				}
	...
	RealType model_file_load_texture(VarType name, IntType res)
	{
		...
		if (ext != /*".png"*/ STR(6) && ext != /*".jpg"*/ STR(725) && ext != /*".jpeg"*/ STR(726))
			return IntType(0);
	...
	IntType model_load(VarType map, StringType dir)
	{
		withOne (obj_model, (new obj_model)->id, noone)
		{
			...
			{
				log({ /*"Could not find name for model"*/ STR(1845) });
				return null_;
			}
			
			if (dev_mode_debug_names && !text_exists(/*"model"*/ STR(532) + self->name))
			...
			self->pattern_type = value_get_string(DsMap(map).Value(/*"pattern_type"*/ STR(876)), /*""*/ STR(0));
			self->states_map = null_;
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(812))))
			{
				self->states_map = ds_map_create();
				VarType curstate = ds_map_find_first((IntType)(DsMap(map).Value(/*"states"*/ STR(812))));
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"modelstate"*/ STR(761) + curstate))
						log({ /*"model/state/"*/ STR(1846) + curstate + dev_mode_name_translation_message });
					withOne (obj_model_state, (new obj_model_state)->id, self->id)
					{
						self->name = curstate;
						IntType valuelist = ds_map_find_value((IntType)(DsMap(map).Value(/*"states"*/ STR(812))), curstate);
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							self->value_name[v] = DsMap(curvalue).Value(/*"value"*/ STR(814));
							...
							if (dev_mode_debug_names && !text_exists(/*"modelstatevalue"*/ STR(764) + self->value_name.Value(v)))
								log({ /*"model/state/value/"*/ STR(1847) + self->value_name.Value(v) + dev_mode_name_translation_message });
							...
							if (is_string(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1848))))
							{
								IntType shapetexnamemap = ds_string_map_create();
								DsMap(shapetexnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"shape_texture"*/ STR(1848));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1848))))
								{
									self->value_shape_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1848)), true);
									self->value_shape_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1848)), true);
									self->value_shape_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1848)), true);
								}
							
							if (ds_list_valid(DsMap(curvalue).Value(/*"hide"*/ STR(736))))
							{
								self->value_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"hide"*/ STR(736))));
							}
							if (ds_list_valid(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1849))))
							{
								self->value_shape_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_shape_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1849))));
							}
							if (ds_map_valid(DsMap(curvalue).Value(/*"shape_color"*/ STR(1850))))
							{
								self->value_color_name_map[v] = ds_map_create();
								ds_map_merge(self->value_color_name_map.Value(v), DsMap(curvalue).Value(/*"shape_color"*/ STR(1850)), true);
							}
							if (!is_undefined(DsMap(curvalue).Value(/*"pattern_type"*/ STR(876))))
								self->value_pattern_type[v] = DsMap(curvalue).Value(/*"pattern_type"*/ STR(876));
						}
						DsMap(ObjType(obj_model, self.otherId)->states_map)[curstate] = self->id;
						curstate = ds_map_find_next((IntType)(DsMap(map).Value(/*"states"*/ STR(812))), curstate);
					}
					
				}
				
			}
			if (is_string(DsMap(map).Value(/*"default_state"*/ STR(816))))
				self->default_state = string_get_state_vars(DsMap(map).Value(/*"default_state"*/ STR(816)));
			else
	...
	RealType model_part_fill_shape_alpha_map(VarType part, IntType alphamap, VarType res, IntType texnamemap, IntType shapetexnamemap)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			{
				if (self->type == /*"plane"*/ STR(1840) && self->is3d > 0)
				{
	...
	RealType model_part_fill_shape_vbuffer_map(VarType part, IntType vbufmap, IntType alphamap, VecType bend)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			withOne (obj_model_shape, DsList(idInt(part, shape_list)).Value(s), noone)
			{
				...
				if (self->type == /*"block"*/ STR(525) && self->bend_shape > 0 && isbent)
	...
	VarType model_shape_generate_block(ScopeAny self, VecType bend)
	{
		...
		VarType bendsize;
		BoolType invangle;
		sharpbend = global::_app->project_bend_style == /*"blocky"*/ STR(1851) && sVar(bend_size) == null_ && ((sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && sVec(bend_axis).Real(Z_)));
		bendsize = (sVar(bend_size) == null_ ? VarType((global::_app->project_bend_style == /*"realistic"*/ STR(1852) ? 4.0 : 1.0)) : sVar(bend_size));
