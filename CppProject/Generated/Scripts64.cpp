...
namespace CppProject
{
	...
	VarType text_control_name(VarType keybind)
	{
		...
		switch ((IntType)keybind.Value(e_keybind_key_CHAR))
		{
			case vk_left:
			{
				char_ = text_get({ /*"keyleft"*/ STR(3766) });
				break;
			}
			case vk_right:
			{
				char_ = text_get({ /*"keyright"*/ STR(3767) });
				break;
			}
			case vk_up:
			{
				char_ = text_get({ /*"keyup"*/ STR(3768) });
				break;
			}
			case vk_down:
			{
				char_ = text_get({ /*"keydown"*/ STR(3769) });
				break;
			}
			case vk_enter:
			{
				char_ = text_get({ /*"keyenter"*/ STR(3770) });
				break;
			}
			case vk_escape:
			{
				char_ = text_get({ /*"keyescape"*/ STR(3771) });
				break;
			}
			case vk_space:
			{
				char_ = text_get({ /*"keyspace"*/ STR(3772) });
				break;
			}
			case vk_lshift:
			{
				char_ = text_get({ /*"keyleftshift"*/ STR(3773) });
				break;
			}
			case vk_rshift:
			{
				char_ = text_get({ /*"keyrightshift"*/ STR(3774) });
				break;
			}
			case vk_lalt:
			{
				char_ = text_get({ /*"keyleftalt"*/ STR(3775) });
				break;
			}
			case vk_ralt:
			{
				char_ = text_get({ /*"keyrightalt"*/ STR(3776) });
				break;
			}
			case vk_lcontrol:
			{
				char_ = text_get({ /*"keyleftcontrol"*/ STR(3777) });
				break;
			}
			case vk_rcontrol:
			{
				char_ = text_get({ /*"keyrightcontrol"*/ STR(3778) });
				break;
			}
			case vk_backspace:
			{
				char_ = text_get({ /*"keybackspace"*/ STR(3779) });
				break;
			}
			case vk_tab:
			{
				char_ = text_get({ /*"keytab"*/ STR(3780) });
				break;
			}
			case vk_home:
			{
				char_ = text_get({ /*"keyhome"*/ STR(3781) });
				break;
			}
			case vk_end:
			{
				char_ = text_get({ /*"keyend"*/ STR(3782) });
				break;
			}
			case vk_delete:
			{
				char_ = text_get({ /*"keydelete"*/ STR(3783) });
				break;
			}
			case vk_insert:
			{
				char_ = text_get({ /*"keyinsert"*/ STR(3784) });
				break;
			}
			case vk_pageup:
			{
				char_ = text_get({ /*"keypageup"*/ STR(3785) });
				break;
			}
			case vk_pagedown:
			{
				char_ = text_get({ /*"keypagedown"*/ STR(3786) });
				break;
			}
			case vk_pause:
			{
				char_ = text_get({ /*"keypause"*/ STR(3787) });
				break;
			}
			case vk_printscreen:
			{
				char_ = text_get({ /*"keyprintscreen"*/ STR(3788) });
				break;
			}
			case vk_f1:
			{
		...
		if (keybind.Value(e_keybind_key_ALT) > 0)
			text = text_get({ /*"keyalt"*/ STR(3801) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2926) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_CTRL) > 0)
			text = text_get({ /*"keycontrol"*/ STR(3802) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2926) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_SHIFT) > 0)
			text = text_get({ /*"keyshift"*/ STR(3803) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2926) + text) : /*""*/ STR(0));
		return text;
	}
	...
	VarType text_get(VarArgs argument)
	{
		...
		{
			if (!text_exists(key))
				return /*"<No text found for \""*/ STR(3804) + key + /*"\">"*/ STR(3805);
	...
	IntType text_max_width(VarArgs argument)
	{
		...
		IntType wid = IntType(0);
		for (IntType a = IntType(0); a < argument_count; a++)
			wid = max({ wid, string_width(text_get({ argument[a] }) + /*":"*/ STR(782)) });
		return wid;
	}
	...
	RealType time_get_rotation(VarType str)
	{
		...
		colons = string_count(/*":"*/ STR(782), str);
		days = IntType(0);
		hours = IntType(0);
		minutes = IntType(0);
		if (colons > IntType(2))
		...
		if (colons == IntType(1))
		{
			colonpos = string_pos(/*":"*/ STR(782), str);
	...
	void tl_event_create(Scope<obj_timeline> self)
	{
		...
		self->temp = null_;
		self->text = text_get({ /*"timelineeditortextsample"*/ STR(3806) });
