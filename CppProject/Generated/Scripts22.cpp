...
namespace CppProject
{
	VarType block_load_model_file(StringType fname, IntType res)
	{
		...
		withOne (obj_block_load_model_file, (new obj_block_load_model_file)->id, noone)
		{
			...
			if (res == null_ && is_string(DsMap(map).Value(/*"parent"*/ STR(837))))
				self->parent = block_load_model_file(global::load_assets_dir + mc_models_directory + string_replace(DsMap(map).Value(/*"parent"*/ STR(837)), /*"minecraft:"*/ STR(834), /*""*/ STR(0)) + /*".json"*/ STR(723));
	...
	VarType block_load_model_file_texture(Scope<obj_block_load_model_file> self, VarType name, IntType res)
	{
		...
		if (string_pos(/*"assets/minecraft_"*/ STR(850), name) == IntType(1))
			name = string_replace(name, /*"assets/minecraft_"*/ STR(850), /*"block/"*/ STR(817));
		if (res == null_)
			return name;
		if (string_pos(/*"blocks/"*/ STR(851), name) == IntType(1))
		{
			name = string_replace(name, /*"blocks/"*/ STR(851), /*"block/"*/ STR(817));
		...
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(20) + string_replace(name, /*"blocks/"*/ STR(851), /*""*/ STR(0)) + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(20) + string_replace(name, /*"block/"*/ STR(817), /*""*/ STR(0)) + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(20) + filename_name(name) + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/"*/ STR(852) + name + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/item/"*/ STR(853) + name + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/block/"*/ STR(854) + name + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			return name;
					...
					withOne (obj_block_render_element, (new obj_block_render_element)->id, self->id)
					{
						...
						for (IntType f = IntType(0); f < e_dir_amount; f++)
						{
							...
							else
							{
								...
								if (opaque)
									slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" opaque"*/ STR(857));
								if (slot < IntType(0))
									slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" noalpha"*/ STR(858));
								if (slot < IntType(0))
								...
								if (slot < IntType(0))
								{
									...
									if (slot < IntType(0))
										slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" noalpha"*/ STR(858));
									if (slot < IntType(0))
										slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list), texname + /*" opaque"*/ STR(857));
									if (slot < IntType(0))
									{
										self->face_render[nd] = false;
										continue;
									...
									if (!is_undefined(col) && col == /*"water"*/ STR(813))
										...
										if (alpha > IntType(0))
										{
											...
											else
											{
												...
												switch ((IntType)self->face_block_vbuffer.Value(nd))
												{
													...
													{
														colstr = /*"grass"*/ STR(859);
														break;
													}
													case e_block_vbuffer_FOLIAGE:
													{
														colstr = /*"foliage"*/ STR(860);
														break;
													}
													case e_block_vbuffer_WATER:
													{
														colstr = /*"water"*/ STR(813);
														break;
													}
