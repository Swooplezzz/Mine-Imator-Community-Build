...
namespace CppProject
{
	void minecraft_assets_load_biomes(IntType list, VarType map)
	{
		for (IntType b = IntType(0); b < ds_list_size((IntType)(map)); b++)
		{
			...
			{
				biomeid = self->id;
				self->name = DsMap(biome).Value(/*"name"*/ STR(815));
				self->group = value_get_real(DsMap(biome).Value(/*"group"*/ STR(1772)), false);
				self->txy = vec2(IntType(0));
				if (is_string(DsMap(biome).Value(/*"foliage"*/ STR(860))))
				{
					self->color_foliage = hex_to_color(DsMap(biome).Value(/*"foliage"*/ STR(860)));
				...
				else
				{
					...
					self->txy = value_get_point2D(DsMap(biome).Value(/*"foliage"*/ STR(860)), vec2(IntType(0), IntType(0)));
					self->hardcoded = false;
				}
				
				if (is_string(DsMap(biome).Value(/*"grass"*/ STR(859))))
					self->color_grass = hex_to_color(DsMap(biome).Value(/*"grass"*/ STR(859)));
				if (is_string(DsMap(biome).Value(/*"water"*/ STR(813))))
					self->color_water = hex_to_color(DsMap(biome).Value(/*"water"*/ STR(813)));
				self->biome_base = null_;
				self->biome_variants = null_;
				self->variants_extend = false;
				if (ds_list_valid(DsMap(biome).Value(/*"variant"*/ STR(811))))
				{
					self->biome_variants = ds_list_create();
					VarType biomevariants = DsMap(biome).Value(/*"variant"*/ STR(811));
					for (IntType v = IntType(0); v < ds_list_size((IntType)(biomevariants)); v++)
					{
						...
						{
							self->name = DsMap(variant).Value(/*"name"*/ STR(815));
							self->group = false;
							...
							if (is_string(DsMap(variant).Value(/*"foliage"*/ STR(860))))
							{
								self->color_foliage = hex_to_color(DsMap(variant).Value(/*"foliage"*/ STR(860)));
								self->hardcoded = true;
							}
							else
								if (ds_list_valid(DsMap(variant).Value(/*"foliage"*/ STR(860))))
								{
									self->color_foliage = c_white;
									self->txy = value_get_point2D(DsMap(variant).Value(/*"foliage"*/ STR(860)), vec2(IntType(0), IntType(0)));
									self->hardcoded = false;
								}
							
							if (is_string(DsMap(variant).Value(/*"grass"*/ STR(859))))
								self->color_grass = hex_to_color(DsMap(variant).Value(/*"grass"*/ STR(859)));
							else
								self->color_grass = ObjType(obj_biome, self.otherId)->color_grass;
							
							if (is_string(DsMap(variant).Value(/*"water"*/ STR(813))))
								self->color_water = hex_to_color(DsMap(variant).Value(/*"water"*/ STR(813)));
							else
		...
		while (!is_undefined(key))
		{
			switch (StringType(key).id)
			{
				...
				default:
				{
					...
					newid = state_vars_get_value(statevars, /*"id"*/ STR(87));
					if (newid != null_)
						newidnomc = string_replace(newid, /*"minecraft:"*/ STR(834), /*""*/ STR(0));
					block = null_;
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1780), /*"b"*/ STR(1781), /*"c"*/ STR(1782), /*"d"*/ STR(1783), /*"e"*/ STR(1240), /*"f"*/ STR(1241), /*"g"*/ STR(1784), /*"h"*/ STR(1785), /*"i"*/ STR(1786), /*"j"*/ STR(1787), /*"k"*/ STR(1788), /*"l"*/ STR(1789), /*"m"*/ STR(1790), /*"n"*/ STR(1242), /*"o"*/ STR(1791), /*"p"*/ STR(1792), /*"q"*/ STR(1793), /*"r"*/ STR(1794), /*"s"*/ STR(1795), /*"t"*/ STR(1243), /*"u"*/ STR(1244), /*"v"*/ STR(1796), /*"w"*/ STR(1797), /*"x"*/ STR(846), /*"y"*/ STR(829), /*"z"*/ STR(847) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->name = DsMap(tempmap).Value(/*"name"*/ STR(815));
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(570));
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1798))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1798));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real(DsMap(tempmap).Value(/*"size"*/ STR(1015)), IntType(8));
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real(DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1799)), false);
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1135) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1135) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		sStr(window_state) = /*"load_assets"*/ STR(84);
		global::load_assets_stage = /*"unzip"*/ STR(1302);
		...
		{
			self->save_id = /*"default"*/ STR(103);
			self->type = e_res_type_PACK;
			self->display_name = /*"Minecraft"*/ STR(1800);
		...
		{
			log({ /*"Could not load "*/ STR(1801) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1802), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1803)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1803) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1804));
			splash = DsList(splashlist).Value(irandom((IntType)(ds_list_size((IntType)(splashlist)) - IntType(1))));
			splashfile = splash_directory + DsMap(splash).Value(/*"file"*/ STR(122));
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1805));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1806), version });
		global::load_assets_file = minecraft_directory + version + /*".midata"*/ STR(101);
		global::load_assets_zip_file = minecraft_directory + version + /*".zip"*/ STR(7);
		if (!file_exists_lib(global::load_assets_file))
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1807), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1808), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1809), global::load_assets_file });
			ds_map_destroy(global::load_assets_type_map);
			return false;
		}
		VarType format = DsMap(global::load_assets_map).Value(/*"format"*/ STR(93));
		if (!is_real(format))
			format = e_minecraft_assets_FORMAT_110_PRE_1;
		if (format != minecraft_assets_format)
		{
			log({ /*"Unsupported archive, format"*/ STR(1810), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1811));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1135) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(20);
	...
	{
		StringType fname = minecraft_directory + minecraft_version + /*".midata"*/ STR(101);
		log({ /*"Reloading models"*/ STR(1812) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1809), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1747));
			if (is_undefined(characterslist))
			{
				log({ /*"No character list found"*/ STR(1748) });
				break;
			}
			for (IntType i = IntType(0); i < ds_list_size((IntType)(characterslist)); i++)
			{
				...
				if (!(model > 0))
				{
					log({ /*"Could not load model"*/ STR(1749) });
					continue;
				}
				DsMap(self->model_name_map)[ObjType(obj_model, model)->name] = model;
				ds_list_add({ self->char_list, model });
			}
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1751));
			if (is_undefined(specialblockslist))
			{
				log({ /*"No special block list found"*/ STR(1752) });
				break;
			}
	...
	BoolType minecraft_assets_startup(ScopeAny self)
	{
		...
		if (sVar(setting_minecraft_assets_new_version) != /*""*/ STR(0))
		{
			...
			sStr(window_state) = /*"new_assets"*/ STR(83);
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1813) + name + /*"]"*/ STR(1238) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1814) + name + /*"]"*/ STR(1238) : /*""*/ STR(0));
	...
	{
		ArrType keys = string_split(name, /*":"*/ STR(784));
		...
		if (string_contains(name, /*"#"*/ STR(855)))
	...
	{
		if (array_length(VarType::CreateRef(global::armor_update)) > IntType(0) && global::_app->window_busy != /*"popup"*/ STR(1619) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
			VarType obj = null_;
	...
	ArrType minecraft_update_armor_generate(ArrType data, VarType res)
	{
		...
		for (IntType i = IntType(0); i < IntType(4); i++)
		{
			...
			if (basemat == /*"none"*/ STR(904))
				continue;
			dye = data.Value(piece + IntType(1));
			trimpat = data.Value(piece + IntType(2));
			trimmat = data.Value(piece + IntType(3));
			layernum = ((i == IntType(2)) ? /*"2"*/ STR(1119) : /*"1"*/ STR(1118));
			basetex = /*"models/armor/"*/ STR(1815) + basemat + /*"_layer_"*/ STR(1816) + layernum;
			...
			surface_set_target(armorsurf);
		{
			...
			if (basemat == /*"leather"*/ STR(1817))
			{
				draw_texture({ basesprite, IntType(0), IntType(0), IntType(1), IntType(1), dye, IntType(1) });
				draw_texture({ DsMap(idInt(res, model_texture_map)).Value(basetex + /*"_overlay"*/ STR(1818)), IntType(0), IntType(0) });
			}
			else
			...
			if (trimpat != /*"none"*/ STR(904) && trimmat != /*"none"*/ STR(904))
			{
				VarType palette = DsMap(idInt(res, model_texture_map)).Value(/*"trims/color_palettes/"*/ STR(1819) + trimmat + (trimmat == basemat ? /*"_darker"*/ STR(1820) : /*""*/ STR(0)));
				surface_set_target(armorsurf);
			{
				...
				withOne (Object, global::render_shader_obj, noone)
				{
					...
					shader_palette_set(self, palette, DsMap(idInt(res, model_texture_map)).Value(/*"trims/color_palettes/trim_palette"*/ STR(1821)));
				}
				
				if (i == IntType(2))
					draw_sprite((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"trims/models/armor/"*/ STR(1822) + trimpat + /*"_leggings"*/ STR(1823))), IntType(0), IntType(0), IntType(0));
				else
					draw_sprite((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"trims/models/armor/"*/ STR(1822) + trimpat)), IntType(0), IntType(0), IntType(0));
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1619) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(812) ? /*"entity/banner_base"*/ STR(1824) : /*"entity/shield_base"*/ STR(1825));
		patterndir = (type == /*"banner"*/ STR(812) ? /*"entity/banner/"*/ STR(1826) : /*"entity/shield/"*/ STR(1827));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1828), fn });
		show_message(/*"The file "*/ STR(1829) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1830));
		return false;
	}
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		{
			log({ /*"Could not find model file"*/ STR(835), fname });
			return null_;
		}
		IntType map = json_load({ fname });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse model file"*/ STR(836), fname });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(815))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(816) });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(570))))
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1831) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1832))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1833) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1834))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1835) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(815));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(8) + self->name))
				log({ /*"model/"*/ STR(1836) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string(DsMap(map).Value(/*"description"*/ STR(1837)), /*""*/ STR(0));
			self->texture_name = DsMap(map).Value(/*"texture"*/ STR(570));
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string(DsMap(map).Value(/*"texture_material"*/ STR(1838)), /*""*/ STR(0));
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string(DsMap(map).Value(/*"texture_normal"*/ STR(1839)), /*""*/ STR(0));
			...
			self->texture_size = value_get_point2D(DsMap(map).Value(/*"texture_size"*/ STR(1832)));
			...
			self->player_skin = value_get_real(DsMap(map).Value(/*"player_skin"*/ STR(1840)), false);
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(885)), vec3(IntType(1), IntType(1), IntType(1)));
			self->model_color = value_get_string(DsMap(map).Value(/*"model_color"*/ STR(1841)), /*"none"*/ STR(904));
			...
			self->floor_box_uvs = value_get_real(DsMap(map).Value(/*"floor_box_uvs"*/ STR(1842)), false);
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1834));
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1843))) && !(DsMap(map).Value(/*"visible"*/ STR(1843)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(815))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(816) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1844) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			if (ObjType(obj_model_part, DsList(ObjType(obj_model_file, root)->file_part_list).Value(i))->name == DsMap(map).Value(/*"name"*/ STR(815)))
			{
				log({ /*"Duplicate part name found"*/ STR(1845), DsMap(map).Value(/*"name"*/ STR(815)) });
				return null_;
			}
		}
		...
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(815));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(774) + self->name))
				log({ /*"model/part/"*/ STR(1846) + self->name + dev_mode_name_translation_message });
			self->depth = value_get_real(DsMap(map).Value(/*"depth"*/ STR(637)), IntType(0));
			IntType pos;
			...
			self->color_inherit = value_get_real(DsMap(map).Value(/*"color_inherit"*/ STR(1847)), true);
			self->color_blend = value_get_color(DsMap(map).Value(/*"color_blend"*/ STR(1848)), c_white);
			self->color_alpha = value_get_real(DsMap(map).Value(/*"color_alpha"*/ STR(1849)), IntType(1));
			if (is_real(/*"color_emissive"*/ STR(1850)))
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_emissive"*/ STR(1850)), IntType(0));
			else
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_brightness"*/ STR(1851)), IntType(0));
			
			self->color_mix = value_get_color(DsMap(map).Value(/*"color_mix"*/ STR(1852)), c_black);
			self->color_mix_percent = value_get_real(DsMap(map).Value(/*"color_mix_percent"*/ STR(1853)), IntType(0));
			self->part_mixing_shapes = false;
			...
			self->rotation = value_get_point3D(DsMap(map).Value(/*"rotation"*/ STR(842)), vec3(IntType(0), IntType(0), IntType(0)));
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(885)), vec3(IntType(1), IntType(1), IntType(1)));
			self->scale = vec3_mul(self->scale, idVar(self.otherId, scale));
			self->floor_box_uvs = idVar(self.otherId, floor_box_uvs);
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1854)), false);
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idVar(self.otherId, locked) > 0)
				self->locked = true;
			else
				self->locked = value_get_real(DsMap(map).Value(/*"locked"*/ STR(937)), false);
			
			self->lock_bend = true;
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idInt(self.otherId, bend_part) != null_)
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1855))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1855))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1855));
				if (self->lock_bend > 0)
				{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1856)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1857))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1857));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1858)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1859))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1860) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1859));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1861)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(1015)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(899))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1862) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(899))).id)
				{
					case 203: // right
					{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1864) });
						return null_;
				}
				
				self->bend_axis = ArrType::From({ false, false, false });
				ArrType axis = ArrType();
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(828))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(828))).id)
					{
						case 846: // x
						{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1865) });
							return null_;
					}
					
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"axis"*/ STR(828))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(828)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(828))), i)).id)
							{
								case 846: // x
								{
									self->bend_axis.Real(X_) = true;
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1866) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1867))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1867));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1867))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1867)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1867))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1868))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1868));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1868))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1868)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1868))), i);
					}
				
				self->bend_direction = ArrType::From({ IntType(0), IntType(0), IntType(0) });
				self->bend_direction_legacy = false;
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(926))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(926))).id)
					{
						case 1869: // forward
						{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1872) });
							return null_;
					}
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(926))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(926)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(926))), i)).id)
							{
								case 1869: // forward
								{
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1873))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1873)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1873)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1873))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1873)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1873))), i);
					}
					else
				...
				if (is_real(DsMap(bendmap).Value(/*"angle"*/ STR(844))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_default_angle = vec3(DsMap(bendmap).Value(/*"angle"*/ STR(844)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"angle"*/ STR(844))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(844)))); i++)
							self->bend_default_angle.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(844))), i);
					}
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1874));
