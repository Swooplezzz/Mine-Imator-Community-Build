...
namespace CppProject
{
	void minecraft_assets_load_biomes(IntType list, VarType map)
	{
		for (IntType b = IntType(0); b < ds_list_size((IntType)(map)); b++)
		{
			...
			{
				biomeid = self->id;
				self->name = DsMap(biome).Value(/*"name"*/ STR(816));
				self->group = value_get_real(DsMap(biome).Value(/*"group"*/ STR(1784)), false);
				self->txy = vec2(IntType(0));
				if (is_string(DsMap(biome).Value(/*"foliage"*/ STR(861))))
				{
					self->color_foliage = hex_to_color(DsMap(biome).Value(/*"foliage"*/ STR(861)));
				...
				else
				{
					...
					self->txy = value_get_point2D(DsMap(biome).Value(/*"foliage"*/ STR(861)), vec2(IntType(0), IntType(0)));
					self->hardcoded = false;
				}
				
				if (is_string(DsMap(biome).Value(/*"grass"*/ STR(860))))
					self->color_grass = hex_to_color(DsMap(biome).Value(/*"grass"*/ STR(860)));
				if (is_string(DsMap(biome).Value(/*"water"*/ STR(814))))
					self->color_water = hex_to_color(DsMap(biome).Value(/*"water"*/ STR(814)));
				self->biome_base = null_;
				self->biome_variants = null_;
				self->variants_extend = false;
				if (ds_list_valid(DsMap(biome).Value(/*"variant"*/ STR(812))))
				{
					self->biome_variants = ds_list_create();
					VarType biomevariants = DsMap(biome).Value(/*"variant"*/ STR(812));
					for (IntType v = IntType(0); v < ds_list_size((IntType)(biomevariants)); v++)
					{
						...
						{
							self->name = DsMap(variant).Value(/*"name"*/ STR(816));
							self->group = false;
							...
							if (is_string(DsMap(variant).Value(/*"foliage"*/ STR(861))))
							{
								self->color_foliage = hex_to_color(DsMap(variant).Value(/*"foliage"*/ STR(861)));
								self->hardcoded = true;
							}
							else
								if (ds_list_valid(DsMap(variant).Value(/*"foliage"*/ STR(861))))
								{
									self->color_foliage = c_white;
									self->txy = value_get_point2D(DsMap(variant).Value(/*"foliage"*/ STR(861)), vec2(IntType(0), IntType(0)));
									self->hardcoded = false;
								}
							
							if (is_string(DsMap(variant).Value(/*"grass"*/ STR(860))))
								self->color_grass = hex_to_color(DsMap(variant).Value(/*"grass"*/ STR(860)));
							else
								self->color_grass = ObjType(obj_biome, self.otherId)->color_grass;
							
							if (is_string(DsMap(variant).Value(/*"water"*/ STR(814))))
								self->color_water = hex_to_color(DsMap(variant).Value(/*"water"*/ STR(814)));
							else
		...
		while (!is_undefined(key))
		{
			switch (StringType(key).id)
			{
				...
				default:
				{
					...
					if (newid != null_)
						newidnomc = string_replace(newid, /*"minecraft:"*/ STR(835), /*""*/ STR(0));
					block = null_;
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1792), /*"b"*/ STR(1793), /*"c"*/ STR(1794), /*"d"*/ STR(1795), /*"e"*/ STR(1252), /*"f"*/ STR(1253), /*"g"*/ STR(1796), /*"h"*/ STR(1797), /*"i"*/ STR(1798), /*"j"*/ STR(1799), /*"k"*/ STR(1800), /*"l"*/ STR(1801), /*"m"*/ STR(1802), /*"n"*/ STR(1254), /*"o"*/ STR(1803), /*"p"*/ STR(1804), /*"q"*/ STR(1805), /*"r"*/ STR(1806), /*"s"*/ STR(1807), /*"t"*/ STR(1255), /*"u"*/ STR(1256), /*"v"*/ STR(1808), /*"w"*/ STR(1809), /*"x"*/ STR(847), /*"y"*/ STR(830), /*"z"*/ STR(848) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->name = DsMap(tempmap).Value(/*"name"*/ STR(816));
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(571));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real(DsMap(tempmap).Value(/*"size"*/ STR(1016)), IntType(8));
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1148) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1148) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		global::load_assets_stage = /*"unzip"*/ STR(1314);
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1148) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(20);
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1825) + name + /*"]"*/ STR(1250) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1826) + name + /*"]"*/ STR(1250) : /*""*/ STR(0));
	...
	{
		ArrType keys = string_split(name, /*":"*/ STR(785));
		...
		if (string_contains(name, /*"#"*/ STR(856)))
	...
	{
		if (array_length(VarType::CreateRef(global::armor_update)) > IntType(0) && global::_app->window_busy != /*"popup"*/ STR(1631) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
			VarType obj = null_;
	...
	ArrType minecraft_update_armor_generate(ArrType data, VarType res)
	{
			...
			surface_set_target(armorsurf);
		{
			...
			if (trimpat != /*"none"*/ STR(905) && trimmat != /*"none"*/ STR(905))
			{
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1631) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(813) ? /*"entity/banner_base"*/ STR(1836) : /*"entity/shield_base"*/ STR(1837));
		patterndir = (type == /*"banner"*/ STR(813) ? /*"entity/banner/"*/ STR(1838) : /*"entity/shield/"*/ STR(1839));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		{
			log({ /*"Could not find model file"*/ STR(836), fname });
			return null_;
		}
		IntType map = json_load({ fname });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse model file"*/ STR(837), fname });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(816))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(817) });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(571))))
		{
		...
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(816));
			...
			self->texture_name = DsMap(map).Value(/*"texture"*/ STR(571));
			...
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(886)), vec3(IntType(1), IntType(1), IntType(1)));
			self->model_color = value_get_string(DsMap(map).Value(/*"model_color"*/ STR(1853)), /*"none"*/ STR(905));
			...
			self->floor_box_uvs = value_get_real(DsMap(map).Value(/*"floor_box_uvs"*/ STR(1854)), false);
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1855))) && !(DsMap(map).Value(/*"visible"*/ STR(1855)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(816))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(817) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1856) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			if (ObjType(obj_model_part, DsList(ObjType(obj_model_file, root)->file_part_list).Value(i))->name == DsMap(map).Value(/*"name"*/ STR(816)))
			{
				log({ /*"Duplicate part name found"*/ STR(1857), DsMap(map).Value(/*"name"*/ STR(816)) });
				return null_;
			}
		}
		withOne (obj_model_part, (new obj_model_part)->id, self->id)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(775) + self->name))
				log({ /*"model/part/"*/ STR(1858) + self->name + dev_mode_name_translation_message });
			...
			self->color_inherit = value_get_real(DsMap(map).Value(/*"color_inherit"*/ STR(1859)), true);
			self->color_blend = value_get_color(DsMap(map).Value(/*"color_blend"*/ STR(1860)), c_white);
			self->color_alpha = value_get_real(DsMap(map).Value(/*"color_alpha"*/ STR(1861)), IntType(1));
			if (is_real(/*"color_emissive"*/ STR(1862)))
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_emissive"*/ STR(1862)), IntType(0));
			else
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_brightness"*/ STR(1863)), IntType(0));
			
			self->color_mix = value_get_color(DsMap(map).Value(/*"color_mix"*/ STR(1864)), c_black);
			self->color_mix_percent = value_get_real(DsMap(map).Value(/*"color_mix_percent"*/ STR(1865)), IntType(0));
			self->part_mixing_shapes = false;
			...
			self->rotation = value_get_point3D(DsMap(map).Value(/*"rotation"*/ STR(843)), vec3(IntType(0), IntType(0), IntType(0)));
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(886)), vec3(IntType(1), IntType(1), IntType(1)));
			self->scale = vec3_mul(self->scale, idVar(self.otherId, scale));
			self->floor_box_uvs = idVar(self.otherId, floor_box_uvs);
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1866)), false);
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idVar(self.otherId, locked) > 0)
				self->locked = true;
			else
				self->locked = value_get_real(DsMap(map).Value(/*"locked"*/ STR(938)), false);
			
			self->lock_bend = true;
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idInt(self.otherId, bend_part) != null_)
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1867))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1867))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1867));
				if (self->lock_bend > 0)
				{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1868)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1869))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1869));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1870)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1871))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1872) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1871));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1873)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(1016)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(900))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1874) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(900))).id)
				{
					case 204: // right
					{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1876) });
						return null_;
				}
				
				self->bend_axis = ArrType::From({ false, false, false });
				ArrType axis = ArrType();
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(829))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(829))).id)
					{
						case 847: // x
						{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1877) });
							return null_;
					}
					
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"axis"*/ STR(829))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(829)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(829))), i)).id)
							{
								case 847: // x
								{
									self->bend_axis.Real(X_) = true;
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1878) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1879))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1879));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1879))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1879)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1879))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1880))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1880));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1880))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1880)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1880))), i);
					}
				
				self->bend_direction = ArrType::From({ IntType(0), IntType(0), IntType(0) });
				self->bend_direction_legacy = false;
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(927))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(927))).id)
					{
						case 1881: // forward
						{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1884) });
							return null_;
					}
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(927))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(927)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(927))), i)).id)
							{
								case 1881: // forward
								{
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1885))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1885)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1885)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1885))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1885)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1885))), i);
					}
					else
				...
				if (is_real(DsMap(bendmap).Value(/*"angle"*/ STR(845))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_default_angle = vec3(DsMap(bendmap).Value(/*"angle"*/ STR(845)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"angle"*/ STR(845))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(845)))); i++)
							self->bend_default_angle.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(845))), i);
					}
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1886));
