...
namespace CppProject
{
					...
					if (app_mouse_box(self, sVar(content_x) + (nextcoord2d.Real(X_) - IntType(13)), sVar(content_y) + (nextcoord2d.Real(Y_) - IntType(13)), IntType(26), IntType(26)) && sBool(content_mouseon) && ObjType(obj_view, view)->control_mouseon_last == null_)
					{
						...
						if (sBool(mouse_left_pressed))
						{
							...
							if (sVar(setting_timeline_select_jump) > 0)
								tl_jump(pointtl);
							sVar(window_busy) = /*"viewpathpointclick"*/ STR(4012);
						}
					}
				}
			}
	...
	RealType view_toolbar_draw(ScopeAny self, IntType view, RealType xx, RealType yy)
	{
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtoolselect"*/ STR(4013), xx, yy, IntType(24), IntType(24), sBool(setting_tool_select), icons_SELECT, null_, false, /*"viewtoolselecttip"*/ STR(4014) }))
			{
		...
		if (draw_button_icon(self, { /*"viewtoolmove"*/ STR(4015), xx, yy, IntType(24), IntType(24), sVar(setting_tool_move), icons_MOVE, null_, false, /*"viewtoolmovetip"*/ STR(4016) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (draw_button_icon(self, { /*"viewtoolrotate"*/ STR(4017), xx, yy, IntType(24), IntType(24), sBool(setting_tool_rotate), icons_ROTATE, null_, false, /*"viewtoolrotatetip"*/ STR(4018) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtoolscale"*/ STR(4019), xx, yy, IntType(24), IntType(24), sBool(setting_tool_scale), icons_SCALE, null_, false, /*"viewtoolscaletip"*/ STR(4020) }))
			{
				if (sVar(setting_separate_tool_modes) > 0)
				{
					action_tools_disable_all(self);
		...
		if (draw_button_icon(self, { /*"viewtoolbend"*/ STR(4021), xx, yy, IntType(24), IntType(24), sBool(setting_tool_bend), icons_BEND, null_, false, /*"viewtoolbendtip"*/ STR(4022) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtooltransform"*/ STR(4023), xx, yy, IntType(24), IntType(24), sBool(setting_tool_transform), icons_MULTITRANSFORM, null_, false, /*"viewtooltransformtip"*/ STR(4024) }))
			{
				action_tools_disable_all(self);
	...
	void view_update(ScopeAny self, IntType view, VarType cam)
	{
		...
		if (sBool(content_mouseon) && (sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(119)))
		{
			...
			if (sBool(mouse_left_pressed))
			{
				sVar(window_busy) = /*"viewclick"*/ STR(4025);
			...
			if ((!(cam > 0) || editcamobj) && sBool(mouse_right_pressed))
			{
				...
				sVar(window_busy) = /*"viewmovecamera"*/ STR(4026);
				sVar(window_focus) = string(view);
				if (cam > 0)
		...
		if (((((sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(119)) && sBool(content_mouseon)) || (sVar(window_busy) == /*"viewrotatecamera"*/ STR(4027) && sVar(window_focus) == string(view)))) && sReal(mouse_wheel) != IntType(0))
		{
			if (!(cam > 0))
		...
		{
			if (sVar(window_busy) == /*"viewclick"*/ STR(4025))
			{
				sInt(mouse_cursor) = cr_handpoint;
				if ((!(cam > 0) || editcamobj) && sReal(mouse_move) > IntType(5))
				{
					...
					{
						sVar(window_busy) = /*"viewpancamera"*/ STR(4028);
						sVar(window_focus) = string(view);
						if (cam > 0)
							action_tl_select_single(self, { cam });
					}
					else
					{
						sInt(view_click_x) = display_mouse_get_x();
						sInt(view_click_y) = display_mouse_get_y();
						sVar(window_busy) = /*"viewrotatecamera"*/ STR(4027);
						if (cam > 0)
							action_tl_select_single(self, { cam });
					}
			...
			if (sVar(window_busy) == /*"viewrotatecamera"*/ STR(4027))
			{
			...
			if (sVar(window_busy) == /*"viewmovecamera"*/ STR(4026))
			{
				global::render_samples = -IntType(1);
				if (cam == null_)
			...
			if (sVar(window_busy) == /*"viewpancamera"*/ STR(4028))
			{
		...
		if (sVar(window_busy) == /*"viewpathpointclick"*/ STR(4012))
			sVar(window_busy) = /*""*/ STR(0);
	}
	...
	RealType window_draw(ScopeAny self)
	{
		switch ((IntType)window_get_current())
		{
			...
			case e_window_TIMELINE:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(text_get({ /*"tabtimeline"*/ STR(3281) }) + /*" - Mine-imator"*/ STR(703));
				break;
			}
	...
	RealType window_draw_export(ScopeAny self)
	{
		...
		if (timelefthours > IntType(0))
			timeleftstr += text_get({ ((timelefthours == IntType(1)) ? /*"exporttimelefthour"*/ STR(4029) : /*"exporttimelefthours"*/ STR(4030)), string(timelefthours) }) + /*", "*/ STR(1057);
		if (timeleftmins > IntType(0))
			timeleftstr += text_get({ ((timeleftmins == IntType(1)) ? /*"exporttimeleftminute"*/ STR(4031) : /*"exporttimeleftminutes"*/ STR(4032)), string(timeleftmins) }) + /*" "*/ STR(21) + text_get({ /*"exporttimeleftand"*/ STR(4033) }) + /*" "*/ STR(21);
		timeleftstr += text_get({ ((timeleftsecs == IntType(1)) ? /*"exporttimeleftsecond"*/ STR(4034) : /*"exporttimeleftseconds"*/ STR(4035)), string(timeleftsecs) });
		draw_label(self, { text_get({ /*"exporttimeleft"*/ STR(4036), timeleftstr }), framex + (RealType)framew / 2.0, framey + frameh + IntType(33), fa_center, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_heading_big) });
		VarType loadtext, loadw;
		RealType sw, sh;
		loadtext = text_get({ /*"exportloading"*/ STR(4037), string(floor(perc * IntType(100))) });
		loadw = framew;
		...
		if (usesamples > 0)
			samplecount = text_get({ /*"exportsamples"*/ STR(4038), string(max({ global::render_samples, IntType(1) })), string(sVar(project_render_samples)) });
		VarType text = /*""*/ STR(0);
		if (sStr(window_state) == /*"export_movie"*/ STR(53))
			text = text_get({ /*"exportframe"*/ STR(4039), string(sReal(exportmovie_frame)), string(totalframes) }) + ((usesamples > 0) ? (/*" ("*/ STR(1155) + samplecount + /*")"*/ STR(1131)) : /*""*/ STR(0));
		else
			if (usesamples > 0)
				text = samplecount;
		...
		draw_label(self, { sStr(window_state) == /*"export_movie"*/ STR(53) ? text_get({ /*"exportmovietitle"*/ STR(4040) }) : text_get({ /*"exportimagetitle"*/ STR(4041) }), framex + (RealType)framew / 2.0, framey - IntType(35), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading_big) });
		draw_label(self, { text_get({ /*"exportstop"*/ STR(4042) }), framex + (RealType)framew / 2.0, framey - IntType(16), fa_center, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		return 0.0;
	}
	...
	RealType window_draw_load_assets(ScopeAny self)
	{
		...
		{
			global::load_assets_stage = /*"exit"*/ STR(4043);
			return IntType(0);
		}
		else
			if (global::load_assets_stage == /*"exit"*/ STR(4043))
			{
		...
		draw_dropshadow(self, xoff, yoff, IntType(740), IntType(450), global::c_accent, IntType(1));
		draw_image({ ID_spr_jonathan_splash, IntType(0), xoff + (740.0 / 2.0), yoff + IntType(100) });
		draw_box(xoff, yoff, IntType(740), IntType(450), false, global::c_level_middle, IntType(1));
		draw_sprite_ext(ID_spr_startupasset_gradient, IntType(0), (IntType)(xoff), (IntType)(yoff), 1.0, 1.0, 0.0, merge_color((IntType)(global::c_accent), c_white, 0.2), .5);
		...
		draw_label(self, { /*"Mine-imator "*/ STR(4044) + string(mineimator_version), xoff + IntType(95), yoff + IntType(289), fa_middle, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_splash_bold) });
		...
		draw_label(self, { text_get({ /*"startuploadingassets"*/ STR(4045) }), xoff + IntType(12), yoff + IntType(418), fa_left, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_splash_bold_big) });
		...
		draw_label(self, { string(floor(global::load_assets_progress * IntType(100))) + /*"%"*/ STR(220), xoff + IntType(180), yoff + IntType(435), fa_right, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_splash_bold_big) });
		...
		{
			draw_label(self, { text_get({ /*"startupsplashauthor"*/ STR(4046) }), xoff + IntType(95), yoff + IntType(289) + IntType(34), fa_middle, fa_top, global::c_text_main, global::a_text_main, sInt(font_splash_bold) });
		...
		if (dev_mode)
			draw_label(self, { /*"DEV MODE ENABLED"*/ STR(4047), xoff + IntType(95), yoff + IntType(4), fa_middle, fa_top, global::c_accent, IntType(1), sInt(font_heading_big) });
